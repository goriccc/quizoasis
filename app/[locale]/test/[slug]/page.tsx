...
const Phase3FoodBalanceGameTestClient = dynamic(
  () => import('@/components/Phase3FoodBalanceGameTestClient'),
  { ssr: false }
);
...
const Phase3ExamStudyTypeTestClient = dynamic(
  () => import('@/components/Phase3ExamStudyTypeTestClient'),
  { ssr: false }
);

const Phase3LoveExpectationDiagnosisTestClient = dynamic(
  () => import('@/components/Phase3LoveExpectationDiagnosisTestClient'),
  { ssr: false }
);
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!test && slug === 'phase3-food-balance-game') {
    test = {
      slug: 'phase3-food-balance-game',
      title: {
        ko: '밸런스 게임 — 음식 극한편',
        en: 'Balance Game — Food Extreme',
        ja: 'バランスゲーム 食べ物編（極限）',
        'zh-CN': '平衡游戏 · 美食篇（极限）',
        'zh-TW': '平衡遊戲 · 美食篇（極限）',
        vi: 'Trò cân bằng — ẩm thực (cực hạn)',
        id: 'Permainan seimbang — makanan (ekstrem)',
      },
      description: {
        ko: '12라운드 극한 2지선다로 나의 식성 스펙트럼을 분석합니다. 친구에게 공유하면 선택이 달라 반응이 터집니다.',
        en: '12 rounds of extreme food A/B choices reveal your taste spectrum. Share with friends for guaranteed reactions.',
        ja: '12ラウンドの極限2択で食性スペクトラムを分析。友達に共有すると反応が炸裂します。',
        'zh-CN': '12轮极限二选一分析你的食性光谱。分享给朋友，选择不同反应保证炸裂。',
        'zh-TW': '12輪極限二選一分析你的食性光譜。分享給朋友，選擇不同反應保證炸裂。',
        vi: '12 vòng chọn ẩm thực cực hạn phân tích phổ vị giác. Chia sẻ với bạn bè để xem phản ứng.',
        id: '12 ronde pilihan makanan ekstrem menganalisis spektrum selera. Bagikan ke teman untuk reaksi garanti.',
      },
      ...
      play_count: 0,
      tags: {
        ko: ['밸런스게임', '음식', '먹방', '극한선택', '취향'],
        en: ['balance game', 'food', 'mukbang', 'extreme choice', 'taste'],
        ja: ['バランスゲーム', '食べ物', 'モッパン', '極限選択', '好み'],
        'zh-CN': ['平衡游戏', '美食', '吃播', '极限选择', '口味'],
        'zh-TW': ['平衡遊戲', '美食', '吃播', '極限選擇', '口味'],
        vi: ['trò cân bằng', 'ẩm thực', 'mukbang', 'lựa chọn cực hạn', 'gu'],
        id: ['permainan seimbang', 'makanan', 'mukbang', 'pilihan ekstrem', 'selera'],
      },
    } as Awaited<ReturnType<typeof getTestBySlug>>;
  }
  ...
  if (!test && slug === 'phase3-exam-study-type') {
    test = {
      slug: 'phase3-exam-study-type',
      title: {
        ko: '나의 시험 공부 유형',
        en: 'My Exam Study Type',
        ja: '私の試験勉強タイプ',
        'zh-CN': '我的考试学习类型',
        'zh-TW': '我的考試學習類型',
        vi: 'Kiểu học thi của tôi',
        id: 'Tipe Belajar Ujian-ku',
      },
      description: {
        ko: '12가지 공부 상황으로 나의 학습 스타일과 최적 공부법을 분석합니다. 친구에게 공유하면 공감 폭발 보장.',
        en: 'Analyze your learning style and best study methods with 12 situations. Share with friends for guaranteed relatable reactions.',
        ja: '12の勉強シチュエーションで学習スタイルと最適な勉強法を分析。友達に共有すると共感が炸裂。',
        'zh-CN': '通过12种学习情境分析你的学习风格和最佳方法。分享给朋友保证共鸣炸裂。',
        'zh-TW': '透過12種學習情境分析你的學習風格和最佳方法。分享給朋友保證共鳴炸裂。',
        vi: 'Phân tích phong cách học và cách học tối ưu qua 12 tình huống. Chia sẻ với bạn bè để xem phản ứng.',
        id: 'Analisis gaya belajar dan metode terbaik lewat 12 situasi. Bagikan ke teman untuk reaksi relatable.',
      },
      ...
      play_count: 0,
      tags: {
        ko: ['공부유형', '시험공부', '학생공감', '벼락치기', '학습스타일'],
        en: ['study type', 'exam prep', 'student life', 'cramming', 'learning style'],
        ja: ['勉強タイプ', '試験勉強', '学生共感', '一夜漬け', '学習スタイル'],
        'zh-CN': ['学习类型', '考试复习', '学生共鸣', '临时抱佛脚', '学习风格'],
        'zh-TW': ['學習類型', '考試複習', '學生共鳴', '臨時抱佛腳', '學習風格'],
        vi: ['kiểu học', 'ôn thi', 'sinh viên', 'cày đêm', 'phong cách học'],
        id: ['tipe belajar', 'persiapan ujian', 'mahasiswa', 'mengasah', 'gaya belajar'],
      },
    } as Awaited<ReturnType<typeof getTestBySlug>>;
  }

  if (!test && slug === 'phase3-love-expectation-diagnosis') {
    test = {
      slug: 'phase3-love-expectation-diagnosis',
      title: {
        ko: '나의 연애 기대치 진단',
        en: 'My Love Expectation Diagnosis',
        ja: '私の恋愛期待値診断',
        'zh-CN': '我的恋爱期待值诊断',
        'zh-TW': '我的戀愛期待值診斷',
        vi: 'Chẩn đoán Kỳ vọng Tình yêu của tôi',
        id: 'Diagnosis Ekspektasi Cinta-ku',
      },
      description: {
        ko: '12가지 질문으로 나의 연애 기대치가 실제로 어느 수준인지 솔직하게 측정합니다. 6개 영역별 점수와 건강한 방향까지 확인하세요.',
        en: 'Measure your real love expectations honestly with 12 questions. See scores across 6 domains plus healthy direction tips.',
        ja: '12の質問で恋愛期待値の実際のレベルを正直に測定。6領域のスコアと健全な方向性まで確認。',
        'zh-CN': '通过12个问题诚实测量你的恋爱期待值实际水平。含6个领域得分与健康方向建议。',
        'zh-TW': '透過12個問題誠實測量你的戀愛期待值實際水平。含6個領域得分與健康方向建議。',
        vi: 'Đo trung thực kỳ vọng tình yêu qua 12 câu hỏi. Xem điểm 6 lĩnh vực và hướng lành mạnh.',
        id: 'Ukur ekspektasi cinta secara jujur lewat 12 pertanyaan. Lihat skor 6 domain dan arah sehat.',
      },
      ...
      play_count: 0,
      tags: {
        ko: ['연애기대치', '연애스타일', '커플', '자존감', '연애상담'],
        en: ['love expectations', 'dating style', 'couple', 'self-esteem', 'relationship advice'],
        ja: ['恋愛期待値', '恋愛スタイル', 'カップル', '自己肯定感', '恋愛相談'],
        'zh-CN': ['恋爱期待值', '恋爱风格', '情侣', '自尊', '恋爱咨询'],
...
export default async function TestPage({ params }: Props) {
  ...
  if (slug === 'phase3-eagle-eye-ultimate') {
    ...
    if (slug === 'phase3-food-balance-game') {
      const { phase3FoodBalanceGameQuestions, phase3FoodBalanceGameResults } = await import('@/lib/phase3FoodBalanceGameData');
      const test = (await getTestBySlug(slug)) || {
        slug: 'phase3-food-balance-game',
        title: {
          ko: '밸런스 게임 — 음식 극한편',
      ...
      return (
        <>
          <Phase3FoodBalanceGameTestClient
            locale={locale}
            slug={test.slug}
            title={typeof test.title === 'object' ? test.title[locale] || test.title.ko : test.title}
            description={
            ...
            questions={phase3FoodBalanceGameQuestions}
            results={phase3FoodBalanceGameResults}
            questionCount={phase3FoodBalanceGameQuestions.length}
            thumbnail={test.thumbnail}
            playCount={test.play_count}
          />
        </>
    ...
    if (slug === 'phase3-exam-study-type') {
      const { phase3ExamStudyTypeQuestions, phase3ExamStudyTypeResults } = await import('@/lib/phase3ExamStudyTypeData');
      const test = (await getTestBySlug(slug)) || {
        slug: 'phase3-exam-study-type',
        title: {
          ko: '나의 시험 공부 유형',
      ...
      return (
        <>
          <Phase3ExamStudyTypeTestClient
            ...
            questions={phase3ExamStudyTypeQuestions}
            results={phase3ExamStudyTypeResults}
            questionCount={phase3ExamStudyTypeQuestions.length}
            thumbnail={test.thumbnail}
            playCount={test.play_count}
          />
        </>
    ...
    if (slug === 'phase3-love-expectation-diagnosis') {
      const { phase3LoveExpectationDiagnosisQuestions, phase3LoveExpectationDiagnosisResults } = await import('@/lib/phase3LoveExpectationDiagnosisData');
      const test = (await getTestBySlug(slug)) || {
        slug: 'phase3-love-expectation-diagnosis',
        title: {
          ko: '나의 연애 기대치 진단',
      ...
      return (
        <>
          <Phase3LoveExpectationDiagnosisTestClient
            locale={locale}
            slug={test.slug}
            title={typeof test.title === 'object' ? test.title[locale] || test.title.ko : test.title}
            description={
