...
export default function ReportPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      console.log('[report-form] Submitting:', { title, details, email, category, locale });
      ...
      console.log('[report-form] Response:', { status: res.status, result });
      
      if (res.ok && result.ok) {
        setDone(true);
      } else {
        console.error('[report-form] Submission failed:', result);
      ...
      <form className="space-y-4" onSubmit={onSubmit}>
        <div>
          ...
          <select className="w-full border rounded px-3 py-2" value={category} onChange={(e) => setCategory(e.target.value)}>
            ...
            <option value="result">{t('categories.result')}</option>
