"""Generate lib/phase3ApprovalSeekingLevelData.ts with full 7-language localeMap() content."""
...
def fmt_result(r: dict) -> str:
    ...
    parts.append(f"    cautionsTitleKey: '{r['cautionsTitleKey']}',")
    parts.append("  },")
    return "\n".join(parts)
...
def _load_translations():
    spec = importlib.util.spec_from_file_location(
        "approval_seeking_level_translations",
        Path(__file__).resolve().parent / "build_phase3_approval_seeking_level_translations.py",
    )
...
def main() -> None:
    ...
    out = Path(__file__).resolve().parents[1] / "lib" / "phase3ApprovalSeekingLevelData.ts"
