"""Generate lib/phase3FamilyBalanceGameData.ts (Korean complete, other locales skeleton).

Source: scripts/_family_balance_game_doc_extract.txt
"""
...
DOC_PATH = ROOT / "scripts" / "_family_balance_game_doc_extract.txt"
OUT_PATH = ROOT / "lib" / "phase3FamilyBalanceGameData.ts"

LOCALES = ("ko", "en", "ja", "zh-CN", "zh-TW", "vi", "id")
OPTION_SCORES = {"A": 0, "B": 1}

TYPE_META = {
    1: {"type": "Type1", "emoji": "🧊", "title_en": "Cool Independent Type", "score_band": "0~1"},
    2: {"type": "Type2", "emoji": "🔍", "title_en": "Reason-First Type", "score_band": "2~3"},
    3: {"type": "Type3", "emoji": "⚖️", "title_en": "Balanced Family Type", "score_band": "4~6"},
    4: {"type": "Type4", "emoji": "🏠", "title_en": "Family-First Type", "score_band": "7~9"},
    5: {"type": "Type5", "emoji": "💛", "title_en": "Ultra Family Type", "score_band": "10~11"},
    6: {"type": "Type6", "emoji": "🌟", "title_en": "Family Legend Type", "score_band": "12"},
}
...
def parse_doc() -> tuple[list[dict], dict[int, dict]]:
    ...
    for k, start in enumerate(type_idx[:6]):
        ...
        summary_lines = [bln for bln in block if bln.startswith("가족 스타일:")]
        family_summary = summary_lines[0].split("가족 스타일:", 1)[1].strip() if summary_lines else ""
        family_style_type = summary_lines[1].split("가족 스타일:", 1)[1].strip() if len(summary_lines) > 1 else ""
        ...
        for bln in block:
            if bln.startswith('"'):
                ...
                continue
            if after_quote:
                if bln.startswith(("가족 스타일:", "관계 키워드:")):
                    break
                if bln.strip():
                    desc_lines.append(bln.strip())
        description = " ".join(desc_lines).strip()

        keywords = find_line(block, "관계 키워드:")
        strengths = parse_list_block(
            block,
            "이 유형의 강점:",
            ["이 유형이 가족에게서 듣는 말:", "이 유형의 주의점:", "한 줄 평:", "인증 문구:"],
        )
        family_says = parse_list_block(
            block,
            "이 유형이 가족에게서 듣는 말:",
        ...
        results[type_num] = {
            ...
            "description": "\n\n".join(desc_sections),
            "family_style_type": family_style_type,
            "keywords": keywords,
            "family_says": family_says,
            "cautions": cautions,
...
HEADER = """/** 밸런스 게임 — 가족 극한편 · phase3-family-balance-game · 12라운드 2지선다 · A=0/B=1 · 7개 로케일 */
...
out.append("export const phase3FamilyBalanceGameResults: Phase3FamilyBalanceGameResult[] = [")
for tn in range(1, 7):
    r = RESULTS[tn]
    meta = TYPE_META[tn]
    ...
    out.append(f"    empathyLevel: {fmt_locale_map(ml(r['family_style_type']))},")
    out.append(f"    characteristics: {fmt_locale_map(ml(r['keywords']))},")
    out.append(f"    goodMatch: {fmt_locale_map(ml(r['family_says']))},")
