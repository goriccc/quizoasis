"""Generate lib/phase3RelationshipEnergyData.ts (Korean complete, other locales skeleton).

Source: scripts/_relationship_energy_doc_extract.txt
"""
...
DOC_PATH = ROOT / "scripts" / "_relationship_energy_doc_extract.txt"
OUT_PATH = ROOT / "lib" / "phase3RelationshipEnergyData.ts"

LOCALES = ("ko", "en", "ja", "zh-CN", "zh-TW", "vi", "id")
OPTION_SCORES = {"A": 0, "B": 1}

TYPE_META = {
    1: {"type": "Type1", "emoji": "🔋", "title_en": "Energy-Saving Type"},
    2: {"type": "Type2", "emoji": "🌿", "title_en": "Selective Focus Type"},
    3: {"type": "Type3", "emoji": "⚖️", "title_en": "Balance Management Type"},
    4: {"type": "Type4", "emoji": "🌟", "title_en": "Relationship Vitality Type"},
    5: {"type": "Type5", "emoji": "💛", "title_en": "Relationship Immersion Type"},
    6: {"type": "Type6", "emoji": "❤️", "title_en": "Complete Devotion Type"},
}
...
def parse_doc() -> tuple[list[dict], dict[int, dict]]:
    ...
    for k, start in enumerate(type_idx[:6]):
        ...
        summary_lines = [bln for bln in block if bln.startswith("관계 에너지 유형:")]
        relationship_summary = summary_lines[0].split("관계 에너지 유형:", 1)[1].strip() if summary_lines else ""
        relationship_type = summary_lines[1].split("관계 에너지 유형:", 1)[1].strip() if len(summary_lines) > 1 else ""
        ...
        for bln in block:
            if bln.startswith('"'):
                ...
                continue
            if after_quote:
                if bln.startswith(("관계 에너지 유형:", "관계 키워드:")):
                    break
                if bln.strip():
                    desc_lines.append(bln.strip())
        description = " ".join(desc_lines).strip()

        relationship_keywords = find_line(block, "관계 키워드:")

        strengths = parse_list_block(
            block,
            "이 유형의 강점:",
            ["이 유형이 관계에서 힘든 이유:", "이 유형에게 맞는 관계 방식:", "한 줄 평:", "인증 문구:"],
        )

        cautions = parse_list_block(
            block,
            "이 유형이 관계에서 힘든 이유:",
            ["이 유형에게 맞는 관계 방식:", "한 줄 평:", "인증 문구:"],
        )

        relationship_style = parse_list_block(
            block,
            "이 유형에게 맞는 관계 방식:",
            ["한 줄 평:", "인증 문구:"],
        )
...
out.append("export const phase3RelationshipEnergyQuestions: Phase3RelationshipEnergyQuestion[] = [")
...
out.append("export const phase3RelationshipEnergyResults: Phase3RelationshipEnergyResult[] = [")
for tn in range(1, 7):
    r = RESULTS[tn]
    meta = TYPE_META[tn]
    ...
    out.append(f"    relationshipEnergySummary: {fmt_locale_map(ml(r['relationship_summary']))},")
    out.append(f"    quote: {fmt_locale_map(ml(r['quote']))},")
    out.append(f"    description: {fmt_locale_map(ml(r['description']))},")
    out.append(f"    relationshipEnergyType: {fmt_locale_map(ml(r['relationship_type']))},")
    out.append(f"    relationshipKeywords: {fmt_locale_map(ml(r['relationship_keywords']))},")
    out.append(f"    strengths: {fmt_locale_map(ml(r['strengths']))},")
    out.append(f"    cautions: {fmt_locale_map(ml(r['cautions']))},")
    out.append(f"    relationshipStyle: {fmt_locale_map(ml(r['relationship_style']))},")
