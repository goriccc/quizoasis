"""Generate lib/phase3CuriosityTypeData.ts (Korean complete, other locales skeleton).

Source: scripts/_curiosity_type_doc_extract.txt
"""
...
DOC_PATH = ROOT / "scripts" / "_curiosity_type_doc_extract.txt"
OUT_PATH = ROOT / "lib" / "phase3CuriosityTypeData.ts"

LOCALES = ("ko", "en", "ja", "zh-CN", "zh-TW", "vi", "id")
OPTION_SCORES = {"A": 0, "B": 1, "C": 2, "D": 3}

TYPE_META = {
    1: {"type": "Type1", "emoji": "🔧", "title_en": "Practical Explorer Type"},
    2: {"type": "Type2", "emoji": "📚", "title_en": "Deep Specialist Type"},
    3: {"type": "Type3", "emoji": "🕸️", "title_en": "Connection Seeker Type"},
    4: {"type": "Type4", "emoji": "🗺️", "title_en": "Broad Explorer Type"},
    5: {"type": "Type5", "emoji": "❓", "title_en": "Question Explorer Type"},
    6: {"type": "Type6", "emoji": "🌟", "title_en": "Wonder Explorer Type"},
}
...
def parse_doc() -> tuple[list[dict], dict[int, dict]]:
    ...
    for k, start in enumerate(type_idx[:6]):
        ...
        curiosity_lines = [bln for bln in block if bln.startswith("호기심 유형:")]
        curiosity_summary = curiosity_lines[0].split("호기심 유형:", 1)[1].strip() if curiosity_lines else ""
        curiosity_type = curiosity_lines[1].split("호기심 유형:", 1)[1].strip() if len(curiosity_lines) > 1 else ""
        ...
        if quote_i is not None:
            for bln in block[quote_i + 1 :]:
                if bln.startswith("호기심 유형:"):
                    break
                if bln.strip():
                    desc_lines.append(bln.strip())
        description = " ".join(desc_lines).strip()

        explore_keywords = find_line(block, "탐구 키워드:")

        strengths = parse_list_block(
            block,
            "이 유형의 강점:",
            ["이 유형의 주의점:", "이 유형에게 맞는 학습 콘텐츠:", "호기심을 더 키우고 싶다면:", "이 호기심을 잘 활용하려면:", "인증 문구:", "한 줄 평:"],
        )

        cautions = parse_list_block(
            block,
            "이 유형의 주의점:",
            ["이 유형에게 맞는 학습 콘텐츠:", "호기심을 더 키우고 싶다면:", "이 호기심을 잘 활용하려면:", "인증 문구:", "한 줄 평:"],
        )

        content_recommend = find_line(block, "이 유형에게 맞는 학습 콘텐츠:")

        grow_curiosity = find_line(block, "호기심을 더 키우고 싶다면:")
        if not grow_curiosity:
            grow_curiosity = find_line(block, "이 호기심을 잘 활용하려면:")
...
HEADER = """/** 나의 호기심 유형 진단 — phase3-curiosity-type · 12문항 4지선다 · A=0/B=1/C=2/D=3 · 7개 로케일 */
...
out.append("export const phase3CuriosityTypeQuestions: Phase3CuriosityTypeQuestion[] = [")
...
out.append("export const phase3CuriosityTypeResults: Phase3CuriosityTypeResult[] = [")
for tn in range(1, 7):
    r = RESULTS[tn]
    meta = TYPE_META[tn]
    ...
    out.append(f"    curiositySummary: {fmt_locale_map(ml(r['curiosity_summary']))},")
    out.append(f"    quote: {fmt_locale_map(ml(r['quote']))},")
    out.append(f"    description: {fmt_locale_map(ml(r['description']))},")
    out.append(f"    curiosityType: {fmt_locale_map(ml(r['curiosity_type']))},")
    out.append(f"    exploreKeywords: {fmt_locale_map(ml(r['explore_keywords']))},")
    out.append(f"    strengths: {fmt_locale_map(ml(r['strengths']))},")
    out.append(f"    cautions: {fmt_locale_map(ml(r['cautions']))},")
    out.append(f"    contentRecommend: {fmt_locale_map(ml(r['content_recommend']))},")
    out.append(f"    growCuriosity: {fmt_locale_map(ml(r['grow_curiosity']))},")
