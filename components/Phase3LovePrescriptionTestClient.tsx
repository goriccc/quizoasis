...
import {
  Phase3LovePrescriptionQuestion,
  Phase3LovePrescriptionResult,
  calculatePhase3LovePrescriptionResult,
...
interface Phase3LovePrescriptionTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3LovePrescriptionQuestion[];
  results: Phase3LovePrescriptionResult[];
...
export default function Phase3LovePrescriptionTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3LovePrescriptionTestClientProps) {
  const t = useTranslations('phase3LovePrescriptionTest');
  ...
  const [result, setResult] = useState<Phase3LovePrescriptionResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3LovePrescriptionQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3LovePrescriptionQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3LovePrescriptionResult(answersArray);
      const prescriptionResult = results.find(r => r.type === resultType);
  ...
  if (showResult && result) {
    ...
    const resultDiagnosis =
      result.diagnosisName[locale as keyof typeof result.diagnosisName] || result.diagnosisName.ko;
    const resultOpening =
      result.openingLine[locale as keyof typeof result.openingLine] || result.openingLine.ko;
    const resultLongDescription = result.description[locale as keyof typeof result.description] || result.description.ko;
    const rx1 = result.prescription1[locale as keyof typeof result.prescription1] || result.prescription1.ko;
    const rx2 = result.prescription2[locale as keyof typeof result.prescription2] || result.prescription2.ko;
    const rx3 = result.prescription3[locale as keyof typeof result.prescription3] || result.prescription3.ko;
    const resultDosage = result.dosage[locale as keyof typeof result.dosage] || result.dosage.ko;
    const resultPrecautions =
      result.precautions[locale as keyof typeof result.precautions] || result.precautions.ko;
    const resultSideEffects =
      result.sideEffects[locale as keyof typeof result.sideEffects] || result.sideEffects.ko;
    const resultExtraRx = result.extraRx[locale as keyof typeof result.extraRx] || result.extraRx.ko;
    const resultPeriod =
      result.prescriptionPeriod[locale as keyof typeof result.prescriptionPeriod] ||
      result.prescriptionPeriod.ko;
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            <div className="text-center mb-3 bg-white rounded-2xl shadow-lg p-4 md:p-5 border border-teal-100/70">
              ...
              <p className="text-lg font-bold text-gray-800 mb-3">{resultDiagnosis}</p>
              <p className="text-base font-semibold text-gray-700 mb-3 leading-relaxed">{resultOpening}</p>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3 border border-teal-100/60">
              <h3 className="text-base font-bold text-teal-800 mb-2">{t('ui.prescription1')}</h3>
              <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap">{rx1}</p>
            </div>
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3 border border-pink-100/60">
              <h3 className="text-base font-bold text-teal-800 mb-2">{t('ui.prescription2')}</h3>
              <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap">{rx2}</p>
            </div>
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3 border border-teal-100/50">
              <h3 className="text-base font-bold text-teal-800 mb-2">{t('ui.prescription3')}</h3>
              <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap">{rx3}</p>
            </div>

            <div className="bg-white rounded-xl shadow-lg p-4 mb-3 border border-emerald-100/60">
              <h3 className="text-base font-bold text-gray-800 mb-2">📌 {t('ui.dosage')}</h3>
              <p className="text-sm text-gray-700 leading-relaxed">{resultDosage}</p>
            </div>
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3 border border-amber-100/60">
              <h3 className="text-base font-bold text-gray-800 mb-2">⚠️ {t('ui.precautions')}</h3>
              <p className="text-sm text-gray-700 leading-relaxed">{resultPrecautions}</p>
            </div>
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3 border border-rose-100/60">
              <h3 className="text-base font-bold text-gray-800 mb-2">💬 {t('ui.sideEffects')}</h3>
              <p className="text-sm text-gray-700 leading-relaxed">{resultSideEffects}</p>
            </div>
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3 border border-pink-100/60">
              <h3 className="text-base font-bold text-gray-800 mb-2">💊 {t('ui.extraRx')}</h3>
              <p className="text-sm text-gray-700 leading-relaxed">{resultExtraRx}</p>
            </div>
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3 border border-teal-100/50">
              <h3 className="text-base font-bold text-gray-800 mb-2">📅 {t('ui.prescriptionPeriod')}</h3>
              <p className="text-sm text-gray-700 leading-relaxed">{resultPeriod}</p>
            </div>
