...
export default function FaceReadingTestClient({ 
  ...
  const resultData = useMemo(() => {
    ...
    const personalities = result?.personality?.[locale as keyof typeof result.personality] || result?.personality?.ko || [];
  ...
  if (showResult && result && resultData) {
    const { resultTitle, resultDescription, resultStrengths, resultRecommendations, personalities, resultAdvice, wealthFortune, loveFortune, successFortune } = resultData;
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <div className="space-y-1">
                {Array.isArray(personalities) && personalities.length > 0 ? (
                  ...
                  <span className="text-sm text-gray-600">성격 분석 정보가 없습니다.</span>
                )}
              </div>
            </div>
