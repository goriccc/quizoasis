...
import { Phase2YoutubeChannelQuestion, Phase2YoutubeChannelResult, calculatePhase2YoutubeChannelResult } from '@/lib/phase2_youtube_channel_data';
...
interface Phase2YoutubeChannelTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase2YoutubeChannelQuestion[];
  results: Phase2YoutubeChannelResult[];
...
export default function Phase2YoutubeChannelTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase2YoutubeChannelTestClientProps) {
  const t = useTranslations('phase2YoutubeChannelTest');
  ...
  const [result, setResult] = useState<Phase2YoutubeChannelResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2YoutubeChannelQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase2YoutubeChannelQuestion[]) => {
  ...
  const handleAnswer = (optionIndex: number) => {
    // 현재 질문의 원래 인덱스를 찾아서 답변 저장 (선택한 옵션의 인덱스)
    const currentOriginalIndex = originalQuestionIndices[currentQuestion];
    
    // 셔플링된 옵션 배열에서 선택한 옵션 객체 찾기
    const shuffledOptions = shuffledOptionsMap[currentQuestion] || shuffledQuestions[currentQuestion].options;
    const selectedOption = shuffledOptions[optionIndex];
    
    // 원래 질문의 옵션 배열에서 해당 옵션의 인덱스 찾기
    const originalQuestion = questions[currentOriginalIndex];
    const originalOptionIndex = originalQuestion.options.findIndex(opt => 
      opt.text.ko === selectedOption.text.ko // 텍스트로 비교 (더 안전)
    );
    const finalOptionIndex = originalOptionIndex >= 0 ? originalOptionIndex : optionIndex;
    
    const newAnswers = { ...answers, [currentOriginalIndex]: finalOptionIndex };
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산 (answers는 원래 질문 인덱스를 키로, 선택한 옵션 인덱스를 값으로 가짐)
      const resultType = calculatePhase2YoutubeChannelResult(newAnswers, questions);
      const youtubeChannelResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (youtubeChannelResult) {
        setResult(youtubeChannelResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (youtubeChannelResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(youtubeChannelResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 유튜브 채널 결과:', youtubeChannelResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const resultMainContent = result.mainContent[locale as keyof typeof result.mainContent] || result.mainContent.ko;
    const resultCheatKey = result.cheatKey[locale as keyof typeof result.cheatKey] || result.cheatKey.ko;
    const resultCollabPartner = result.collabPartner[locale as keyof typeof result.collabPartner] || result.collabPartner.ko;
            ...
            <div className="grid grid-cols-1 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="flex flex-wrap justify-start gap-2">
                  {resultMainContent.split(/[,、，]/).map((content, idx) => (
                    <span
                      key={idx}
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="flex flex-wrap justify-start gap-2">
                  {resultCheatKey.split(/[,、，]/).map((key, idx) => (
                    <span
                      key={idx}
                      className="inline-block bg-gradient-to-r from-blue-100 to-cyan-100 text-blue-800 text-sm font-semibold px-3 py-1 rounded-full shadow-sm"
                    >
                      {key.trim()}
                    </span>
                  ))}
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="space-y-2">
                  {(() => {
                    // "Type 6 (뷰티/패션 유튜버)" 형식에서 Type 추출
                    const matchType = resultCollabPartner.match(/Type\s*(\d+)/i);
                    if (matchType) {
                      ...
                      if (matchResult) {
                        ...
                        return (
                          <div className="bg-gradient-to-r from-blue-100 to-cyan-100 rounded-lg px-3 py-2">
