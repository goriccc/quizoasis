...
import { StressReliefQuestion, StressReliefResult, calculateStressReliefResult } from '../lib/stressReliefData';
...
interface StressReliefTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: StressReliefQuestion[];
  results: StressReliefResult[];
...
export default function StressReliefTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  playCount = 0,
  similarTests = []
}: StressReliefTestClientProps) {
  const t = useTranslations();
  const [started, setStarted] = useState(false);
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [answers, setAnswers] = useState<any[]>([]);
  const [result, setResult] = useState<StressReliefResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<StressReliefQuestion[]>(questions);
    ...
    if (locale !== 'ko' && !started && aliProducts.length === 0) {
      const loadProducts = async () => {
        try {
          ...
          console.error(t('stressReliefTest.ui.productLoadError'), error);
        }
      };
      loadProducts();
    }
    ...
    if (similarTests.length === 0) {
      const loadTests = async () => {
        try {
          ...
          console.error(t('stressReliefTest.ui.testLoadError'), error);
        }
      };
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: StressReliefQuestion[]) => {
  ...
  const handleAnswer = (scores: any) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculateStressReliefResult(newAnswers);
      const stressReliefResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (stressReliefResult) {
        setResult(stressReliefResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (stressReliefResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(stressReliefResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 스트레스 해소 결과:', stressReliefResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  const calculateResult = (finalAnswers: any[]) => {
    const resultType = calculateStressReliefResult(finalAnswers);
    const stressReliefResult = results.find(r => r.type === resultType);
  ...
  const handleShareResult = async () => {
    ...
    const shareText = t('stressReliefTest.shareMessages.default', { type: resultTitle }) + `\n\n${`https://myquizoasis.com${window.location.pathname}`}`;
    ...
    if (navigator.share) {
      try {
        ...
        if (error instanceof Error && error.name !== 'AbortError') {
          console.error(t('stressReliefTest.ui.shareError'), error);
        }
      }
    } else {
      try {
        await navigator.clipboard.writeText(shareText);
        alert(t('stressReliefTest.ui.resultCopied'));
      } catch (error) {
        console.error(t('stressReliefTest.ui.clipboardError'), error);
        alert(t('stressReliefTest.ui.shareUnavailable'));
      }
    }
  };
  ...
  const shareToWeChat = async () => {
    ...
    const shareText = result 
      ? t('stressReliefTest.shareMessages.default', { type: resultTitle }) + `\n\n${url}`
      : `${title}\n\n${url}`;
    ...
    try {
      ...
      alert(t('stressReliefTest.ui.wechatCopy'));
    } catch (error) {
      alert(t('stressReliefTest.ui.shareUnavailable'));
    }
  };
  ...
  const shareToWhatsApp = () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareText = result 
      ? encodeURIComponent(t('stressReliefTest.shareMessages.default', { type: resultTitle }))
  ...
  const shareToKakao = () => {
    ...
    if (!window.Kakao || !window.Kakao.isInitialized()) {
      alert(t('stressReliefTest.ui.kakaoInit'));
      return;
    }
    ...
    try {
      window.Kakao.Share.sendDefault({
        ...
        buttons: [
          {
            title: t('stressReliefTest.ui.testButton'),
            link: {
              mobileWebUrl: currentUrl,
              webUrl: currentUrl,
            },
      ...
      console.error(t('stressReliefTest.ui.shareError'), error);
      alert(t('stressReliefTest.ui.kakaoError'));
    }
  };
  ...
  const copyLink = () => {
    ...
    alert(t('stressReliefTest.ui.linkCopied'));
  };
      ...
      <div className="min-h-screen bg-white">
        <div className="max-w-4xl mx-auto">
          <div className="relative w-full overflow-hidden mb-3" style={{ aspectRatio: '680/384' }}>
            <Image
              src={getThumbnailUrl(thumbnail || 'test_054_stress_relief.jpg')}
              alt={title}
              fill
          ...
          <div className="px-4">
            ...
            <div className="my-6">
              <AdSensePlaceholder 
                ...
                className="mx-auto"
                label={t('stressReliefTest.ui.adsenseTitle')}
              />
            </div>

            <div className="text-gray-600 mb-6 leading-relaxed text-center space-y-4">
              <p className="font-bold text-gray-700">{t('stressReliefTest.startMessage.line1')}</p>
              <p>{t('stressReliefTest.startMessage.line2')}</p>
              <p>{t('stressReliefTest.startMessage.line3')}</p>
              <p>{t('stressReliefTest.startMessage.line4')}</p>
              <p>{t('stressReliefTest.startMessage.line5')}</p>
            </div>
            ...
            <div className="mb-8 text-center">
              ...
              <div className="flex justify-center gap-2">
                <button onClick={copyLink} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/link.jpeg" alt={t('stressReliefTest.ui.linkCopy')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToKakao} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/kakao.jpeg" alt={t('stressReliefTest.ui.kakao')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToTelegram} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/telegram.jpeg" alt={t('stressReliefTest.ui.telegram')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToWeChat} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/wechat.jpeg" alt={t('stressReliefTest.ui.wechat')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToLine} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/line.jpeg" alt={t('stressReliefTest.ui.line')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToWhatsApp} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/whatsapp.jpeg" alt={t('stressReliefTest.ui.whatsapp')} width={46} height={46} className="rounded-lg" />
                </button>
              </div>
            </div>
            ...
            {similarTestsState.length > 0 && (
              ...
              <h2 className="text-xl font-bold text-gray-800 mb-6">
                {t('stressReliefTest.ui.similarTests')}
              </h2>
      ...
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-4">
        ...
        <div className="mb-8 w-full max-w-[680px]">
          <AdSensePlaceholder 
            ...
            style={{ width: '100%', height: '250px' }}
            className="mx-auto"
                label={t('stressReliefTest.ui.adsenseLoading')}
          />
        </div>
  ...
  if (showResult && result) {
    ...
    const resultRecommendedActivities = result.recommendedActivities[locale as keyof typeof result.recommendedActivities] || result.recommendedActivities.ko;
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                🎯 {t('stressReliefTest.ui.recommendedActivities')}
              </h3>
              <div className="flex flex-wrap gap-2">
                {resultRecommendedActivities.split(/,\s+|，\s*|、\s*/).map((activity: string, index: number) => (
                  <span
                    key={index}
            ...
            <div className="my-6 px-4">
              <AdSensePlaceholder 
                ...
                style={{ width: '100%', height: '250px' }}
                className="mx-auto"
                label={t('stressReliefTest.ui.adsenseResult')}
              />
            </div>
        ...
        <div>
          ...
          <div className="mt-8 px-4">
            <AdSensePlaceholder 
              ...
              style={{ width: '100%', height: '250px' }}
              className="mx-auto"
              label={t('stressReliefTest.ui.adsenseBottom')}
            />
          </div>
