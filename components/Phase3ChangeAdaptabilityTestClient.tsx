...
import {
  Phase3ChangeAdaptabilityQuestion,
  Phase3ChangeAdaptabilityResult,
  calculatePhase3ChangeAdaptabilityResult,
  calculatePhase3ChangeAdaptabilityDomainScores,
} from '@/lib/phase3ChangeAdaptabilityData';
...
interface Phase3ChangeAdaptabilityTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3ChangeAdaptabilityQuestion[];
  results: Phase3ChangeAdaptabilityResult[];
...
export default function Phase3ChangeAdaptabilityTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3ChangeAdaptabilityTestClientProps) {
  const t = useTranslations('phase3ChangeAdaptabilityTest');
  ...
  const [result, setResult] = useState<Phase3ChangeAdaptabilityResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3ChangeAdaptabilityQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3ChangeAdaptabilityQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      const scores = calculatePhase3ChangeAdaptabilityDomainScores(answersArray);
      setDomainScores(scores);

      const resultType = calculatePhase3ChangeAdaptabilityResult(answersArray);
  ...
  const handleRetake = () => {
    ...
    setResult(null);
    setShowResult(false);
    setDomainScores({ cognitive: 0, emotional: 0, behavioral: 0, uncertainty: 0, resilience: 0, learning: 0, total: 0 });
  ...
  if (showResult && result) {
    ...
    const resultChangeAdaptability =
      result.changeAdaptabilityIndex[locale as keyof typeof result.changeAdaptabilityIndex] || result.changeAdaptabilityIndex.ko;
    const resultChangeAdaptabilityType =
      result.changeAdaptabilityType[locale as keyof typeof result.changeAdaptabilityType] || result.changeAdaptabilityType.ko;
    const resultIndexScore =
      result.indexScore[locale as keyof typeof result.indexScore] || result.indexScore.ko;
    ...
    const domainConfig = [
      { key: 'cognitive', label: t('ui.cognitiveDomain'), score: domainScores.cognitive, max: 6 },
      { key: 'emotional', label: t('ui.emotionalDomain'), score: domainScores.emotional, max: 6 },
      { key: 'behavioral', label: t('ui.behavioralDomain'), score: domainScores.behavioral, max: 6 },
      { key: 'uncertainty', label: t('ui.uncertaintyDomain'), score: domainScores.uncertainty, max: 6 },
      { key: 'resilience', label: t('ui.resilienceDomain'), score: domainScores.resilience, max: 6 },
      { key: 'learning', label: t('ui.learningDomain'), score: domainScores.learning, max: 6 },
    ];
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  🎯 {t('ui.adaptabilityType')}
                </h3>
                <p className="text-sm font-bold text-gray-700 leading-relaxed text-center">
                  {resultChangeAdaptabilityType}
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                ...
                {domainConfig.map((domain) => (
                  <div key={domain.key}>
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2.5">
                      <div
                        className="bg-gradient-to-r from-teal-500 to-cyan-400 h-2.5 rounded-full transition-all"
...
{isType6 && resultCertification && (
            ...
            <div className="mt-8 mb-6 px-4">
              <button
                ...
                className="w-full bg-gradient-to-r from-teal-500 to-emerald-500 text-white font-bold py-4 px-6 rounded-xl hover:from-teal-600 hover:to-emerald-600 transition-all shadow-md flex items-center justify-center gap-3"
              >
