...
import {
  Phase3GritIndexQuestion,
  Phase3GritIndexResult,
  calculatePhase3GritIndexResult,
} from '@/lib/phase3GritIndexData';
...
interface Phase3GritIndexTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3GritIndexQuestion[];
  results: Phase3GritIndexResult[];
...
export default function Phase3GritIndexTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3GritIndexTestClientProps) {
  const t = useTranslations('phase3GritIndexTest');
  ...
  const [result, setResult] = useState<Phase3GritIndexResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3GritIndexQuestion[]>([]);
  const [originalQuestionIndices, setOriginalQuestionIndices] = useState<number[]>([]); // 셔플링된 질문의 원래 인덱스 매핑
  const [perseveranceScore, setPerseveranceScore] = useState<number>(0);
  const [passionScore, setPassionScore] = useState<number>(0);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3GritIndexQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 축 점수 계산 (원본 질문 인덱스를 기준으로 합산)
      const perseveranceIndices = [0, 2, 4, 6, 8, 10]; // Q1,3,5,7,9,11
      const passionIndices = [1, 3, 5, 7, 9, 11]; // Q2,4,6,8,10,12
      const perseveranceTotal = perseveranceIndices.reduce((sum, idx) => sum + (answersArray[idx] ?? 0), 0);
      const passionTotal = passionIndices.reduce((sum, idx) => sum + (answersArray[idx] ?? 0), 0);
      setPerseveranceScore(perseveranceTotal);
      setPassionScore(passionTotal);
      
      // 결과 계산
      const resultType = calculatePhase3GritIndexResult(answersArray);
  ...
  if (showResult && result) {
    ...
    const resultGritIndex = result.gritIndex[locale as keyof typeof result.gritIndex] || result.gritIndex.ko;
    const resultPerseveranceDiagnosis = result.perseveranceDiagnosis[
      locale as keyof typeof result.perseveranceDiagnosis
    ] || result.perseveranceDiagnosis.ko;
    const resultPassionDiagnosis = result.passionDiagnosis[locale as keyof typeof result.passionDiagnosis] || result.passionDiagnosis.ko;
    const resultActionItems = result.actionItems[locale as keyof typeof result.actionItems] || result.actionItems.ko;
    const resultDuckworthQuote =
      result.duckworthQuote[locale as keyof typeof result.duckworthQuote] || result.duckworthQuote.ko;
    const resultCommonTraits = result.commonTraits[locale as keyof typeof result.commonTraits] || result.commonTraits.ko;
    ...
    const displayGritIndex = (() => {
      const lines = resultGritIndex.split('\n');
      if (lines.length < 2) return resultGritIndex;
      const first = lines[0];
      const rest = lines.slice(1).join('\n');
      // Type line starts with an emoji (e.g. 🌱). It's duplicated because the big emoji is already shown above.
      // ES target compatibility: avoid unicode property escapes / `u` flag.
      // Remove leading non-(Korean/latin/digit) chars (emoji + following spaces).
      const cleanedRest = rest.replace(/^[^\uAC00-\uD7A3A-Za-z0-9]+/, '').trim();
      return cleanedRest ? `${first}\n${cleanedRest}` : first;
    })();

    const actionItemsList = resultActionItems ? resultActionItems.split('\n').map((s) => s.trim()).filter(Boolean) : [];
    const isType1 = result.type === 'Type1';
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <p className="text-sm font-bold text-gray-700 leading-relaxed whitespace-pre-line text-center">
                {displayGritIndex}
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  💪 {t('ui.perseveranceDiagnosis')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed text-left">
                  {resultPerseveranceDiagnosis}
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  🔥 {t('ui.passionDiagnosis')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed text-left">
                  {resultPassionDiagnosis}
                </p>
              </div>
            </div>
            ...
            {isType1 && resultDuckworthQuote && (
              <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  ✍️ {t('ui.duckworthQuoteTitle')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-line text-left">
                  &ldquo;{resultDuckworthQuote}&rdquo;
                </p>
              </div>
            )}
