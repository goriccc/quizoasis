...
import {
  Phase3IdealTypeDnaAnalysisQuestion,
  Phase3IdealTypeDnaAnalysisResult,
  calculatePhase3IdealTypeDnaAnalysisResult,
...
interface Phase3IdealTypeDnaAnalysisTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3IdealTypeDnaAnalysisQuestion[];
  results: Phase3IdealTypeDnaAnalysisResult[];
...
export default function Phase3IdealTypeDnaAnalysisTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3IdealTypeDnaAnalysisTestClientProps) {
  const t = useTranslations('phase3IdealTypeDnaAnalysisTest');
  ...
  const [result, setResult] = useState<Phase3IdealTypeDnaAnalysisResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3IdealTypeDnaAnalysisQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3IdealTypeDnaAnalysisQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3IdealTypeDnaAnalysisResult(answersArray);
      const idealResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (idealResult) {
        setResult(idealResult);
      }
    }
  };
          ...
          <div className="px-4">
            ...
            <div className="flex justify-center mb-4">
              <button
                ...
                className="bg-gradient-to-r from-rose-500 to-amber-500 hover:from-rose-600 hover:to-amber-600 text-white font-bold py-4 px-8 rounded-full shadow-lg transform hover:scale-105 transition-all duration-200"
              >
  ...
  if (showResult && result) {
    ...
    const resultDnaTypeLabel =
      result.dnaTypeLabel[locale as keyof typeof result.dnaTypeLabel] || result.dnaTypeLabel.ko;
    const resultExcitingPoint =
      result.excitingPoint[locale as keyof typeof result.excitingPoint] || result.excitingPoint.ko;
    const resultCommonGround =
      result.commonGround[locale as keyof typeof result.commonGround] || result.commonGround.ko;
    const resultIdealKeywords =
      result.idealKeywords[locale as keyof typeof result.idealKeywords] || result.idealKeywords.ko;
    const resultFlirtLine =
      result.flirtLine[locale as keyof typeof result.flirtLine] || result.flirtLine.ko;
    const resultDetectionTip =
      result.detectionTip[locale as keyof typeof result.detectionTip] || result.detectionTip.ko;
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <p className="text-sm text-gray-700 leading-relaxed">{resultExcitingPoint}</p>
            </div>

            <div className="bg-white rounded-xl shadow-lg p-4 md:p-5 mb-3 border border-pink-100/80">
              <h3 className="text-base font-bold text-gray-800 mb-3 text-left">
                🔍 {t('ui.commonGround')}
              </h3>
              <div className="flex flex-wrap justify-center md:justify-start gap-2">
                {resultCommonGround.split(/[,、，]/).map((item, index) => (
                  <span
                    key={index}
                    className="inline-block bg-gradient-to-r from-pink-50 to-rose-100 text-gray-800 px-3 py-1.5 rounded-full text-sm font-medium"
                  >
                    {item.trim()}
                  </span>
                ))}
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 md:p-5 mb-3 border border-rose-100/80">
              <h3 className="text-base font-bold text-gray-800 mb-3 text-left">
                ⭐ {t('ui.idealKeywords')}
              </h3>
              <div className="flex flex-wrap justify-center md:justify-start gap-2">
                {resultIdealKeywords.split(/[,、，]/).map((char, index) => (
                  <span
                    key={index}
                    className="inline-block bg-gradient-to-r from-rose-100 to-amber-100 text-rose-900 px-3 py-1.5 rounded-full text-sm font-medium"
                  >
                    {char.trim()}
                  </span>
                ))}
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <p className="text-sm text-gray-700 leading-relaxed">{resultFlirtLine}</p>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <p className="text-sm text-gray-700 leading-relaxed">{resultDetectionTip}</p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 px-4">
              ...
              <Link
                href={`/${locale}`}
                className="flex-1 bg-gradient-to-r from-rose-500 to-amber-500 text-white font-bold py-4 px-6 rounded-xl hover:from-rose-600 hover:to-amber-600 transition-all text-center shadow-md"
              >
          ...
          <div className="space-y-4 px-4">
            {optionsArray.map((option, index) => {
              ...
              const colors = [
                ...
                'from-teal-50 to-cyan-100 hover:from-teal-100 hover:to-cyan-200 border-teal-200 hover:border-teal-400',
              ];
              const bgColors = ['bg-rose-600', 'bg-pink-600', 'bg-amber-600', 'bg-teal-600'];
