...
import { HonestyVsRestraintQuestion, HonestyVsRestraintResult, calculateHonestyVsRestraintResult } from '@/lib/honestyVsRestraintData';
...
interface HonestyVsRestraintTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: HonestyVsRestraintQuestion[];
  results: HonestyVsRestraintResult[];
...
export default function HonestyVsRestraintTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  playCount = 0,
  similarTests = []
}: HonestyVsRestraintTestClientProps) {
  const t = useTranslations();
  const [started, setStarted] = useState(false);
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [answers, setAnswers] = useState<any[]>([]);
  const [result, setResult] = useState<HonestyVsRestraintResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<HonestyVsRestraintQuestion[]>(questions);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: HonestyVsRestraintQuestion[]) => {
  ...
  const handleAnswer = (scores: any) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculateHonestyVsRestraintResult(newAnswers);
      const honestyResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (honestyResult) {
        setResult(honestyResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (honestyResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(honestyResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 솔직/절제 결과:', honestyResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  const calculateResult = (finalAnswers: any[]) => {
    const resultType = calculateHonestyVsRestraintResult(finalAnswers);
    const honestyResult = results.find(r => r.type === resultType);
  ...
  const handleShareResult = async () => {
    ...
    const shareMessage = t('honestyVsRestraintTest.shareMessages.default', { type: resultTitle });
    ...
    if (navigator.share) {
      ...
      try {
        ...
        alert(t('honestyVsRestraintTest.alerts.resultCopied'));
      } catch (error) {
        console.error('클립보드 복사 실패:', error);
        alert(t('honestyVsRestraintTest.alerts.shareFailed'));
      }
    }
  };
  ...
  const shareToWeChat = async () => {
    ...
    const shareMessage = result 
      ? t('honestyVsRestraintTest.shareMessages.wechat', { type: resultTitle })
      : title;
    const shareText = `${shareMessage}\n\n${url}`;
    ...
    try {
      ...
      alert(t('honestyVsRestraintTest.alerts.wechatCopy'));
    } catch (error) {
      alert(t('honestyVsRestraintTest.alerts.shareFailed'));
    }
  };
  ...
  const shareToWhatsApp = () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareMessage = result 
      ? t('honestyVsRestraintTest.shareMessages.whatsapp', { type: resultTitle })
      : title;
  ...
  const shareToKakao = () => {
    ...
    if (!window.Kakao || !window.Kakao.isInitialized()) {
      alert(t('honestyVsRestraintTest.alerts.kakaoInit'));
      return;
    }
    ...
    const shareDescription = result 
      ? t('honestyVsRestraintTest.shareMessages.kakao', { type: resultTitle })
      : description;
    ...
    try {
      ...
      alert(t('honestyVsRestraintTest.alerts.kakaoError'));
    }
  };
  ...
  const shareToTelegram = () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareMessage = result 
      ? t('honestyVsRestraintTest.shareMessages.telegram', { type: resultTitle })
      : title;
  ...
  const copyLink = () => {
    ...
    alert(t('honestyVsRestraintTest.alerts.linkCopied'));
  };
      ...
      <div className="min-h-screen bg-white">
        <div className="max-w-4xl mx-auto">
          <div className="relative w-full overflow-hidden mb-3" style={{ aspectRatio: '680/384' }}>
            <Image
              src={getThumbnailUrl(thumbnail || 'test_208_honesty_vs_restraint.jpg')}
              alt={title}
              fill
          ...
          <div className="px-4">
            ...
            <div className="text-gray-600 mb-6 leading-relaxed text-center space-y-4">
              <p className="font-bold text-gray-700">{t('honestyVsRestraintTest.startMessage.line1')}</p>
              <p>{t('honestyVsRestraintTest.startMessage.line2')}</p>
              <p>{t('honestyVsRestraintTest.startMessage.line3')}</p>
              <p>{t('honestyVsRestraintTest.startMessage.line4')}</p>
              <p className="whitespace-pre-line">{t('honestyVsRestraintTest.startMessage.line5')}</p>
              <p className="whitespace-pre-line">{t('honestyVsRestraintTest.startMessage.line6')}</p>
            </div>
            ...
            <div className="mb-8 text-center">
              ...
              <div className="flex justify-center gap-2">
                <button onClick={copyLink} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/link.jpeg" alt={t('honestyVsRestraintTest.ui.linkCopy')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToKakao} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/kakao.jpeg" alt={t('honestyVsRestraintTest.ui.kakao')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToTelegram} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/telegram.jpeg" alt={t('honestyVsRestraintTest.ui.telegram')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToWeChat} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/wechat.jpeg" alt={t('honestyVsRestraintTest.ui.wechat')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToLine} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/line.jpeg" alt={t('honestyVsRestraintTest.ui.line')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToWhatsApp} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/whatsapp.jpeg" alt={t('honestyVsRestraintTest.ui.whatsapp')} width={46} height={46} className="rounded-lg" />
                </button>
              </div>
            </div>
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-3">
                  ✅ {t('honestyVsRestraintTest.result.strengths')}
                </h3>
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-3">
                  ⚠️ {t('honestyVsRestraintTest.result.weaknesses')}
                </h3>
                <div className="flex flex-wrap gap-2">
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                📊 {t('honestyVsRestraintTest.result.score')}
              </h3>
              <div className="flex flex-wrap gap-2">
                {resultScore.split(/\s*\/\s*/).map((scoreItem, index) => (
                  <span
                    key={index}
                    className="bg-gradient-to-r from-blue-100 to-purple-100 px-4 py-2 rounded-full text-sm font-medium text-gray-800 shadow-sm"
                  >
                    {scoreItem}
                  </span>
                ))}
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                💡 {t('honestyVsRestraintTest.result.advice')}
              </h3>
