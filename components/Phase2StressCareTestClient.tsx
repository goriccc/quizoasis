...
import { Phase2StressCareQuestion, Phase2StressCareResult, calculatePhase2StressCareResult } from '@/lib/phase2_stress_care_data';
...
interface Phase2StressCareTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase2StressCareQuestion[];
  results: Phase2StressCareResult[];
...
export default function Phase2StressCareTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase2StressCareTestClientProps) {
  const t = useTranslations('phase2StressCareTest');
  ...
  const [result, setResult] = useState<Phase2StressCareResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2StressCareQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase2StressCareQuestion[]) => {
  ...
  const handleAnswer = (selectedTypes: string[]) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase2StressCareResult(newAnswers);
      const stressCareResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (stressCareResult) {
        setResult(stressCareResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (stressCareResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(stressCareResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 스트레스 케어 결과:', stressCareResult.type, '→ 검색 키워드:', keywords[0]);
            ...
            <div className="space-y-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {resultSymptoms.split(/[,、，]/).map((symptom, index) => (
                    <span
                      key={index}
                      className="inline-block px-4 py-2 bg-gradient-to-r from-red-100 to-pink-100 text-red-700 font-medium rounded-full text-sm border border-red-200"
                    >
                      {symptom.trim()}
                    </span>
                  ))}
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  🌟 {t('ui.recommendedActivities')}
                </h3>
                <div className="flex flex-wrap gap-2">
                  {resultRecommendedActivities.split(/[,、，]/).map((activity, index) => (
                    <span
                      key={index}
