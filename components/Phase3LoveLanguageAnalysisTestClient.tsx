...
import {
  Phase3LoveLanguageAnalysisQuestion,
  Phase3LoveLanguageAnalysisResult,
  calculatePhase3LoveLanguageAnalysisResult,
  calculatePhase3LoveLanguageAnalysisDomainScores,
} from '@/lib/phase3LoveLanguageAnalysisData';
...
interface Phase3LoveLanguageAnalysisTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3LoveLanguageAnalysisQuestion[];
  results: Phase3LoveLanguageAnalysisResult[];
...
export default function Phase3LoveLanguageAnalysisTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3LoveLanguageAnalysisTestClientProps) {
  const t = useTranslations('phase3LoveLanguageAnalysisTest');
  ...
  const [result, setResult] = useState<Phase3LoveLanguageAnalysisResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3LoveLanguageAnalysisQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3LoveLanguageAnalysisQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      const scores = calculatePhase3LoveLanguageAnalysisDomainScores(answersArray);
      setDomainScores(scores);

      const resultType = calculatePhase3LoveLanguageAnalysisResult(answersArray);
  ...
  const handleRetake = () => {
    ...
    setResult(null);
    setShowResult(false);
    setDomainScores({ wordsAffirmation: 0, qualityTime: 0, receivingGifts: 0, actsOfService: 0, physicalTouch: 0, conflictDevotion: 0, total: 0 });
  ...
  if (showResult && result) {
    ...
    const resultLanguageSummary =
      result.languageSummary[locale as keyof typeof result.languageSummary] || result.languageSummary.ko;
    const resultLanguageType =
      result.languageType[locale as keyof typeof result.languageType] || result.languageType.ko;
    const resultLoveKeywords =
      result.loveKeywords[locale as keyof typeof result.loveKeywords] || result.loveKeywords.ko;
    ...
    const resultLovePattern = result.lovePattern[locale as keyof typeof result.lovePattern] || result.lovePattern.ko;
    ...
    const domainConfig = [
      { key: 'wordsAffirmation', label: t('ui.wordsAffirmationDomain'), score: domainScores.wordsAffirmation, max: 6 },
      { key: 'qualityTime', label: t('ui.qualityTimeDomain'), score: domainScores.qualityTime, max: 6 },
      { key: 'receivingGifts', label: t('ui.receivingGiftsDomain'), score: domainScores.receivingGifts, max: 6 },
      { key: 'actsOfService', label: t('ui.actsOfServiceDomain'), score: domainScores.actsOfService, max: 6 },
      { key: 'physicalTouch', label: t('ui.physicalTouchDomain'), score: domainScores.physicalTouch, max: 6 },
      { key: 'conflictDevotion', label: t('ui.conflictDevotionDomain'), score: domainScores.conflictDevotion, max: 6 },
    ];
...
{isType6 && resultCertification && (
            ...
            <div className="mt-8 mb-6 px-4">
              <button
                ...
                className="w-full bg-gradient-to-r from-amber-500 to-rose-600 text-white font-bold py-4 px-6 rounded-xl hover:from-amber-600 hover:to-rose-700 transition-all shadow-md flex items-center justify-center gap-3"
              >
