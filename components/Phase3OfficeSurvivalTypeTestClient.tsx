...
import {
  Phase3OfficeSurvivalTypeQuestion,
  Phase3OfficeSurvivalTypeResult,
  calculatePhase3OfficeSurvivalTypeResult,
...
interface Phase3OfficeSurvivalTypeTestClientProps {
  ...
  title: string;
  description: string;
  questions: Phase3OfficeSurvivalTypeQuestion[];
  results: Phase3OfficeSurvivalTypeResult[];
...
function getResultShareSnippet(t: (key: string) => string, result: Phase3OfficeSurvivalTypeResult): string {
  switch (result.type) {
    case 'Type1':
...
export default function Phase3OfficeSurvivalTypeTestClient({
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3OfficeSurvivalTypeTestClientProps) {
  const t = useTranslations('phase3OfficeSurvivalTypeTest');
  ...
  const [result, setResult] = useState<Phase3OfficeSurvivalTypeResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3OfficeSurvivalTypeQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3OfficeSurvivalTypeQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3OfficeSurvivalTypeResult(answersArray);
  ...
  if (showResult && result) {
    ...
    const resultStrengths = result.strengthsTags[locale as keyof typeof result.strengthsTags] || result.strengthsTags.ko;
    const resultNickname = result.nickname[locale as keyof typeof result.nickname] || result.nickname.ko;
    const resultWeaknesses = result.weaknesses[locale as keyof typeof result.weaknesses] || result.weaknesses.ko;
    const resultBoss = result.bossView[locale as keyof typeof result.bossView] || result.bossView.ko;
    const resultPeer = result.peerView[locale as keyof typeof result.peerView] || result.peerView.ko;
    const resultSurvival = result.survivalStrategy[locale as keyof typeof result.survivalStrategy] || result.survivalStrategy.ko;
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {resultStrengths.split(/[,、，]/).map((char, index) => (
                    <span
                      key={index}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultNickname}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  ⚠️ {t('ui.weaknesses')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">{resultWeaknesses}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultBoss}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  👥 {t('ui.peerView')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">{resultPeer}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  🛡️ {t('ui.survivalStrategy')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">{resultSurvival}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  ⚡ {t('ui.caution')}
              ...
              <div className="bg-white rounded-xl shadow-lg p-4 border border-amber-100">
                ...
                <p className="text-sm text-gray-800 font-medium leading-relaxed">&ldquo;{resultQuote}&rdquo;</p>
              </div>
            </div>
