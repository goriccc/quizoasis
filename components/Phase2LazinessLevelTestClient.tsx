...
import { Phase2LazinessLevelQuestion, Phase2LazinessLevelResult, calculatePhase2LazinessLevelResult } from '@/lib/phase2_laziness_level_data';
...
interface Phase2LazinessLevelTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase2LazinessLevelQuestion[];
  results: Phase2LazinessLevelResult[];
...
,
  ...
  const [result, setResult] = useState<Phase2LazinessLevelResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2LazinessLevelQuestion[]>([]);
...
// 질문 섞기 함수
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase2LazinessLevelResult(answersArray);
      const lazinessLevelResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (lazinessLevelResult) {
        setResult(lazinessLevelResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (lazinessLevelResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(lazinessLevelResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 게으름 지수 결과:', lazinessLevelResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const resultLazinessLevel = result.lazinessLevel[locale as keyof typeof result.lazinessLevel] || result.lazinessLevel.ko;
