...
import {
  Phase3SelfEsteemShieldStrengthQuestion,
  Phase3SelfEsteemShieldStrengthResult,
  calculatePhase3SelfEsteemShieldStrengthResult,
...
interface Phase3SelfEsteemShieldStrengthTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3SelfEsteemShieldStrengthQuestion[];
  results: Phase3SelfEsteemShieldStrengthResult[];
...
export default function Phase3SelfEsteemShieldStrengthTestClient({
  locale,
  slug,
  title,
  description,
  ...
  isLatestTest = false,
  badgeType = null,
}: Phase3SelfEsteemShieldStrengthTestClientProps) {
  const t = useTranslations('phase3SelfEsteemShieldStrengthTest');
  ...
  const [result, setResult] = useState<Phase3SelfEsteemShieldStrengthResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3SelfEsteemShieldStrengthQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3SelfEsteemShieldStrengthQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3SelfEsteemShieldStrengthResult(answersArray);
  ...
  if (showResult && result) {
    ...
    const resultShieldStrength =
      result.shieldStrength[locale as keyof typeof result.shieldStrength] || result.shieldStrength.ko;
    const resultPattern = result.pattern
      ? result.pattern[locale as keyof typeof result.pattern] || result.pattern.ko
      : null;
    ...
    const resultActions = result.actions
      ? result.actions[locale as keyof typeof result.actions] || result.actions.ko
      : null;
    const resultRemainingTask = result.remainingTask
      ? result.remainingTask[locale as keyof typeof result.remainingTask] || result.remainingTask.ko
      : null;
    const resultCanDo = result.canDo
      ? result.canDo[locale as keyof typeof result.canDo] || result.canDo.ko
      : null;
    const resultCaution = result.caution
      ? result.caution[locale as keyof typeof result.caution] || result.caution.ko
      : null;
    const resultToOthers = result.toOthers
      ? result.toOthers[locale as keyof typeof result.toOthers] || result.toOthers.ko
      : null;
    const resultAffirmation = result.affirmation
      ? result.affirmation[locale as keyof typeof result.affirmation] || result.affirmation.ko
      : null;
          ...
          <div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-lg font-bold text-violet-600 text-center leading-snug">
                  {resultShieldStrength}
                </p>
              </div>
            </div>
              ...
              <div className="mb-3">
                <div className="bg-white rounded-xl shadow-lg p-4">
                  ...
                  <div className="flex flex-wrap gap-2 justify-center">
                    {chipSplit(resultPattern).map((char, idx) => (
                      <span
                        key={idx}
                        className="inline-block bg-gradient-to-r from-violet-100 to-fuchsia-100 text-violet-800 text-sm font-semibold px-3 py-1 rounded-full shadow-sm"
                      >
                        {char}
                      </span>
                    ))}
            ...
            {resultStrength ? (
              <div className="mb-3">
                <div className="bg-white rounded-xl shadow-lg p-4">
                  ...
                  <div className="flex flex-wrap gap-2 justify-center">
                    {chipSplit(resultStrength).map((char, idx) => (
                      <span
                        key={idx}
                        className="inline-block bg-gradient-to-r from-emerald-100 to-teal-100 text-emerald-800 text-sm font-semibold px-3 py-1 rounded-full shadow-sm"
                      >
                        {char}
                      </span>
                    ))}
            ...
            {resultRemainingTask ? (
              <div className="mb-3">
                <div className="bg-white rounded-xl shadow-lg p-4">
                  ...
                  <p className="text-sm text-gray-700 leading-relaxed text-center">{resultRemainingTask}</p>
                </div>
              </div>
            ) : null}
            ...
            {resultCanDo ? (
              <div className="mb-3">
                <div className="bg-white rounded-xl shadow-lg p-4">
                  ...
                  <p className="text-sm text-gray-700 leading-relaxed">{resultCanDo}</p>
                </div>
              </div>
            ) : null}
            ...
            {resultCaution ? (
              <div className="mb-3">
                <div className="bg-amber-50 border border-amber-200 rounded-xl shadow-lg p-4">
                  <h3 className="text-base font-bold text-amber-900 mb-2 text-left w-full">
                    {t('ui.caution')}
                  </h3>
                  <p className="text-sm text-amber-950 leading-relaxed">{resultCaution}</p>
                </div>
              </div>
            ) : null}
            ...
            {resultToOthers ? (
              <div className="mb-3">
                <div className="bg-white rounded-xl shadow-lg p-4">
                  ...
                  <p className="text-sm text-gray-700 leading-relaxed text-center">{resultToOthers}</p>
                </div>
              </div>
            ) : null}
            ...
            {resultAffirmation ? (
              <div className="mb-3">
                <div className="bg-gradient-to-r from-violet-50 to-amber-50 rounded-xl shadow-lg p-4 border border-violet-100">
                  <h3 className="text-base font-bold text-gray-800 mb-2 text-left w-full">
                    {t('ui.affirmation')}
                  </h3>
                  <p className="text-sm text-gray-800 leading-relaxed text-center font-semibold">
                    {resultAffirmation}
                  </p>
                </div>
              </div>
