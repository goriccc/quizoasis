...
import { Phase2AreYouTQuestion, Phase2AreYouTResult, calculatePhase2AreYouTResult } from '@/lib/phase2_are_you_T_data';
...
interface Phase2AreYouTTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase2AreYouTQuestion[];
  results: Phase2AreYouTResult[];
...
export default function Phase2AreYouTTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  playCount = 0,
  similarTests = []
}: Phase2AreYouTTestClientProps) {
  const t = useTranslations('phase2AreYouTTest');
  ...
  const [result, setResult] = useState<Phase2AreYouTResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2AreYouTQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase2AreYouTQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase2AreYouTResult(answersArray);
      const areYouTResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (areYouTResult) {
        setResult(areYouTResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (areYouTResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(areYouTResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] T/F 감성 결과:', areYouTResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const resultTLevel = result.tLevel[locale as keyof typeof result.tLevel] || result.tLevel.ko;
    const resultMainLines = result.mainLines[locale as keyof typeof result.mainLines] || result.mainLines.ko;
            ...
            <div className="mb-3">
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {resultMainLines.split(/[,、，]/).map((line, index) => (
                    <span
                      key={index}
