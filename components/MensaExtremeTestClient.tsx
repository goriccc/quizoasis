...
import { MensaExtremeQuestion, MensaExtremeResult, calculateMensaExtremeResult } from '@/lib/mensaExtremeData';
...
interface MensaExtremeTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: MensaExtremeQuestion[];
  results: MensaExtremeResult[];
...
export default function MensaExtremeTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: MensaExtremeTestClientProps) {
  const t = useTranslations();
  const [started, setStarted] = useState(false);
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [answers, setAnswers] = useState<boolean[]>([]);
  const [result, setResult] = useState<MensaExtremeResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<MensaExtremeQuestion[]>([]);
  ...
  useEffect(() => {
    if (!started && aliProducts.length === 0) {
      const loadProducts = async () => {
        try {
          const keyword = locale === 'ko' ? 'trending products' : 'brain training';
  ...
  const copyLink = () => {
    ...
    alert(t('mensaExtremeTest.alerts.linkCopied'));
  };
  ...
  const shareToKakao = () => {
    ...
    if (!window.Kakao || !window.Kakao.isInitialized()) {
      alert(t('mensaExtremeTest.alerts.kakaoInit'));
      return;
    }
    ...
    const shareDescription = result 
      ? t('mensaExtremeTest.shareMessages.kakao', { type: resultTitle })
      : description;
    ...
    try {
      ...
      alert(t('mensaExtremeTest.alerts.kakaoError'));
    }
  };
  ...
  const shareToTelegram = () => {
    ...
    const shareText = result 
      ? t('mensaExtremeTest.shareMessages.telegram', { type: resultTitle })
      : title;
  ...
  const shareToWeChat = async () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareText = result 
      ? t('mensaExtremeTest.shareMessages.wechat', { type: resultTitle })
      : title;
    
    try {
      await navigator.clipboard.writeText(url);
      alert(t('mensaExtremeTest.alerts.wechatCopy'));
    } catch (error) {
      console.error('클립보드 복사 실패:', error);
      alert(t('mensaExtremeTest.alerts.shareFailed'));
    }
  };
  ...
  const shareToLine = () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareText = result 
      ? t('mensaExtremeTest.shareMessages.line', { type: resultTitle })
      : title;
    const text = encodeURIComponent(shareText);
  ...
  const shareToWhatsApp = () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareText = result 
      ? t('mensaExtremeTest.shareMessages.whatsapp', { type: resultTitle })
      : title;
      ...
      <div className="min-h-screen bg-white">
        <div className="max-w-4xl mx-auto">
          <div className="relative w-full overflow-hidden mb-3" style={{ aspectRatio: '680/384' }}>
            <Image
              src={getThumbnailUrl(thumbnail || 'test_103_mensa_extreme.jpg')}
              alt={title}
              fill
          ...
          <div className="px-4">
            ...
            <div className="text-gray-600 mb-6 leading-relaxed text-center space-y-4">
              <p className="font-bold text-gray-700">{t('mensaExtremeTest.startMessage.line1')}</p>
              <p>{t('mensaExtremeTest.startMessage.line2')}</p>
              <p>{t('mensaExtremeTest.startMessage.line3')}</p>
              <p>{t('mensaExtremeTest.startMessage.line4')}</p>
              <p className="whitespace-pre-line">{t('mensaExtremeTest.startMessage.line5')}</p>
              <p>{t('mensaExtremeTest.startMessage.line6')}</p>
            </div>
  ...
  const handleAnswer = (isCorrect: boolean) => {
    ...
    if (nextQuestion >= shuffledQuestions.length) {
      ...
      // 결과 계산
      const resultType = calculateMensaExtremeResult(newAnswers);
      const mensaResult = results.find(r => r.type === resultType);
      
      if (mensaResult) {
        setResult(mensaResult);
      }
    } else {
      // 다음 문항으로 이동
      ...
      <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-2xl p-8 max-w-md w-full mx-4 text-center shadow-2xl">
          <h2 className="text-2xl font-bold text-gray-800 mb-4">
            🎉 {t('mensaExtremeTest.ui.testCompleted')}
          </h2>
          ...
          >
            {t('mensaExtremeTest.ui.viewResults')}
          </button>
        </div>
      </div>
    );
          ...
          <div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                📊 {t('mensaExtremeTest.ui.score')}
              </h3>
              <p className="text-center text-3xl font-bold text-blue-600">
                {t('mensaExtremeTest.ui.scoreFormat', { 
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-3">
                  💪 {t('mensaExtremeTest.ui.strengths')}
                </h3>
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-3">
                  🎯 {t('mensaExtremeTest.ui.recommendations')}
                </h3>
                <div className="flex flex-wrap gap-2">
            ...
            <div className="mt-8 mb-6 px-4">
              <button
                onClick={() => {
                  const resultTitle = result.title[locale as keyof typeof result.title] || result.title.ko;
                  const shareText = t('mensaExtremeTest.shareMessages.default', { type: resultTitle });
                  const url = `https://myquizoasis.com${window.location.pathname}`;
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-600">{t('mensaExtremeTest.ui.preparingQuestions')}</p>
        </div>
      </div>
    );
  }
      ...
      {showHint && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 max-w-md w-full mx-4 text-center shadow-2xl">
            <h3 className="text-xl font-bold text-gray-800 mb-4">{t('mensaExtremeTest.ui.hint')}</h3>
