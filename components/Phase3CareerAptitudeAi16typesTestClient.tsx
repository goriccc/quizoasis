...
import { Phase3CareerAptitudeAi16typesQuestion, Phase3CareerAptitudeAi16typesResult, calculatePhase3CareerAptitudeAi16typesResult } from '@/lib/phase3CareerAptitudeAi16typesData';
...
interface Phase3CareerAptitudeAi16typesTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3CareerAptitudeAi16typesQuestion[];
  results: Phase3CareerAptitudeAi16typesResult[];
...
export default function Phase3CareerAptitudeAi16typesTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3CareerAptitudeAi16typesTestClientProps) {
  const t = useTranslations('phase3CareerAptitudeAi16typesTest');
  const tGlobal = useTranslations(); // 글로벌 번역 (mbti 등)
  const [started, setStarted] = useState(false);
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [answers, setAnswers] = useState<Record<number, string>>({}); // 원래 질문 인덱스를 키로 axisTag 저장
  const [result, setResult] = useState<Phase3CareerAptitudeAi16typesResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3CareerAptitudeAi16typesQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3CareerAptitudeAi16typesQuestion[]) => {
  ...
  // 답변 처리
  const handleAnswer = (axisTag: string) => {
    // 현재 질문의 원래 인덱스를 찾아 axisTag 저장
    const currentOriginalIndex = originalQuestionIndices[currentQuestion];
    const newAnswers = { ...answers, [currentOriginalIndex]: axisTag };
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 원래 질문 순서대로 axisTag 배열 재구성
      const answersArray = questions.map((_, idx) => newAnswers[idx] ?? '');
      
      // 결과 계산
      const resultType = calculatePhase3CareerAptitudeAi16typesResult(answersArray);
      ...
      if (careerResult && locale !== 'ko') {
        ...
        console.log('🔮 [시작] 직업 적성 결과:', careerResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  const getResultShareText = () => {
    if (!result) return '';
    return (
      result.shareMessage[locale as keyof typeof result.shareMessage] ||
      result.shareMessage.ko
    );
  };
  ...
  const handleShareResult = async () => {
    ...
    const body = getResultShareText() || t('shareMessages.default', { type: resultTitle });
  ...
  const shareToLine = () => {
    ...
    const shareText = result 
      ? encodeURIComponent(getResultShareText() || t('shareMessages.line', { type: resultTitle }))
  ...
  const shareToWeChat = async () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareText = result 
      ? `${getResultShareText() || t('shareMessages.wechat', { type: resultTitle })}\n\n${url}`
  ...
  const shareToWhatsApp = () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareText = result 
      ? encodeURIComponent(getResultShareText() || t('shareMessages.whatsapp', { type: resultTitle }))
  ...
  const shareToKakao = () => {
    ...
    const shareDescription = result 
      ? (getResultShareText() || t('shareMessages.kakao', { type: resultTitle }))
  ...
  const shareToTelegram = () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareText = result 
      ? (getResultShareText() || t('shareMessages.telegram', { type: resultTitle }))
  ...
  if (showResult && result) {
    ...
    const resultCoreCompetencies = result.coreCompetencies[locale as keyof typeof result.coreCompetencies] || result.coreCompetencies.ko;
    const resultMatchingJobs = result.matchingJobs[locale as keyof typeof result.matchingJobs] || result.matchingJobs.ko;
    const resultGrowthPath = result.growthPath[locale as keyof typeof result.growthPath] || result.growthPath.ko;
    const resultCaution = result.caution[locale as keyof typeof result.caution] || result.caution.ko;
    const resultFirstCareer = result.firstCareer[locale as keyof typeof result.firstCareer] || result.firstCareer.ko;
          ...
          <div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <div className="flex flex-wrap justify-center gap-2">
                {resultCoreCompetencies.split(/[,、，]/).map((char, index) => (
                  <span
                    key={index}
          ...
          <div className="space-y-4 px-4">
            {optionsArray.map((option, index) => {
              ...
              return (
                <button
                  key={index}
                  onClick={() => handleAnswer(option.axisTag)}
