...
import { Phase2TeaTherapyQuestion, Phase2TeaTherapyResult, calculatePhase2TeaTherapyResult } from '@/lib/phase2_tea_therapy_data';
...
interface Phase2TeaTherapyTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase2TeaTherapyQuestion[];
  results: Phase2TeaTherapyResult[];
...
export default function Phase2TeaTherapyTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase2TeaTherapyTestClientProps) {
  const t = useTranslations('phase2TeaTherapyTest');
  ...
  const [result, setResult] = useState<Phase2TeaTherapyResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2TeaTherapyQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase2TeaTherapyQuestion[]) => {
  ...
  const handleAnswer = (selectedTypes: string[]) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase2TeaTherapyResult(newAnswers);
      const teaTherapyResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (teaTherapyResult) {
        setResult(teaTherapyResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (teaTherapyResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(teaTherapyResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 힐링 티 결과:', teaTherapyResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const resultRecommendedTeas = result.recommendedTeas[locale as keyof typeof result.recommendedTeas] || result.recommendedTeas.ko;
    const resultEffects = result.effects[locale as keyof typeof result.effects] || result.effects.ko;
    const resultTeaFoods = result.teaFoods[locale as keyof typeof result.teaFoods] || result.teaFoods.ko;
            ...
            <div className="space-y-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  🍵 {t('ui.recommendedTeas')}
                </h3>
                <div className="flex flex-wrap gap-2">
                  {resultRecommendedTeas.split(/[,、，]/).map((tea, index) => (
                    <span
                      key={index}
                      className="inline-block px-4 py-2 bg-gradient-to-r from-green-100 to-emerald-100 text-green-700 font-medium rounded-full text-sm border border-green-200"
                    >
                      {tea.trim()}
                    </span>
                  ))}
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {resultTeaFoods.split(/[,、，]/).map((food, index) => (
                    <span
                      key={index}
                      className="inline-block px-4 py-2 bg-gradient-to-r from-amber-100 to-orange-100 text-amber-700 font-medium rounded-full text-sm border border-amber-200"
                    >
                      {food.trim()}
                    </span>
                  ))}
