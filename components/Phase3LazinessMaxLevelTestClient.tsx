...
import {
  Phase3LazinessMaxLevelQuestion,
  Phase3LazinessMaxLevelResult,
  calculatePhase3LazinessMaxLevelResult,
...
interface Phase3LazinessMaxLevelTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3LazinessMaxLevelQuestion[];
  results: Phase3LazinessMaxLevelResult[];
...
export default function Phase3LazinessMaxLevelTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3LazinessMaxLevelTestClientProps) {
  const t = useTranslations('phase3LazinessMaxLevelTest');
  ...
  const [result, setResult] = useState<Phase3LazinessMaxLevelResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3LazinessMaxLevelQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3LazinessMaxLevelQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3LazinessMaxLevelResult(answersArray);
  ...
  if (showResult && result) {
    ...
    const resultLazinessGrade = result.lazinessGrade[locale as keyof typeof result.lazinessGrade] || result.lazinessGrade.ko;
    const resultLazinessIndex = result.lazinessIndex[locale as keyof typeof result.lazinessIndex] || result.lazinessIndex.ko;
