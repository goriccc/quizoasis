...
import {
  Phase3RelationshipEnergyQuestion,
  Phase3RelationshipEnergyResult,
  calculatePhase3RelationshipEnergyResult,
  calculatePhase3RelationshipEnergyDomainScores,
} from '@/lib/phase3RelationshipEnergyData';
...
interface Phase3RelationshipEnergyTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3RelationshipEnergyQuestion[];
  results: Phase3RelationshipEnergyResult[];
...
export default function Phase3RelationshipEnergyTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3RelationshipEnergyTestClientProps) {
  const t = useTranslations('phase3RelationshipEnergyTest');
  ...
  const [result, setResult] = useState<Phase3RelationshipEnergyResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3RelationshipEnergyQuestion[]>([]);
  const [originalQuestionIndices, setOriginalQuestionIndices] = useState<number[]>([]); // 셔플링된 질문의 원래 인덱스 매핑
  const [domainScores, setDomainScores] = useState({
    energyDirection: 0,
    relationshipMaintenance: 0,
    togetherTimeReaction: 0,
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3RelationshipEnergyQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      const scores = calculatePhase3RelationshipEnergyDomainScores(answersArray);
      setDomainScores(scores);

      const resultType = calculatePhase3RelationshipEnergyResult(answersArray);
  ...
  const handleRetake = () => {
    ...
    setResult(null);
    setShowResult(false);
    setDomainScores({ energyDirection: 0, relationshipMaintenance: 0, togetherTimeReaction: 0, relationshipFatigue: 0, contactPattern: 0, relationshipPriority: 0, total: 0 });
  ...
  if (showResult && result) {
    ...
    const resultRelationshipEnergySummary =
      result.relationshipEnergySummary[locale as keyof typeof result.relationshipEnergySummary] || result.relationshipEnergySummary.ko;
    const resultRelationshipEnergyType =
      result.relationshipEnergyType[locale as keyof typeof result.relationshipEnergyType] || result.relationshipEnergyType.ko;
    const resultRelationshipKeywords =
      result.relationshipKeywords[locale as keyof typeof result.relationshipKeywords] || result.relationshipKeywords.ko;
    ...
    const resultRelationshipStyle = result.relationshipStyle[locale as keyof typeof result.relationshipStyle] || result.relationshipStyle.ko;
    ...
    const domainConfig = [
      { key: 'energyDirection', label: t('ui.energyDirectionDomain'), score: domainScores.energyDirection, max: 2 },
      { key: 'relationshipMaintenance', label: t('ui.relationshipMaintenanceDomain'), score: domainScores.relationshipMaintenance, max: 2 },
      { key: 'togetherTimeReaction', label: t('ui.togetherTimeReactionDomain'), score: domainScores.togetherTimeReaction, max: 2 },
      { key: 'relationshipFatigue', label: t('ui.relationshipFatigueDomain'), score: domainScores.relationshipFatigue, max: 2 },
      { key: 'contactPattern', label: t('ui.contactPatternDomain'), score: domainScores.contactPattern, max: 2 },
      { key: 'relationshipPriority', label: t('ui.relationshipPriorityDomain'), score: domainScores.relationshipPriority, max: 2 },
    ];
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  🎯 {t('ui.relationshipEnergyType')}
                </h3>
                <p className="text-sm font-bold text-gray-700 leading-relaxed text-center">
                  {resultRelationshipEnergyType}
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  🔑 {t('ui.relationshipKeywordsTitle')}
                </h3>
                <p className="text-sm font-bold text-gray-700 leading-relaxed text-center">
                  {resultRelationshipKeywords}
                </p>
              </div>
            </div>
                ...
                {domainConfig.map((domain) => (
                  <div key={domain.key}>
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2.5">
                      <div
                        className="bg-gradient-to-r from-rose-500 to-orange-400 h-2.5 rounded-full transition-all"
              ...
              <div className="mt-4 space-y-2 text-sm text-gray-700">
                <p>
                  📌 {t('ui.mostNaturalFlowDomain')}: <span className="font-semibold">{strongestLabels.join(' · ')}</span>
                </p>
                <p>
                  📌 {t('ui.mostEnergyDrainDomain')}: <span className="font-semibold">{weakestLabels.join(' · ')}</span>
                </p>
              </div>
            </div>
            ...
            {resultRelationshipStyle && (
              <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  💡 {t('ui.relationshipStyleTitle')}
                </h3>
                <ul className="text-sm text-gray-700 leading-relaxed list-disc list-inside pl-[2ch]">
                  {resultRelationshipStyle.split('\n').map((item, idx) => (
                    item.trim() ? (
...
{isType6 && resultCertification && (
            ...
            <div className="mt-8 mb-6 px-4">
              <button
                ...
                className="w-full bg-gradient-to-r from-rose-500 to-orange-500 text-white font-bold py-4 px-6 rounded-xl hover:from-rose-600 hover:to-orange-600 transition-all shadow-md flex items-center justify-center gap-3"
              >
