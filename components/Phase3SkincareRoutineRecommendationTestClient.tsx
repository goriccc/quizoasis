...
import {
  Phase3SkincareRoutineRecommendationQuestion,
  Phase3SkincareRoutineRecommendationResult,
...
interface Phase3SkincareRoutineRecommendationTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3SkincareRoutineRecommendationQuestion[];
  results: Phase3SkincareRoutineRecommendationResult[];
...
export default function Phase3SkincareRoutineRecommendationTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3SkincareRoutineRecommendationTestClientProps) {
  const t = useTranslations('phase3SkincareRoutineRecommendationTest');
  ...
  const [result, setResult] = useState<Phase3SkincareRoutineRecommendationResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3SkincareRoutineRecommendationQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3SkincareRoutineRecommendationQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3SkincareRoutineRecommendationResult(answersArray);
      const skincareResult = results.find(r => r.type === resultType);
      
      if (skincareResult) {
        setResult(skincareResult);
      }
    }
  };
          ...
          <div className="px-4">
            ...
            <div className="flex justify-center mb-4">
              <button
                ...
                className="bg-gradient-to-r from-teal-500 to-pink-400 hover:from-teal-600 hover:to-pink-500 text-white font-bold py-4 px-8 rounded-full shadow-lg transform hover:scale-105 transition-all duration-200"
              >
      ...
      <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-2xl p-8 max-w-md w-full mx-4 text-center shadow-2xl">
          ...
          <button
            ...
            className="w-full bg-gradient-to-r from-teal-500 to-pink-400 text-white py-4 px-6 rounded-xl text-xl font-bold hover:from-teal-600 hover:to-pink-500 transition-all duration-300 shadow-lg"
          >
  ...
  if (showResult && result) {
    ...
    const resultSkinType =
      result.skinTypeDiagnosis[locale as keyof typeof result.skinTypeDiagnosis] ||
      result.skinTypeDiagnosis.ko;
    const resultLead =
      result.leadQuote[locale as keyof typeof result.leadQuote] || result.leadQuote.ko;
    const resultLongDescription = result.description[locale as keyof typeof result.description] || result.description.ko;
    const morning = result.morningRoutine[locale as keyof typeof result.morningRoutine] || result.morningRoutine.ko;
    const evening = result.eveningRoutine[locale as keyof typeof result.eveningRoutine] || result.eveningRoutine.ko;
    const keyIng =
      result.keyIngredients[locale as keyof typeof result.keyIngredients] || result.keyIngredients.ko;
    const avoidIng =
      result.avoidIngredients[locale as keyof typeof result.avoidIngredients] || result.avoidIngredients.ko;
    const weekly = result.weeklySpecial[locale as keyof typeof result.weeklySpecial] || result.weeklySpecial.ko;
    const essence =
      result.routineEssence[locale as keyof typeof result.routineEssence] || result.routineEssence.ko;
    const addToday =
      result.addTodayIngredient[locale as keyof typeof result.addTodayIngredient] ||
      result.addTodayIngredient.ko;

    return (
      <div className="min-h-screen bg-gradient-to-br from-teal-50 via-white to-pink-50">
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            <div className="text-center mb-3 bg-white rounded-2xl shadow-lg p-4 md:p-5 border border-teal-100/70">
              <h2 className="text-xl font-bold text-gray-800 mb-3">
                {tGlobal('mbti.yourResult')}
              </h2>
              <div className="text-6xl mb-3">{result.emoji}</div>
              <h1 className="text-xl md:text-2xl font-bold mb-3 text-teal-800 leading-snug">
                {resultTitle}
              </h1>
              <p className="text-sm font-semibold text-teal-600 mb-2">{t('ui.diagnosisLabel')}</p>
              <p className="text-lg font-bold text-gray-800 mb-3">{resultSkinType}</p>
              <p className="text-base font-semibold text-gray-700 mb-3 leading-relaxed">{resultLead}</p>
              {resultLongDescription.trim() ? (
                <p className="text-base text-gray-600 leading-relaxed text-left">{resultLongDescription}</p>
              ) : null}
            </div>

            <div className="bg-white rounded-xl shadow-lg p-4 mb-3 border border-teal-100/60">
              <h3 className="text-base font-bold text-teal-800 mb-2">{t('ui.morningRoutine')}</h3>
              <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap">{morning}</p>
            </div>
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3 border border-pink-100/60">
              <h3 className="text-base font-bold text-teal-800 mb-2">{t('ui.eveningRoutine')}</h3>
              <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap">{evening}</p>
            </div>

            <div className="bg-white rounded-xl shadow-lg p-4 mb-3 border border-emerald-100/60">
              <h3 className="text-base font-bold text-gray-800 mb-2">{t('ui.keyIngredients')}</h3>
              <p className="text-sm text-gray-700 leading-relaxed">{keyIng}</p>
            </div>
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3 border border-amber-100/60">
              <h3 className="text-base font-bold text-gray-800 mb-2">{t('ui.avoidIngredients')}</h3>
              <p className="text-sm text-gray-700 leading-relaxed">{avoidIng}</p>
            </div>
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3 border border-sky-100/60">
              <h3 className="text-base font-bold text-gray-800 mb-2">{t('ui.weeklySpecial')}</h3>
              <p className="text-sm text-gray-700 leading-relaxed">{weekly}</p>
            </div>
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3 border border-rose-100/60">
              <h3 className="text-base font-bold text-gray-800 mb-2">{t('ui.routineEssence')}</h3>
              <p className="text-sm text-gray-700 leading-relaxed">{essence}</p>
            </div>
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3 border border-pink-100/60">
              <h3 className="text-base font-bold text-gray-800 mb-2">{t('ui.addTodayIngredient')}</h3>
              <p className="text-sm text-gray-700 leading-relaxed">{addToday}</p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 px-4">
              ...
              <Link
                href={`/${locale}`}
                className="flex-1 bg-gradient-to-r from-teal-500 to-pink-400 text-white font-bold py-4 px-6 rounded-xl hover:from-teal-600 hover:to-pink-500 transition-all text-center shadow-md"
              >
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
            <div
              className="bg-gradient-to-r from-teal-500 to-pink-400 h-full rounded-full transition-all duration-300 ease-out"
        ...
        <div>
          ...
          <div className="space-y-4 px-4">
            {optionsArray.map((option, index) => {
              const raw =
                option.text[locale as keyof typeof option.text] || option.text.ko;
              // 답변 문장 안의 "A. " 형태만 제거 (좌측 원형 라벨 A~D는 유지)
              const optionText = raw.replace(/^[ABCD]\.\s*/, '').trim();
              const label = String.fromCharCode(65 + index);
              const colors = [
                'from-teal-50 to-emerald-50 hover:from-teal-100 hover:to-emerald-100 border-teal-200 hover:border-teal-400',
                'from-cyan-50 to-teal-50 hover:from-cyan-100 hover:to-teal-100 border-cyan-200 hover:border-cyan-400',
                'from-pink-50 to-rose-50 hover:from-pink-100 hover:to-rose-100 border-pink-200 hover:border-pink-400',
                'from-fuchsia-50 to-pink-50 hover:from-fuchsia-100 hover:to-pink-100 border-fuchsia-200 hover:border-fuchsia-400',
              ];
              const bgColors = ['bg-teal-600', 'bg-cyan-600', 'bg-pink-600', 'bg-fuchsia-600'];
