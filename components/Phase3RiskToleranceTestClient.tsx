...
import {
  Phase3RiskToleranceQuestion,
  Phase3RiskToleranceResult,
  calculatePhase3RiskToleranceResult,
  calculatePhase3RiskToleranceDomainScores,
} from '@/lib/phase3RiskToleranceData';
...
interface Phase3RiskToleranceTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3RiskToleranceQuestion[];
  results: Phase3RiskToleranceResult[];
...
export default function Phase3RiskToleranceTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3RiskToleranceTestClientProps) {
  const t = useTranslations('phase3RiskToleranceTest');
  ...
  const [result, setResult] = useState<Phase3RiskToleranceResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3RiskToleranceQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3RiskToleranceQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      const scores = calculatePhase3RiskToleranceDomainScores(answersArray);
      setDomainScores(scores);

      const resultType = calculatePhase3RiskToleranceResult(answersArray);
      const potentialResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (potentialResult) {
        setResult(potentialResult);
      }
    }
  };
  ...
  const handleRetake = () => {
    ...
    setResult(null);
    setShowResult(false);
    setDomainScores({ financial: 0, social: 0, physical: 0, career: 0, uncertainty: 0, total: 0 });
  ...
  const handleShareResult = async () => {
    ...
    const resultTitle = result.title[locale as keyof typeof result.title] || '';
    const customShare = result.shareLine[locale as keyof typeof result.shareLine] || '';
    const shareText = `${customShare || t('shareMessages.default', { type: resultTitle })}\n\n${`https://myquizoasis.com${window.location.pathname}`}`;
  ...
  const shareToLine = () => {
    ...
    const shareName = result
      ? (result.shareLine[locale as keyof typeof result.shareLine] || result.title[locale as keyof typeof result.title] || '')
      : '';
    const shareText = result 
      ? encodeURIComponent(shareName || t('shareMessages.line', { type: result.title[locale as keyof typeof result.title] || '' }))
  ...
  const shareToWeChat = async () => {
    ...
    const shareName = result
      ...
      : '';
    const shareText = result 
      ? `${shareName || t('shareMessages.wechat', { type: result.title[locale as keyof typeof result.title] || '' })}\n\n${url}`
  ...
  const shareToWhatsApp = () => {
    ...
    const shareName = result
      ...
      : '';
    const shareText = result 
      ? encodeURIComponent(shareName || t('shareMessages.whatsapp', { type: result.title[locale as keyof typeof result.title] || '' }))
  ...
  const shareToKakao = () => {
    ...
    const shareDescription = result 
      ? (shareName || t('shareMessages.kakao', { type: result.title[locale as keyof typeof result.title] || '' }))
  ...
  const shareToTelegram = () => {
    ...
    const shareName = result
      ...
      : '';
    const shareText = result 
      ? (shareName || t('shareMessages.telegram', { type: result.title[locale as keyof typeof result.title] || '' }))
  ...
  if (showResult && result) {
    ...
    const resultQuote = result.quote[locale as keyof typeof result.quote] || result.quote.ko;
    const resultDescription = result.description[locale as keyof typeof result.description] || result.description.ko;
    const resultRiskLevel = result.riskLevel[locale as keyof typeof result.riskLevel] || result.riskLevel.ko;
    const resultRiskType = result.riskType[locale as keyof typeof result.riskType] || result.riskType.ko;
    const resultRiskAcceptance =
      result.riskAcceptanceLevel[locale as keyof typeof result.riskAcceptanceLevel] || result.riskAcceptanceLevel.ko;
    const resultFinancial =
      result.financialDiagnosis[locale as keyof typeof result.financialDiagnosis] || result.financialDiagnosis.ko;
    const resultSocial =
      result.socialDiagnosis[locale as keyof typeof result.socialDiagnosis] || result.socialDiagnosis.ko;
    const resultPhysical =
      result.physicalDiagnosis[locale as keyof typeof result.physicalDiagnosis] || result.physicalDiagnosis.ko;
    const resultCareer =
      result.careerDiagnosis[locale as keyof typeof result.careerDiagnosis] || result.careerDiagnosis.ko;
    const resultUncertainty =
      result.uncertaintyDiagnosis[locale as keyof typeof result.uncertaintyDiagnosis] || result.uncertaintyDiagnosis.ko;
    ...
    const resultInvestment =
      result.investmentStyle[locale as keyof typeof result.investmentStyle] || result.investmentStyle.ko;
    const resultEntrepreneurship =
      result.entrepreneurshipStyle[locale as keyof typeof result.entrepreneurshipStyle] || result.entrepreneurshipStyle.ko;
    ...
    const resultOneLiner = result.oneLiner[locale as keyof typeof result.oneLiner] || result.oneLiner.ko;

    const domainConfig = [
      { key: 'financial', label: t('ui.financialDomain'), score: domainScores.financial, max: 9 },
      { key: 'social', label: t('ui.socialDomain'), score: domainScores.social, max: 9 },
      { key: 'physical', label: t('ui.physicalDomain'), score: domainScores.physical, max: 6 },
      { key: 'career', label: t('ui.careerDomain'), score: domainScores.career, max: 6 },
      { key: 'uncertainty', label: t('ui.uncertaintyDomain'), score: domainScores.uncertainty, max: 6 },
    ];
    const sortedDomains = [...domainConfig].sort((a, b) => b.score - a.score);
    const maxScore = sortedDomains[0]?.score ?? 0;
    const minScore = sortedDomains[sortedDomains.length - 1]?.score ?? 0;
    const strongestLabels = domainConfig.filter((d) => d.score === maxScore).map((d) => d.label);
    const weakestLabels = domainConfig.filter((d) => d.score === minScore).map((d) => d.label);
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  📈 {t('ui.riskAcceptanceLevel')}
                </h3>
                <p className="text-sm font-bold text-gray-700 leading-relaxed text-center">
                  {resultRiskAcceptance}
                </p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <p className="text-sm font-bold text-gray-700 mb-3 text-center">
                {t('ui.totalScore')}: {domainScores.total}{t('ui.scoreUnit')} ({resultTitleDisplay} {result.emoji})
              </p>
                  ...
                  <div key={domain.key}>
                    <div className="flex justify-between text-sm text-gray-700 mb-1">
                      ...
                      <span className="font-bold">
                        {domain.score}/{domain.max}{t('ui.scoreUnit')}
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2.5">
                      <div
                        className="bg-gradient-to-r from-red-500 to-orange-400 h-2.5 rounded-full transition-all"
                        style={{ width: `${Math.round((domain.score / domain.max) * 100)}%` }}
                      />
                    </div>
                  </div>
                ))}
              ...
              <div className="mt-4 space-y-2 text-sm text-gray-700">
                <p>
                  📌 {t('ui.strongestDomain')}: <span className="font-semibold">{strongestLabels.join(' · ')}</span>
                </p>
                <p>
                  📌 {t('ui.conservativeDomain')}: <span className="font-semibold">{weakestLabels.join(' · ')}</span>
                </p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <div className="space-y-2 text-sm text-gray-700">
                <p><span className="font-semibold">{t('ui.financialDomain')}:</span> {resultFinancial}</p>
                <p><span className="font-semibold">{t('ui.socialDomain')}:</span> {resultSocial}</p>
                <p><span className="font-semibold">{t('ui.physicalDomain')}:</span> {resultPhysical}</p>
                <p><span className="font-semibold">{t('ui.careerDomain')}:</span> {resultCareer}</p>
                <p><span className="font-semibold">{t('ui.uncertaintyDomain')}:</span> {resultUncertainty}</p>
              </div>
            </div>
            ...
            {(resultInvestment || resultEntrepreneurship) && (
              <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  💡 {t('ui.investmentCareerDirection')}
                </h3>
                <div className="space-y-2 text-sm text-gray-700">
                  {resultInvestment && (
                    <p><span className="font-semibold">{t('ui.investmentStyleTitle')}:</span> {resultInvestment}</p>
                  )}
                  {resultEntrepreneurship && (
                    <p><span className="font-semibold">{t('ui.entrepreneurshipStyleTitle')}:</span> {resultEntrepreneurship}</p>
                  )}
                </div>
              </div>
            )}
            ...
            {isType6 && resultCertification && (
              <div className="bg-gradient-to-r from-purple-100 to-pink-100 rounded-xl shadow-lg p-4 mb-3">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  🏆 {t('ui.certificationPhrase')}
                </h3>
                <p className="text-sm font-semibold text-purple-800 leading-relaxed text-center">
                  {resultCertification}
                </p>
              </div>
            )}
