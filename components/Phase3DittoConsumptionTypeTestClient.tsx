...
import {
  Phase3DittoConsumptionTypeQuestion,
  Phase3DittoConsumptionTypeResult,
  calculatePhase3DittoConsumptionTypeResult,
...
interface Phase3DittoConsumptionTypeTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3DittoConsumptionTypeQuestion[];
  results: Phase3DittoConsumptionTypeResult[];
...
export default function Phase3DittoConsumptionTypeTestClient({
  locale,
  slug,
  title,
  description,
  ...
  isLatestTest = false,
  badgeType = null,
}: Phase3DittoConsumptionTypeTestClientProps) {
  const t = useTranslations('phase3DittoConsumptionTypeTest');
  ...
  const [result, setResult] = useState<Phase3DittoConsumptionTypeResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3DittoConsumptionTypeQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3DittoConsumptionTypeQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3DittoConsumptionTypeResult(answersArray);
      const dittoResult = results.find(r => r.type === resultType);

      if (dittoResult) {
        setResult(dittoResult);
      }
    }
  };
  ...
  if (showResult && result) {
    ...
    const resultDittoType =
      result.dittoType[locale as keyof typeof result.dittoType] || result.dittoType.ko;
    const resultMainChannel =
      result.mainChannel[locale as keyof typeof result.mainChannel] || result.mainChannel.ko;
    const resultTriggerCondition =
      result.triggerCondition[locale as keyof typeof result.triggerCondition] || result.triggerCondition.ko;
    const resultStrength =
      result.strength[locale as keyof typeof result.strength] || result.strength.ko;
    const resultTrap =
      result.trap[locale as keyof typeof result.trap] || result.trap.ko;
    const resultBlackHistory =
      result.blackHistory[locale as keyof typeof result.blackHistory] || result.blackHistory.ko;
    const resultSmartStrategy =
      result.smartStrategy[locale as keyof typeof result.smartStrategy] || result.smartStrategy.ko;
    const resultBrandApproach =
      result.brandApproach[locale as keyof typeof result.brandApproach] || result.brandApproach.ko;
    const resultTodayTask =
      result.todayTask[locale as keyof typeof result.todayTask] || result.todayTask.ko;
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultMainChannel}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left w-full">
                  {t('ui.triggerCondition')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">{resultTriggerCondition}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultTrap}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultBlackHistory}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-line">{resultSmartStrategy}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultBrandApproach}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultTodayTask}</p>
              </div>
            </div>
