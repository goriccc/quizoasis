...
import {
  Phase3ShortformAddictionTypeQuestion,
  Phase3ShortformAddictionTypeResult,
  calculatePhase3ShortformAddictionTypeResult,
...
interface Phase3ShortformAddictionTypeTestClientProps {
  ...
  title: string;
  description: string;
  questions: Phase3ShortformAddictionTypeQuestion[];
  results: Phase3ShortformAddictionTypeResult[];
...
export default function Phase3ShortformAddictionTypeTestClient({
  locale,
  slug,
  title,
  description,
  ...
  isLatestTest = false,
  badgeType = null,
}: Phase3ShortformAddictionTypeTestClientProps) {
  const t = useTranslations('phase3ShortformAddictionTypeTest');
  ...
  const [result, setResult] = useState<Phase3ShortformAddictionTypeResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3ShortformAddictionTypeQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3ShortformAddictionTypeQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3ShortformAddictionTypeResult(answersArray);
      const addictionResult = results.find(r => r.type === resultType);

      if (addictionResult) {
        setResult(addictionResult);
      }
    }
  };
  ...
  const handleShareResult = async () => {
    ...
    const shareText = `${t('shareMessages.default', { level: result.shareLevel, typeName })}\n\n${`https://myquizoasis.com${window.location.pathname}`}`;
  ...
  const shareToLine = () => {
    ...
    const shareText = result
      ? encodeURIComponent(t('shareMessages.line', { level: result.shareLevel, typeName }))
  ...
  const shareToWeChat = async () => {
    ...
    const typeName = result
      ...
      : '';
    const shareText = result
      ? `${t('shareMessages.wechat', { level: result.shareLevel, typeName })}\n\n${url}`
  ...
  const shareToWhatsApp = () => {
    ...
    const typeName = result
      ...
      : '';
    const shareText = result
      ? encodeURIComponent(t('shareMessages.whatsapp', { level: result.shareLevel, typeName }))
  ...
  const shareToKakao = () => {
    ...
    const shareDescription = result
      ? t('shareMessages.kakao', { level: result.shareLevel, typeName })
  ...
  const shareToTelegram = () => {
    ...
    const typeName = result
      ...
      : '';
    const shareText = result
      ? t('shareMessages.telegram', { level: result.shareLevel, typeName })
  ...
  if (showResult && result) {
    ...
    const resultAddictionLevel =
      result.addictionLevel[locale as keyof typeof result.addictionLevel] || result.addictionLevel.ko;
          ...
          <div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {resultSymptoms.split(/[,、，]/).map((char, index) => (
                    <span
                      key={index}
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-base text-gray-700 leading-relaxed">{resultPrescription}</p>
              </div>
            </div>
