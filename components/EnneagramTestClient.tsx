...
import { EnneagramQuestion, EnneagramResult, calculateEnneagramResult } from '@/lib/enneagramData';
...
interface EnneagramTestClientProps {
  locale: string;
  slug: string;
  title: string | Record<string, string>;
  description: string | Record<string, string>;
  questions: EnneagramQuestion[];
  results: EnneagramResult[];
...
export default function EnneagramTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false
}: EnneagramTestClientProps) {
  const t = useTranslations('enneagramTest');
  const tGlobal = useTranslations();
  // title과 description을 locale에 맞게 변환
  const displayTitle = typeof title === 'object' 
    ? (title[locale as keyof typeof title] || title.ko || '9가지 성격! 에니어그램 테스트')
    : title;
  const displayDescription = typeof description === 'object'
    ? (description[locale as keyof typeof description] || description.ko || '나를 움직이는 내면의 힘은 무엇일까요?')
    : description;
  ...
  const [shuffledQuestions, setShuffledQuestions] = useState<EnneagramQuestion[]>(questions);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: EnneagramQuestion[]) => {
  ...
  const handleAnswer = (scores: any) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculateEnneagramResult(newAnswers);
      const enneagramResult = results.find(r => r.type === resultType);
      ...
      if (enneagramResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(enneagramResult.type, locale);
        const loadStartTime = Date.now();
            console.log('🔮 [시작] 에니어그램 결과:', enneagramResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  const calculateResult = (finalAnswers: any[]) => {
    const resultType = calculateEnneagramResult(finalAnswers);
    const enneagramResult = results.find(r => r.type === resultType);
      ...
      <div className="min-h-screen bg-white">
        <div className="max-w-4xl mx-auto">
          <div className="relative w-full overflow-hidden mb-3" style={{ aspectRatio: '680/384' }}>
            <Image
              src={getThumbnailUrl(thumbnail || 'phase2_test_145_enneagram.jpg')}
              alt={displayTitle}
              fill
  ...
  // Type 번호에서 Type 정보 추출하는 함수
  const getTypeInfo = (matchText: string) => {
    // "Type 7" 또는 "Type7" 형식에서 번호 추출
    const typeMatch = matchText.match(/Type\s*(\d+)/i);
    if (!typeMatch) return null;
    
    const typeNumber = typeMatch[1];
    const typeKey = `Type${typeNumber}`;
    const matchedResult = results.find(r => r.type === typeKey);
    
    if (!matchedResult) return null;
    
    const matchedTitle = matchedResult.title[locale as keyof typeof matchedResult.title] || matchedResult.title.ko;
    const matchedEmoji = matchedResult.emoji;
    
    // 원본 텍스트에서 설명 부분 추출 (괄호 안의 내용)
    const descriptionMatch = matchText.match(/\(([^)]+)\)/);
  ...
  if (showResult && result) {
    ...
    const resultCoreValue = result.coreValue[locale as keyof typeof result.coreValue] || result.coreValue.ko;
    const resultCaution = result.caution[locale as keyof typeof result.caution] || result.caution.ko;
    const resultIdealMatchText = result.idealMatch[locale as keyof typeof result.idealMatch] || result.idealMatch.ko;
    const resultWorstMatchText = result.worstMatch[locale as keyof typeof result.worstMatch] || result.worstMatch.ko;
    
    // Type 정보 추출
    const idealMatchInfo = getTypeInfo(resultIdealMatchText);
    const worstMatchInfo = getTypeInfo(resultWorstMatchText);
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              {idealMatchInfo ? (
                <p className="text-sm text-gray-700 leading-relaxed">
                  {idealMatchInfo.emoji} {idealMatchInfo.title}
                  {idealMatchInfo.description && ` - ${idealMatchInfo.description}`}
                </p>
              ) : (
                <p className="text-sm text-gray-700 leading-relaxed">
                  {resultIdealMatchText}
                </p>
              )}
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              {worstMatchInfo ? (
                <p className="text-sm text-gray-700 leading-relaxed">
                  {worstMatchInfo.emoji} {worstMatchInfo.title}
                  {worstMatchInfo.description && ` - ${worstMatchInfo.description}`}
                </p>
              ) : (
                <p className="text-sm text-gray-700 leading-relaxed">
                  {resultWorstMatchText}
                </p>
              )}
            </div>
