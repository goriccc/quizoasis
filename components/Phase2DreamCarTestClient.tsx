...
import { Phase2DreamCarQuestion, Phase2DreamCarResult, calculatePhase2DreamCarResult } from '@/lib/phase2_dream_car_data';
...
interface Phase2DreamCarTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase2DreamCarQuestion[];
  results: Phase2DreamCarResult[];
...
export default function Phase2DreamCarTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  playCount = 0,
  similarTests = []
}: Phase2DreamCarTestClientProps) {
  const t = useTranslations('phase2DreamCarTest');
  ...
  const [result, setResult] = useState<Phase2DreamCarResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2DreamCarQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase2DreamCarQuestion[]) => {
  ...
  const handleAnswer = (selectedTypes: string[]) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase2DreamCarResult(newAnswers);
      const dreamCarResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (dreamCarResult) {
        setResult(dreamCarResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (dreamCarResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(dreamCarResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 드림카 결과:', dreamCarResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const resultRecommendedModels = result.recommendedModels[locale as keyof typeof result.recommendedModels] || result.recommendedModels.ko;
            ...
            <div className="space-y-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  🚘 {t('ui.recommendedModels')}
                </h3>
                <div className="flex flex-wrap gap-2">
                  {resultRecommendedModels.split(/[,、，]/).map((model, index) => (
                    <span
                      key={index}
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {resultKeywords.split(/[,、，]/).map((keyword, index) => (
                    <span
                      key={index}
