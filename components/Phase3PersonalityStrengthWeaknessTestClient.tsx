...
import {
  Phase3PersonalityStrengthWeaknessQuestion,
  Phase3PersonalityStrengthWeaknessResult,
  calculatePhase3PersonalityStrengthWeaknessResult,
...
interface Phase3PersonalityStrengthWeaknessTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3PersonalityStrengthWeaknessQuestion[];
  results: Phase3PersonalityStrengthWeaknessResult[];
...
export default function Phase3PersonalityStrengthWeaknessTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3PersonalityStrengthWeaknessTestClientProps) {
  const t = useTranslations('phase3PersonalityStrengthWeaknessTest');
  ...
  const [result, setResult] = useState<Phase3PersonalityStrengthWeaknessResult | null>(null);
  const [totalScore, setTotalScore] = useState<number | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3PersonalityStrengthWeaknessQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3PersonalityStrengthWeaknessQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      const sumScore = answersArray.reduce((a, b) => a + b, 0);
      setTotalScore(sumScore);

      const resultType = calculatePhase3PersonalityStrengthWeaknessResult(answersArray);
      const resolved = results.find(r => r.type === resultType);

      if (resolved) {
        setResult(resolved);
      }

      if (resolved && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(resolved.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 성격 팩폭 결과:', resolved.type, '→ 검색 키워드:', keywords[0]);
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="px-4">
            ...
            <div className="text-gray-600 mb-6 leading-relaxed text-center space-y-4">
              ...
              <p className="font-semibold text-gray-800">{t('startMessage.line6')}</p>
            </div>
  ...
  if (showResult && result) {
    ...
    const resultPersonalityLabel =
      result.personalityLabel[locale as keyof typeof result.personalityLabel] || result.personalityLabel.ko;
    const resultStrengthRoast =
      result.strengthRoast[locale as keyof typeof result.strengthRoast] || result.strengthRoast.ko;
    const resultWeaknessRoast =
      result.weaknessRoast[locale as keyof typeof result.weaknessRoast] || result.weaknessRoast.ko;
    const resultStrengthKw =
      result.strengthKeywords[locale as keyof typeof result.strengthKeywords] || result.strengthKeywords.ko;
    const resultWeaknessKw =
      result.weaknessKeywords[locale as keyof typeof result.weaknessKeywords] || result.weaknessKeywords.ko;
          ...
          <div>
            <div className="text-center mb-3 bg-white rounded-2xl shadow-lg p-4 md:p-5">
              ...
              {totalScore !== null && (
                <p className="text-sm text-gray-500 mb-3">
                  {t('ui.totalScore', { score: totalScore })}
                </p>
              )}
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                🎭 {t('ui.personalityType')}
              </h3>
              <p className="text-xl font-bold text-purple-600 text-center">{resultPersonalityLabel}</p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  ✨ {t('ui.strengthKeywords')}
                </h3>
                <div className="flex flex-wrap justify-center gap-2">
                  {resultStrengthKw.split(/[,、，]/).map((char, index) => (
                    <span
                      key={index}
                      className="inline-block bg-gradient-to-r from-amber-50 to-yellow-100 text-amber-900 px-3 py-1.5 rounded-full text-sm font-medium"
                    >
                      {char.trim()}
                    </span>
                  ))}
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  ⚡ {t('ui.weaknessKeywords')}
                </h3>
                <div className="flex flex-wrap justify-center gap-2">
                  {resultWeaknessKw.split(/[,、，]/).map((char, index) => (
                    <span
                      key={index}
                      className="inline-block bg-gradient-to-r from-slate-100 to-gray-200 text-gray-800 px-3 py-1.5 rounded-full text-sm font-medium"
                    >
                      {char.trim()}
                    </span>
                  ))}
            ...
            <div className="space-y-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4 text-left">
                <h3 className="text-base font-bold text-gray-800 mb-2">🌟 {t('ui.strengthRoast')}</h3>
                <p className="text-base text-gray-700 leading-relaxed whitespace-pre-line">{resultStrengthRoast}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4 text-left">
                <h3 className="text-base font-bold text-gray-800 mb-2">💥 {t('ui.weaknessRoast')}</h3>
                <p className="text-base text-gray-700 leading-relaxed whitespace-pre-line">{resultWeaknessRoast}</p>
              </div>
            </div>
