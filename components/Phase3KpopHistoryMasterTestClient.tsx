...
import {
  Phase3KpopHistoryMasterQuestion,
  Phase3KpopHistoryMasterResult,
  Phase3KpopHistoryMasterOption,
  calculatePhase3KpopHistoryMasterResult,
...
interface Phase3KpopHistoryMasterTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3KpopHistoryMasterQuestion[];
  results: Phase3KpopHistoryMasterResult[];
...
export default function Phase3KpopHistoryMasterTestClient({
  locale,
  slug,
  title,
  description,
  ...
  isLatestTest = false,
  badgeType = null,
}: Phase3KpopHistoryMasterTestClientProps) {
  const t = useTranslations('phase3KpopHistoryMasterTest');
  ...
  const [result, setResult] = useState<Phase3KpopHistoryMasterResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3KpopHistoryMasterQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3KpopHistoryMasterQuestion[]) => {
  ...
  const handleAnswer = (option: Phase3KpopHistoryMasterOption) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      const resultType = calculatePhase3KpopHistoryMasterResult(answersArray);
  ...
  if (showResult && result) {
    ...
    const resultHistoryGrade = result.historyGrade[locale as keyof typeof result.historyGrade] || result.historyGrade.ko;
