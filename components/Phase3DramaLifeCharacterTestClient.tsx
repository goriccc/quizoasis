...
import {
  Phase3DramaLifeCharacterQuestion,
  Phase3DramaLifeCharacterResult,
  calculatePhase3DramaLifeCharacterResult,
...
interface Phase3DramaLifeCharacterTestClientProps {
  ...
  title: string;
  description: string;
  questions: Phase3DramaLifeCharacterQuestion[];
  results: Phase3DramaLifeCharacterResult[];
...
export default function Phase3DramaLifeCharacterTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3DramaLifeCharacterTestClientProps) {
  const t = useTranslations('phase3DramaLifeCharacterTest');
  ...
  const [result, setResult] = useState<Phase3DramaLifeCharacterResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3DramaLifeCharacterQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3DramaLifeCharacterQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3DramaLifeCharacterResult(answersArray);
      const dramaResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (dramaResult) {
        setResult(dramaResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (dramaResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(dramaResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 드라마 캐릭터 결과:', dramaResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const characterTypeLabel =
      result.characterTypeLabel[locale as keyof typeof result.characterTypeLabel] || result.characterTypeLabel.ko;
    const narrativeKeywords =
      result.narrativeKeywords[locale as keyof typeof result.narrativeKeywords] || result.narrativeKeywords.ko;
    const dramaQuote = result.dramaQuote[locale as keyof typeof result.dramaQuote] || result.dramaQuote.ko;
    const narrativePosition =
      result.narrativePosition[locale as keyof typeof result.narrativePosition] || result.narrativePosition.ko;
    const realLifeStory =
      result.realLifeStory[locale as keyof typeof result.realLifeStory] || result.realLifeStory.ko;
    const genreMatch = result.genreMatch[locale as keyof typeof result.genreMatch] || result.genreMatch.ko;
    const characterMessage =
      result.characterMessage[locale as keyof typeof result.characterMessage] || result.characterMessage.ko;
          ...
          <div>
            ...
            <div className="space-y-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-base text-indigo-800 font-medium leading-relaxed">{characterTypeLabel}</p>
              </div>

              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  ⭐ {t('ui.narrativeKeywords')}
                </h3>
                <div className="flex flex-wrap justify-center gap-2">
                  {narrativeKeywords.split(/[,，、]/).map((char, index) => (
                    <span
                      key={index}
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-base text-gray-800 font-medium italic leading-relaxed">{dramaQuote}</p>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="bg-white rounded-xl shadow-lg p-4">
                  <h3 className="text-base font-bold text-gray-800 mb-2">
                    📍 {t('ui.narrativePosition')}
                  </h3>
                  <p className="text-sm text-gray-700 leading-relaxed">{narrativePosition}</p>
                </div>
                <div className="bg-white rounded-xl shadow-lg p-4">
                  <h3 className="text-base font-bold text-gray-800 mb-2">
                    🎬 {t('ui.genreMatch')}
                  </h3>
                  <p className="text-sm text-gray-700 leading-relaxed">{genreMatch}</p>
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{realLifeStory}</p>
              </div>

              <div className="bg-white rounded-xl shadow-lg p-4 border-2 border-amber-100">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  🎬 {t('ui.characterMessage')}
                </h3>
                <p className="text-base text-amber-900 font-medium leading-relaxed">{characterMessage}</p>
              </div>
            </div>
