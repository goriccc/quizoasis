...
import { Phase2BodySignalQuestion, Phase2BodySignalResult, calculatePhase2BodySignalResult } from '@/lib/phase2_body_signal_data';
...
interface Phase2BodySignalTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase2BodySignalQuestion[];
  results: Phase2BodySignalResult[];
...
export default function Phase2BodySignalTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false
}: Phase2BodySignalTestClientProps) {
  const t = useTranslations('phase2BodySignalTest');
  ...
  const [result, setResult] = useState<Phase2BodySignalResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2BodySignalQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase2BodySignalQuestion[]) => {
  ...
  const handleAnswer = (selectedTypes: string[]) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase2BodySignalResult(newAnswers);
      const bodySignalResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (bodySignalResult) {
        setResult(bodySignalResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (bodySignalResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(bodySignalResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 몸 신호 결과:', bodySignalResult.type, '→ 검색 키워드:', keywords[0]);
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {resultSymptoms.split(/[,、，]/).map((symptom, index) => (
                    <span
                      key={index}
                      className="inline-block px-4 py-2 bg-gradient-to-r from-red-100 to-orange-100 text-red-700 font-medium rounded-full text-sm border border-red-200"
                    >
                      {symptom.trim()}
                    </span>
                  ))}
        ...
        <div>
          ...
          <div className="space-y-4 px-4">
            {optionsArray.map((option, index) => {
              ...
              const bgColors = ['bg-purple-600', 'bg-pink-600', 'bg-blue-600'];
