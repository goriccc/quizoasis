...
import {
  Phase3LeadershipStyleQuestion,
  Phase3LeadershipStyleResult,
  calculatePhase3LeadershipStyleResult,
...
interface Phase3LeadershipStyleTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3LeadershipStyleQuestion[];
  results: Phase3LeadershipStyleResult[];
...
export default function Phase3LeadershipStyleTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3LeadershipStyleTestClientProps) {
  const t = useTranslations('phase3LeadershipStyleTest');
  ...
  const [result, setResult] = useState<Phase3LeadershipStyleResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3LeadershipStyleQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3LeadershipStyleQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3LeadershipStyleResult(answersArray);
  ...
  if (showResult && result) {
    ...
    const resultLeadershipStyle = result.leadershipStyle[locale as keyof typeof result.leadershipStyle] || result.leadershipStyle.ko;
    const resultLeadershipKeywords = result.leadershipKeywords[locale as keyof typeof result.leadershipKeywords] || result.leadershipKeywords.ko;
    const resultTeamRole = result.teamRole[locale as keyof typeof result.teamRole] || result.teamRole.ko;
    const resultStrength = result.strength[locale as keyof typeof result.strength] || result.strength.ko;
    const resultWeakness = result.weakness[locale as keyof typeof result.weakness] || result.weakness.ko;
    const resultGoodEnvironment = result.goodEnvironment[locale as keyof typeof result.goodEnvironment] || result.goodEnvironment.ko;
    const resultGoodMatch = result.goodMatch[locale as keyof typeof result.goodMatch] || result.goodMatch.ko;
    const resultCoverLetterPhrase = result.coverLetterPhrase[locale as keyof typeof result.coverLetterPhrase] || result.coverLetterPhrase.ko;
    const resultCoverLetterMaterial = result.coverLetterMaterial[locale as keyof typeof result.coverLetterMaterial] || result.coverLetterMaterial.ko;
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  👑 {t('ui.leadershipStyle')}
                </h3>
                <p className="text-base font-semibold text-purple-700 text-center">
                  {resultLeadershipStyle}
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  ⭐ {t('ui.leadershipKeywords')}
                </h3>
                <div className="flex flex-wrap justify-center gap-2">
                  {resultLeadershipKeywords.split(/[·]/).map((keyword, index) => (
                    <span
                      key={index}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  🌱 {t('ui.goodEnvironment')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed text-left">
                  {resultGoodEnvironment}
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                📝 {t('ui.coverLetterPhrase')}
              </h3>
              <p className="text-sm text-gray-700 leading-relaxed text-left">
                &ldquo;{resultCoverLetterPhrase}&rdquo;
              </p>
            </div>

            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                💡 {t('ui.coverLetterMaterial')}
              </h3>
              <p className="text-sm text-gray-700 leading-relaxed text-left">
                {resultCoverLetterMaterial}
              </p>
            </div>
