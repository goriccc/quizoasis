...
import {
  Phase3CafeWorkGradeQuestion,
  Phase3CafeWorkGradeResult,
  calculatePhase3CafeWorkGradeResult,
...
interface Phase3CafeWorkGradeTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3CafeWorkGradeQuestion[];
  results: Phase3CafeWorkGradeResult[];
...
export default function Phase3CafeWorkGradeTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3CafeWorkGradeTestClientProps) {
  const t = useTranslations('phase3CafeWorkGradeTest');
  ...
  const [result, setResult] = useState<Phase3CafeWorkGradeResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3CafeWorkGradeQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3CafeWorkGradeQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3CafeWorkGradeResult(answersArray);
  ...
  if (showResult && result) {
    ...
    const resultCafeGrade = result.cafeGrade[locale as keyof typeof result.cafeGrade] || result.cafeGrade.ko;
      ...
      <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-blue-50">
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            ...
            {result.sections.slice(1).map((section, index) => {
