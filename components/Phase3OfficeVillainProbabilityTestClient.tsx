...
import {
  Phase3OfficeVillainProbabilityQuestion,
  Phase3OfficeVillainProbabilityResult,
  calculatePhase3OfficeVillainProbabilityResult,
...
interface Phase3OfficeVillainProbabilityTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3OfficeVillainProbabilityQuestion[];
  results: Phase3OfficeVillainProbabilityResult[];
...
export default function Phase3OfficeVillainProbabilityTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3OfficeVillainProbabilityTestClientProps) {
  const t = useTranslations('phase3OfficeVillainProbabilityTest');
  ...
  const [result, setResult] = useState<Phase3OfficeVillainProbabilityResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3OfficeVillainProbabilityQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3OfficeVillainProbabilityQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3OfficeVillainProbabilityResult(answersArray);
