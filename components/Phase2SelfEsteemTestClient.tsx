...
import { Phase2SelfEsteemQuestion, Phase2SelfEsteemResult, calculatePhase2SelfEsteemResult } from '@/lib/phase2SelfEsteemData';
...
interface Phase2SelfEsteemTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase2SelfEsteemQuestion[];
  results: Phase2SelfEsteemResult[];
...
export default function Phase2SelfEsteemTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  playCount = 0,
  similarTests = []
}: Phase2SelfEsteemTestClientProps) {
  const t = useTranslations('phase2SelfEsteemTest');
  ...
  const [result, setResult] = useState<Phase2SelfEsteemResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2SelfEsteemQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase2SelfEsteemQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase2SelfEsteemResult(answersArray);
      const selfEsteemResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (selfEsteemResult) {
        setResult(selfEsteemResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (selfEsteemResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(selfEsteemResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 자존감 결과:', selfEsteemResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  const handleShareResult = async () => {
    ...
    const resultLevel = (result.selfEsteemLevel[locale as keyof typeof result.selfEsteemLevel] || result.selfEsteemLevel.ko).match(/Lv\.\s*\d+/)?.[0] || result.selfEsteemLevel[locale as keyof typeof result.selfEsteemLevel] || result.selfEsteemLevel.ko;
  ...
  const shareToLine = () => {
    ...
    const resultLevel = result ? ((result.selfEsteemLevel[locale as keyof typeof result.selfEsteemLevel] || result.selfEsteemLevel.ko).match(/Lv\.\s*\d+/)?.[0] || result.selfEsteemLevel[locale as keyof typeof result.selfEsteemLevel] || result.selfEsteemLevel.ko) : '';
    const shareText = result 
      ? encodeURIComponent(t('shareMessages.line', { level: resultLevel, type: resultTitle }))
  ...
  const shareToWeChat = async () => {
    ...
    const resultLevel = result ? ((result.selfEsteemLevel[locale as keyof typeof result.selfEsteemLevel] || result.selfEsteemLevel.ko).match(/Lv\.\s*\d+/)?.[0] || result.selfEsteemLevel[locale as keyof typeof result.selfEsteemLevel] || result.selfEsteemLevel.ko) : '';
    const shareText = result 
      ? `${t('shareMessages.wechat', { level: resultLevel, type: resultTitle })}\n\n${url}`
  ...
  const shareToWhatsApp = () => {
    ...
    const resultLevel = result ? ((result.selfEsteemLevel[locale as keyof typeof result.selfEsteemLevel] || result.selfEsteemLevel.ko).match(/Lv\.\s*\d+/)?.[0] || result.selfEsteemLevel[locale as keyof typeof result.selfEsteemLevel] || result.selfEsteemLevel.ko) : '';
    const shareText = result 
      ? encodeURIComponent(t('shareMessages.whatsapp', { level: resultLevel, type: resultTitle }))
  ...
  const shareToKakao = () => {
    ...
    const shareDescription = result 
      ? t('shareMessages.kakao', { level: resultLevel, type: resultTitle })
  ...
  const shareToTelegram = () => {
    ...
    const resultLevel = result ? ((result.selfEsteemLevel[locale as keyof typeof result.selfEsteemLevel] || result.selfEsteemLevel.ko).match(/Lv\.\s*\d+/)?.[0] || result.selfEsteemLevel[locale as keyof typeof result.selfEsteemLevel] || result.selfEsteemLevel.ko) : '';
    const shareText = result 
      ? t('shareMessages.telegram', { level: resultLevel, type: resultTitle })
  ...
  if (showResult && result) {
    ...
    const resultSelfEsteemLevel = result.selfEsteemLevel[locale as keyof typeof result.selfEsteemLevel] || result.selfEsteemLevel.ko;
    ...
    const resultRecommendedActivity = result.recommendedActivity[locale as keyof typeof result.recommendedActivity] || result.recommendedActivity.ko;
    
    // 레벨 추출 (예: "Lv. 10 (돌봄 필요)" -> "Lv. 10")
    const resultLevel = resultSelfEsteemLevel.match(/Lv\.\s*\d+/)?.[0] || resultSelfEsteemLevel;
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-3">
                  💖 {t('ui.recommendedActivity')}
                </h3>
                <div className="flex flex-wrap justify-center gap-2">
                  {resultRecommendedActivity.split(/[,、，]/).map((activity, index) => (
                    <span
                      key={index}
                      className="inline-block bg-gradient-to-r from-pink-100 to-purple-100 text-pink-800 px-4 py-2 rounded-full text-sm font-medium"
                    >
                      {activity.trim()}
                    </span>
                  ))}
