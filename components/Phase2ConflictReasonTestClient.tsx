...
import { Phase2ConflictReasonQuestion, Phase2ConflictReasonResult, calculatePhase2ConflictReasonResult } from '@/lib/phase2_conflict_reason_data';
...
interface Phase2ConflictReasonTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase2ConflictReasonQuestion[];
  results: Phase2ConflictReasonResult[];
...
export default function Phase2ConflictReasonTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  playCount = 0,
  similarTests = []
}: Phase2ConflictReasonTestClientProps) {
  const t = useTranslations('phase2ConflictReasonTest');
  ...
  const [result, setResult] = useState<Phase2ConflictReasonResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2ConflictReasonQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase2ConflictReasonQuestion[]) => {
  ...
  const handleAnswer = (selectedTypes: string[]) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase2ConflictReasonResult(newAnswers);
      const conflictReasonResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (conflictReasonResult) {
        setResult(conflictReasonResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (conflictReasonResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(conflictReasonResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 갈등 원인 결과:', conflictReasonResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const resultConflictCauses = result.conflictCauses[locale as keyof typeof result.conflictCauses] || result.conflictCauses.ko;
    const resultSolution = result.solution[locale as keyof typeof result.solution] || result.solution.ko;
            ...
            <div className="space-y-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  ⚠️ {t('ui.conflictCauses')}
                </h3>
                <div className="flex flex-wrap gap-2">
                  {resultConflictCauses.split(/[,、，]/).map((cause, index) => (
                    <span
                      key={index}
