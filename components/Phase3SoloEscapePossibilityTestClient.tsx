...
import {
  Phase3SoloEscapePossibilityQuestion,
  Phase3SoloEscapePossibilityResult,
  calculatePhase3SoloEscapePossibilityResult,
...
interface Phase3SoloEscapePossibilityTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3SoloEscapePossibilityQuestion[];
  results: Phase3SoloEscapePossibilityResult[];
...
export default function Phase3SoloEscapePossibilityTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3SoloEscapePossibilityTestClientProps) {
  const t = useTranslations('phase3SoloEscapePossibilityTest');
  ...
  const [result, setResult] = useState<Phase3SoloEscapePossibilityResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3SoloEscapePossibilityQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3SoloEscapePossibilityQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3SoloEscapePossibilityResult(answersArray);
      const soloResult = results.find(r => r.type === resultType);
      
      if (soloResult) {
        setResult(soloResult);
      }
    }
  };
  ...
  const handleShareResult = async () => {
    ...
    const shareText = `${t('shareMessages.default', { percent: result.sharePercent, type: shareName })}\n\n${`https://myquizoasis.com${window.location.pathname}`}`;
  ...
  const shareToLine = () => {
    ...
    const shareText = result 
      ? encodeURIComponent(t('shareMessages.line', { percent: result.sharePercent, type: shareName }))
  ...
  const shareToWeChat = async () => {
    ...
    const shareName = result ? (result.shareTypeName[locale as keyof typeof result.shareTypeName] || result.shareTypeName.ko) : '';
    const shareText = result 
      ? `${t('shareMessages.wechat', { percent: result.sharePercent, type: shareName })}\n\n${url}`
  ...
  const shareToWhatsApp = () => {
    ...
    const shareName = result ? (result.shareTypeName[locale as keyof typeof result.shareTypeName] || result.shareTypeName.ko) : '';
    const shareText = result 
      ? encodeURIComponent(t('shareMessages.whatsapp', { percent: result.sharePercent, type: shareName }))
  ...
  const shareToKakao = () => {
    ...
    const shareDescription = result 
      ? t('shareMessages.kakao', { percent: result.sharePercent, type: shareName })
  ...
  const shareToTelegram = () => {
    ...
    const shareName = result ? (result.shareTypeName[locale as keyof typeof result.shareTypeName] || result.shareTypeName.ko) : '';
    const shareText = result 
      ? t('shareMessages.telegram', { percent: result.sharePercent, type: shareName })
          ...
          <div className="px-4">
            ...
            <div className="flex justify-center mb-4">
              <button
                ...
                className="bg-gradient-to-r from-pink-500 to-amber-400 hover:from-pink-600 hover:to-amber-500 text-white font-bold py-4 px-8 rounded-full shadow-lg transform hover:scale-105 transition-all duration-200"
              >
      ...
      <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-2xl p-8 max-w-md w-full mx-4 text-center shadow-2xl">
          ...
          <button
            ...
            className="w-full bg-gradient-to-r from-pink-500 to-amber-400 text-white py-4 px-6 rounded-xl text-xl font-bold hover:from-pink-600 hover:to-amber-500 transition-all duration-300 shadow-lg"
          >
  ...
  if (showResult && result) {
    ...
    const resultCurrentStatus =
      result.currentStatus[locale as keyof typeof result.currentStatus] || result.currentStatus.ko;
    const resultRealReason =
      result.realReason[locale as keyof typeof result.realReason] || result.realReason.ko;
    const resultPrescription =
      result.prescription[locale as keyof typeof result.prescription] || result.prescription.ko;
    const resultEscapeTiming =
      result.escapeTiming[locale as keyof typeof result.escapeTiming] || result.escapeTiming.ko;
    const resultGoodMatch = result.goodMatch[locale as keyof typeof result.goodMatch] || result.goodMatch.ko;
    const resultTrapToAvoid =
      result.trapToAvoid[locale as keyof typeof result.trapToAvoid] || result.trapToAvoid.ko;

    return (
      <div className="min-h-screen bg-gradient-to-br from-pink-50 via-yellow-50 to-rose-50">
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            <div className="text-center mb-3 bg-white rounded-2xl shadow-lg p-4 md:p-5 border border-pink-100/60">
              <h2 className="text-xl font-bold text-gray-800 mb-3">
                {tGlobal('mbti.yourResult')}
              </h2>
              <div className="text-6xl mb-3">{result.emoji}</div>
              <p className="text-sm font-medium text-rose-600 mb-1">{t('ui.escapePossibilityLabel')}</p>
              <p className="text-4xl md:text-5xl font-black text-rose-500 mb-3 tabular-nums">
                {result.sharePercent}%
              </p>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3 border border-pink-100/50">
              <h3 className="text-base font-bold text-gray-800 mb-2">
                📍 {t('ui.currentStatus')}
              </h3>
              <p className="text-sm text-gray-700 leading-relaxed">{resultCurrentStatus}</p>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3 border border-amber-100/60">
              ...
              <p className="text-sm text-gray-700 leading-relaxed">{resultRealReason}</p>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3 border border-yellow-100/60">
              <h3 className="text-base font-bold text-gray-800 mb-2">
                💘 {t('ui.escapeTiming')}
              </h3>
              <p className="text-sm text-gray-700 leading-relaxed">{resultEscapeTiming}</p>
            </div>

            <div className="grid grid-cols-2 gap-2 sm:gap-3 mb-3 items-stretch">
              <div className="bg-white rounded-xl shadow-lg p-3 sm:p-4 border border-pink-100/50 min-w-0 h-full flex flex-col">
                <h3 className="text-sm sm:text-base font-bold text-gray-800 mb-2 sm:mb-3 break-words">
                  💖 {t('ui.goodMatch')}
                </h3>
                  ...
                  {(() => {
                    ...
                    if (matchType) {
                      ...
                      if (matchResult) {
                        ...
                        return (
                          <div className="bg-gradient-to-r from-pink-100 to-amber-100 rounded-lg px-3 py-2">
                            <div className="flex items-start gap-2 sm:gap-2.5 min-w-0">
                              <span className="text-lg sm:text-xl flex-shrink-0">{matchResult.emoji}</span>
                              <span className="text-xs sm:text-sm font-medium text-gray-800 break-words leading-snug">{matchTitle}</span>
                            </div>
                          </div>
                        );
                      }
              ...
              <div className="bg-white rounded-xl shadow-lg p-3 sm:p-4 border border-orange-100/60 min-w-0 h-full flex flex-col">
                <h3 className="text-sm sm:text-base font-bold text-gray-800 mb-2 sm:mb-3 break-words">
                  💔 {t('ui.trapToAvoid')}
                </h3>
                <div className="space-y-2 flex-1 min-w-0">
                  {(() => {
                    const matchType = resultTrapToAvoid.match(/Type\s*(\d+)/i);
                    if (matchType) {
                      const typeNumber = matchType[1];
                      const matchTypeKey = `Type${typeNumber}`;
                      const matchResult = results.find(r => r.type === matchTypeKey);
            ...
            <div className="flex flex-col sm:flex-row gap-4 px-4">
              ...
              <Link
                href={`/${locale}`}
                className="flex-1 bg-gradient-to-r from-pink-500 to-amber-400 text-white font-bold py-4 px-6 rounded-xl hover:from-pink-600 hover:to-amber-500 transition-all text-center shadow-md"
              >
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
            <div
              className="bg-gradient-to-r from-pink-500 to-amber-400 h-full rounded-full transition-all duration-300 ease-out"
        ...
        <div>
          ...
          <div className="space-y-4 px-4">
            {optionsArray.map((option, index) => {
              ...
              const colors = [
                'from-rose-50 to-rose-100 hover:from-rose-100 hover:to-rose-200 border-rose-200 hover:border-rose-400',
                'from-pink-50 to-pink-100 hover:from-pink-100 hover:to-pink-200 border-pink-200 hover:border-pink-400',
                'from-red-50 to-rose-100 hover:from-red-100 hover:to-rose-200 border-red-200 hover:border-red-400',
                'from-fuchsia-50 to-pink-100 hover:from-fuchsia-100 hover:to-pink-200 border-fuchsia-200 hover:border-fuchsia-400',
              ];
              const bgColors = ['bg-rose-600', 'bg-pink-600', 'bg-red-600', 'bg-fuchsia-700'];
