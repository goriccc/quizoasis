...
import {
  Phase3FirstImpressionColorScannerQuestion,
  Phase3FirstImpressionColorScannerResult,
  calculatePhase3FirstImpressionColorScannerResult,
...
interface Phase3FirstImpressionColorScannerTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3FirstImpressionColorScannerQuestion[];
  results: Phase3FirstImpressionColorScannerResult[];
...
export default function Phase3FirstImpressionColorScannerTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3FirstImpressionColorScannerTestClientProps) {
  const t = useTranslations('phase3FirstImpressionColorScannerTest');
  ...
  const [result, setResult] = useState<Phase3FirstImpressionColorScannerResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3FirstImpressionColorScannerQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3FirstImpressionColorScannerQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3FirstImpressionColorScannerResult(answersArray);
  ...
  if (showResult && result) {
    ...
    const resultFirstImpressionColor =
      result.firstImpressionColor[locale as keyof typeof result.firstImpressionColor] || result.firstImpressionColor.ko;
    const resultColorKeywords =
      result.colorKeywords[locale as keyof typeof result.colorKeywords] ||
      result.colorKeywords.ko;
    const resultFirstFeeling =
      result.firstFeeling[locale as keyof typeof result.firstFeeling] || result.firstFeeling.ko;
    const resultShiningMoment =
      result.shiningMoment[locale as keyof typeof result.shiningMoment] || result.shiningMoment.ko;
    const resultReverseCharm =
      result.reverseCharm[locale as keyof typeof result.reverseCharm] || result.reverseCharm.ko;
    const resultMatchingColors =
      result.matchingColors[locale as keyof typeof result.matchingColors] || result.matchingColors.ko;
    const resultPersonalColorLink =
      result.personalColorLink[locale as keyof typeof result.personalColorLink] || result.personalColorLink.ko;
          ...
          <div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-5">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {resultColorKeywords.split(/[,、，]/).map((part, index) => (
                    <span
                      key={index}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultFirstFeeling}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  {t('ui.shiningMoment')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">{resultShiningMoment}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultReverseCharm}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultMatchingColors}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  {t('ui.personalColorLink')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">{resultPersonalColorLink}</p>
              </div>
            </div>
