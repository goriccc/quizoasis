...
import { WorkLifeBalanceQuestion, WorkLifeBalanceResult, calculateWorkLifeBalanceResult } from '@/lib/workLifeBalanceData';
...
interface WorkLifeBalanceTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: WorkLifeBalanceQuestion[];
  results: WorkLifeBalanceResult[];
...
const getCompatibilityDescription = (myType: string, partnerType: string, t: any): string => {
  ...
  return t(`workLifeBalanceTest.result.compatibility.${key}`) || '';
};

export default function WorkLifeBalanceTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  playCount = 0,
  similarTests = []
}: WorkLifeBalanceTestClientProps) {
  const t = useTranslations();
  const [started, setStarted] = useState(false);
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [answers, setAnswers] = useState<any[]>([]);
  const [result, setResult] = useState<WorkLifeBalanceResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<WorkLifeBalanceQuestion[]>(questions);
  ...
  }, [currentQuestion, started, shuffledOptionsMap]);
  ...
  useEffect(() => {
    if (locale !== 'ko' && !started && aliProducts.length === 0) {
      const loadProducts = async () => {
        try {
          const products = await searchAliExpressProducts('work life balance', 4, locale);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: WorkLifeBalanceQuestion[]) => {
  ...
  const handleAnswer = (answer: string) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const calculatedResult = calculateWorkLifeBalanceResult(newAnswers, shuffledQuestions);
      setResult(calculatedResult);
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (calculatedResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(calculatedResult.type as any, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 워라밸 결과:', calculatedResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  const handleShareResult = async () => {
    ...
    const shareText = t('workLifeBalanceTest.shareMessages.default', { resultTitle }) + `\n\n${`https://myquizoasis.com${window.location.pathname}`}`;
  ...
  const shareToWeChat = async () => {
    ...
    const shareText = result 
      ? t('workLifeBalanceTest.shareMessages.default', { resultTitle }) + `\n\n${url}`
      : `${title}\n\n${url}`;
  ...
  const shareToWhatsApp = () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareText = result 
      ? encodeURIComponent(t('workLifeBalanceTest.shareMessages.default', { resultTitle }))
      ...
      <div className="min-h-screen bg-white">
        <div className="max-w-4xl mx-auto">
          <div className="relative w-full overflow-hidden mb-3" style={{ aspectRatio: '680/384' }}>
            <Image
              src={getThumbnailUrl(thumbnail || 'test_051_work_life_balance.jpg')}
              alt={title}
              fill
          ...
          <div className="px-4">
            ...
            <div className="text-gray-600 mb-6 leading-relaxed text-center space-y-4">
              <p className="font-bold text-gray-700">{t('workLifeBalanceTest.startMessage.line1')}</p>
              <p>{t('workLifeBalanceTest.startMessage.line2')}</p>
              <p>{t('workLifeBalanceTest.startMessage.line3')}</p>
              <p>{t('workLifeBalanceTest.startMessage.line4')}</p>
              <p className="whitespace-pre-line">{t('workLifeBalanceTest.startMessage.line5')}</p>
              <p>{t('workLifeBalanceTest.startMessage.line6')}</p>
            </div>
  ...
  if (showResult && result) {
    ...
    const resultPros = typeof result.pros === 'object' && !Array.isArray(result.pros)
      ? result.pros[locale] || result.pros.ko
      : result.pros;
    const resultCons = typeof result.cons === 'object' && !Array.isArray(result.cons)
      ? result.cons[locale] || result.cons.ko
      : result.cons;
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-3">
                  ✅ {t('workLifeBalanceTest.result.pros')}
                </h3>
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-3">
                  ⚠️ {t('workLifeBalanceTest.result.cons')}
                </h3>
                <div className="flex flex-wrap gap-2">
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                💼 {t('workLifeBalanceTest.result.recommendedJobs')}
              </h3>
              <div className="flex flex-wrap gap-2">
                {resultRecommendedJobs.split(/,\s+|，\s*|、\s*/).map((job, index) => (
                  <span
                    key={index}
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                💡 {t('workLifeBalanceTest.result.advice')}
              </h3>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-6">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                🤝 {t('workLifeBalanceTest.result.compatibility')}
              </h3>
              <div className="space-y-2">
                <div className="p-3 bg-green-50 rounded-lg">
                  <span className="text-sm font-medium text-green-800 block mb-1">
                    💖 {t('workLifeBalanceTest.result.bestMatch')}
                  </span>
                  <span className="text-sm text-green-600">
                    {typeof result.compatibility.bestMatch === 'string' 
                      ? result.compatibility.bestMatch 
                      : result.compatibility.bestMatch[locale] || result.compatibility.bestMatch.ko}
                  </span>
                </div>
                <div className="p-3 bg-yellow-50 rounded-lg">
                  <span className="text-sm font-medium text-yellow-800 block mb-1">
                    ⚠️ {t('workLifeBalanceTest.result.caution')}
                  </span>
                  <span className="text-sm text-yellow-600">
                    {typeof result.compatibility.caution === 'string' 
                      ? result.compatibility.caution 
                      : result.compatibility.caution[locale] || result.compatibility.caution.ko}
                  </span>
                </div>
              </div>
            </div>
  ...
  const questionText = typeof question.question === 'string' 
    ? question.question 
    : question.question[locale] || question.question.ko;
  const progress = ((currentQuestion + 1) / shuffledQuestions.length) * 100;
  
  const optionsArray = shuffledOptionsMap[currentQuestion] || ['A', 'B', 'C', 'D'];
        ...
        <div>
          ...
          <div className="space-y-4 px-4">
            {optionsArray.map((option, index) => {
              const optionData = question[`option${option}` as keyof WorkLifeBalanceQuestion] as Record<string, string> | string;
              const optionText = typeof optionData === 'string' 
                ? optionData 
                : optionData[locale] || optionData.ko;
              const label = option;
              const colors = [
