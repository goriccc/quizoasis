...
import {
  Phase3SoloDiningTypeQuestion,
  Phase3SoloDiningTypeResult,
  calculatePhase3SoloDiningTypeResult,
...
interface Phase3SoloDiningTypeTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3SoloDiningTypeQuestion[];
  results: Phase3SoloDiningTypeResult[];
...
export default function Phase3SoloDiningTypeTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3SoloDiningTypeTestClientProps) {
  const t = useTranslations('phase3SoloDiningTypeTest');
  ...
  const [result, setResult] = useState<Phase3SoloDiningTypeResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3SoloDiningTypeQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3SoloDiningTypeQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3SoloDiningTypeResult(answersArray);
  ...
  if (showResult && result) {
    ...
    const resultSoloDiningType =
      result.soloDiningType[locale as keyof typeof result.soloDiningType] || result.soloDiningType.ko;
    const resultSoloDiningKeywords =
      result.soloDiningKeywords[locale as keyof typeof result.soloDiningKeywords] ||
      result.soloDiningKeywords.ko;
    const resultFavoritePlaces =
      result.favoritePlaces[locale as keyof typeof result.favoritePlaces] || result.favoritePlaces.ko;
    const resultFavoriteMenus =
      result.favoriteMenus[locale as keyof typeof result.favoriteMenus] || result.favoriteMenus.ko;
    ...
    const resultHiddenSide =
      result.hiddenSide[locale as keyof typeof result.hiddenSide] || result.hiddenSide.ko;
    const resultUpgradeTip =
      result.upgradeTip[locale as keyof typeof result.upgradeTip] || result.upgradeTip.ko;
