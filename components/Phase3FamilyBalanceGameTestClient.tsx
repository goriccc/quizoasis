...
import {
  Phase3FamilyBalanceGameQuestion,
  Phase3FamilyBalanceGameResult,
  calculatePhase3FamilyBalanceGameResult,
} from '@/lib/phase3FamilyBalanceGameData';
...
const FAMILY_BALANCE_ROUND_STRIP: Record<string, RegExp[]> = {
...
function stripFamilyBalanceRoundPrefix(locale: string, raw: string): { body: string; matched: boolean } {
  const list = FAMILY_BALANCE_ROUND_STRIP[locale] ?? FAMILY_BALANCE_ROUND_STRIP.ko;
  let s = raw;
...
function formatFamilyBalanceRoundQuestionText(
  locale: string,
  displayRound1Based: number,
  rawQuestionText: string,
  originalQuestionId: number
): string {
  const { body, matched } = stripFamilyBalanceRoundPrefix(locale, rawQuestionText);
...
function parseFamilyBalanceDescription(text: string): {
  mainBody: string;
  strengths: string;
  oneLiner: string;
} {
...
interface Phase3FamilyBalanceGameTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3FamilyBalanceGameQuestion[];
  results: Phase3FamilyBalanceGameResult[];
...
export default function Phase3FamilyBalanceGameTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3FamilyBalanceGameTestClientProps) {
  const t = useTranslations('phase3FamilyBalanceGameTest');
  ...
  const [result, setResult] = useState<Phase3FamilyBalanceGameResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3FamilyBalanceGameQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3FamilyBalanceGameQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3FamilyBalanceGameResult(answersArray);
          ...
          <div className="px-4">
            ...
            <div className="flex justify-center mb-4">
              <button
                ...
                className="bg-gradient-to-r from-orange-500 to-rose-500 hover:from-orange-600 hover:to-rose-600 text-white font-bold py-4 px-8 rounded-full shadow-lg transform hover:scale-105 transition-all duration-200"
              >
  ...
  if (showResult && result) {
    ...
    const { mainBody: resultMainBody, strengths: resultStrengths, oneLiner: resultOneLiner } =
      parseFamilyBalanceDescription(resultLongDescription);
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  📊 {t('ui.familyStyleType')}
                </h3>
                <p className="text-2xl font-bold text-orange-600 text-center" style={{ fontSize: '1.5em' }}>
                  {resultEmpathyLevel}
                </p>
              </div>
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {resultCharacteristics
                    ...
                    <span
                      key={index}
                      className="inline-block bg-gradient-to-r from-orange-100 to-amber-100 text-orange-800 px-3 py-1.5 rounded-full text-sm font-medium"
                    >
                      {char}
                    </span>
                  ))}
...
{resultBadMatch && (
            ...
            <div className="flex flex-col sm:flex-row gap-4 px-4">
              ...
              <Link
                href={`/${locale}`}
                className="flex-1 bg-gradient-to-r from-orange-500 to-rose-500 text-white font-bold py-4 px-6 rounded-xl hover:from-orange-600 hover:to-rose-600 transition-all text-center shadow-md"
              >
  ...
  const questionText = formatFamilyBalanceRoundQuestionText(
    locale,
    currentQuestion + 1,
    questionTextRaw,
    question.id
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
            <div
              className="bg-gradient-to-r from-orange-500 to-rose-500 h-full rounded-full transition-all duration-300 ease-out"
