...
import {
  Phase3SleepTypePrescriptionQuestion,
  Phase3SleepTypePrescriptionResult,
  calculatePhase3SleepTypePrescriptionResult,
...
interface Phase3SleepTypePrescriptionTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3SleepTypePrescriptionQuestion[];
  results: Phase3SleepTypePrescriptionResult[];
...
export default function Phase3SleepTypePrescriptionTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3SleepTypePrescriptionTestClientProps) {
  const t = useTranslations('phase3SleepTypePrescriptionTest');
  ...
  const [result, setResult] = useState<Phase3SleepTypePrescriptionResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3SleepTypePrescriptionQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3SleepTypePrescriptionQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3SleepTypePrescriptionResult(answersArray);
      const sleepResult = results.find(r => r.type === resultType);
  ...
  if (showResult && result) {
    ...
    const scoreBand = result.scoreBand[locale as keyof typeof result.scoreBand] || result.scoreBand.ko;
    const sleepTypeLine = result.sleepTypeLine[locale as keyof typeof result.sleepTypeLine] || result.sleepTypeLine.ko;
    ...
    const problemLine = result.problemLine[locale as keyof typeof result.problemLine] || result.problemLine.ko;
    const prescriptionText = result.prescriptionText[locale as keyof typeof result.prescriptionText] || result.prescriptionText.ko;
    const todayTask = result.todayTask[locale as keyof typeof result.todayTask] || result.todayTask.ko;
    const warningLine = result.warningLine
      ? result.warningLine[locale as keyof typeof result.warningLine] || result.warningLine.ko
      : null;
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            <div className="text-center mb-3 bg-white rounded-2xl shadow-lg p-4 md:p-5">
              ...
              <p className="text-sm font-semibold text-teal-700 mb-3">{scoreBand}</p>
              <p className="text-base font-semibold text-gray-800 mb-2 text-left">{sleepTypeLine}</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-sm font-bold text-gray-800 mb-2 text-left">⚠️ {t('ui.problem')}</h3>
                <p className="text-sm text-gray-700 leading-relaxed text-left">{problemLine}</p>
              </div>
            </div>
            ...
            {warningLine && (
              <div className="mb-3 bg-gradient-to-r from-red-50 to-rose-50 rounded-xl shadow-lg p-4 border border-rose-100">
                <h3 className="text-base font-bold text-gray-800 mb-2">🩺 {t('ui.professionalGuide')}</h3>
                <p className="text-sm text-gray-800 leading-relaxed">{warningLine}</p>
              </div>
            )}
            ...
            <div className="mb-3 bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl shadow-lg p-4 border border-orange-100">
              ...
              <p className="text-sm text-gray-800 leading-relaxed">{todayTask}</p>
            </div>
