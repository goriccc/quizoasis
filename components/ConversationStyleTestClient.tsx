...
import { ConversationStyleQuestion, ConversationStyleResult, calculateConversationStyleResult } from '@/lib/conversationStyleData';
...
interface ConversationStyleTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: ConversationStyleQuestion[];
  results: ConversationStyleResult[];
...
export default function ConversationStyleTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: ConversationStyleTestClientProps) {
  const t = useTranslations('conversationStyleTest');
  ...
  const [result, setResult] = useState<ConversationStyleResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<ConversationStyleQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: ConversationStyleQuestion[]) => {
  ...
  const handleAnswer = (types: string[]) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      const resultType = calculateConversationStyleResult(answersArray);
      const conversationStyleResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (conversationStyleResult) {
        setResult(conversationStyleResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (conversationStyleResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(conversationStyleResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 대화 스타일 결과:', conversationStyleResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const resultConversationShare = result.conversationShare[locale as keyof typeof result.conversationShare] || result.conversationShare.ko;
