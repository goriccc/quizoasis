...
import {
  Phase3ElementaryMathAdultsQuizQuestion,
  Phase3ElementaryMathAdultsQuizResult,
  Phase3ElementaryMathAdultsQuizOption,
  calculatePhase3ElementaryMathAdultsQuizResult,
...
interface Phase3ElementaryMathAdultsQuizTestClientProps {
  ...
  title: string;
  description: string;
  questions: Phase3ElementaryMathAdultsQuizQuestion[];
  results: Phase3ElementaryMathAdultsQuizResult[];
...
export default function Phase3ElementaryMathAdultsQuizTestClient({
  locale,
  slug,
  title,
  description,
  ...
  isLatestTest = false,
  badgeType = null,
}: Phase3ElementaryMathAdultsQuizTestClientProps) {
  const t = useTranslations('phase3ElementaryMathAdultsQuizTest');
  ...
  const [result, setResult] = useState<Phase3ElementaryMathAdultsQuizResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3ElementaryMathAdultsQuizQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3ElementaryMathAdultsQuizQuestion[]) => {
  ...
  const handleAnswer = (option: Phase3ElementaryMathAdultsQuizOption) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      const resultType = calculatePhase3ElementaryMathAdultsQuizResult(answersArray);
  ...
  if (showResult && result) {
    ...
    const resultTodo = result.todoToday[locale as keyof typeof result.todoToday] || result.todoToday.ko;
        ...
        <div>
          <div className="mb-6 px-4">
            <h2 className="text-2xl md:text-3xl font-bold text-gray-800 mb-2 text-center leading-relaxed whitespace-pre-line">
              <StackedFractionText text={questionText} />
            </h2>
                ...
                >
                  <div className="flex items-center">
                    ...
                    <span className="text-base">
                      <StackedFractionText text={optionText} />
                    </span>
                  </div>
                </button>
              );
          ...
          <div className="mt-4 px-4 flex flex-col gap-2">
            <button
              ...
              className="w-full py-3.5 rounded-xl font-bold border-2 border-amber-400 bg-gradient-to-r from-amber-100 to-sky-100 text-amber-950 hover:from-amber-200 hover:to-sky-200 shadow-sm"
            >
          ...
          {explainOpen && (
            ...
            >
              <div
                className="bg-white rounded-t-2xl sm:rounded-2xl w-full max-w-lg max-h-[78vh] overflow-y-auto p-6 shadow-2xl border border-amber-200"
              ...
              >
                ...
                <p className="text-gray-700 text-sm leading-relaxed mb-5 whitespace-pre-line">
                  <StackedFractionText text={hintExplanation} />
                </p>
                ...
                <p className="text-gray-700 text-sm leading-relaxed mb-6 whitespace-pre-line">
                  <StackedFractionText text={hintTraps} />
                </p>
                <button
                  type="button"
                  onClick={() => setExplainOpen(false)}
                  className="w-full py-3 rounded-xl font-bold bg-gradient-to-r from-amber-500 to-rose-500 text-white hover:from-amber-600 hover:to-rose-600"
                >
                  {t('ui.closeModal')}
                </button>
              </div>
