...
import {
  Phase3PersonalBrandingKeywordsQuestion,
  Phase3PersonalBrandingKeywordsResult,
  calculatePhase3PersonalBrandingKeywordsResult,
...
interface Phase3PersonalBrandingKeywordsTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3PersonalBrandingKeywordsQuestion[];
  results: Phase3PersonalBrandingKeywordsResult[];
...
export default function Phase3PersonalBrandingKeywordsTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3PersonalBrandingKeywordsTestClientProps) {
  const t = useTranslations('phase3PersonalBrandingKeywordsTest');
  ...
  const [result, setResult] = useState<Phase3PersonalBrandingKeywordsResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3PersonalBrandingKeywordsQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3PersonalBrandingKeywordsQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3PersonalBrandingKeywordsResult(answersArray);
  ...
  if (showResult && result) {
    ...
    const resultEnglishKeywords = result.englishKeywords[locale as keyof typeof result.englishKeywords] || result.englishKeywords.ko;
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <div className="flex flex-wrap gap-2 justify-center">
                {resultKeywords.split(/[·・]/).map((kw, idx) => (
                  <span
                    key={idx}
                    className="inline-block bg-gradient-to-r from-indigo-100 to-cyan-100 text-indigo-800 text-sm font-semibold px-3 py-1 rounded-full shadow-sm"
                  >
                    #{kw.trim()}
                  </span>
                ))}
