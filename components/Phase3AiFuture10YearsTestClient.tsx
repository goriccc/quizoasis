...
import {
  Phase3AiFuture10YearsQuestion,
  Phase3AiFuture10YearsResult,
  calculatePhase3AiFuture10YearsResult,
  formatPhase3AiFuture10YearsText,
...
interface Phase3AiFuture10YearsTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3AiFuture10YearsQuestion[];
  results: Phase3AiFuture10YearsResult[];
...
export default function Phase3AiFuture10YearsTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3AiFuture10YearsTestClientProps) {
  const t = useTranslations('phase3AiFuture10YearsTest');
  ...
  const [result, setResult] = useState<Phase3AiFuture10YearsResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3AiFuture10YearsQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3AiFuture10YearsQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3AiFuture10YearsResult(answersArray);
  ...
  if (showResult && result) {
    ...
    const resultLongDescription = formatPhase3AiFuture10YearsText(
