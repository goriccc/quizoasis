...
import {
  Phase3GaslightingDefensePowerQuestion,
  Phase3GaslightingDefensePowerResult,
  calculatePhase3GaslightingDefensePowerResult,
...
interface Phase3GaslightingDefensePowerTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3GaslightingDefensePowerQuestion[];
  results: Phase3GaslightingDefensePowerResult[];
...
export default function Phase3GaslightingDefensePowerTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3GaslightingDefensePowerTestClientProps) {
  const t = useTranslations('phase3GaslightingDefensePowerTest');
  ...
  const [result, setResult] = useState<Phase3GaslightingDefensePowerResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3GaslightingDefensePowerQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3GaslightingDefensePowerQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3GaslightingDefensePowerResult(answersArray);
      const defenseResult = results.find(r => r.type === resultType);
  ...
  if (showResult && result) {
    ...
    const resultDefenseLevel = result.defenseLevel[locale as keyof typeof result.defenseLevel] || result.defenseLevel.ko;
    const resultDefenseIndex = result.defenseIndex[locale as keyof typeof result.defenseIndex] || result.defenseIndex.ko;
    const resultPatterns = result.patterns
      ? (result.patterns[locale as keyof typeof result.patterns] || result.patterns.ko)
      : null;
    const resultStrength = result.strength
      ? (result.strength[locale as keyof typeof result.strength] || result.strength.ko)
      : null;
    const resultNeeds = result.needs
      ? (result.needs[locale as keyof typeof result.needs] || result.needs.ko)
      : null;
    const resultGuideTitle = result.guideTitle[locale as keyof typeof result.guideTitle] || result.guideTitle.ko;
    const resultGuide = result.guide[locale as keyof typeof result.guide] || result.guide.ko;
    const resultCaution = result.caution
      ? (result.caution[locale as keyof typeof result.caution] || result.caution.ko)
      : null;
    const resultExpertNote = result.expertNote
      ? (result.expertNote[locale as keyof typeof result.expertNote] || result.expertNote.ko)
      : null;
    return (
            ...
            {resultPatterns && (
              <div className="bg-white rounded-xl shadow-lg p-4 md:p-5 mb-3 border border-purple-100/80">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {resultPatterns.split(/[,、，]/).map((char, index) => (
                    <span
                      key={index}
              ...
              <div className="bg-white rounded-xl shadow-lg p-4 md:p-5 mb-3 border border-purple-100/80">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {resultStrength.split(/[,、，]/).map((char, index) => (
                    <span
                      key={index}
                      className="inline-block bg-gradient-to-r from-blue-100 to-cyan-100 text-blue-800 px-3 py-1.5 rounded-full text-sm font-medium"
                    >
                      {char.trim()}
                    </span>
                  ))}
