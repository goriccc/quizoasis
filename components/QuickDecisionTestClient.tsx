...
import { QuickDecisionQuestion, QuickDecisionResult } from '@/lib/quickDecisionData';
...
interface QuickDecisionTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: QuickDecisionQuestion[];
  results: QuickDecisionResult[];
...
const getCompatibilityDescription = (myType: string, partnerType: string, t: any): string => {
  ...
  return t(`quickDecisionTest.result.compatibility.${key}`) || '';
};

export default function QuickDecisionTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  playCount = 0,
  similarTests = []
}: QuickDecisionTestClientProps) {
  const t = useTranslations();
  const [started, setStarted] = useState(false);
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [answers, setAnswers] = useState<any[]>([]);
  const [result, setResult] = useState<QuickDecisionResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<QuickDecisionQuestion[]>(questions);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: QuickDecisionQuestion[]) => {
  ...
  const handleAnswer = (scores: any) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      const quickDecisionResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (quickDecisionResult) {
        setResult(quickDecisionResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (quickDecisionResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(quickDecisionResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 찰나의 순간 결과:', quickDecisionResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  const handleShareResult = async () => {
    ...
    const shareText = t('quickDecisionTest.shareMessages.default', { type: resultTitle }) + `\n\n${`https://myquizoasis.com${window.location.pathname}`}`;
    ...
    if (navigator.share) {
      ...
      try {
        ...
        alert(t('quickDecisionTest.alerts.resultCopied'));
      } catch (error) {
        console.error('클립보드 복사 실패:', error);
        alert(t('quickDecisionTest.alerts.shareFailed'));
      }
    }
  };
  ...
  const shareToWeChat = async () => {
    ...
    const shareText = result 
      ? t('quickDecisionTest.shareMessages.wechat', { type: resultTitle }) + `\n\n${url}`
      : `${title}\n\n${url}`;
    ...
    try {
      ...
      alert(t('quickDecisionTest.alerts.wechatCopy'));
    } catch (error) {
      alert(t('quickDecisionTest.alerts.shareFailed'));
    }
  };
  ...
  const shareToWhatsApp = () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareText = result 
      ? encodeURIComponent(t('quickDecisionTest.shareMessages.whatsapp', { type: resultTitle }))
  ...
  const shareToKakao = () => {
    ...
    if (!window.Kakao || !window.Kakao.isInitialized()) {
      alert(t('quickDecisionTest.alerts.kakaoInit'));
      return;
    }
    ...
    const shareDescription = result 
      ? t('quickDecisionTest.shareMessages.kakao', { type: resultTitle })
      : description;
    ...
    try {
      ...
      alert(t('quickDecisionTest.alerts.kakaoError'));
    }
  };
  ...
  const shareToTelegram = () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareText = result 
      ? t('quickDecisionTest.shareMessages.telegram', { type: resultTitle })
      : title;
  ...
  const copyLink = () => {
    ...
    alert(t('quickDecisionTest.alerts.linkCopied'));
  };
      ...
      <div className="min-h-screen bg-white">
        <div className="max-w-4xl mx-auto">
          <div className="relative w-full overflow-hidden mb-3" style={{ aspectRatio: '680/384' }}>
            <Image
              src={getThumbnailUrl(thumbnail || 'test_076_quick_decision.jpg')}
              alt={title}
              fill
          ...
          <div className="px-4">
            ...
            <div className="text-gray-600 mb-6 leading-relaxed text-center space-y-3">
              <p className="font-bold text-gray-700">{t('quickDecisionTest.startMessage.title')}</p>
              <p className="whitespace-pre-line">{t('quickDecisionTest.startMessage.description')}</p>
            </div>
            ...
            <div className="flex justify-center mb-4">
              ...
              >
                {t('quickDecisionTest.startMessage.startButton')}
              </button>
            </div>
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {resultPros.length > 0 ? (
                    ...
                    <span className="text-gray-500 text-sm">{t('quickDecisionTest.ui.noData')}</span>
                  )}
                </div>
              </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                💡 {t('quickDecisionTest.result.advice')}
              </h3>
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-3">
                  💚 {t('quickDecisionTest.result.compatibleTypes')}
                </h3>
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-3">
                  ⚠️ {t('quickDecisionTest.result.warningTypes')}
                </h3>
                <div className="space-y-1">
            ...
            <div className="mt-8 mb-6 px-4">
              ...
              >
                ...
                {t('quickDecisionTest.ui.shareResult')}
              </button>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 px-4">
              ...
              >
                {t('quickDecisionTest.ui.retake')}
              </button>
              <Link
                href={`/${locale}`}
                className="flex-1 bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold py-4 px-6 rounded-xl hover:from-purple-700 hover:to-pink-700 transition-all text-center shadow-md"
              >
                {t('quickDecisionTest.ui.otherTests')}
              </Link>
            </div>
