...
import {
  Phase3AiEraJobSurvivalScoreQuestion,
  Phase3AiEraJobSurvivalScoreResult,
  calculatePhase3AiEraJobSurvivalScoreResult,
...
interface Phase3AiEraJobSurvivalScoreTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3AiEraJobSurvivalScoreQuestion[];
  results: Phase3AiEraJobSurvivalScoreResult[];
...
export default function Phase3AiEraJobSurvivalScoreTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3AiEraJobSurvivalScoreTestClientProps) {
  const t = useTranslations('phase3AiEraJobSurvivalScoreTest');
  ...
  const [result, setResult] = useState<Phase3AiEraJobSurvivalScoreResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3AiEraJobSurvivalScoreQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3AiEraJobSurvivalScoreQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3AiEraJobSurvivalScoreResult(answersArray);
  ...
  if (showResult && result) {
    ...
    const resultSurvivalScore = result.survivalScore[locale as keyof typeof result.survivalScore] || result.survivalScore.ko;
    const resultAiRiskLevel = result.aiRiskLevel[locale as keyof typeof result.aiRiskLevel] || result.aiRiskLevel.ko;
