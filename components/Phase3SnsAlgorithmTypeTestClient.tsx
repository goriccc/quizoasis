...
import {
  Phase3SnsAlgorithmTypeQuestion,
  Phase3SnsAlgorithmTypeResult,
  calculatePhase3SnsAlgorithmTypeResult,
...
interface Phase3SnsAlgorithmTypeTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3SnsAlgorithmTypeQuestion[];
  results: Phase3SnsAlgorithmTypeResult[];
...
export default function Phase3SnsAlgorithmTypeTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3SnsAlgorithmTypeTestClientProps) {
  const t = useTranslations('phase3SnsAlgorithmTypeTest');
  ...
  const [result, setResult] = useState<Phase3SnsAlgorithmTypeResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3SnsAlgorithmTypeQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3SnsAlgorithmTypeQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3SnsAlgorithmTypeResult(answersArray);
          ...
          <div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed text-center">{resultSaveFolder}</p>
              </div>
            </div>
