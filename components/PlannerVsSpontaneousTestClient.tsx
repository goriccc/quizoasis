...
import { PlannerVsSpontaneousQuestion, PlannerVsSpontaneousResult, calculatePlannerVsSpontaneousResult } from '@/lib/plannerVsSpontaneousData';
...
interface PlannerVsSpontaneousTestClientProps {
  ...
  title: string;
  description: string;
  questions: PlannerVsSpontaneousQuestion[];
  results: PlannerVsSpontaneousResult[];
...
export default function PlannerVsSpontaneousTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  playCount = 0,
  similarTests = []
}: PlannerVsSpontaneousTestClientProps) {
  const t = useTranslations('plannerVsSpontaneousTest');
  ...
  const [result, setResult] = useState<PlannerVsSpontaneousResult | null>(null);
  ...
  const handleAnswer = (answer: string) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 테스트 완료
      const calculatedResult = calculatePlannerVsSpontaneousResult(newAnswers);
  ...
  const shuffleQuestions = (questionList: PlannerVsSpontaneousQuestion[]) => {
    ...
    try {
      ...
      window.Kakao.Share.sendDefault({
        ...
        content: {
          ...
          imageUrl: getThumbnailUrl(thumbnail || 'test_219_planner_vs_spontaneous.jpg'),
          link: {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-4">
        ...
        <div className="flex flex-col items-center justify-center">
          ...
          <p className="mt-4 text-lg text-gray-700">{t('ui.analyzing')}</p>
        </div>
      ...
      <div className="min-h-screen bg-white">
        <div className="max-w-4xl mx-auto">
          <div className="relative w-full overflow-hidden mb-3" style={{ aspectRatio: '680/384' }}>
            <Image
              src={getThumbnailUrl(thumbnail || 'test_219_planner_vs_spontaneous.jpg')}
              alt={title}
              fill
          ...
          <div className="px-4">
            ...
            <p className="text-sm font-bold text-center mb-6" style={{ color: '#669df6' }}>
              총 {formatPlayCount(displayPlayCount, locale as Locale)}{t('ui.participants')}
            </p>
                          ...
                          <div className="p-4">
                            <div className="flex items-center justify-end gap-3">
                              ...
                              <div className="font-semibold text-gray-800 group-hover:text-primary-600 transition-colors flex items-center gap-1.5 text-sm flex-shrink-0">
                                <Play size={14} />
                                <span>{formatPlayCount(test.playCount || (test as any).play_count || 0, locale as Locale)}</span>
                              </div>
                            </div>
                          </div>
                        </div>
  ...
  if (showResult && result) {
    const resultTitle = result.title[localeKey as keyof typeof result.title] || result.title.ko;
    const resultDescription = result.description[localeKey as keyof typeof result.description] || result.description.ko;
    const resultCharacteristics = result.characteristics[localeKey as keyof typeof result.characteristics] || result.characteristics.ko;
    const resultStrengths = result.strengths[localeKey as keyof typeof result.strengths] || result.strengths.ko;
    const resultWeaknesses = result.weaknesses[localeKey as keyof typeof result.weaknesses] || result.weaknesses.ko;
    const resultAdvice = result.advice[localeKey as keyof typeof result.advice] || result.advice.ko;
  ...
  const questionText = question.question[localeKey as keyof typeof question.question] || question.question.ko;
  const optionA = question.options.a[localeKey as keyof typeof question.options.a] || question.options.a.ko;
  const optionB = question.options.b[localeKey as keyof typeof question.options.b] || question.options.b.ko;
        ...
        <div>
          ...
          <div className="space-y-4 px-4">
            <button
              ...
              className="w-full bg-gradient-to-r from-purple-50 to-purple-100 hover:from-purple-100 hover:to-purple-200 border-2 border-purple-200 hover:border-purple-400 text-gray-800 font-medium py-3 px-4 rounded-xl transition-all transform hover:scale-102 text-left"
            >
              <div className="flex items-center">
                <div className="w-7 h-7 bg-purple-600 text-white rounded-full flex items-center justify-center font-bold mr-3 flex-shrink-0 text-sm">
                  A
                </div>
                <span className="text-base">{optionA}</span>
              </div>
            </button>

            <button
              onClick={() => handleAnswer('b')}
              className="w-full bg-gradient-to-r from-pink-50 to-pink-100 hover:from-pink-100 hover:to-pink-200 border-2 border-pink-200 hover:border-pink-400 text-gray-800 font-medium py-3 px-4 rounded-xl transition-all transform hover:scale-102 text-left"
            >
              <div className="flex items-center">
                <div className="w-7 h-7 bg-pink-600 text-white rounded-full flex items-center justify-center font-bold mr-3 flex-shrink-0 text-sm">
                  B
                </div>
                <span className="text-base">{optionB}</span>
              </div>
            </button>
          </div>
