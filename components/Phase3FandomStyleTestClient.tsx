...
import {
  Phase3FandomStyleQuestion,
  Phase3FandomStyleResult,
  calculatePhase3FandomStyleResult,
...
interface Phase3FandomStyleTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3FandomStyleQuestion[];
  results: Phase3FandomStyleResult[];
...
export default function Phase3FandomStyleTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3FandomStyleTestClientProps) {
  const t = useTranslations('phase3FandomStyleTest');
  ...
  const [result, setResult] = useState<Phase3FandomStyleResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3FandomStyleQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3FandomStyleQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3FandomStyleResult(answersArray);
  ...
  if (showResult && result) {
    ...
    const resultFandomType = result.fandomType[locale as keyof typeof result.fandomType] || result.fandomType.ko;
    const resultFandomKeywords = result.fandomKeywords[locale as keyof typeof result.fandomKeywords] || result.fandomKeywords.ko;
    const resultFandomStyle = result.fandomStyle[locale as keyof typeof result.fandomStyle] || result.fandomStyle.ko;
    ...
    const resultBestBiasType = result.bestBiasType[locale as keyof typeof result.bestBiasType] || result.bestBiasType.ko;
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {resultFandomKeywords.split(/[·]/).map((keyword, index) => (
                    <span
                      key={index}
