...
import { Phase2HomebodyLevelQuestion, Phase2HomebodyLevelResult, calculatePhase2HomebodyLevelResult } from '@/lib/phase2_homebody_level_data';
...
interface Phase2HomebodyLevelTestClientProps {
  ...
  title: string;
  description: string;
  questions: Phase2HomebodyLevelQuestion[];
  results: Phase2HomebodyLevelResult[];
...
export default function Phase2HomebodyLevelTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  playCount = 0,
  similarTests = []
}: Phase2HomebodyLevelTestClientProps) {
  const t = useTranslations('phase2HomebodyLevelTest');
  ...
  const [result, setResult] = useState<Phase2HomebodyLevelResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2HomebodyLevelQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase2HomebodyLevelQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase2HomebodyLevelResult(answersArray);
      const homebodyLevelResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (homebodyLevelResult) {
        setResult(homebodyLevelResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (homebodyLevelResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(homebodyLevelResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 집순이 레벨 결과:', homebodyLevelResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  const handleShareResult = async () => {
    ...
    const homebodyLevelText = result.homebodyLevel[locale as keyof typeof result.homebodyLevel] || result.homebodyLevel.ko;
    const levelMatch = homebodyLevelText.match(/Lv\.\s*(\d+)/);
    const level = levelMatch ? levelMatch[1] : '';
    const shareText = `${t('shareMessages.default', { level: level, type: resultTitle })}\n\n${`https://myquizoasis.com${window.location.pathname}`}`;
  ...
  const shareToLine = () => {
    ...
    const shareText = result 
      ? encodeURIComponent(t('shareMessages.line', { level: level, type: resultTitle }))
  ...
  const shareToWeChat = async () => {
    ...
    const level = levelMatch ? levelMatch[1] : '';
    const shareText = result 
      ? `${t('shareMessages.wechat', { level: level, type: resultTitle })}\n\n${url}`
  ...
  const shareToWhatsApp = () => {
    ...
    const level = levelMatch ? levelMatch[1] : '';
    const shareText = result 
      ? encodeURIComponent(t('shareMessages.whatsapp', { level: level, type: resultTitle }))
  ...
  const shareToKakao = () => {
    ...
    const shareDescription = result 
      ? t('shareMessages.kakao', { level: level, type: resultTitle })
  ...
  const shareToTelegram = () => {
    ...
    const level = levelMatch ? levelMatch[1] : '';
    const shareText = result 
      ? t('shareMessages.telegram', { level: level, type: resultTitle })
  ...
  if (showResult && result) {
    ...
    const resultHomebodyLevel = result.homebodyLevel[locale as keyof typeof result.homebodyLevel] || result.homebodyLevel.ko;
    const resultHabitat = result.habitat[locale as keyof typeof result.habitat] || result.habitat.ko;
