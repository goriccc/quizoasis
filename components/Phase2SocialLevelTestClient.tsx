...
import { Phase2SocialLevelQuestion, Phase2SocialLevelResult, calculatePhase2SocialLevelResult } from '@/lib/phase2_social_level_data';
...
interface Phase2SocialLevelTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase2SocialLevelQuestion[];
  results: Phase2SocialLevelResult[];
...
export default function Phase2SocialLevelTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase2SocialLevelTestClientProps) {
  const t = useTranslations('phase2SocialLevelTest');
  ...
  const [result, setResult] = useState<Phase2SocialLevelResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2SocialLevelQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase2SocialLevelQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase2SocialLevelResult(answersArray);
      const socialLevelResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (socialLevelResult) {
        setResult(socialLevelResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (socialLevelResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(socialLevelResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 사회성 레벨 결과:', socialLevelResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const resultSocialLevel = result.socialLevel[locale as keyof typeof result.socialLevel] || result.socialLevel.ko;
