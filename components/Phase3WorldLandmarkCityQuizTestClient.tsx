...
import {
  Phase3WorldLandmarkCityQuizQuestion,
  Phase3WorldLandmarkCityQuizResult,
  Phase3WorldLandmarkCityQuizOption,
  calculatePhase3WorldLandmarkCityQuizResult,
...
interface Phase3WorldLandmarkCityQuizTestClientProps {
  ...
  title: string;
  description: string;
  questions: Phase3WorldLandmarkCityQuizQuestion[];
  results: Phase3WorldLandmarkCityQuizResult[];
...
export default function Phase3WorldLandmarkCityQuizTestClient({
  locale,
  slug,
  title,
  description,
  ...
  isLatestTest = false,
  badgeType = null,
}: Phase3WorldLandmarkCityQuizTestClientProps) {
  const t = useTranslations('phase3WorldLandmarkCityQuizTest');
  ...
  const [result, setResult] = useState<Phase3WorldLandmarkCityQuizResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3WorldLandmarkCityQuizQuestion[]>([]);
  /** 답 선택 후 정답/해설 확인 단계 */
  ...
  useEffect(() => {
    if (!started && aliProducts.length === 0) {
      const loadProducts = async () => {
        try {
          const keyword = locale === 'ko' ? '여행 기념품' : 'travel souvenir';
  ...
  useEffect(() => {
    if (result && locale !== 'ko') {
      const loadProducts = async () => {
        try {
          const keyword = 'landmark travel souvenir';
          const products = await searchAliExpressProducts(keyword, 4, locale);
          setAliProducts(products);
        } catch (error) {
          console.error('상품 로드 실패:', error);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3WorldLandmarkCityQuizQuestion[]) => {
  ...
  const finishQuizWithAnswers = (finalAnswers: Record<number, number>) => {
    ...
    const resultType = calculatePhase3WorldLandmarkCityQuizResult(answersArray);
    ...
    if (quizResult && locale !== 'ko') {
      ...
      searchAliExpressProducts('landmark travel souvenir', 4, locale)
        .then(products => {
  ...
  const handleOptionSelect = (option: Phase3WorldLandmarkCityQuizOption) => {
