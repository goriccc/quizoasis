...
import { Phase2HiddenTalentQuestion, Phase2HiddenTalentResult, calculatePhase2HiddenTalentResult } from '@/lib/phase2_hidden_talent_data';
...
interface Phase2HiddenTalentTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase2HiddenTalentQuestion[];
  results: Phase2HiddenTalentResult[];
...
export default function Phase2HiddenTalentTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false
}: Phase2HiddenTalentTestClientProps) {
  const t = useTranslations('phase2HiddenTalentTest');
  ...
  const [result, setResult] = useState<Phase2HiddenTalentResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2HiddenTalentQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase2HiddenTalentQuestion[]) => {
  ...
  const handleAnswer = (optionIndex: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      const resultType = calculatePhase2HiddenTalentResult(newAnswers, questions);
      const hiddenTalentResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (hiddenTalentResult) {
        setResult(hiddenTalentResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (hiddenTalentResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(hiddenTalentResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 숨겨진 재능 결과:', hiddenTalentResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const resultHiddenTalent = result.hiddenTalent[locale as keyof typeof result.hiddenTalent] || result.hiddenTalent.ko;
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {resultHiddenTalent.split(/[,、，]/).map((talent, index) => (
                    <span
                      key={index}
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  ⭐ {t('ui.recommendedFields')}
                </h3>
                <div className="flex flex-wrap justify-center gap-2">
                  {resultRecommendedFields.split(/[,、，]/).map((field, index) => (
                    <span
                      key={index}
