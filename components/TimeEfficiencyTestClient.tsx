...
import { TimeEfficiencyQuestion, TimeEfficiencyResult, calculateTimeEfficiencyResult } from '@/lib/timeEfficiencyData';
...
interface TimeEfficiencyTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: TimeEfficiencyQuestion[];
  results: TimeEfficiencyResult[];
...
const getCompatibilityDescription = (myType: string, partnerType: string, t: any): string => {
  ...
  return t(`timeEfficiencyTest.result.compatibility.${key}`) || '';
};

export default function TimeEfficiencyTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  playCount = 0,
  similarTests = []
}: TimeEfficiencyTestClientProps) {
  const t = useTranslations();
  const [started, setStarted] = useState(false);
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [answers, setAnswers] = useState<Array<{ questionId: number; scores: any }>>([]);
  const [result, setResult] = useState<TimeEfficiencyResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<TimeEfficiencyQuestion[]>(questions);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: TimeEfficiencyQuestion[]) => {
  ...
  const handleAnswer = (scores: any) => {
    ...
    const newAnswers = [...answers, answerWithQuestionId];
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculateTimeEfficiencyResult(newAnswers);
      const timeEfficiencyResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (timeEfficiencyResult) {
        setResult(timeEfficiencyResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (timeEfficiencyResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(timeEfficiencyResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 시간 효율성 결과:', timeEfficiencyResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  const calculateResult = (finalAnswers: any[]) => {
    const resultType = calculateTimeEfficiencyResult(finalAnswers);
    const timeEfficiencyResult = results.find(r => r.type === resultType);
  ...
  const handleShareResult = async () => {
    ...
    const shareText = t('timeEfficiencyTest.shareMessages.default', { type: resultTitle }) + `\n\n${`https://myquizoasis.com${window.location.pathname}`}`;
    ...
    if (navigator.share) {
      ...
      try {
        ...
        alert(t('timeEfficiencyTest.ui.clipboardCopySuccess'));
      } catch (error) {
        console.error('클립보드 복사 실패:', error);
        alert(t('timeEfficiencyTest.ui.shareFeatureError'));
      }
    }
  };
  ...
  const shareToWeChat = async () => {
    ...
    const shareText = result 
      ? t('timeEfficiencyTest.shareMessages.wechat', { type: resultTitle }) + `\n\n${url}`
      : `${title}\n\n${url}`;
    ...
    try {
      ...
      alert(t('timeEfficiencyTest.ui.wechatLinkCopy'));
    } catch (error) {
      alert(t('timeEfficiencyTest.ui.shareFeatureError'));
    }
  };
  ...
  const shareToWhatsApp = () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareText = result 
      ? encodeURIComponent(t('timeEfficiencyTest.shareMessages.whatsapp', { type: resultTitle }))
  ...
  const shareToKakao = () => {
    ...
    if (!window.Kakao || !window.Kakao.isInitialized()) {
      alert(t('timeEfficiencyTest.ui.kakaoInitializing'));
      return;
    }
    ...
    const shareDescription = result 
      ? t('timeEfficiencyTest.shareMessages.kakao', { type: resultTitle })
      : description;
    ...
    try {
      ...
      alert(t('timeEfficiencyTest.ui.kakaoShareError'));
    }
  };
  ...
  const shareToTelegram = () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareText = result 
      ? t('timeEfficiencyTest.shareMessages.telegram', { type: resultTitle })
      : title;
  ...
  const copyLink = () => {
    ...
    alert(t('timeEfficiencyTest.ui.linkCopySuccess'));
  };
      ...
      <div className="min-h-screen bg-white">
        <div className="max-w-4xl mx-auto">
          <div className="relative w-full overflow-hidden mb-3" style={{ aspectRatio: '680/384' }}>
            <Image
              src={getThumbnailUrl(thumbnail || 'test_079_time_efficiency.jpg')}
              alt={title}
              fill
          ...
          <div className="px-4">
            ...
            <div className="text-gray-600 mb-6 leading-relaxed text-center space-y-4">
              <p className="font-bold text-gray-700">{t('timeEfficiencyTest.startMessage.line1')}</p>
              <p>{t('timeEfficiencyTest.startMessage.line2')}</p>
              <p>{t('timeEfficiencyTest.startMessage.line3')}</p>
              <p>{t('timeEfficiencyTest.startMessage.line4')}</p>
              <p className="whitespace-pre-line">{t('timeEfficiencyTest.startMessage.line5')}</p>
              <p>{t('timeEfficiencyTest.startMessage.line6')}</p>
            </div>
            ...
            <div className="mb-8 text-center">
              ...
              <div className="flex justify-center gap-2">
                ...
                <button onClick={shareToTelegram} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/telegram.jpeg" alt="텔레그램 벽지" width={46} height={46} className="rounded-lg" />
                </button>
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                💡 {t('timeEfficiencyTest.ui.advice')}
              </h3>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                💖 {t('timeEfficiencyTest.ui.compatibleTypes')}
              </h3>
              <p className="text-sm text-gray-700 leading-relaxed">
                {result.type === "Type1" && t('timeEfficiencyTest.compatibility.Type1_Type2')}
                {result.type === "Type2" && t('timeEfficiencyTest.compatibility.Type2_Type1')}
                {result.type === "Type3" && t('timeEfficiencyTest.compatibility.Type3_all')}
                {result.type === "Type4" && t('timeEfficiencyTest.compatibility.Type4_Type4')}
                {result.type === "Type5" && t('timeEfficiencyTest.compatibility.Type5_Type5')}
                {result.type === "Type6" && t('timeEfficiencyTest.compatibility.Type6_Type6')}
              </p>
            </div>

            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                ⚠️ {t('timeEfficiencyTest.ui.cautionTypes')}
              </h3>
              <p className="text-sm text-gray-700 leading-relaxed">
                {result.type === "Type1" && t('timeEfficiencyTest.caution.Type1_Type6')}
                {result.type === "Type2" && t('timeEfficiencyTest.caution.Type2_Type5')}
                {result.type === "Type3" && t('timeEfficiencyTest.caution.Type3_Type1')}
                {result.type === "Type4" && t('timeEfficiencyTest.caution.Type4_Type1')}
                {result.type === "Type5" && t('timeEfficiencyTest.caution.Type5_Type1_Type2')}
                {result.type === "Type6" && t('timeEfficiencyTest.caution.Type6_Type1')}
              </p>
            </div>
