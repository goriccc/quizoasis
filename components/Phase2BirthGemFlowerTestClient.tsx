...
import { Phase2BirthGemFlowerQuestion, Phase2BirthGemFlowerResult, calculatePhase2BirthGemFlowerResult } from '@/lib/phase2_birth_gem_flower_data';
...
interface Phase2BirthGemFlowerTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase2BirthGemFlowerQuestion[];
  results: Phase2BirthGemFlowerResult[];
...
export default function Phase2BirthGemFlowerTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false
}: Phase2BirthGemFlowerTestClientProps) {
  const t = useTranslations('phase2BirthGemFlowerTest');
  const tGlobal = useTranslations(); // 글로벌 번역 (mbti 등)
  const [started, setStarted] = useState(false);
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [birthMonth, setBirthMonth] = useState<number>(1); // 태어난 달 (1-12)
  const [answers, setAnswers] = useState<Array<{ questionId: number; selectedTypes: string[] }>>([]); // 질문별 선택된 타입들
  const [result, setResult] = useState<Phase2BirthGemFlowerResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2BirthGemFlowerQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase2BirthGemFlowerQuestion[]) => {
  ...
  const handleStartTest = () => {
    if (birthMonth < 1 || birthMonth > 12) {
      alert(t('alerts.selectBirthMonth') || 'Please select your birth month.');
      return;
    }
  ...
  const handleAnswer = (selectedTypes: string[]) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase2BirthGemFlowerResult(birthMonth, newAnswers);
      const birthGemFlowerResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (birthGemFlowerResult) {
        setResult(birthGemFlowerResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (birthGemFlowerResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(birthGemFlowerResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 탄생석/탄생화 결과:', birthGemFlowerResult.type, '→ 검색 키워드:', keywords[0]);
            ...
            <div className="flex flex-col items-center mb-4 space-y-4">
              <div className="flex items-center gap-3">
                ...
                <select
                  ...
                  className="px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 text-gray-800 bg-white"
                >
  ...
  if (showResult && result) {
    ...
    const resultGem = result.gem[locale as keyof typeof result.gem] || result.gem.ko;
    const resultFlower = result.flower[locale as keyof typeof result.flower] || result.flower.ko;
