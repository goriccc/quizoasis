...
import {
  Phase3FallInLoveSpeedQuestion,
  Phase3FallInLoveSpeedResult,
  calculatePhase3FallInLoveSpeedResult,
...
interface Phase3FallInLoveSpeedTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3FallInLoveSpeedQuestion[];
  results: Phase3FallInLoveSpeedResult[];
...
export default function Phase3FallInLoveSpeedTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3FallInLoveSpeedTestClientProps) {
  const t = useTranslations('phase3FallInLoveSpeedTest');
  ...
  const [result, setResult] = useState<Phase3FallInLoveSpeedResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3FallInLoveSpeedQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3FallInLoveSpeedQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3FallInLoveSpeedResult(answersArray);
