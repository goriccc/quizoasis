...
import {
  Phase3ReincarnationJobQuestion,
  Phase3ReincarnationJobResult,
  calculatePhase3ReincarnationJobResult,
  parsePhase3ReincarnationDescription,
...
interface Phase3ReincarnationJobTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3ReincarnationJobQuestion[];
  results: Phase3ReincarnationJobResult[];
...
export default function Phase3ReincarnationJobTestClient({
  locale,
  slug,
  title,
  description,
  ...
  isLatestTest = false,
  badgeType = null,
}: Phase3ReincarnationJobTestClientProps) {
  const t = useTranslations('phase3ReincarnationJobTest');
  ...
  const [result, setResult] = useState<Phase3ReincarnationJobResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3ReincarnationJobQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3ReincarnationJobQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3ReincarnationJobResult(answersArray);
  ...
  const shareToLine = () => {
    ...
    const shareText = result
      ? encodeURIComponent(getResultShareLine())
  ...
  const shareToKakao = () => {
    ...
    const shareDescription = result ? getResultShareLine() : t('shareMessages.startKakao');
  ...
  const shareToTelegram = () => {
    const url = encodeURIComponent(`https://myquizoasis.com${window.location.pathname}`);
    const shareText = result ? getResultShareLine() : t('shareMessages.startTelegram');
  ...
  if (showResult && result) {
    ...
    const resultEra = result.era[locale as keyof typeof result.era] || result.era.ko;
    ...
    const detailParts = parsePhase3ReincarnationDescription(resultLongDescription, locale);
    const hasStructuredDetail =
      detailParts.dayInLife || detailParts.quote || detailParts.struggles || detailParts.lifeConnection;
          ...
          <div>
            <div className="mb-3 bg-white rounded-2xl shadow-lg p-4 md:p-6">
              ...
              <div className="space-y-5 text-left max-w-xl mx-auto">
                ...
                <div className="flex flex-col sm:flex-row sm:items-start gap-1 sm:gap-3 border-b border-gray-100 pb-4">
                  <span className="text-sm font-bold text-gray-500 shrink-0 sm:min-w-[7rem]">
                    {t('ui.resultLabels.era')}
                    <span className="text-gray-400 font-normal"> :</span>
                  </span>
                  <p className="text-base font-medium text-indigo-700 leading-relaxed flex-1">
                    {resultEra}
                  </p>
                </div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="rounded-lg bg-gray-50/80 p-3 border border-gray-100 space-y-4">
                    {hasStructuredDetail ? (
                      <>
                        {detailParts.intro ? (
                        ...
                        {detailParts.dayInLife ? (
                          <div className="flex flex-col sm:flex-row sm:items-start gap-1 sm:gap-3 border-b border-gray-200/80 pb-3">
                            <span className="text-sm font-bold text-gray-500 shrink-0 sm:min-w-[7.5rem]">
                              {t('ui.resultLabels.dayInLife')}
                              <span className="text-gray-400 font-normal"> :</span>
                            </span>
                            <p className="text-base text-gray-700 leading-relaxed flex-1 min-w-0">
                              {detailParts.dayInLife}
                            </p>
                          </div>
                        ) : null}
                        {detailParts.quote ? (
                          <div className="flex flex-col sm:flex-row sm:items-start gap-1 sm:gap-3 border-b border-gray-200/80 pb-3">
                            <span className="text-sm font-bold text-gray-500 shrink-0 sm:min-w-[7.5rem]">
                              {t('ui.resultLabels.quote')}
                              <span className="text-gray-400 font-normal"> :</span>
                            </span>
                            <p className="text-base text-gray-800 font-medium italic leading-relaxed flex-1 min-w-0">
                              {detailParts.quote}
                            </p>
                          </div>
                        ) : null}
                        {detailParts.struggles ? (
                          <div className="flex flex-col sm:flex-row sm:items-start gap-1 sm:gap-3 border-b border-gray-200/80 pb-3">
                            <span className="text-sm font-bold text-gray-500 shrink-0 sm:min-w-[7.5rem]">
                              {t('ui.resultLabels.struggles')}
                              <span className="text-gray-400 font-normal"> :</span>
                            </span>
                            <p className="text-base text-gray-700 leading-relaxed flex-1 min-w-0">
                              {detailParts.struggles}
                        ...
                        {detailParts.lifeConnection ? (
                          <div className="flex flex-col sm:flex-row sm:items-start gap-1 sm:gap-3">
                            <span className="text-sm font-bold text-gray-500 shrink-0 sm:min-w-[7.5rem]">
                              {t('ui.resultLabels.lifeConnection')}
                              <span className="text-gray-400 font-normal"> :</span>
                            </span>
                            <p className="text-base text-gray-700 leading-relaxed flex-1 min-w-0">
                              {detailParts.lifeConnection}
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {resultGoodMatch
                    ...
                    .map((name, index) => (
                      <span
                        key={index}
                        className="inline-block bg-gradient-to-r from-emerald-100 to-teal-100 text-emerald-900 px-3 py-1.5 rounded-full text-sm font-medium"
                      >
                        {name}
                      </span>
                    ))}
