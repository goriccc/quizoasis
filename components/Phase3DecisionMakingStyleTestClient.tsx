...
import {
  Phase3DecisionMakingStyleQuestion,
  Phase3DecisionMakingStyleResult,
  calculatePhase3DecisionMakingStyleResult,
  calculatePhase3DecisionMakingStyleDomainScores,
} from '@/lib/phase3DecisionMakingStyleData';
...
interface Phase3DecisionMakingStyleTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3DecisionMakingStyleQuestion[];
  results: Phase3DecisionMakingStyleResult[];
...
export default function Phase3DecisionMakingStyleTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3DecisionMakingStyleTestClientProps) {
  const t = useTranslations('phase3DecisionMakingStyleTest');
  ...
  const [result, setResult] = useState<Phase3DecisionMakingStyleResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3DecisionMakingStyleQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3DecisionMakingStyleQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      const scores = calculatePhase3DecisionMakingStyleDomainScores(answersArray);
      setDomainScores(scores);

      const resultType = calculatePhase3DecisionMakingStyleResult(answersArray);
  ...
  const handleRetake = () => {
    ...
    setResult(null);
    setShowResult(false);
    setDomainScores({ start: 0, info: 0, others: 0, uncertainty: 0, regret: 0, speed: 0, total: 0 });
  ...
  if (showResult && result) {
    ...
    const resultDecisionStyle =
      result.decisionStyle[locale as keyof typeof result.decisionStyle] || result.decisionStyle.ko;
    const resultDecisionType =
      result.decisionType[locale as keyof typeof result.decisionType] || result.decisionType.ko;
    const resultDecisionKeywords =
      result.decisionKeywords[locale as keyof typeof result.decisionKeywords] || result.decisionKeywords.ko;
    ...
    const resultBestSituations =
      result.bestSituations[locale as keyof typeof result.bestSituations] || result.bestSituations.ko;
    const resultRiskySituations =
      result.riskySituations[locale as keyof typeof result.riskySituations] || result.riskySituations.ko;
    const resultImprovement =
      result.improvementDirection[locale as keyof typeof result.improvementDirection] || result.improvementDirection.ko;
    ...
    const strengthsList = resultStrengths
      ...
      : [];
    const weaknessesList = resultWeaknesses
      ? resultWeaknesses.split('\n').map((s) => s.trim()).filter(Boolean)
      : [];

    const domainConfig = [
      { key: 'start', label: t('ui.startDomain'), score: domainScores.start, max: 6 },
      { key: 'info', label: t('ui.infoDomain'), score: domainScores.info, max: 6 },
      { key: 'others', label: t('ui.othersDomain'), score: domainScores.others, max: 6 },
      { key: 'uncertainty', label: t('ui.uncertaintyDomain'), score: domainScores.uncertainty, max: 6 },
      { key: 'regret', label: t('ui.regretDomain'), score: domainScores.regret, max: 6 },
      { key: 'speed', label: t('ui.speedDomain'), score: domainScores.speed, max: 6 },
    ];
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  🔑 {t('ui.decisionKeywords')}
                </h3>
                <p className="text-sm font-bold text-gray-700 leading-relaxed text-center">
                  {resultDecisionKeywords}
                </p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <div className="mt-4 space-y-2 text-sm text-gray-700">
                ...
                <p>
                  📌 {t('ui.intuitiveDomain')}: <span className="font-semibold">{weakestLabels.join(' · ')}</span>
                </p>
              </div>
            </div>
            ...
            {(resultBestSituations || resultRiskySituations) && (
              <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  🧭 {t('ui.situationGuideTitle')}
                </h3>
                <div className="space-y-2 text-sm text-gray-700">
                  {resultBestSituations && (
                    <p><span className="font-semibold">{t('ui.bestSituationsTitle')}:</span> {resultBestSituations}</p>
                  )}
                  {resultRiskySituations && (
                    <p><span className="font-semibold">{t('ui.riskySituationsTitle')}:</span> {resultRiskySituations}</p>
                  )}
                </div>
              </div>
            )}
            ...
            {resultImprovement && (
              <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  ✅ {t('ui.improvementDirectionTitle')}
                </h3>
