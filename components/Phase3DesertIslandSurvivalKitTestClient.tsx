...
import {
  Phase3DesertIslandSurvivalKitQuestion,
  Phase3DesertIslandSurvivalKitResult,
  calculatePhase3DesertIslandSurvivalKitResult,
} from '@/lib/phase3DesertIslandSurvivalKitData';
...
const DESERT_ISLAND_KIT_ROUND_STRIP: Record<string, RegExp[]> = {
...
function stripDesertIslandKitRoundPrefix(locale: string, raw: string): { body: string; matched: boolean } {
  const list = DESERT_ISLAND_KIT_ROUND_STRIP[locale] ?? DESERT_ISLAND_KIT_ROUND_STRIP.ko;
  let s = raw;
...
function formatDesertIslandKitQuestionText(
  locale: string,
  displayRound1Based: number,
  rawQuestionText: string,
  originalQuestionId: number
): string {
  const { body, matched } = stripDesertIslandKitRoundPrefix(locale, rawQuestionText);
  const textBody = matched ? body : rawQuestionText;
  const useFinal = displayRound1Based === 5 && originalQuestionId === 5;
  switch (locale) {
    case 'en':
      return useFinal
        ? `Q${displayRound1Based}. Final choice. ${textBody}`
        : `Q${displayRound1Based}. ${textBody}`;
    case 'ja':
      return useFinal
        ? `Q${displayRound1Based}。最後の選択。${textBody}`
        : `Q${displayRound1Based}。${textBody}`;
    case 'zh-CN':
      return useFinal
        ? `Q${displayRound1Based}。最后一选。${textBody}`
        : `Q${displayRound1Based}。${textBody}`;
    case 'zh-TW':
      return useFinal
        ? `Q${displayRound1Based}。最後一選。${textBody}`
        : `Q${displayRound1Based}。${textBody}`;
    case 'vi':
      return useFinal
        ? `Q${displayRound1Based}. Lựa chọn cuối. ${textBody}`
        : `Q${displayRound1Based}. ${textBody}`;
    case 'id':
      return useFinal
        ? `Q${displayRound1Based}. Pilihan terakhir. ${textBody}`
        : `Q${displayRound1Based}. ${textBody}`;
    default:
      return useFinal
        ? `Q${displayRound1Based}. 마지막 선택. ${textBody}`
        : `Q${displayRound1Based}. ${textBody}`;
  }
}
...
function parseDesertIslandKitDescription(text: string): {
  mainBody: string;
  strengths: string;
  oneLiner: string;
} {
...
interface Phase3DesertIslandSurvivalKitTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3DesertIslandSurvivalKitQuestion[];
  results: Phase3DesertIslandSurvivalKitResult[];
...
export default function Phase3DesertIslandSurvivalKitTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3DesertIslandSurvivalKitTestClientProps) {
  const t = useTranslations('phase3DesertIslandSurvivalKitTest');
  ...
  const [result, setResult] = useState<Phase3DesertIslandSurvivalKitResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3DesertIslandSurvivalKitQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3DesertIslandSurvivalKitQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3DesertIslandSurvivalKitResult(answersArray);
          ...
          <div className="px-4">
            ...
            <div className="flex justify-center mb-4">
              <button
                ...
                className="bg-gradient-to-r from-teal-500 to-orange-500 hover:from-teal-600 hover:to-orange-600 text-white font-bold py-4 px-8 rounded-full shadow-lg transform hover:scale-105 transition-all duration-200"
              >
  ...
  if (showResult && result) {
    ...
    const { mainBody: resultMainBody, strengths: resultStrengths, oneLiner: resultOneLiner } =
      parseDesertIslandKitDescription(resultLongDescription);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-cyan-50 via-amber-50 to-orange-50">
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  🏝️ {t('ui.survivalStyleType')}
                </h3>
                <p className="text-2xl font-bold text-teal-600 text-center" style={{ fontSize: '1.5em' }}>
                  {resultEmpathyLevel}
                </p>
              </div>
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {resultCharacteristics
                    ...
                    <span
                      key={index}
                      className="inline-block bg-gradient-to-r from-cyan-100 to-amber-100 text-teal-800 px-3 py-1.5 rounded-full text-sm font-medium"
                    >
                      {char}
                    </span>
                  ))}
...
{resultBadMatch && (
            ...
            {isType6 && resultCertification && (
              <div className="bg-gradient-to-r from-cyan-100 to-amber-100 rounded-xl shadow-lg p-4 mb-3">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  🏆 {t('ui.certificationPhrase')}
                </h3>
                <p className="text-sm font-semibold text-teal-800 leading-relaxed text-center">
                  {resultCertification}
                </p>
              </div>
            )}
            ...
            <div className="mt-8 mb-6 px-4">
              <button
                ...
                className="w-full bg-gradient-to-r from-teal-500 to-orange-500 text-white font-bold py-4 px-6 rounded-xl hover:from-teal-600 hover:to-orange-600 transition-all shadow-md flex items-center justify-center gap-3"
              >
            ...
            <div className="flex flex-col sm:flex-row gap-4 px-4">
              ...
              <Link
                href={`/${locale}`}
                className="flex-1 bg-gradient-to-r from-teal-500 to-orange-500 text-white font-bold py-4 px-6 rounded-xl hover:from-teal-600 hover:to-orange-600 transition-all text-center shadow-md"
              >
  ...
  const questionText = formatDesertIslandKitQuestionText(
    locale,
    currentQuestion + 1,
    questionTextRaw,
    question.id
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
            <div
              className="bg-gradient-to-r from-teal-500 to-orange-500 h-full rounded-full transition-all duration-300 ease-out"
