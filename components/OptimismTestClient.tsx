...
import { OptimismQuestion, OptimismResult, calculateOptimismResult } from '@/lib/optimismData';
...
interface OptimismTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: OptimismQuestion[];
  results: OptimismResult[];
...
export default function OptimismTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  playCount = 0,
  similarTests = []
}: OptimismTestClientProps) {
  const t = useTranslations();
  const [started, setStarted] = useState(false);
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [answers, setAnswers] = useState<any[]>([]);
  const [result, setResult] = useState<OptimismResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<OptimismQuestion[]>(questions);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: OptimismQuestion[]) => {
  ...
  const handleAnswer = (scores: any) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculateOptimismResult(newAnswers);
      const optimismResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (optimismResult) {
        setResult(optimismResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (optimismResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(optimismResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 낙관 결과:', optimismResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  const calculateResult = (finalAnswers: any[]) => {
    const resultType = calculateOptimismResult(finalAnswers);
    const optimismResult = results.find(r => r.type === resultType);
  ...
  const handleShareResult = async () => {
    ...
    if (navigator.share) {
      ...
      try {
        ...
        alert(t('optimismTest.alerts.resultCopied'));
      } catch (error) {
        console.error('클립보드 복사 실패:', error);
        alert(t('optimismTest.alerts.shareFailed'));
      }
    }
  };
  ...
  const shareToWeChat = async () => {
    ...
    const shareMessage = result 
      ? t('optimismTest.shareMessages.wechat', { type: resultTitle })
      : title;
    const shareText = `${shareMessage}\n\n${url}`;
    ...
    try {
      ...
      alert(t('optimismTest.alerts.wechatCopy'));
    } catch (error) {
      alert(t('optimismTest.alerts.shareFailed'));
    }
  };
  ...
  const shareToWhatsApp = () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareMessage = result 
      ? t('optimismTest.shareMessages.whatsapp', { type: resultTitle })
      : title;
  ...
  const shareToKakao = () => {
    ...
    if (!window.Kakao || !window.Kakao.isInitialized()) {
      alert(t('optimismTest.alerts.kakaoInit'));
      return;
    }
    ...
    const shareDescription = result 
      ? t('optimismTest.shareMessages.kakao', { type: resultTitle })
      : description;
    ...
    try {
      ...
      alert(t('optimismTest.alerts.kakaoError'));
    }
  };
  ...
  const shareToTelegram = () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareMessage = result 
      ? t('optimismTest.shareMessages.telegram', { type: resultTitle })
      : title;
  ...
  const copyLink = () => {
    ...
    alert(t('optimismTest.alerts.linkCopied'));
  };
      ...
      <div className="min-h-screen bg-white">
        <div className="max-w-4xl mx-auto">
          <div className="relative w-full overflow-hidden mb-3" style={{ aspectRatio: '680/384' }}>
            <Image
              src={getThumbnailUrl(thumbnail || 'test_204_optimism_vs_pessimism.jpg')}
              alt={title}
              fill
          ...
          <div className="px-4">
            ...
            <div className="text-gray-600 mb-6 leading-relaxed text-center space-y-4">
              <p className="font-bold text-gray-700">{t('optimismTest.startMessage.line1')}</p>
              <p>{t('optimismTest.startMessage.line2')}</p>
              <p>{t('optimismTest.startMessage.line3')}</p>
              <p>{t('optimismTest.startMessage.line4')}</p>
              <p className="whitespace-pre-line">{t('optimismTest.startMessage.line5')}</p>
              <p>{t('optimismTest.startMessage.line6')}</p>
            </div>
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-3">
                  ✅ {t('optimismTest.result.strengths')}
                </h3>
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-3">
                  ⚠️ {t('optimismTest.result.weaknesses')}
                </h3>
                <div className="flex flex-wrap gap-2">
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                📊 {t('optimismTest.result.score')}
              </h3>
              <div className="flex flex-wrap gap-2">
                {resultScore.split(/[,，、]\s*/).map((scoreItem, index) => (
                  <span
                    key={index}
