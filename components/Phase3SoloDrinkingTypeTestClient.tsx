...
import {
  ...
  Phase3SoloDrinkingTypeResult,
  calculatePhase3SoloDrinkingTypeResult,
...
interface Phase3SoloDrinkingTypeTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3SoloDrinkingTypeQuestion[];
  results: Phase3SoloDrinkingTypeResult[];
...
export default function Phase3SoloDrinkingTypeTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3SoloDrinkingTypeTestClientProps) {
  const t = useTranslations('phase3SoloDrinkingTypeTest');
  ...
  const [result, setResult] = useState<Phase3SoloDrinkingTypeResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3SoloDrinkingTypeQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3SoloDrinkingTypeQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3SoloDrinkingTypeResult(answersArray);
  ...
  if (showResult && result) {
    ...
    const resultSoloDiningType =
      result.soloDrinkingType[locale as keyof typeof result.soloDrinkingType] || result.soloDrinkingType.ko;
    const resultSoloDiningKeywords =
      result.soloDrinkingKeywords[locale as keyof typeof result.soloDrinkingKeywords] ||
      result.soloDrinkingKeywords.ko;
    const resultFavoritePlaces =
      result.favoriteDrinks[locale as keyof typeof result.favoriteDrinks] || result.favoriteDrinks.ko;
    const resultFavoriteMenus =
      result.favoriteSnacks[locale as keyof typeof result.favoriteSnacks] || result.favoriteSnacks.ko;
    const resultPersonalityTrait =
      result.personalityTrait[locale as keyof typeof result.personalityTrait] || result.personalityTrait.ko;
    const resultHiddenSide =
      result.drinkingHabit[locale as keyof typeof result.drinkingHabit] || result.drinkingHabit.ko;
    const resultUpgradeTip =
      result.drinkingTip[locale as keyof typeof result.drinkingTip] || result.drinkingTip.ko;
          ...
          <div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-5">
                <h3 className="text-base font-bold text-gray-800 mb-4 text-left">
                  {t('ui.soloDrinkingKeywords')}
                </h3>
                <div className="flex flex-wrap justify-center gap-2">
                  {resultSoloDiningKeywords.split(/[·、，]/).map((part, index) => (
                    <span
                      key={index}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultFavoritePlaces}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  {t('ui.favoriteSnacks')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">{resultFavoriteMenus}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  {t('ui.personalityTrait')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">{resultPersonalityTrait}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultHiddenSide}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultUpgradeTip}</p>
              </div>
            </div>
