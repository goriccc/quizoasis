...
import {
  Phase3OfficeBalanceGameQuestion,
  Phase3OfficeBalanceGameResult,
  calculatePhase3OfficeBalanceGameResult,
...
function stripOfficeBalanceRoundPrefix(locale: string, raw: string): { body: string; matched: boolean } {
  const list = OFFICE_BALANCE_ROUND_STRIP[locale] ?? OFFICE_BALANCE_ROUND_STRIP.ko;
  let s = raw;
...
function formatOfficeBalanceRoundQuestionText(
  locale: string,
  displayRound1Based: number,
  rawQuestionText: string,
  originalQuestionId: number
): string {
  const { body, matched } = stripOfficeBalanceRoundPrefix(locale, rawQuestionText);
  if (!matched) return rawQuestionText;
  const useFinal = displayRound1Based === 10 && originalQuestionId === 10;
  switch (locale) {
    case 'en':
      return useFinal
...
/** 결과 본문: 이중 줄바꿈 단위로 나누고, 콜론이 있으면 앞·뒤를 제목·본문으로 분리해 별도 섹션으로 표시 */
function renderOfficeBalanceResultDescription(text: string) {
  ...
  return segments.map((segment, index) => {
    ...
    if (hasLabeledSection) {
      const heading = stripSectionHeadingEmoji(trimmed.slice(0, colonIndex).trim());
      const body = trimmed.slice(colonIndex + 1).trim();
      return (
        <div
          key={index}
          className="mb-4 last:mb-0 rounded-xl border border-purple-100 bg-white/70 px-4 py-3 text-left shadow-sm"
        >
          <h4 className="text-sm font-bold text-gray-900 mb-2">{heading}</h4>
          <p className="text-sm text-gray-600 leading-relaxed whitespace-pre-wrap">{body}</p>
        </div>
      );
    }
    return (
      <p key={index} className="text-base text-gray-600 leading-relaxed mb-4 last:mb-0 whitespace-pre-wrap text-left">
        {trimmed}
      </p>
    );
  });
...
interface Phase3OfficeBalanceGameTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3OfficeBalanceGameQuestion[];
  results: Phase3OfficeBalanceGameResult[];
...
export default function Phase3OfficeBalanceGameTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3OfficeBalanceGameTestClientProps) {
  const t = useTranslations('phase3OfficeBalanceGameTest');
  ...
  const [result, setResult] = useState<Phase3OfficeBalanceGameResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3OfficeBalanceGameQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3OfficeBalanceGameQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3OfficeBalanceGameResult(answersArray);
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            <div className="text-center mb-3 bg-white rounded-2xl shadow-lg p-4 md:p-5">
              ...
              <div className="text-base leading-relaxed">{renderOfficeBalanceResultDescription(resultLongDescription)}</div>
            </div>
  ...
  const questionText = formatOfficeBalanceRoundQuestionText(
    locale,
    currentQuestion + 1,
    questionTextRaw,
    question.id
