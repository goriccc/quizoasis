...
import {
  Phase3DopamineSelfControlIndexQuestion,
  Phase3DopamineSelfControlIndexResult,
  calculatePhase3DopamineSelfControlIndexResult,
...
interface Phase3DopamineSelfControlIndexTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3DopamineSelfControlIndexQuestion[];
  results: Phase3DopamineSelfControlIndexResult[];
...
export default function Phase3DopamineSelfControlIndexTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3DopamineSelfControlIndexTestClientProps) {
  const t = useTranslations('phase3DopamineSelfControlIndexTest');
  ...
  const [result, setResult] = useState<Phase3DopamineSelfControlIndexResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3DopamineSelfControlIndexQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3DopamineSelfControlIndexQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3DopamineSelfControlIndexResult(answersArray);
      const dopamineResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (dopamineResult) {
        setResult(dopamineResult);
      }
    }
  };
  ...
  const handleShareResult = async () => {
    ...
    const customShare = result.shareMessage[locale as keyof typeof result.shareMessage] || result.shareMessage.ko;
  ...
  const shareToLine = () => {
    ...
    const shareName = result ? (result.shareMessage[locale as keyof typeof result.shareMessage] || result.shareMessage.ko || (result.title[locale as keyof typeof result.title] || result.title.ko)) : '';
  ...
  if (showResult && result) {
    ...
    const resultControlLevel = result.controlLevel[locale as keyof typeof result.controlLevel] || result.controlLevel.ko;
    const resultControlIndex = result.controlIndex[locale as keyof typeof result.controlIndex] || result.controlIndex.ko;
    const resultStrength = result.strength
      ? (result.strength[locale as keyof typeof result.strength] || result.strength.ko)
      : null;
    const resultCaution = result.caution
      ? (result.caution[locale as keyof typeof result.caution] || result.caution.ko)
      : null;
    const resultCommonTitle = result.commonTitle
      ? (result.commonTitle[locale as keyof typeof result.commonTitle] || result.commonTitle.ko)
      : null;
    const resultCommon = result.common
      ? (result.common[locale as keyof typeof result.common] || result.common.ko)
      : null;
    const resultPrescriptionTitle = result.prescriptionTitle
      ? (result.prescriptionTitle[locale as keyof typeof result.prescriptionTitle] || result.prescriptionTitle.ko)
      : null;
    const resultPrescription = result.prescription
      ? (result.prescription[locale as keyof typeof result.prescription] || result.prescription.ko)
      : null;
    const resultEncouragement = result.encouragement
      ? (result.encouragement[locale as keyof typeof result.encouragement] || result.encouragement.ko)
      : null;
    const resultTodayTask = result.todayTask[locale as keyof typeof result.todayTask] || result.todayTask.ko;
      ...
      <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-blue-50">
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            ...
            {resultStrength && (
              <div className="bg-white rounded-xl shadow-lg p-4 md:p-5 mb-3 border border-purple-100/80">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {resultStrength.split(/[,、，]/).map((char, index) => (
                    <span
                      key={index}
            ...
            {resultCommonTitle && resultCommon && (
              <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  {resultCommonTitle}
                </h3>
            ...
            {resultPrescriptionTitle && resultPrescription && (
              <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  {resultPrescriptionTitle}
                </h3>
            ...
            {resultEncouragement && (
              <div className="bg-white rounded-xl shadow-lg p-4 mb-3 border border-rose-100">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  🤍 {t('ui.encouragement')}
                </h3>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <p className="text-sm text-gray-700 leading-relaxed text-center font-semibold">
                {resultTodayTask}
              </p>
            </div>
