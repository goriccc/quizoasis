...
export default function FacePsychStateTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  const t = useTranslations('facePsychStateTest');
  ...
  const [psychologyMessage, setPsychologyMessage] = useState<string>('');
  ...
  const analyzeFace = async (imageData: string) => {
    ...
    try {
      ...
      if (landmarks && landmarks.length > 0) {
        ...
        // Generate psychology message based on emotions
        const message = generateTruePsychologyMessage(emotions);
        setPsychologyMessage(message);
        ...
        // Attach emotion analysis to result
        (faceResult as any).emotions = emotions;
        (faceResult as any).landmarks = landmarks;
        (faceResult as any).blendshapes = blendshapes;
  ...
  const analyzeEmotionsFromBlendshapes = (blendshapes: Array<{categoryName: string, score: number}>): Record<string, number> => {
    ...
    for (const blend of blendshapes) {
      ...
      if (name.includes('smile') || name.includes('mouthSmile')) {
        scores.happy = (scores.happy || 0) + blend.score;
      }
      if (name.includes('frown') || name.includes('mouthFrown')) {
        scores.sad = (scores.sad || 0) + blend.score;
      }
      if (name.includes('jawOpen') && blend.score > 0.3) {
    ...
    // Calculate neutral as inverse of strong emotions
    ...
    // If neutral is dominant, emphasize it
    if (emotions.neutral > 0.5 && emotionSum < 0.5) {
      emotions.neutral = 0.8;
      for (const key in emotions) {
        if (key !== 'neutral') {
          emotions[key as keyof typeof emotions] = emotions[key as keyof typeof emotions] * 0.2;
        }
      }
    }
  ...
  const shareToTelegram = () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareText = result ? t('shareMessages.telegram', { type: resultTitle }) : title;
  ...
  const shareToWeChat = async () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareText = result ? t('shareMessages.wechat', { type: resultTitle }) : title;
  ...
  const shareToLine = () => {
    const url = encodeURIComponent(`https://myquizoasis.com${window.location.pathname}`);
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareText = result ? t('shareMessages.line', { type: resultTitle }) : title;
    const text = encodeURIComponent(shareText);
  ...
  const shareToWhatsApp = () => {
    const url = encodeURIComponent(`https://myquizoasis.com${window.location.pathname}`);
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareText = result ? t('shareMessages.whatsapp', { type: resultTitle }) : title;
    const text = encodeURIComponent(shareText);
  ...
  const resultData = useMemo(() => {
    ...
    return { resultTitle, resultDescription, resultStrengths, resultRecommendations, resultAdvice, whenFortune, styleFortune, warningFortune };
  }, [result, locale]);
  ...
  if (showResult && result && resultData) {
    const { resultTitle, resultDescription } = resultData;
    const emotions = (result as any).emotions || { happy: 0.2, sad: 0.1, neutral: 0.4, angry: 0.05, surprised: 0.05, fearful: 0.05, disgusted: 0.05, contempt: 0.05 };
    const emotionLabels: Record<string, string> = {
      happy: t('ui.emotion.happy'),
      sad: t('ui.emotion.sad'),
      neutral: t('ui.emotion.neutral'),
    ...
    // Use pre-generated psychology message from state
    // Generate short summary (30 chars max)
    const generateSummary = () => {
      const hasSad = emotions.sad > 0.15;
      const hasWorry = emotions.fearful > 0.1;
      const isHappy = emotions.happy > 0.4;
      const dominantKey = dominantEmotion[0];
      
      if (locale === 'ko') {
        if (isHappy && hasSad) return "밝은 표정 속 조용한 감정이";
        if (isHappy) return "전반적으로 긍정적인 에너지";
        if (dominantKey === 'sad') return "내면에 깊은 감정이 자리";
        if (dominantKey === 'angry') return "경직되고 긴장된 상태";
        if (hasWorry) return "표면적 평온 뒤 걱정이";
        return "복합적인 감정 상태";
      } else if (locale === 'ja') {
        if (isHappy && hasSad) return "明るい表情の中の静かな感情";
        if (isHappy) return "全体的にポジティブなエネルギー";
        if (dominantKey === 'sad') return "内面に深い感情が宿る";
        if (dominantKey === 'angry') return "硬直して緊張した状態";
        if (hasWorry) return "表面的な平穏の後ろの心配";
        return "複合的な感情状態";
      } else if (locale === 'zh-CN' || locale === 'zh-TW') {
        if (isHappy && hasSad) return "明亮表情中的安静情感";
        if (isHappy) return "整体积极的能量";
        if (dominantKey === 'sad') return "内心深处的感受";
        if (dominantKey === 'angry') return "僵硬紧张的狀態";
        if (hasWorry) return "表面平静后的担忧";
        return "复杂的情感状态";
      } else if (locale === 'vi') {
        if (isHappy && hasSad) return "Cảm xúc yên tĩnh trong biểu cảm sáng";
        if (isHappy) return "Năng lượng tích cực tổng thể";
        if (dominantKey === 'sad') return "Cảm xúc sâu bên trong";
        if (dominantKey === 'angry') return "Trạng thái căng thẳng";
        if (hasWorry) return "Lo lắng sau sự bình yên bề mặt";
        return "Trạng thái cảm xúc phức tạp";
      } else if (locale === 'id') {
        if (isHappy && hasSad) return "Emosi tenang dalam ekspresi cerah";
        if (isHappy) return "Energi positif secara keseluruhan";
        if (dominantKey === 'sad') return "Emosi mendalam di dalam";
        if (dominantKey === 'angry') return "Keadaan tegang dan kaku";
        if (hasWorry) return "Kekhawatiran di balik ketenangan permukaan";
        return "Keadaan emosional yang kompleks";
      } else {
        if (isHappy && hasSad) return "Quiet emotions behind bright expression";
        if (isHappy) return "Overall positive energy";
        if (dominantKey === 'sad') return "Deep emotions within";
        if (dominantKey === 'angry') return "Tense and strained state";
        if (hasWorry) return "Worry behind surface calm";
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            <div className="text-center mb-3 bg-white rounded-2xl shadow-lg p-4 md:p-5">
              <h2 className="text-xl font-bold text-gray-800 mb-3">{t('ui.yourPsychState')}</h2>
              <div className="text-6xl mb-2">{emotionEmojis[dominantEmotion[0]] || '😊'}</div>
              <h1 className="text-2xl md:text-3xl font-bold mb-2 text-gray-800">{emotionLabels[dominantEmotion[0]]}</h1>
              <p className="text-base text-gray-600 mb-1">{generateSummary()}</p>
            </div>
            
            {/* 겉으로 보이는 표정 섹션 (감정 그래프) */}
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-4 text-left">😊 {t('ui.visibleExpression')}</h3>
              <div className="space-y-3">
                 {Object.entries(emotions).sort(([,a], [,b]) => (b as number) - (a as number)).map(([emotion, score]) => (
                  <div key={emotion} className="w-full">
                    <div className="flex justify-between items-center mb-1">
                      <span className="text-sm font-medium text-gray-700">{emotionLabels[emotion]}</span>
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2.5">
                      <div
                        ...
                        style={{
                          width: `${Math.max((score as number) * 100, 2)}%`,
            ...
            {/* 진짜 심리 상태 섹션 */}
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3 text-left">💭 {t('ui.truePsychState')}</h3>
              <p className="text-sm text-gray-700 leading-relaxed text-left">{psychologyMessage}</p>
            </div>
            <div className="mt-8 mb-6 px-4">
              <button
                onClick={() => {
                  const rt = result.title[locale as keyof typeof result.title] || result.title.ko;
                  const shareText = t('shareMessages.default', { type: rt });
                  const url = `https://myquizoasis.com${window.location.pathname}`;
                  const shareMessage = `${shareText}\n\n${url}`;
                  if (navigator.share) {
                    navigator.share({ title: rt, text: shareMessage, url });
                  } else {
