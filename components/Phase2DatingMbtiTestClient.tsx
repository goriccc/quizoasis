...
import { Phase2DatingMbtiQuestion, Phase2DatingMbtiResult, calculatePhase2DatingMbtiResult } from '@/lib/phase2_dating_mbti_data';
...
interface Phase2DatingMbtiTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase2DatingMbtiQuestion[];
  results: Phase2DatingMbtiResult[];
...
export default function Phase2DatingMbtiTestClient({ 
  ...
  const [result, setResult] = useState<Phase2DatingMbtiResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2DatingMbtiQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase2DatingMbtiQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase2DatingMbtiResult(answersArray);
      const datingMbtiResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (datingMbtiResult) {
        setResult(datingMbtiResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (datingMbtiResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(datingMbtiResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 연애 MBTI 결과:', datingMbtiResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const resultDatingStyle = result.datingStyle[locale as keyof typeof result.datingStyle] || result.datingStyle.ko;
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {resultDatingStyle.split(/[,、，]/).map((char, index) => (
                    <span
                      key={index}
