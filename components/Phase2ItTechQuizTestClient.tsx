...
import { Phase2ItTechQuizQuestion, Phase2ItTechQuizResult, calculatePhase2ItTechQuizResult } from '@/lib/phase2_it_tech_quiz_data';
...
interface Phase2ItTechQuizTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase2ItTechQuizQuestion[];
  results: Phase2ItTechQuizResult[];
...
export default function Phase2ItTechQuizTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  playCount = 0,
  similarTests = []
}: Phase2ItTechQuizTestClientProps) {
  const t = useTranslations('phase2ItTechQuizTest');
  ...
  const [result, setResult] = useState<Phase2ItTechQuizResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2ItTechQuizQuestion[]>([]);
  ...
  useEffect(() => {
    if (result) {
      const loadProducts = async () => {
        try {
          // IT 테크 퀴즈에 맞는 키워드 사용 (한국어일 때는 시즌/추천 상품, 다른 언어는 결과 타입 기반)
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase2ItTechQuizQuestion[]) => {
  ...
  const handleAnswer = (selectedOptionIndex: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산 (정답 체크)
      const resultType = calculatePhase2ItTechQuizResult(newAnswers, questions);
      const itTechQuizResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (itTechQuizResult) {
        setResult(itTechQuizResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (itTechQuizResult) {
        // 한국어일 때는 시즌/추천 상품, 다른 언어는 결과 타입 기반
        const keyword = locale === 'ko' ? 'trending products' : getProductKeywordsForDating(itTechQuizResult.type, locale)[0];
        const loadStartTime = Date.now();
        console.log('🔮 [시작] IT 상식 퀴즈 결과:', itTechQuizResult.type, '→ 검색 키워드:', keyword);
  ...
  if (showResult && result) {
    ...
    const resultItLevel = result.itLevel[locale as keyof typeof result.itLevel] || result.itLevel.ko;
