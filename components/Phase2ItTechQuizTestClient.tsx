...
export default function Phase2ItTechQuizTestClient({ 
  ...
  const handleAnswer = (selectedOptionIndex: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      if (itTechQuizResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(itTechQuizResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] IT 상식 퀴즈 결과:', itTechQuizResult.type, '→ 검색 키워드:', keywords[0]);
