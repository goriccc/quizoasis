...
import {
  Phase3MyHashtagGeneratorQuestion,
  Phase3MyHashtagGeneratorResult,
  calculatePhase3MyHashtagGeneratorResult,
...
interface Phase3MyHashtagGeneratorTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3MyHashtagGeneratorQuestion[];
  results: Phase3MyHashtagGeneratorResult[];
...
export default function Phase3MyHashtagGeneratorTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3MyHashtagGeneratorTestClientProps) {
  const t = useTranslations('phase3MyHashtagGeneratorTest');
  ...
  const [result, setResult] = useState<Phase3MyHashtagGeneratorResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3MyHashtagGeneratorQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3MyHashtagGeneratorQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3MyHashtagGeneratorResult(answersArray);
      const hashtagResult = results.find(r => r.type === resultType);
  ...
  if (showResult && result) {
    ...
    const hashtagsBlock =
      result.hashtagsBlock[locale as keyof typeof result.hashtagsBlock] || result.hashtagsBlock.ko;
    ...
    const hashtagTendency =
      result.hashtagTendency[locale as keyof typeof result.hashtagTendency] || result.hashtagTendency.ko;
    const bioSuggestion =
      result.bioSuggestion[locale as keyof typeof result.bioSuggestion] || result.bioSuggestion.ko;
    const goodPosts = result.goodPosts[locale as keyof typeof result.goodPosts] || result.goodPosts.ko;
    const feedVibe = result.feedVibe[locale as keyof typeof result.feedVibe] || result.feedVibe.ko;
    const extraHashtags =
      result.extraHashtags[locale as keyof typeof result.extraHashtags] || result.extraHashtags.ko;

    const hashtagLines = hashtagsBlock.split('\n').map((s) => s.trim()).filter(Boolean);
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            <div className="text-center mb-3 bg-white rounded-2xl shadow-lg p-4 md:p-5">
              ...
              <p className="text-base font-bold text-gray-800 mb-2 text-left">
                🏷️ {t('ui.myHashtags')}
              </p>
              <div className="text-left mb-4 space-y-1">
                {hashtagLines.map((line, idx) => (
                  <p key={idx} className="text-base font-semibold text-pink-600">
                    {line}
                  </p>
                ))}
              </div>
            ...
            <div className="mb-3 bg-white rounded-xl shadow-lg p-4 md:p-5">
              <h3 className="text-sm font-bold text-gray-800 mb-2 text-left">
                🎯 {t('ui.hashtagTendency')}
              </h3>
              <p className="text-sm text-gray-700 leading-relaxed text-left">{hashtagTendency}</p>
            </div>
            ...
            <div className="mb-3 bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl shadow-lg p-4 border border-orange-100">
              ...
              <p className="text-sm text-gray-800 leading-relaxed text-left">{bioSuggestion}</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-1 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed text-left">{goodPosts}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-sm font-bold text-gray-800 mb-2 text-left">
                  🎨 {t('ui.feedVibe')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed text-left">{feedVibe}</p>
              </div>
            </div>
            ...
            <div className="mb-3 bg-white rounded-xl shadow-lg p-4 md:p-5">
              ...
              <p className="text-sm text-gray-700 leading-relaxed text-left">{extraHashtags}</p>
            </div>
