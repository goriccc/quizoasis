...
import { JobStrengthQuestion, JobStrengthResult, calculateJobStrengthResult } from '@/lib/jobStrengthData';
...
interface JobStrengthTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: JobStrengthQuestion[];
  results: JobStrengthResult[];
...
export default function JobStrengthTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  playCount = 0,
  similarTests = []
}: JobStrengthTestClientProps) {
  const t = useTranslations();
  const [started, setStarted] = useState(false);
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [answers, setAnswers] = useState<any[]>([]);
  const [result, setResult] = useState<JobStrengthResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<JobStrengthQuestion[]>(questions);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: JobStrengthQuestion[]) => {
  ...
  const handleAnswer = (scores: any) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculateJobStrengthResult(newAnswers);
      const jobStrengthResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (jobStrengthResult) {
        setResult(jobStrengthResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (jobStrengthResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(jobStrengthResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 직무 강점 결과:', jobStrengthResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  const calculateResult = (finalAnswers: any[]) => {
    const resultType = calculateJobStrengthResult(finalAnswers);
    const jobStrengthResult = results.find(r => r.type === resultType);
  ...
  const handleShareResult = async () => {
    ...
    const shareText = t('jobStrengthTest.share.message', { resultTitle }) + `\n\n${`https://myquizoasis.com${window.location.pathname}`}`;
    ...
    if (navigator.share) {
      ...
      try {
        ...
        alert(t('jobStrengthTest.alerts.linkCopied'));
      } catch (error) {
        console.error('클립보드 복사 실패:', error);
        alert(t('jobStrengthTest.alerts.shareError'));
      }
    }
  };
  ...
  const shareToWeChat = async () => {
    ...
    const shareText = result 
      ? t('jobStrengthTest.share.message', { resultTitle }) + `\n\n${url}`
      : `${title}\n\n${url}`;
  ...
  const shareToWhatsApp = () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareText = result 
      ? encodeURIComponent(t('jobStrengthTest.share.message', { resultTitle }))
  ...
  const shareToKakao = () => {
    ...
    try {
      ...
      alert(t('jobStrengthTest.alerts.kakaoShareError'));
    }
  };
      ...
      <div className="min-h-screen bg-white">
        <div className="max-w-4xl mx-auto">
          <div className="relative w-full overflow-hidden mb-3" style={{ aspectRatio: '680/384' }}>
            <Image
              src={getThumbnailUrl(thumbnail || 'test_047_job_strength.jpg')}
              alt={title}
              fill
          ...
          <div className="px-4">
            ...
            <div className="text-gray-600 mb-6 leading-relaxed text-center space-y-4">
              <p className="font-bold text-gray-700">{t('jobStrengthTest.startMessage.title')}</p>
              <p>{t('jobStrengthTest.startMessage.line1')}</p>
              <p>{t('jobStrengthTest.startMessage.line2')}</p>
              <p>{t('jobStrengthTest.startMessage.line3')}</p>
              <p className="whitespace-pre-line">{t('jobStrengthTest.startMessage.line4')}</p>
            </div>
            ...
            <div className="flex justify-center mb-4">
              ...
              >
{t('jobStrengthTest.ui.startTest')}
              </button>
            </div>
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-4">
        ...
        <div className="flex flex-col items-center justify-center">
          ...
          <p className="mt-4 text-lg text-gray-700">{t('jobStrengthTest.ui.analyzing')}</p>
        </div>
      ...
      <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-2xl p-8 max-w-md w-full mx-4 text-center shadow-2xl">
          <h2 className="text-2xl font-bold text-gray-800 mb-4">
            {t('jobStrengthTest.ui.testComplete')}
          </h2>
          ...
          >
            {t('jobStrengthTest.ui.viewResult')}
          </button>
        </div>
      </div>
    );
  ...
  if (showResult && result) {
    ...
    const resultLongDescription = result.longDescription ? 
    ...
    const resultRecommendedCompanies = result.recommendedCompanies[locale as keyof typeof result.recommendedCompanies] || result.recommendedCompanies.ko;
          ...
          <div>
            <div className="text-center mb-3 bg-white rounded-2xl shadow-lg p-4 md:p-5">
              <h2 className="text-xl font-bold text-gray-800 mb-3">
                {t('jobStrengthTest.ui.yourResult')}
              </h2>
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-3">
                  {t('jobStrengthTest.ui.strengths')}
                </h3>
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-3">
                  {t('jobStrengthTest.ui.weaknesses')}
                </h3>
                <div className="flex flex-wrap gap-2">
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                {t('jobStrengthTest.ui.recommendedJobs')}
              </h3>
              <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-line">
                {resultRecommendedJobs}
              </p>
            </div>

            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                {t('jobStrengthTest.ui.recommendedCompanies')}
              </h3>
              <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-line">
                {resultRecommendedCompanies}
              </p>
            </div>

            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                {t('jobStrengthTest.ui.advice')}
              </h3>
              <p className="text-sm text-gray-700 leading-relaxed">
                {resultAdvice}
              </p>
            ...
            <div className="mt-8 mb-6 px-4">
              ...
              >
                ...
                {t('jobStrengthTest.ui.shareResult')}
              </button>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 px-4">
              ...
              >
                {t('jobStrengthTest.ui.retakeTest')}
              </button>
              <Link
                href={`/${locale}`}
                className="flex-1 bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold py-4 px-6 rounded-xl hover:from-purple-700 hover:to-pink-700 transition-all text-center shadow-md"
              >
                {t('jobStrengthTest.ui.otherTests')}
              </Link>
            </div>

            <div className="mt-8 mb-8 text-center px-4">
              <h2 className="text-lg font-bold text-gray-800 mb-4">
                {t('jobStrengthTest.ui.shareWithFriends')}
              </h2>
              <div className="flex justify-center gap-2">
                <button onClick={copyLink} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/link.jpeg" alt="링크 복사" width={46} height={46} className="rounded-lg" />
            ...
            {popularTestsState.length > 0 && (
              <div className="mb-8 pb-4">
                <h2 className="text-xl font-bold text-gray-800 mb-6">
                  {t('jobStrengthTest.ui.popularTestsTop5')}
                </h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
