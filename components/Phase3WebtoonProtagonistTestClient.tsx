...
import {
  Phase3WebtoonProtagonistQuestion,
  Phase3WebtoonProtagonistResult,
  calculatePhase3WebtoonProtagonistResult,
...
interface Phase3WebtoonProtagonistTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3WebtoonProtagonistQuestion[];
  results: Phase3WebtoonProtagonistResult[];
...
export default function Phase3WebtoonProtagonistTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3WebtoonProtagonistTestClientProps) {
  const t = useTranslations('phase3WebtoonProtagonistTest');
  ...
  const [result, setResult] = useState<Phase3WebtoonProtagonistResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3WebtoonProtagonistQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3WebtoonProtagonistQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3WebtoonProtagonistResult(answersArray);
  ...
  const handleShareResult = async () => {
    ...
    const customShare = result.shareLine[locale as keyof typeof result.shareLine] || result.shareLine.ko;
  ...
  const shareToLine = () => {
    ...
    const shareName = result ? (result.shareLine[locale as keyof typeof result.shareLine] || result.shareLine.ko || (result.title[locale as keyof typeof result.title] || result.title.ko)) : '';
  ...
  if (showResult && result) {
    ...
    const resultProtagonistType = result.protagonistType[locale as keyof typeof result.protagonistType] || result.protagonistType.ko;
    const resultAppearingGenre = result.appearingGenre[locale as keyof typeof result.appearingGenre] || result.appearingGenre.ko;
    const resultNarrativePattern = result.narrativePattern[locale as keyof typeof result.narrativePattern] || result.narrativePattern.ko;
    const resultStrength = result.strength[locale as keyof typeof result.strength] || result.strength.ko;
    const resultCharmPoint = result.charmPoint[locale as keyof typeof result.charmPoint] || result.charmPoint.ko;
    const resultShiningScene = result.shiningScene[locale as keyof typeof result.shiningScene] || result.shiningScene.ko;
    const resultRecommendedGenre = result.recommendedGenre[locale as keyof typeof result.recommendedGenre] || result.recommendedGenre.ko;
          ...
          <div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <div className="flex flex-wrap justify-center gap-2">
                {resultAppearingGenre.split(/[·]/).map((genre, index) => (
                  <span
                    key={index}
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                📖 {t('ui.narrativePattern')}
              </h3>
              <p className="text-sm text-gray-700 leading-relaxed text-center">
                {resultNarrativePattern}
              </p>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                💡 {t('ui.recommendedGenre')}
              </h3>
              <p className="text-sm text-gray-700 leading-relaxed text-left">
                {resultRecommendedGenre}
              </p>
            </div>
