...
import {
  Phase3SuddenPoorDefenseIndexQuestion,
  Phase3SuddenPoorDefenseIndexResult,
  calculatePhase3SuddenPoorDefenseIndexResult,
...
interface Phase3SuddenPoorDefenseIndexTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3SuddenPoorDefenseIndexQuestion[];
  results: Phase3SuddenPoorDefenseIndexResult[];
...
export default function Phase3SuddenPoorDefenseIndexTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3SuddenPoorDefenseIndexTestClientProps) {
  const t = useTranslations('phase3SuddenPoorDefenseIndexTest');
  ...
  const [result, setResult] = useState<Phase3SuddenPoorDefenseIndexResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3SuddenPoorDefenseIndexQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3SuddenPoorDefenseIndexQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3SuddenPoorDefenseIndexResult(answersArray);
  ...
  if (showResult && result) {
    ...
    const resultDefenseScore = result.defenseScore[locale as keyof typeof result.defenseScore] || result.defenseScore.ko;
