...
import {
  Phase3FoodBalanceGameQuestion,
  Phase3FoodBalanceGameResult,
  calculatePhase3FoodBalanceGameResult,
} from '@/lib/phase3FoodBalanceGameData';
...
const FOOD_BALANCE_ROUND_STRIP: Record<string, RegExp[]> = {
  ko: [/^Round\s*12\s*\.\s*국물 전쟁 최종 라운드\s*/i, /^Round\s*\d+\s*\.\s*/i],
  en: [/^Round\s*12\s*\.\s*/i, /^Round\s*\d+\s*\.\s*/i],
  ja: [/^Round\s*12\s*\.\s*/i, /^Round\s*\d+\s*\.\s*/i],
  'zh-CN': [/^Round\s*12\s*\.\s*/i, /^Round\s*\d+\s*\.\s*/i],
  'zh-TW': [/^Round\s*12\s*\.\s*/i, /^Round\s*\d+\s*\.\s*/i],
  vi: [/^Round\s*12\s*\.\s*/i, /^Round\s*\d+\s*\.\s*/i],
  id: [/^Round\s*12\s*\.\s*/i, /^Round\s*\d+\s*\.\s*/i],
};

function stripFoodBalanceRoundPrefix(locale: string, raw: string): { body: string; matched: boolean } {
  const list = FOOD_BALANCE_ROUND_STRIP[locale] ?? FOOD_BALANCE_ROUND_STRIP.ko;
  let s = raw;
...
function formatFoodBalanceRoundQuestionText(
  locale: string,
  displayRound1Based: number,
  rawQuestionText: string,
  originalQuestionId: number,
): string {
  const { body, matched } = stripFoodBalanceRoundPrefix(locale, rawQuestionText);
  if (!matched) return rawQuestionText;
  const useFinal = displayRound1Based === 12 && originalQuestionId === 12;
  if (useFinal) {
    return locale === 'ko'
      ? `Round ${displayRound1Based}. 국물 전쟁 최종 라운드. ${body}`
      : `Round ${displayRound1Based}. Final round. ${body}`;
  }
  return `Round ${displayRound1Based}. ${body}`;
}

interface Phase3FoodBalanceGameTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3FoodBalanceGameQuestion[];
  results: Phase3FoodBalanceGameResult[];
...
export default function Phase3FoodBalanceGameTestClient({
  locale,
  slug,
  title,
  description,
  ...
  isLatestTest = false,
  badgeType = null,
}: Phase3FoodBalanceGameTestClientProps) {
  const t = useTranslations('phase3FoodBalanceGameTest');
  const tGlobal = useTranslations();
  const [started, setStarted] = useState(false);
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [answers, setAnswers] = useState<Record<number, number>>({});
  const [result, setResult] = useState<Phase3FoodBalanceGameResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3FoodBalanceGameQuestion[]>([]);
  const [originalQuestionIndices, setOriginalQuestionIndices] = useState<number[]>([]);
  const [shuffledOptionsMap, setShuffledOptionsMap] = useState<Record<number, Phase3FoodBalanceGameQuestion['options']>>({});
  ...
  const shuffleQuestions = (questionList: Phase3FoodBalanceGameQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      const resultType = calculatePhase3FoodBalanceGameResult(answersArray);
      const foodResult = results.find((r) => r.type === resultType);
      if (foodResult) {
          ...
          <div className="px-4">
            ...
            <div className="flex justify-center mb-4">
              <button
                ...
                className="bg-gradient-to-r from-amber-500 to-red-600 hover:from-amber-600 hover:to-red-700 text-white font-bold py-4 px-8 rounded-full shadow-lg transform hover:scale-105 transition-all duration-200"
              >
          ...
          <div>
              ...
              <div className="px-4 py-5 md:px-6">
                <div className="mb-4">
                  <div
                    ...
                    style={{
                      background:
                        'linear-gradient(to right, #FFF8E7, #FFE4B5, #FFB347, #FF7F50, #E63946, #DC143C, #8B0000)',
                    }}
                  >
                    <div
                  ...
                  <div className="flex justify-between text-xs text-gray-600 mt-2">
                    <span>{t('ui.mildLabel')}</span>
                    <span>{t('ui.spicyLabel')}</span>
                  </div>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-4 text-center">
                  <p className="text-base font-bold text-gray-800">
                    {t('ui.mySpiceScore')}: {resultEmotionTemperature}
                  </p>
                  <p className="text-base font-bold text-gray-800">
                    {t('ui.tasteType')}: {resultEmotionColor} {result.emoji}
                  </p>
                </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-5">
                <h3 className="text-base font-bold text-gray-800 mb-4 text-left">{t('ui.tasteKeywords')}</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">{t('ui.foodSays')}</h3>
                <p className="text-sm text-gray-700 leading-relaxed">{resultStrengthAtTemp}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">{t('ui.foodTrait')}</h3>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">{t('ui.goodRestaurants')}</h3>
            ...
            {resultColorCodes && (
              <div className="mb-3">
                <div className="bg-white rounded-xl shadow-lg p-4">
                  <h3 className="text-base font-bold text-gray-800 mb-2 text-left">{t('ui.extraTrait')}</h3>
  ...
  const questionText = formatFoodBalanceRoundQuestionText(
    locale,
    currentQuestion + 1,
    questionTextRaw,
    question.id,
  ...
  const optionStyles = [
    {
      gradient: 'from-amber-50 to-amber-100 hover:from-amber-100 hover:to-amber-200 border-amber-200 hover:border-amber-400',
      badge: 'bg-amber-600',
    },
    {
      gradient: 'from-red-50 to-red-100 hover:from-red-100 hover:to-red-200 border-red-200 hover:border-red-400',
      badge: 'bg-red-600',
    },
  ];
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
            <div
              className="bg-gradient-to-r from-amber-500 to-red-600 h-full rounded-full transition-all duration-300 ease-out"
