...
import {
  Phase3SoulmateFinderQuestion,
  Phase3SoulmateFinderResult,
  calculatePhase3SoulmateFinderResult,
...
interface Phase3SoulmateFinderTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3SoulmateFinderQuestion[];
  results: Phase3SoulmateFinderResult[];
...
export default function Phase3SoulmateFinderTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3SoulmateFinderTestClientProps) {
  const t = useTranslations('phase3SoulmateFinderTest');
  ...
  const [result, setResult] = useState<Phase3SoulmateFinderResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3SoulmateFinderQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3SoulmateFinderQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3SoulmateFinderResult(answersArray);
  ...
  if (showResult && result) {
    ...
    const resultSoulType = result.soulType[locale as keyof typeof result.soulType] || result.soulType.ko;
    const resultSoulKeywords = result.soulKeywords[locale as keyof typeof result.soulKeywords] || result.soulKeywords.ko;
    const resultTwinSoul = result.twinSoul?.[locale as keyof typeof result.twinSoul] || result.twinSoul?.ko;
    const resultDestinySoul = result.destinySoul?.[locale as keyof typeof result.destinySoul] || result.destinySoul?.ko;
          ...
          <div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <div className="flex flex-wrap justify-center gap-2">
                {resultSoulKeywords.split(/[,、，]/).map((keyword, index) => (
                  <span
                    key={index}
            ...
            {(resultTwinSoul || resultDestinySoul) && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
                ...
                {resultDestinySoul && (
                  <div className="bg-white rounded-xl shadow-lg p-4">
                    ...
                    <p className="text-base font-semibold text-pink-700 text-center">
                      {resultDestinySoul}
                    </p>
                  </div>
                )}
