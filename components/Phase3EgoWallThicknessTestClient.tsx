...
import {
  Phase3EgoWallThicknessQuestion,
  Phase3EgoWallThicknessResult,
  calculatePhase3EgoWallThicknessResult,
...
interface Phase3EgoWallThicknessTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3EgoWallThicknessQuestion[];
  results: Phase3EgoWallThicknessResult[];
...
export default function Phase3EgoWallThicknessTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3EgoWallThicknessTestClientProps) {
  const t = useTranslations('phase3EgoWallThicknessTest');
  ...
  const [result, setResult] = useState<Phase3EgoWallThicknessResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3EgoWallThicknessQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3EgoWallThicknessQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3EgoWallThicknessResult(answersArray);
  ...
  if (showResult && result) {
    ...
    const resultWallThickness =
      result.wallThickness[locale as keyof typeof result.wallThickness] || result.wallThickness.ko;
    const resultStrength =
      result.strength[locale as keyof typeof result.strength] || result.strength.ko;
    const resultShadow =
      result.shadow[locale as keyof typeof result.shadow] || result.shadow.ko;
    const resultNeedNow =
          ...
          <div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-5">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {resultStrength.split(/[·,、，]/).map((part, index) => (
                    <span
                      key={index}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultShadow}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  {t('ui.needNow')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">{resultNeedNow}</p>
              </div>
            </div>
