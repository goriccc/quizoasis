...
interface Phase2FriendshipStyleTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase2FriendshipStyleQuestion[];
  results: Phase2FriendshipStyleResult[];
...
export default function Phase2FriendshipStyleTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase2FriendshipStyleTestClientProps) {
  const t = useTranslations('phase2FriendshipStyleTest');
  ...
  const [result, setResult] = useState<Phase2FriendshipStyleResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2FriendshipStyleQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase2FriendshipStyleQuestion[]) => {
  ...
  const handleAnswer = (shuffledOptionIndex: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase2FriendshipStyleResult(answersArray);
      const friendshipStyleResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (friendshipStyleResult) {
        setResult(friendshipStyleResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (friendshipStyleResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(friendshipStyleResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 우정 스타일 결과:', friendshipStyleResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const resultFriendshipStyle = result.friendshipStyle[locale as keyof typeof result.friendshipStyle] || result.friendshipStyle.ko;
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  🤝 {t('ui.friendshipStyle')}
                </h3>
                <div className="flex flex-wrap justify-center gap-2">
                  {resultFriendshipStyle.split(/[,、，]/).map((char, index) => (
                    <span
                      key={index}
