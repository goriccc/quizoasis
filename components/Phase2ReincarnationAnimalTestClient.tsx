...
import { Phase2ReincarnationAnimalQuestion, Phase2ReincarnationAnimalResult, calculatePhase2ReincarnationAnimalResult } from '@/lib/phase2_reincarnation_animal_data';
...
interface Phase2ReincarnationAnimalTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase2ReincarnationAnimalQuestion[];
  results: Phase2ReincarnationAnimalResult[];
...
export default function Phase2ReincarnationAnimalTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase2ReincarnationAnimalTestClientProps) {
  const t = useTranslations('phase2ReincarnationAnimalTest');
  ...
  const [result, setResult] = useState<Phase2ReincarnationAnimalResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2ReincarnationAnimalQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase2ReincarnationAnimalQuestion[]) => {
  ...
  const handleAnswer = (selectedTypes: string[]) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase2ReincarnationAnimalResult(newAnswers);
      const animalResult = results.find(r => r.type === resultType);
      ...
      if (animalResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(animalResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 환생 동물 결과:', animalResult.type, '→ 검색 키워드:', keywords[0]);
                  ...
                  <div className="space-y-2">
                    {(() => {
                      ...
                      const matchResult = results.find(r => {
                        const matchTitle = r.title[locale as keyof typeof r.title] || r.title.ko;
                        return matchTitle.includes(resultGoodMatch) || resultGoodMatch.includes(matchTitle);
                      });
                      if (matchResult) {
                ...
                <div className="bg-white rounded-xl shadow-lg p-4">
                  ...
                  <div className="space-y-2">
                    {(() => {
                      ...
                      const matchResult = results.find(r => {
                        const matchTitle = r.title[locale as keyof typeof r.title] || r.title.ko;
                        return matchTitle.includes(resultBadMatch) || resultBadMatch.includes(matchTitle);
                      });
                      if (matchResult) {
                        const matchTitle = matchResult.title[locale as keyof typeof matchResult.title] || matchResult.title.ko;
                        return (
