...
import {
  Phase3Balance99UltimateQuestion,
  Phase3Balance99UltimateResult,
  calculatePhase3Balance99UltimateResult,
  encodePhase3Balance99Answers,
  decodePhase3Balance99Answers,
  countSamePhase3Balance99Answers,
  getPhase3Balance99MatchGrade,
  Phase3Balance99MatchGrade,
...
interface Phase3Balance99UltimateTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3Balance99UltimateQuestion[];
  results: Phase3Balance99UltimateResult[];
...
export default function Phase3Balance99UltimateTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3Balance99UltimateTestClientProps) {
  const t = useTranslations('phase3Balance99UltimateTest');
  ...
  const [result, setResult] = useState<Phase3Balance99UltimateResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3Balance99UltimateQuestion[]>([]);
  ...
  const [matchGrade, setMatchGrade] = useState<Phase3Balance99MatchGrade | null>(null);
  ...
  useEffect(() => {
    ...
    if (k) {
      const decoded = decodePhase3Balance99Answers(k);
      if (decoded && decoded.length === 99) {
        setFriendMode(true);
        setHostAnswers(decoded);
      }
    }
  }, []);

  const getCustomShareMessage = (res: Phase3Balance99UltimateResult): string => {
    const aCount = (finalAnswers || []).filter((x) => x === 0).length;
    const bCount = (finalAnswers || []).length - aCount;
    const count = ['Type1', 'Type2', 'Type3'].includes(res.type) ? aCount : bCount;
    const raw = res.shareMessage[locale as keyof typeof res.shareMessage] || res.shareMessage.ko;
    return raw.replace(/\{count\}/g, String(count));
  };

  // 밸런스 게임은 A/B 의미·채점이 고정이므로 선택지 순서는 섞지 않음 (질문 순서만 셔플)
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3Balance99UltimateQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3Balance99UltimateResult(answersArray);
      const potentialResult = results.find(r => r.type === resultType);

      if (friendMode && hostAnswers) {
        const same = countSamePhase3Balance99Answers(hostAnswers, answersArray);
        setSameCount(same);
        setMatchGrade(getPhase3Balance99MatchGrade(same));
      }
  ...
  const handleCopyChallengeLink = async () => {
    if (!finalAnswers || finalAnswers.length !== 99) return;
    const challengeUrl = `${window.location.origin}${window.location.pathname}?k=${encodePhase3Balance99Answers(finalAnswers)}`;
    try {
      await navigator.clipboard.writeText(challengeUrl);
      alert(t('alerts.challengeLinkCopied'));
    } catch (error) {
      console.error('대결 링크 복사 실패:', error);
  ...
  const handleShareResult = async () => {
    ...
    const customShare = getCustomShareMessage(result);
    const shareText = `${customShare}\n\n${`https://myquizoasis.com${window.location.pathname}`}`;
  ...
  const shareToLine = () => {
    ...
    const shareName = result ? getCustomShareMessage(result) : '';
    const shareText = result 
      ? encodeURIComponent(shareName || t('shareMessages.line', { type: result.title[locale as keyof typeof result.title] || result.title.ko }))
  ...
  const shareToWeChat = async () => {
    ...
    const shareName = result ? getCustomShareMessage(result) : '';
    const shareText = result 
      ? `${shareName || t('shareMessages.wechat', { type: result.title[locale as keyof typeof result.title] || result.title.ko })}\n\n${url}`
  ...
  const shareToWhatsApp = () => {
    ...
    const shareName = result ? getCustomShareMessage(result) : '';
    const shareText = result 
      ? encodeURIComponent(shareName || t('shareMessages.whatsapp', { type: result.title[locale as keyof typeof result.title] || result.title.ko }))
  ...
  const shareToKakao = () => {
    ...
    const shareDescription = result 
      ? (shareName || t('shareMessages.kakao', { type: result.title[locale as keyof typeof result.title] || result.title.ko }))
  ...
  const shareToTelegram = () => {
    ...
    const shareName = result ? getCustomShareMessage(result) : '';
    const shareText = result 
      ? (shareName || t('shareMessages.telegram', { type: result.title[locale as keyof typeof result.title] || result.title.ko }))
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="px-4">
            ...
            <div className="text-gray-600 mb-6 leading-relaxed text-center space-y-4">
              {friendMode && (
                <p className="font-bold text-orange-600 bg-orange-50 border border-orange-200 rounded-xl px-4 py-3">
                  {t('ui.friendChallengeBanner')}
                </p>
              )}
              ...
              <p className="font-semibold text-gray-800">{t('startMessage.line5')}</p>
  ...
  if (showResult && result) {
    ...
    const resultSelectionTendency = result.selectionTendency[locale as keyof typeof result.selectionTendency] || result.selectionTendency.ko;
    const resultChoiceCountInfo = result.choiceCountInfo[locale as keyof typeof result.choiceCountInfo] || result.choiceCountInfo.ko;
    const resultFriendCompare = result.friendCompare[locale as keyof typeof result.friendCompare] || result.friendCompare.ko;
    const answersArray = finalAnswers || [];
    const aCount = answersArray.filter((x) => x === 0).length;
    const bCount = answersArray.length - aCount;

    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-blue-50">
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            {matchGrade && sameCount !== null && (
              <div className="text-center mb-3 bg-white rounded-2xl shadow-lg p-4 md:p-5 border-2 border-orange-200">
                <h2 className="text-xl font-bold text-gray-800 mb-3">
                  {t('ui.duelResultTitle')}
                </h2>
                <div className="text-5xl mb-2">{matchGrade.emoji}</div>
                <h3 className="text-xl font-bold text-gray-800 mb-2">
                  {matchGrade.title[locale as keyof typeof matchGrade.title] || matchGrade.title.ko}
                </h3>
                <p className="text-base text-gray-600 mb-3">
                  {matchGrade.verdict[locale as keyof typeof matchGrade.verdict] || matchGrade.verdict.ko}
                </p>
                <p className="text-lg font-semibold text-orange-600">
                  {t('ui.sameCountLabel', { sameCount })}
                </p>
              </div>
            )}
            ...
            <div className="text-center mb-3 bg-white rounded-2xl shadow-lg p-4 md:p-5">
              <h2 className="text-xl font-bold text-gray-800 mb-3">
                {matchGrade ? t('ui.personalResultTitle') : tGlobal('mbti.yourResult')}
              </h2>
              ...
              <p className="text-lg font-semibold text-gray-700 mb-3">
                &ldquo;{resultShortDescription}&rdquo;
              </p>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                {t('ui.selectionTendency')}
              </h3>
              <p className="text-sm text-gray-700 leading-relaxed text-left whitespace-pre-line">
                {resultSelectionTendency}
              </p>
            </div>

            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                {t('ui.choiceCountInfo')}
              </h3>
              <p className="text-base font-semibold text-purple-700 text-center mb-2">
                {t('ui.choiceCountLive', { aCount, bCount })}
              </p>
              <p className="text-sm text-gray-700 leading-relaxed text-left whitespace-pre-line">
                {resultChoiceCountInfo}
              </p>
            </div>
            ...
            {result.sections.map((section, index) => {
              const sectionTitle = section.title[locale as keyof typeof section.title] || section.title.ko;
              const sectionContent = section.content[locale as keyof typeof section.content] || section.content.ko;
              return (
                <div key={index} className="bg-white rounded-xl shadow-lg p-4 mb-3">
                  <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                    {sectionTitle}
                  </h3>
                  <p className="text-sm text-gray-700 leading-relaxed text-left whitespace-pre-line">
            ...
            {finalAnswers && finalAnswers.length === 99 && (
              <div className="mb-6 px-4">
                <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
                  <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                    {t('ui.duelSectionTitle')}
                  </h3>
                ...
                <button
                  ...
                  className="w-full bg-gradient-to-r from-orange-500 to-red-500 text-white font-bold py-4 px-6 rounded-xl hover:from-orange-600 hover:to-red-600 transition-all shadow-md"
                >
                  {t('ui.duelButton')}
                </button>
              </div>
  ...
  const optionsArray = [...question.options].sort((a, b) => a.score - b.score);
        ...
        <div>
          ...
          <div className="space-y-4 px-4">
            {optionsArray.map((option) => {
              ...
              // 밸런스 게임: score 0=A, 1=B 고정 (표시 순서·라벨 모두 score 기준)
              const label = option.score === 0 ? 'A' : option.score === 1 ? 'B' : '?';
              const colorIndex = option.score === 0 ? 0 : 1;
              const colors = [
              ...
              return (
                <button
                  key={`opt-${option.score}`}
                  onClick={() => handleAnswer(option.score)}
                  className={`w-full bg-gradient-to-r ${colors[colorIndex]} border-2 text-gray-800 font-medium py-3 px-4 rounded-xl transition-all transform hover:scale-102 text-left`}
                >
                  <div className="flex items-center">
                    <div className={`w-7 h-7 ${bgColors[colorIndex]} text-white rounded-full flex items-center justify-center font-bold mr-3 flex-shrink-0 text-sm`}>
                      {label}
                    </div>
