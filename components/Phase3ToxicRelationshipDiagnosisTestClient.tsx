...
import {
  Phase3ToxicRelationshipDiagnosisQuestion,
  Phase3ToxicRelationshipDiagnosisResult,
  calculatePhase3ToxicRelationshipDiagnosisResult,
...
interface Phase3ToxicRelationshipDiagnosisTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3ToxicRelationshipDiagnosisQuestion[];
  results: Phase3ToxicRelationshipDiagnosisResult[];
...
export default function Phase3ToxicRelationshipDiagnosisTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3ToxicRelationshipDiagnosisTestClientProps) {
  const t = useTranslations('phase3ToxicRelationshipDiagnosisTest');
  ...
  const [result, setResult] = useState<Phase3ToxicRelationshipDiagnosisResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3ToxicRelationshipDiagnosisQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3ToxicRelationshipDiagnosisQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3ToxicRelationshipDiagnosisResult(answersArray);
      const toxicRelationshipResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (toxicRelationshipResult) {
        setResult(toxicRelationshipResult);
      }
    }
  };
  ...
  if (showResult && result) {
    ...
    const resultDiagnosisType = result.diagnosisType[locale as keyof typeof result.diagnosisType] || result.diagnosisType.ko;
    const resultKeyTraits = result.keyTraits[locale as keyof typeof result.keyTraits] || result.keyTraits.ko;
    const resultStrategy = result.strategy[locale as keyof typeof result.strategy] || result.strategy.ko;
    const resultDeclaration = result.declaration[locale as keyof typeof result.declaration] || result.declaration.ko;
          ...
          <div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 md:p-5 mb-3 border border-purple-100/80">
              ...
              <div className="flex flex-wrap justify-center gap-2">
                {resultKeyTraits.split(/[,、，]/).map((char, index) => (
                  <span
                    key={index}
