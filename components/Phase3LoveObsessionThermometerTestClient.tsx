...
import {
  Phase3LoveObsessionThermometerQuestion,
  Phase3LoveObsessionThermometerResult,
  calculatePhase3LoveObsessionThermometerResult,
...
interface Phase3LoveObsessionThermometerTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3LoveObsessionThermometerQuestion[];
  results: Phase3LoveObsessionThermometerResult[];
...
export default function Phase3LoveObsessionThermometerTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3LoveObsessionThermometerTestClientProps) {
  const t = useTranslations('phase3LoveObsessionThermometerTest');
  ...
  const [result, setResult] = useState<Phase3LoveObsessionThermometerResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3LoveObsessionThermometerQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3LoveObsessionThermometerQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3LoveObsessionThermometerResult(answersArray);
  ...
  if (showResult && result) {
    ...
    const resultLongDescription =
      result.description[locale as keyof typeof result.description] || result.description.ko;
    const resultKeywords = result.keywords[locale as keyof typeof result.keywords] || result.keywords.ko;
          ...
          <div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <div className="flex flex-wrap gap-2 justify-center">
                {resultKeywords.split(/[,、，]/).map((kw, idx) => (
                  <span
                    key={idx}
                    className="inline-block bg-gradient-to-r from-purple-100 to-pink-100 text-purple-800 text-sm font-semibold px-3 py-1 rounded-full shadow-sm"
                  >
                    {kw.trim()}
                  </span>
                ))}
            ...
            {(() => {
              ...
              for (let i = 0; i < result.sections.length; i++) {
                ...
                const sectionTitle =
                  section.title[locale as keyof typeof section.title] || section.title.ko;
                const sectionContent =
                  section.content[locale as keyof typeof section.content] || section.content.ko;
                const next = result.sections[i + 1];
                const nextTitle = next
                  ? next.title[locale as keyof typeof next.title] || next.title.ko
                  : '';
                const isStrengthWeaknessPair =
                  Boolean(next) &&
                  (sectionTitle.includes('강점') ||
                    sectionTitle.includes('Strength') ||
                    sectionTitle.includes('強み') ||
                    sectionTitle.includes('优点') ||
                    sectionTitle.includes('優點') ||
                    sectionTitle.includes('Điểm mạnh') ||
                    sectionTitle.includes('Kelebihan') ||
                    sectionTitle.includes('💪')) &&
                  (nextTitle.includes('약점') ||
                    nextTitle.includes('주의점') ||
                    nextTitle.includes('Weakness') ||
                    nextTitle.includes('Caution') ||
                    nextTitle.includes('弱み') ||
                    nextTitle.includes('注意') ||
                    nextTitle.includes('弱点') ||
                    nextTitle.includes('注意点') ||
                    nextTitle.includes('弱點') ||
                    nextTitle.includes('注意點') ||
                    nextTitle.includes('Điểm yếu') ||
                    nextTitle.includes('Lưu ý') ||
                    nextTitle.includes('Kelemahan') ||
                    nextTitle.includes('Perhatian') ||
                    nextTitle.includes('⚠️'));

                if (isStrengthWeaknessPair && next) {
                  const nextContent =
                    next.content[locale as keyof typeof next.content] || next.content.ko;
                  nodes.push(
                ...
                nodes.push(
                  <div key={i} className="bg-white rounded-xl shadow-lg p-4 mb-3">
                    <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                      {sectionTitle}
                    </h3>
                    <p className="text-sm text-gray-700 leading-relaxed text-center whitespace-pre-line">
