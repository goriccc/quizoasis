...
export default function Phase2LiteratureQuizTestClient({ 
  ...
  const handleAnswer = (selectedOptionIndex: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      if (literatureQuizResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(literatureQuizResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 문학 퀴즈 결과:', literatureQuizResult.type, '→ 검색 키워드:', keywords[0]);
