...
import { Phase2LiteratureQuizQuestion, Phase2LiteratureQuizResult, calculatePhase2LiteratureQuizResult } from '@/lib/phase2_literature_quiz_data';
...
interface Phase2LiteratureQuizTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase2LiteratureQuizQuestion[];
  results: Phase2LiteratureQuizResult[];
...
export default function Phase2LiteratureQuizTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase2LiteratureQuizTestClientProps) {
  const t = useTranslations('phase2LiteratureQuizTest');
  ...
  const [result, setResult] = useState<Phase2LiteratureQuizResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2LiteratureQuizQuestion[]>([]);
  ...
  useEffect(() => {
    if (result) {
      const loadProducts = async () => {
        try {
          // 문학 퀴즈에 맞는 키워드 사용 (한국어일 때는 시즌/추천 상품, 다른 언어는 결과 타입 기반)
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase2LiteratureQuizQuestion[]) => {
  ...
  const handleAnswer = (selectedOptionIndex: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산 (정답 체크)
      const resultType = calculatePhase2LiteratureQuizResult(newAnswers, questions);
      const literatureQuizResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (literatureQuizResult) {
        setResult(literatureQuizResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (literatureQuizResult) {
        // 한국어일 때는 시즌/추천 상품, 다른 언어는 결과 타입 기반
        const keyword = locale === 'ko' ? 'trending products' : getProductKeywordsForDating(literatureQuizResult.type, locale)[0];
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 문학 퀴즈 결과:', literatureQuizResult.type, '→ 검색 키워드:', keyword);
  ...
  if (showResult && result) {
    ...
    const resultLiteratureLevel = result.literatureLevel[locale as keyof typeof result.literatureLevel] || result.literatureLevel.ko;
