...
import {
  Phase3LoveWeaknessMomentQuestion,
  Phase3LoveWeaknessMomentResult,
  calculatePhase3LoveWeaknessMomentResult,
...
interface Phase3LoveWeaknessMomentTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3LoveWeaknessMomentQuestion[];
  results: Phase3LoveWeaknessMomentResult[];
...
export default function Phase3LoveWeaknessMomentTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3LoveWeaknessMomentTestClientProps) {
  const t = useTranslations('phase3LoveWeaknessMomentTest');
  ...
  const [result, setResult] = useState<Phase3LoveWeaknessMomentResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3LoveWeaknessMomentQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3LoveWeaknessMomentQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3LoveWeaknessMomentResult(answersArray);
      const weaknessResult = results.find(r => r.type === resultType);
      
      if (weaknessResult) {
        setResult(weaknessResult);
      }
    }
  };
          ...
          <div className="px-4">
            ...
            <div className="flex justify-center mb-4">
              <button
                ...
                className="bg-gradient-to-r from-rose-500 to-red-600 hover:from-rose-600 hover:to-red-700 text-white font-bold py-4 px-8 rounded-full shadow-lg transform hover:scale-105 transition-all duration-200"
              >
      ...
      <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-2xl p-8 max-w-md w-full mx-4 text-center shadow-2xl">
          ...
          <button
            ...
            className="w-full bg-gradient-to-r from-rose-500 to-red-600 text-white py-4 px-6 rounded-xl text-xl font-bold hover:from-rose-600 hover:to-red-700 transition-all duration-300 shadow-lg"
          >
  ...
  if (showResult && result) {
    ...
    const resultTypeLabel =
      result.typeLabel[locale as keyof typeof result.typeLabel] || result.typeLabel.ko;
    const resultCollapseMoment =
      result.collapseMoment[locale as keyof typeof result.collapseMoment] || result.collapseMoment.ko;
    const resultFatalWeakness =
      result.fatalWeakness[locale as keyof typeof result.fatalWeakness] || result.fatalWeakness.ko;
    const resultDangerSignal =
      result.dangerSignal[locale as keyof typeof result.dangerSignal] || result.dangerSignal.ko;
    const resultEffectiveLine =
      result.effectiveLine[locale as keyof typeof result.effectiveLine] || result.effectiveLine.ko;
    ...
    const resultVulnerableTo =
      result.vulnerableTo[locale as keyof typeof result.vulnerableTo] || result.vulnerableTo.ko;

    return (
      <div className="min-h-screen bg-gradient-to-br from-rose-50 via-pink-50 to-red-50">
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            <div className="text-center mb-3 bg-white rounded-2xl shadow-lg p-4 md:p-5 border border-rose-100/60">
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3 border border-rose-100/50">
              <h3 className="text-base font-bold text-gray-800 mb-2">
                📊 {t('ui.weaknessType')}
              </h3>
              <p className="text-2xl font-bold text-rose-600 text-center" style={{ fontSize: '1.35em' }}>
                {resultTypeLabel}
              </p>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3 border border-pink-100/60">
              ...
              <p className="text-sm text-gray-700 leading-relaxed">{resultCollapseMoment}</p>
            </div>

            <div className="bg-white rounded-xl shadow-lg p-4 mb-3 border border-rose-100/60">
              <h3 className="text-base font-bold text-gray-800 mb-2">
                ⚠️ {t('ui.fatalWeakness')}
              </h3>
              <p className="text-sm text-gray-700 leading-relaxed">{resultFatalWeakness}</p>
            </div>

            <div className="bg-white rounded-xl shadow-lg p-4 mb-3 border border-red-100/60">
              <h3 className="text-base font-bold text-gray-800 mb-2">
                🚨 {t('ui.dangerSignal')}
              </h3>
              <p className="text-sm text-gray-700 leading-relaxed">{resultDangerSignal}</p>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3 border border-pink-100/60">
              ...
              <p className="text-sm text-gray-700 leading-relaxed">{resultEffectiveLine}</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 sm:gap-3 mb-3 items-stretch">
              <div className="bg-white rounded-xl shadow-lg p-3 sm:p-4 border border-rose-100/50 min-w-0 h-full flex flex-col">
                  ...
                  {(() => {
                    ...
                    if (matchType) {
                      ...
                      if (matchResult) {
                        ...
                        return (
                          <div className="bg-gradient-to-r from-rose-100 to-pink-100 rounded-lg px-3 py-2">
              ...
              <div className="bg-white rounded-xl shadow-lg p-3 sm:p-4 border border-orange-100/60 min-w-0 h-full flex flex-col">
                ...
                <div className="space-y-2 flex-1 min-w-0">
                  {(() => {
                    const matchType = resultVulnerableTo.match(/Type\s*(\d+)/i);
                    if (matchType) {
                      const typeNumber = matchType[1];
                      const matchTypeKey = `Type${typeNumber}`;
                      const matchResult = results.find(r => r.type === matchTypeKey);
            ...
            <div className="flex flex-col sm:flex-row gap-4 px-4">
              ...
              <Link
                href={`/${locale}`}
                className="flex-1 bg-gradient-to-r from-rose-500 to-red-600 text-white font-bold py-4 px-6 rounded-xl hover:from-rose-600 hover:to-red-700 transition-all text-center shadow-md"
              >
