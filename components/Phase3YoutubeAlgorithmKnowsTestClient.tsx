...
import {
  Phase3YoutubeAlgorithmKnowsQuestion,
  Phase3YoutubeAlgorithmKnowsResult,
  calculatePhase3YoutubeAlgorithmKnowsResult,
...
interface Phase3YoutubeAlgorithmKnowsTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3YoutubeAlgorithmKnowsQuestion[];
  results: Phase3YoutubeAlgorithmKnowsResult[];
...
export default function Phase3YoutubeAlgorithmKnowsTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3YoutubeAlgorithmKnowsTestClientProps) {
  const t = useTranslations('phase3YoutubeAlgorithmKnowsTest');
  ...
  const [result, setResult] = useState<Phase3YoutubeAlgorithmKnowsResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3YoutubeAlgorithmKnowsQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3YoutubeAlgorithmKnowsQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3YoutubeAlgorithmKnowsResult(answersArray);
      const ytResult = results.find(r => r.type === resultType);
  ...
  if (showResult && result) {
    ...
    const resultAlgoClass = result.algorithmClassification[locale as keyof typeof result.algorithmClassification] || result.algorithmClassification.ko;
    const resultYoutubeSeesMe = result.youtubeSeesMe[locale as keyof typeof result.youtubeSeesMe] || result.youtubeSeesMe.ko;
    const resultExampleVideos = result.exampleVideos[locale as keyof typeof result.exampleVideos] || result.exampleVideos.ko;
    const resultHiddenInterest = result.hiddenInterest[locale as keyof typeof result.hiddenInterest] || result.hiddenInterest.ko;
    ...
    const exampleVideoChips = resultExampleVideos.split(/\s*\/\s*/).map((s) => s.trim()).filter(Boolean);
          ...
          <div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed text-center">{resultYoutubeSeesMe}</p>
              </div>
            </div>

            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-5">
                <h3 className="text-base font-bold text-gray-800 mb-4 text-left w-full">
                  {t('ui.exampleVideos')}
                </h3>
                <div className="flex flex-wrap justify-center gap-2">
                  {exampleVideoChips.map((chip, index) => (
                    <span
                      key={index}
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed text-center">{resultHiddenInterest}</p>
              </div>
            </div>
