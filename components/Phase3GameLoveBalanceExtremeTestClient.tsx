...
import {
  Phase3GameLoveBalanceExtremeQuestion,
  Phase3GameLoveBalanceExtremeResult,
  calculatePhase3GameLoveBalanceExtremeResult,
  stripLoveBalanceRoundPrefix,
...
interface Phase3GameLoveBalanceExtremeTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3GameLoveBalanceExtremeQuestion[];
  results: Phase3GameLoveBalanceExtremeResult[];
...
function getResultShareMessage(
  t: (key: string) => string,
  resultType: string
): string {
  switch (resultType) {
...
export default function Phase3GameLoveBalanceExtremeTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3GameLoveBalanceExtremeTestClientProps) {
  const t = useTranslations('phase3GameLoveBalanceExtremeTest');
  ...
  const [result, setResult] = useState<Phase3GameLoveBalanceExtremeResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3GameLoveBalanceExtremeQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3GameLoveBalanceExtremeQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3GameLoveBalanceExtremeResult(answersArray);
      const loveResult = results.find(r => r.type === resultType);
  ...
  const handleShareResult = async () => {
    ...
    const shareText = `${getResultShareMessage(t, result.type)}\n\n${`https://myquizoasis.com${window.location.pathname}`}`;
  ...
  const shareToLine = () => {
    ...
    const shareText = result
      ? encodeURIComponent(getResultShareMessage(t, result.type))
  ...
  const shareToWeChat = async () => {
    ...
    const shareText = result
      ? `${getResultShareMessage(t, result.type)}\n\n${url}`
  ...
  const shareToKakao = () => {
    ...
    const shareDescription = result ? getResultShareMessage(t, result.type) : t('shareMessages.startKakao');
  ...
  if (showResult && result) {
    ...
    const resultHappyLove =
      result.happyLove[locale as keyof typeof result.happyLove] || result.happyLove.ko;
    const resultHardMoment =
      result.hardMoment[locale as keyof typeof result.hardMoment] || result.hardMoment.ko;
          ...
          <div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-5">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {resultKeywords.split(/[·／/]/).map((part, index) => (
                    <span
                      key={index}
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed text-center">{resultHappyLove}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed text-center">{resultHardMoment}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 bg-gradient-to-r from-green-100 to-emerald-100 rounded-lg px-3 py-2">
                  {resultGoodMatch}
                </p>
              </div>
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 bg-gradient-to-r from-yellow-100 to-orange-100 rounded-lg px-3 py-2">
                  {resultBadMatch}
                </p>
              </div>
            </div>
  ...
  const rawQuestionText =
    question.question[locale as keyof typeof question.question] || question.question.ko;
  /** 셔플 후에도 진행 순서대로 라운드 1~10으로 표시 (문항 본문의 기존 라운드 접두어 제거) */
  const displayRound = currentQuestion + 1;
  const questionBody = stripLoveBalanceRoundPrefix(rawQuestionText);
  const questionText = `${t('ui.roundLead', { current: displayRound })}${questionBody}`;
  const progress = (displayRound / shuffledQuestions.length) * 100;
        ...
        <div className="mb-8">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="text-sm font-bold text-purple-600">
              {t('ui.roundCount', { current: displayRound, total: shuffledQuestions.length })}
            </span>
          </div>
