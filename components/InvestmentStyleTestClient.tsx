...
interface InvestmentStyleTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: InvestmentStyleQuestion[];
  results: InvestmentStyleResult[];
...
const getCompatibilityDescription = (myType: string, partnerType: string, t: any): string => {
  ...
  return t(`investmentStyleTest.result.compatibility.${key}`) || '';
};

export default function InvestmentStyleTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  const [result, setResult] = useState<InvestmentStyleResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<InvestmentStyleQuestion[]>(questions);
  ...
  const handleAnswer = (scores: any) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculateInvestmentStyleResult(newAnswers);
      const investmentResult = results.find(r => r.type === resultType);
      ...
      if (investmentResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(investmentResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 재테크 결과:', investmentResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  const calculateResult = (finalAnswers: any[]) => {
    const resultType = calculateInvestmentStyleResult(finalAnswers);
    const investmentResult = results.find(r => r.type === resultType);
  ...
  const handleShareResult = async () => {
    ...
    const shareText = t('investmentStyleTest.shareMessages.default', { type: resultTitle }) + `\n\n${`https://myquizoasis.com${window.location.pathname}`}`;
    ...
    if (navigator.share) {
      try {
        ...
        if (error instanceof Error && error.name !== 'AbortError') {
          console.error(t('investmentStyleTest.ui.shareError'), error);
        }
      }
    } else {
      try {
        await navigator.clipboard.writeText(shareText);
        alert(t('investmentStyleTest.ui.resultCopied'));
      } catch (error) {
        console.error(t('investmentStyleTest.ui.clipboardError'), error);
        alert(t('investmentStyleTest.ui.shareUnavailable'));
      }
    }
  };
  ...
  const shareToWeChat = async () => {
    ...
    const shareText = result 
      ? t('investmentStyleTest.shareMessages.wechat', { type: resultTitle }) + `\n\n${url}`
      : `${title}\n\n${url}`;
    ...
    try {
      ...
      alert(t('investmentStyleTest.ui.wechatCopy'));
    } catch (error) {
      alert(t('investmentStyleTest.ui.shareUnavailable'));
    }
  };
  ...
  const shareToWhatsApp = () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareText = result 
      ? encodeURIComponent(t('investmentStyleTest.shareMessages.whatsapp', { type: resultTitle }))
  ...
  const shareToKakao = () => {
    ...
    if (!window.Kakao || !window.Kakao.isInitialized()) {
      alert(t('investmentStyleTest.ui.kakaoInit'));
      return;
    }
    ...
    const shareDescription = result 
      ? t('investmentStyleTest.shareMessages.kakao', { type: resultTitle })
      : description;
    ...
    try {
      window.Kakao.Share.sendDefault({
        ...
        buttons: [
          {
            title: t('investmentStyleTest.ui.testButton'),
            link: {
              mobileWebUrl: currentUrl,
              webUrl: currentUrl,
            },
      ...
      console.error(t('investmentStyleTest.ui.kakaoError'), error);
      alert(t('investmentStyleTest.ui.kakaoError'));
    }
  };
  ...
  const shareToTelegram = () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareText = result 
      ? t('investmentStyleTest.shareMessages.telegram', { type: resultTitle })
      : title;
  ...
  const copyLink = () => {
    ...
    alert(t('investmentStyleTest.ui.linkCopied'));
  };
      ...
      <div className="min-h-screen bg-white">
        <div className="max-w-4xl mx-auto">
          <div className="relative w-full overflow-hidden mb-3" style={{ aspectRatio: '680/384' }}>
            <Image
              src={getThumbnailUrl(thumbnail || 'test_056_investment_style.jpg')}
              alt={title}
              fill
          ...
          <div className="px-4">
            ...
            <div className="my-6">
              <AdSensePlaceholder 
                ...
                className="mx-auto"
                label={t('investmentStyleTest.ui.adsenseTitle')}
              />
            </div>

            <div className="text-gray-600 mb-6 leading-relaxed text-center space-y-4">
              <p className="font-bold text-gray-700">{t('investmentStyleTest.startMessage.line1')}</p>
              <p>{t('investmentStyleTest.startMessage.line2')}</p>
              <p>{t('investmentStyleTest.startMessage.line3')}</p>
              <p>{t('investmentStyleTest.startMessage.line4')}</p>
              <p className="whitespace-pre-line">{t('investmentStyleTest.startMessage.line5')}</p>
              <p>{t('investmentStyleTest.startMessage.line6')}</p>
            </div>
            ...
            <div className="mb-8 text-center">
              ...
              <div className="flex justify-center gap-2">
                <button onClick={copyLink} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/link.jpeg" alt={t('investmentStyleTest.ui.linkCopy')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToKakao} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/kakao.jpeg" alt={t('investmentStyleTest.ui.kakao')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToTelegram} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/telegram.jpeg" alt={t('investmentStyleTest.ui.telegram')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToWeChat} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/wechat.jpeg" alt={t('investmentStyleTest.ui.wechat')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToLine} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/line.jpeg" alt={t('investmentStyleTest.ui.line')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToWhatsApp} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/whatsapp.jpeg" alt={t('investmentStyleTest.ui.whatsapp')} width={46} height={46} className="rounded-lg" />
                </button>
              </div>
            </div>
            ...
            {similarTestsState.length > 0 && (
              ...
              <h2 className="text-xl font-bold text-gray-800 mb-6">
                {t('investmentStyleTest.ui.similarTests')}
              </h2>
      ...
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-4">
        ...
        <div className="mb-8 w-full max-w-[680px]">
          <AdSensePlaceholder 
            ...
            style={{ width: '100%', height: '250px' }}
            className="mx-auto"
            label={t('investmentStyleTest.ui.adsenseLoading')}
          />
        </div>
  ...
  if (showResult && result) {
    ...
    const resultRecommendedInvestments = result.recommendedInvestments[locale as keyof typeof result.recommendedInvestments] || result.recommendedInvestments.ko;
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                🎯 {t('investmentStyleTest.ui.recommendedInvestments')}
              </h3>
              <div className="flex flex-wrap gap-2">
                {resultRecommendedInvestments.split(/,\s+|，\s*|、\s*/).map((investment: string, index: number) => (
                  <span
                    key={index}
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                💡 {t('investmentStyleTest.ui.advice')}
              </h3>
            ...
            <div className="my-6 px-4">
              <AdSensePlaceholder 
                ...
                style={{ width: '100%', height: '250px' }}
                className="mx-auto"
                label={t('investmentStyleTest.ui.adsenseResult')}
              />
            </div>
        ...
        <div>
          ...
          <div className="mt-8 px-4">
            <AdSensePlaceholder 
              ...
              style={{ width: '100%', height: '250px' }}
              className="mx-auto"
              label={t('investmentStyleTest.ui.adsenseBottom')}
            />
          </div>
