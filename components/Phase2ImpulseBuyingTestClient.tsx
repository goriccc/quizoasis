...
import { Phase2ImpulseBuyingQuestion, Phase2ImpulseBuyingResult, calculatePhase2ImpulseBuyingResult } from '@/lib/phase2_impulse_buying_data';
...
interface Phase2ImpulseBuyingTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase2ImpulseBuyingQuestion[];
  results: Phase2ImpulseBuyingResult[];
...
export default function Phase2ImpulseBuyingTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase2ImpulseBuyingTestClientProps) {
  const t = useTranslations('phase2ImpulseBuyingTest');
  ...
  const [result, setResult] = useState<Phase2ImpulseBuyingResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2ImpulseBuyingQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase2ImpulseBuyingQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase2ImpulseBuyingResult(answersArray);
      const impulseBuyingResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (impulseBuyingResult) {
        setResult(impulseBuyingResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (impulseBuyingResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(impulseBuyingResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 충동구매 결과:', impulseBuyingResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const resultImpulseLevel = result.impulseLevel[locale as keyof typeof result.impulseLevel] || result.impulseLevel.ko;
