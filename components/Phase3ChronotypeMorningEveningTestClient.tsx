...
import {
  Phase3ChronotypeMorningEveningQuestion,
  Phase3ChronotypeMorningEveningResult,
  calculatePhase3ChronotypeMorningEveningResult,
...
interface Phase3ChronotypeMorningEveningTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3ChronotypeMorningEveningQuestion[];
  results: Phase3ChronotypeMorningEveningResult[];
...
export default function Phase3ChronotypeMorningEveningTestClient({
  locale,
  slug,
  title,
  description,
  ...
  isLatestTest = false,
  badgeType = null,
}: Phase3ChronotypeMorningEveningTestClientProps) {
  const t = useTranslations('phase3ChronotypeMorningEveningTest');
  ...
  const [result, setResult] = useState<Phase3ChronotypeMorningEveningResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3ChronotypeMorningEveningQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3ChronotypeMorningEveningQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3ChronotypeMorningEveningResult(answersArray);
      const chronotypeResult = results.find(r => r.type === resultType);
  ...
  if (showResult && result) {
    ...
    const resultChronotypeLabel =
      result.chronotypeLabel[locale as keyof typeof result.chronotypeLabel] || result.chronotypeLabel.ko;
    const resultEnergyPeak =
      result.energyPeak[locale as keyof typeof result.energyPeak] || result.energyPeak.ko;
    const resultFocusPeak =
      result.focusPeak[locale as keyof typeof result.focusPeak] || result.focusPeak.ko;
    const resultNaturalSleep =
      result.naturalSleep[locale as keyof typeof result.naturalSleep] || result.naturalSleep.ko;
    const resultNaturalWake =
      result.naturalWake[locale as keyof typeof result.naturalWake] || result.naturalWake.ko;
    const resultOptimalSchedule =
      result.optimalSchedule[locale as keyof typeof result.optimalSchedule] || result.optimalSchedule.ko;
    const resultStrength =
      result.strength[locale as keyof typeof result.strength] || result.strength.ko;
    const resultWeakness =
      result.weakness[locale as keyof typeof result.weakness] || result.weakness.ko;
    const resultFamousPeople =
      result.famousPeople[locale as keyof typeof result.famousPeople] || result.famousPeople.ko;
    const resultSocialJetLag =
      result.socialJetLag[locale as keyof typeof result.socialJetLag] || result.socialJetLag.ko;
    const resultScientificFact =
      result.scientificFact[locale as keyof typeof result.scientificFact] || result.scientificFact.ko;
    const resultChronotypeTip =
      result.chronotypeTip[locale as keyof typeof result.chronotypeTip] || result.chronotypeTip.ko;
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultEnergyPeak}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left w-full">
                  {t('ui.focusPeak')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">{resultFocusPeak}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultNaturalSleep}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left w-full">
                  {t('ui.naturalWake')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">{resultNaturalWake}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left w-full">
                  {t('ui.optimalSchedule')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-line">{resultOptimalSchedule}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultWeakness}</p>
              </div>
            </div>

            {resultFamousPeople.trim() && (
              <div className="mb-3">
                <div className="bg-white rounded-xl shadow-lg p-4">
                  <h3 className="text-base font-bold text-gray-800 mb-2 text-left w-full">
                    {t('ui.famousPeople')}
                  </h3>
                  <p className="text-sm text-gray-700 leading-relaxed">{resultFamousPeople}</p>
                </div>
              </div>
            )}

            {resultSocialJetLag.trim() && (
              <div className="mb-3">
                <div className="bg-white rounded-xl shadow-lg p-4">
                  <h3 className="text-base font-bold text-gray-800 mb-2 text-left w-full">
                    {t('ui.socialJetLag')}
                  </h3>
                  <p className="text-sm text-gray-700 leading-relaxed">{resultSocialJetLag}</p>
                </div>
              </div>
            )}

            {resultScientificFact.trim() && (
              <div className="mb-3">
                <div className="bg-white rounded-xl shadow-lg p-4">
                  <h3 className="text-base font-bold text-gray-800 mb-2 text-left w-full">
                    {t('ui.scientificFact')}
                  </h3>
                  <p className="text-sm text-gray-700 leading-relaxed">{resultScientificFact}</p>
                </div>
              </div>
            )}
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultChronotypeTip}</p>
              </div>
            </div>
