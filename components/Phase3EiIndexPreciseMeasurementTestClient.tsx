...
import {
  Phase3EiIndexPreciseMeasurementQuestion,
  Phase3EiIndexPreciseMeasurementResult,
  calculatePhase3EiIndexPreciseMeasurementResult,
  getEiPercentFromRawScore,
...
interface Phase3EiIndexPreciseMeasurementTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3EiIndexPreciseMeasurementQuestion[];
  results: Phase3EiIndexPreciseMeasurementResult[];
...
export default function Phase3EiIndexPreciseMeasurementTestClient({
  locale,
  slug,
  title,
  description,
  ...
  isLatestTest = false,
  badgeType = null,
}: Phase3EiIndexPreciseMeasurementTestClientProps) {
  const t = useTranslations('phase3EiIndexPreciseMeasurementTest');
  ...
  const [result, setResult] = useState<Phase3EiIndexPreciseMeasurementResult | null>(null);
  const [totalRawScore, setTotalRawScore] = useState(0);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3EiIndexPreciseMeasurementQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3EiIndexPreciseMeasurementQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      const resultType = calculatePhase3EiIndexPreciseMeasurementResult(answersArray);
      const eiResult = results.find(r => r.type === resultType);

      if (eiResult) {
        setResult(eiResult);
      }

      if (eiResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(eiResult.type, locale);
  ...
  const formatShareLine = (r: Phase3EiIndexPreciseMeasurementResult) => {
    const ei = getEiPercentFromRawScore(totalRawScore);
    const line = r.shareLine[locale as keyof typeof r.shareLine] || r.shareLine.ko;
    return line.replace(/\{ePercent\}/g, String(ei.e));
  };
  ...
  const shareToLine = () => {
    ...
    const shareText = result
      ? encodeURIComponent(formatShareLine(result))
  ...
  const shareToKakao = () => {
    ...
    const shareDescription = result ? formatShareLine(result) : t('shareMessages.startKakao');
  ...
  const shareToTelegram = () => {
    ...
    const url = encodeURIComponent(`https://myquizoasis.com${window.location.pathname}`);
    const shareText = result ? formatShareLine(result) : t('shareMessages.startTelegram');
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <ul className="list-disc list-outside pl-5 text-left space-y-2 marker:text-gray-500">
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {resultBadMatch.split(/[,、，]/).map((job, index) => (
                    <span
                      key={index}
                      className="inline-block bg-gradient-to-r from-sky-100 to-indigo-100 text-indigo-900 px-3 py-1.5 rounded-full text-sm font-medium"
                    >
                      {job.trim()}
                    </span>
                  ))}
