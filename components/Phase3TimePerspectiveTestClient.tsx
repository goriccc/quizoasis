...
import {
  Phase3TimePerspectiveQuestion,
  Phase3TimePerspectiveResult,
  calculatePhase3TimePerspectiveResult,
  calculatePhase3TimePerspectiveDomainScores,
} from '@/lib/phase3TimePerspectiveData';
...
interface Phase3TimePerspectiveTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3TimePerspectiveQuestion[];
  results: Phase3TimePerspectiveResult[];
...
export default function Phase3TimePerspectiveTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3TimePerspectiveTestClientProps) {
  const t = useTranslations('phase3TimePerspectiveTest');
  ...
  const [result, setResult] = useState<Phase3TimePerspectiveResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3TimePerspectiveQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3TimePerspectiveQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      const scores = calculatePhase3TimePerspectiveDomainScores(answersArray);
      setDomainScores(scores);

      const resultType = calculatePhase3TimePerspectiveResult(answersArray);
  ...
  const handleRetake = () => {
    ...
    setResult(null);
    setShowResult(false);
    setDomainScores({ freeTime: 0, emotion: 0, decision: 0, happiness: 0, difficulty: 0, timeAttitude: 0, total: 0 });
  ...
  if (showResult && result) {
    ...
    const resultTimePerspective =
      result.timePerspectivePattern[locale as keyof typeof result.timePerspectivePattern] || result.timePerspectivePattern.ko;
    const resultCompetitiveType =
      result.timePerspectiveType[locale as keyof typeof result.timePerspectiveType] || result.timePerspectiveType.ko;
    const resultCompetitiveKeywords =
      result.timeKeywords[locale as keyof typeof result.timeKeywords] || result.timeKeywords.ko;
    ...
    const domainConfig = [
      { key: 'freeTime', label: t('ui.freeTimeDomain'), score: domainScores.freeTime, max: 6 },
      { key: 'emotion', label: t('ui.emotionDomain'), score: domainScores.emotion, max: 6 },
      { key: 'decision', label: t('ui.decisionDomain'), score: domainScores.decision, max: 6 },
      { key: 'happiness', label: t('ui.happinessDomain'), score: domainScores.happiness, max: 6 },
      { key: 'difficulty', label: t('ui.difficultyDomain'), score: domainScores.difficulty, max: 6 },
      { key: 'timeAttitude', label: t('ui.timeAttitudeDomain'), score: domainScores.timeAttitude, max: 6 },
    ];
          ...
          <div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
                ...
                {domainConfig.map((domain) => (
                  <div key={domain.key}>
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2.5">
                      <div
                        className="bg-gradient-to-r from-blue-500 to-indigo-400 h-2.5 rounded-full transition-all"
              ...
              <div className="mt-4 space-y-2 text-sm text-gray-700">
                <p>
                  📌 {t('ui.futureOrientedDomain')}: <span className="font-semibold">{strongestLabels.join(' · ')}</span>
                </p>
                <p>
                  📌 {t('ui.pastNegativeDomain')}: <span className="font-semibold">{weakestLabels.join(' · ')}</span>
                </p>
              </div>
            </div>
