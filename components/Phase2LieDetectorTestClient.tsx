...
import { Phase2LieDetectorQuestion, Phase2LieDetectorResult, calculatePhase2LieDetectorResult } from '@/lib/phase2_lie_detector_data';
...
interface Phase2LieDetectorTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase2LieDetectorQuestion[];
  results: Phase2LieDetectorResult[];
...
export default function Phase2LieDetectorTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase2LieDetectorTestClientProps) {
  const t = useTranslations('phase2LieDetectorTest');
  ...
  const [result, setResult] = useState<Phase2LieDetectorResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2LieDetectorQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase2LieDetectorQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase2LieDetectorResult(answersArray);
      const lieDetectorResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (lieDetectorResult) {
        setResult(lieDetectorResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (lieDetectorResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(lieDetectorResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 거짓말 탐지기 결과:', lieDetectorResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  const handleShareResult = async () => {
    ...
    const resultLieLevel = result.lieLevel[locale as keyof typeof result.lieLevel] || result.lieLevel.ko;
    // 레벨에서 숫자 추출 (예: "Lv. 1 (하수)" -> "1")
    const levelMatch = resultLieLevel.match(/Lv\.\s*(\d+)/);
    const level = levelMatch ? levelMatch[1] : '';
    const shareText = `${t('shareMessages.default', { level, type: resultTitle })}\n\n${`https://myquizoasis.com${window.location.pathname}`}`;
  ...
  const shareToLine = () => {
    ...
    const resultLieLevel = result ? (result.lieLevel[locale as keyof typeof result.lieLevel] || result.lieLevel.ko) : '';
    const levelMatch = resultLieLevel ? resultLieLevel.match(/Lv\.\s*(\d+)/) : null;
    const level = levelMatch ? levelMatch[1] : '';
    const shareText = result 
      ? encodeURIComponent(t('shareMessages.line', { level, type: resultTitle }))
  ...
  const shareToWeChat = async () => {
    ...
    const level = levelMatch ? levelMatch[1] : '';
    const shareText = result 
      ? `${t('shareMessages.wechat', { level, type: resultTitle })}\n\n${url}`
  ...
  const shareToWhatsApp = () => {
    ...
    const level = levelMatch ? levelMatch[1] : '';
    const shareText = result 
      ? encodeURIComponent(t('shareMessages.whatsapp', { level, type: resultTitle }))
  ...
  const shareToKakao = () => {
    ...
    const shareDescription = result 
      ? t('shareMessages.kakao', { level, type: resultTitle })
  ...
  const shareToTelegram = () => {
    ...
    const level = levelMatch ? levelMatch[1] : '';
    const shareText = result 
      ? t('shareMessages.telegram', { level, type: resultTitle })
