...
import {
  Phase3MukbangStyleDiagnosisQuestion,
  Phase3MukbangStyleDiagnosisResult,
  calculatePhase3MukbangStyleDiagnosisResult,
...
interface Phase3MukbangStyleDiagnosisTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3MukbangStyleDiagnosisQuestion[];
  results: Phase3MukbangStyleDiagnosisResult[];
...
export default function Phase3MukbangStyleDiagnosisTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3MukbangStyleDiagnosisTestClientProps) {
  const t = useTranslations('phase3MukbangStyleDiagnosisTest');
  ...
  const [result, setResult] = useState<Phase3MukbangStyleDiagnosisResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3MukbangStyleDiagnosisQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3MukbangStyleDiagnosisQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3MukbangStyleDiagnosisResult(answersArray);
  ...
  const getResultShareText = () => {
    if (!result) return '';
    return result.shareMessage[locale as keyof typeof result.shareMessage] || result.shareMessage.ko;
  };
  ...
  if (showResult && result) {
    ...
    const resultMukbangStyle =
      result.mukbangStyle[locale as keyof typeof result.mukbangStyle] || result.mukbangStyle.ko;
    const resultMukbangKeywords =
      result.mukbangKeywords[locale as keyof typeof result.mukbangKeywords] || result.mukbangKeywords.ko;
    const resultEatingTogether =
      result.eatingTogether[locale as keyof typeof result.eatingTogether] || result.eatingTogether.ko;
    const resultFoodStrength =
      result.foodStrength[locale as keyof typeof result.foodStrength] || result.foodStrength.ko;
    const resultAvoidEatingWith = result.avoidEatingWith
      ? result.avoidEatingWith[locale as keyof typeof result.avoidEatingWith] || result.avoidEatingWith.ko
      : '';
    const resultCommunityStance =
      result.communityStance[locale as keyof typeof result.communityStance] || result.communityStance.ko;
          ...
          <div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-5">
                <h3 className="text-base font-bold text-gray-800 mb-4 text-left">
                  {t('ui.mukbangKeywords')}
                </h3>
                <div className="flex flex-wrap justify-center gap-2">
                  {resultMukbangKeywords.split(/[,、，]/).map((part, index) => (
                    <span
                      key={index}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultEatingTogether}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  {t('ui.foodStrength')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">{resultFoodStrength}</p>
              </div>
            </div>

            {resultAvoidEatingWith ? (
              <div className="mb-3">
                <div className="bg-white rounded-xl shadow-lg p-4">
                  <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                    {t('ui.avoidEatingWith')}
                  </h3>
                  <p className="text-sm text-gray-700 leading-relaxed">{resultAvoidEatingWith}</p>
                </div>
              </div>
            ) : null}
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  {t('ui.communityStance')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">{resultCommunityStance}</p>
              </div>
            </div>
