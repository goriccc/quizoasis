...
import {
  Phase3DopamineTypeQuestion,
  Phase3DopamineTypeResult,
  calculatePhase3DopamineTypeResult,
...
interface Phase3DopamineTypeTestClientProps {
  ...
  title: string;
  description: string;
  questions: Phase3DopamineTypeQuestion[];
  results: Phase3DopamineTypeResult[];
...
export default function Phase3DopamineTypeTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3DopamineTypeTestClientProps) {
  const t = useTranslations('phase3DopamineTypeTest');
  ...
  const [result, setResult] = useState<Phase3DopamineTypeResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3DopamineTypeQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3DopamineTypeQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3DopamineTypeResult(answersArray);
  ...
  if (showResult && result) {
    ...
    const resultCategory = result.dopamineCategory[locale as keyof typeof result.dopamineCategory] || result.dopamineCategory.ko;
    const resultCharge = result.chargePoints[locale as keyof typeof result.chargePoints] || result.chargePoints.ko;
    const resultDischarge = result.dischargePoints[locale as keyof typeof result.dischargePoints] || result.dischargePoints.ko;
    const resultMeme = result.memeVersion[locale as keyof typeof result.memeVersion] || result.memeVersion.ko;
    ...
    const resultUpgrade = result.upgrade[locale as keyof typeof result.upgrade] || result.upgrade.ko;
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  ⚡ {t('ui.dopamineCategory')}
                </h3>
                <p className="text-base font-semibold text-purple-600">{resultCategory}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  💬 {t('ui.memeVersion')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">{resultMeme}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultCharge}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  📉 {t('ui.dischargePoints')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">{resultDischarge}</p>
              </div>
            </div>
            ...
            <div className="space-y-3 mb-3">
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultUpgrade}</p>
              </div>
            </div>
