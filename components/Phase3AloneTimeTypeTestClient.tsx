...
import {
  Phase3AloneTimeTypeQuestion,
  Phase3AloneTimeTypeResult,
  calculatePhase3AloneTimeTypeResult,
...
interface Phase3AloneTimeTypeTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3AloneTimeTypeQuestion[];
  results: Phase3AloneTimeTypeResult[];
...
export default function Phase3AloneTimeTypeTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3AloneTimeTypeTestClientProps) {
  const t = useTranslations('phase3AloneTimeTypeTest');
  ...
  const [result, setResult] = useState<Phase3AloneTimeTypeResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3AloneTimeTypeQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3AloneTimeTypeQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3AloneTimeTypeResult(answersArray);
  ...
  if (showResult && result) {
    ...
    const resultSoloTimeType = result.soloTimeType[locale as keyof typeof result.soloTimeType] || result.soloTimeType.ko;
    const resultSoloKeywords = result.soloKeywords[locale as keyof typeof result.soloKeywords] || result.soloKeywords.ko;
    const resultActualBehavior = result.actualBehavior[locale as keyof typeof result.actualBehavior] || result.actualBehavior.ko;
    ...
    const resultDifficultSituation = result.difficultSituation[locale as keyof typeof result.difficultSituation] || result.difficultSituation.ko;
    const resultWhatTheyNeed = result.whatTheyNeed[locale as keyof typeof result.whatTheyNeed] || result.whatTheyNeed.ko;
          ...
          <div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <div className="flex flex-wrap justify-center gap-2">
                {resultSoloKeywords.split(/[·]/).map((keyword, index) => (
                  <span
                    key={index}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  😓 {t('ui.difficultSituation')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed text-left">
                  {resultDifficultSituation}
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
