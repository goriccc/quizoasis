...
import {
  ...
  calculatePhase3SpendingDarkHistoryTypeResult,
...
function getSpendingProductKeyword(resultType: string, loc: string): string {
  const map: Record<string, string> = {
    Type1: loc === 'ko' ? '재테크 다이어리' : 'budget planner',
    Type2: loc === 'ko' ? '생활용품' : 'home essentials',
    Type3: loc === 'ko' ? '습관 트래커' : 'habit tracker',
    Type4: loc === 'ko' ? '미니멀 소품' : 'minimalist living',
    Type5: loc === 'ko' ? '플래너' : 'planner notebook',
    Type6: loc === 'ko' ? '가계부' : 'expense tracker',
  };
  return map[resultType] || 'lifestyle';
}

interface Phase3SpendingDarkHistoryTypeTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3SpendingDarkHistoryQuestion[];
  results: Phase3SpendingDarkHistoryResult[];
...
export default function Phase3SpendingDarkHistoryTypeTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3SpendingDarkHistoryTypeTestClientProps) {
  const t = useTranslations('phase3SpendingDarkHistoryTypeTest');
  ...
  const [result, setResult] = useState<Phase3SpendingDarkHistoryResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3SpendingDarkHistoryQuestion[]>([]);
  ...
  useEffect(() => {
    if (result && locale !== 'ko') {
      const loadProducts = async () => {
        try {
          const kw = getSpendingProductKeyword(result.type, locale);
          const products = await searchAliExpressProducts(kw, 4, locale);
          setAliProducts(products);
        } catch (error) {
          console.error('상품 로드 실패:', error);
        }
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3SpendingDarkHistoryQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3SpendingDarkHistoryTypeResult(answersArray);
      const spendingResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (spendingResult) {
        setResult(spendingResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (spendingResult && locale !== 'ko') {
        const kw = getSpendingProductKeyword(spendingResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 소비 흑역사 유형:', spendingResult.type, '→ 검색 키워드:', kw);
        searchAliExpressProducts(kw, 4, locale)
          .then(products => {
  ...
  const handleShareResult = async () => {
    ...
    const displayType = `${result.emoji} ${resultTitle}`;
    const shareText = `${t('shareMessages.default', { type: displayType })}\n\n${`https://myquizoasis.com${window.location.pathname}`}`;
  ...
  const shareToLine = () => {
    ...
    const displayType = result ? `${result.emoji} ${resultTitle}` : '';
    const shareText = result 
      ? encodeURIComponent(t('shareMessages.line', { type: displayType }))
  ...
  const shareToWeChat = async () => {
    ...
    const displayType = result ? `${result.emoji} ${resultTitle}` : '';
    const shareText = result 
      ? `${t('shareMessages.wechat', { type: displayType })}\n\n${url}`
  ...
  const shareToWhatsApp = () => {
    ...
    const displayType = result ? `${result.emoji} ${resultTitle}` : '';
    const shareText = result 
      ? encodeURIComponent(t('shareMessages.whatsapp', { type: displayType }))
  ...
  const shareToKakao = () => {
    ...
    const shareDescription = result 
      ? t('shareMessages.kakao', { type: displayType })
  ...
  const shareToTelegram = () => {
    ...
    const displayType = result ? `${result.emoji} ${resultTitle}` : '';
    const shareText = result 
      ? t('shareMessages.telegram', { type: displayType })
  ...
  if (showResult && result) {
    ...
    const resultDangerLevel = result.dangerLevel[locale as keyof typeof result.dangerLevel] || result.dangerLevel.ko;
    const resultRepresentative = result.representativeHistory[locale as keyof typeof result.representativeHistory] || result.representativeHistory.ko;
    const resultFutureRisk = result.futureRisk[locale as keyof typeof result.futureRisk] || result.futureRisk.ko;
    const resultPrevention = result.preventionBlock[locale as keyof typeof result.preventionBlock] || result.preventionBlock.ko;
    const resultTodayTodo = result.todayTodo[locale as keyof typeof result.todayTodo] || result.todayTodo.ko;
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  ⭐ {t('ui.representativeHistory')}
                </h3>
                <div className="flex flex-wrap justify-center gap-2">
                  {resultRepresentative.split(/[,、，]/).map((char, index) => (
                    <span
                      key={index}
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3 whitespace-pre-line text-sm text-gray-700 leading-relaxed">
              <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                {t('result.empathyIndexTitle')}
              </h3>
              {t('result.empathyIndexBody')}
            </div>
