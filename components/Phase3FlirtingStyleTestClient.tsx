...
import {
  Phase3FlirtingStyleQuestion,
  Phase3FlirtingStyleResult,
  calculatePhase3FlirtingStyleResult,
...
interface Phase3FlirtingStyleTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3FlirtingStyleQuestion[];
  results: Phase3FlirtingStyleResult[];
...
export default function Phase3FlirtingStyleTestClient({
  locale,
  slug,
  title,
  description,
  ...
  isLatestTest = false,
  badgeType = null,
}: Phase3FlirtingStyleTestClientProps) {
  const t = useTranslations('phase3FlirtingStyleTest');
  ...
  const [result, setResult] = useState<Phase3FlirtingStyleResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3FlirtingStyleQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3FlirtingStyleQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3FlirtingStyleResult(answersArray);
  ...
  if (showResult && result) {
    ...
    const resultFlirtingSkill =
      result.flirtingSkill[locale as keyof typeof result.flirtingSkill] || result.flirtingSkill.ko;
    const resultFlirtingKeywords =
      result.flirtingKeywords[locale as keyof typeof result.flirtingKeywords] || result.flirtingKeywords.ko;
    ...
    const resultGoodMatch =
      result.goodMatch[locale as keyof typeof result.goodMatch] || result.goodMatch.ko;
    const resultFriendTestimony =
      result.friendTestimony[locale as keyof typeof result.friendTestimony] || result.friendTestimony.ko;
          ...
          <div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-lg font-bold text-pink-600 text-center leading-snug">
                  {resultFlirtingSkill}
                </p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left w-full">
                  {t('ui.flirtingKeywords')}
                </h3>
                <div className="flex flex-wrap gap-2 justify-center">
                  {chipSplit(resultFlirtingKeywords).map((char, idx) => (
                    <span
                      key={idx}
                      className="inline-block bg-gradient-to-r from-pink-100 to-purple-100 text-pink-800 text-sm font-semibold px-3 py-1 rounded-full shadow-sm"
                    >
                      {char}
                    </span>
                  ))}
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed text-center">{resultStrength}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed text-center">{resultWeakness}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed text-center">{resultGoodMatch}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left w-full">
                  {t('ui.friendTestimony')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed text-center">
                  &quot;{resultFriendTestimony}&quot;
                </p>
              </div>
            </div>
