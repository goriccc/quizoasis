...
import {
  Phase3GhostingReboundPotentialQuestion,
  Phase3GhostingReboundPotentialResult,
  calculatePhase3GhostingReboundPotentialResult,
...
interface Phase3GhostingReboundPotentialTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3GhostingReboundPotentialQuestion[];
  results: Phase3GhostingReboundPotentialResult[];
...
export default function Phase3GhostingReboundPotentialTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3GhostingReboundPotentialTestClientProps) {
  const t = useTranslations('phase3GhostingReboundPotentialTest');
  ...
  const [result, setResult] = useState<Phase3GhostingReboundPotentialResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3GhostingReboundPotentialQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3GhostingReboundPotentialQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3GhostingReboundPotentialResult(answersArray);
  ...
  if (showResult && result) {
    ...
    const resultPotentialLevel = result.potentialLevel[locale as keyof typeof result.potentialLevel] || result.potentialLevel.ko;
    const resultCowardiceIndex = result.cowardiceIndex[locale as keyof typeof result.cowardiceIndex] || result.cowardiceIndex.ko;
          ...
          <div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <p className="text-base font-semibold text-purple-700 text-center">
                {resultPotentialLevel}
              </p>
              <p className="text-sm text-gray-500 text-center mt-1">
                {resultCowardiceIndex}
              </p>
            </div>
