...
import {
  Phase3LoveGreenFlagFinderQuestion,
  Phase3LoveGreenFlagFinderResult,
  calculatePhase3LoveGreenFlagFinderResult,
...
interface Phase3LoveGreenFlagFinderTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3LoveGreenFlagFinderQuestion[];
  results: Phase3LoveGreenFlagFinderResult[];
...
export default function Phase3LoveGreenFlagFinderTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3LoveGreenFlagFinderTestClientProps) {
  const t = useTranslations('phase3LoveGreenFlagFinderTest');
  ...
  const [result, setResult] = useState<Phase3LoveGreenFlagFinderResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3LoveGreenFlagFinderQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3LoveGreenFlagFinderQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3LoveGreenFlagFinderResult(answersArray);
      const greenFlagResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (greenFlagResult) {
        setResult(greenFlagResult);
      }
    }
  };
          ...
          <div className="px-4">
            ...
            <div className="flex justify-center mb-4">
              <button
                ...
                className="bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white font-bold py-4 px-8 rounded-full shadow-lg transform hover:scale-105 transition-all duration-200"
              >
  ...
  if (showResult && result) {
    ...
    const resultGreenFlagTypeLabel =
      result.greenFlagTypeLabel[locale as keyof typeof result.greenFlagTypeLabel] || result.greenFlagTypeLabel.ko;
    const resultMainTraits = result.mainTraits[locale as keyof typeof result.mainTraits] || result.mainTraits.ko;
    const resultPartnerReceives =
      result.partnerReceives[locale as keyof typeof result.partnerReceives] || result.partnerReceives.ko;
    const resultBrilliantTip =
      result.brilliantTip[locale as keyof typeof result.brilliantTip] || result.brilliantTip.ko;
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <p className="text-2xl font-bold text-emerald-600 text-center" style={{ fontSize: '1.35em' }}>
                {resultGreenFlagTypeLabel}
              </p>
            </div>

            <div className="bg-white rounded-xl shadow-lg p-4 md:p-5 mb-3 border border-emerald-100/80">
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-2">
                💚 {t('ui.partnerReceives')}
              </h3>
              <p className="text-sm text-gray-700 leading-relaxed">{resultPartnerReceives}</p>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <p className="text-sm text-gray-700 leading-relaxed">{resultBrilliantTip}</p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 px-4">
              ...
              <Link
                href={`/${locale}`}
                className="flex-1 bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-bold py-4 px-6 rounded-xl hover:from-emerald-700 hover:to-teal-700 transition-all text-center shadow-md"
              >
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
            <div
              className="bg-gradient-to-r from-emerald-600 to-teal-600 h-full rounded-full transition-all duration-300 ease-out"
        ...
        <div>
          ...
          <div className="space-y-4 px-4">
            {optionsArray.map((option, index) => {
              ...
              const colors = [
                'from-emerald-50 to-emerald-100 hover:from-emerald-100 hover:to-emerald-200 border-emerald-200 hover:border-emerald-400',
                'from-teal-50 to-teal-100 hover:from-teal-100 hover:to-teal-200 border-teal-200 hover:border-teal-400',
                'from-green-50 to-green-100 hover:from-green-100 hover:to-green-200 border-green-200 hover:border-green-400',
                'from-lime-50 to-lime-100 hover:from-lime-100 hover:to-lime-200 border-lime-200 hover:border-lime-400',
              ];
              const bgColors = ['bg-emerald-600', 'bg-teal-600', 'bg-green-600', 'bg-lime-600'];
