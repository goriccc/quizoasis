...
import {
  Phase3WorldGreetingChallengeQuestion,
  Phase3WorldGreetingChallengeResult,
  Phase3WorldGreetingChallengeOption,
  calculatePhase3WorldGreetingChallengeResult,
} from '@/lib/phase3WorldGreetingChallengeData';
...
interface Phase3WorldGreetingChallengeTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3WorldGreetingChallengeQuestion[];
  results: Phase3WorldGreetingChallengeResult[];
...
export default function Phase3WorldGreetingChallengeTestClient({
  locale,
  slug,
  title,
  description,
  ...
  isLatestTest = false,
  badgeType = null,
}: Phase3WorldGreetingChallengeTestClientProps) {
  const t = useTranslations('phase3WorldGreetingChallengeTest');
  ...
  const [result, setResult] = useState<Phase3WorldGreetingChallengeResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3WorldGreetingChallengeQuestion[]>([]);
  /** 답 선택 후 정답/해설 확인 단계 */
  ...
  useEffect(() => {
    if (!started && aliProducts.length === 0) {
      const loadProducts = async () => {
        try {
          const keyword = locale === 'ko' ? '언어 학습' : 'language learning';
  ...
  useEffect(() => {
    if (result && locale !== 'ko') {
      const loadProducts = async () => {
        try {
          const keyword = 'language learning travel';
          const products = await searchAliExpressProducts(keyword, 4, locale);
          setAliProducts(products);
        } catch (error) {
          console.error('상품 로드 실패:', error);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3WorldGreetingChallengeQuestion[]) => {
  ...
  const finishQuizWithAnswers = (finalAnswers: Record<number, number>) => {
    ...
    const resultType = calculatePhase3WorldGreetingChallengeResult(answersArray);
    ...
    if (quizResult && locale !== 'ko') {
      ...
      searchAliExpressProducts('language learning travel', 4, locale)
        .then(products => {
  ...
  const handleOptionSelect = (option: Phase3WorldGreetingChallengeOption) => {
  ...
  if (showResult && result) {
    ...
    const resultLanguageGrade = result.languageGrade[locale as keyof typeof result.languageGrade] || result.languageGrade.ko;
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4 border border-purple-100">
                ...
                <p className="text-lg font-bold text-purple-700 text-center">{resultLanguageGrade}</p>
              </div>
