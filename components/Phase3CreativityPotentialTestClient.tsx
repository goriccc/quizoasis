...
import {
  Phase3CreativityPotentialQuestion,
  Phase3CreativityPotentialResult,
  calculatePhase3CreativityPotentialResult,
...
interface Phase3CreativityPotentialTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3CreativityPotentialQuestion[];
  results: Phase3CreativityPotentialResult[];
...
export default function Phase3CreativityPotentialTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3CreativityPotentialTestClientProps) {
  const t = useTranslations('phase3CreativityPotentialTest');
  ...
  const [result, setResult] = useState<Phase3CreativityPotentialResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3CreativityPotentialQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3CreativityPotentialQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3CreativityPotentialResult(answersArray);
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="px-4">
            ...
            <div className="text-gray-600 mb-6 leading-relaxed text-center space-y-4">
              ...
              <p className="font-semibold text-gray-800">{t('startMessage.line4')}</p>
  ...
  if (showResult && result) {
    ...
    const resultCreativityType = result.creativityType[locale as keyof typeof result.creativityType] || result.creativityType.ko;
    const resultCreativityKeywords = result.creativityKeywords[locale as keyof typeof result.creativityKeywords] || result.creativityKeywords.ko;
    const resultStrongField = result.strongField[locale as keyof typeof result.strongField] || result.strongField.ko;
    ...
    const resultMaximizeMethod = result.maximizeMethod[locale as keyof typeof result.maximizeMethod] || result.maximizeMethod.ko;
    const resultGoodMatch = result.goodMatch[locale as keyof typeof result.goodMatch] || result.goodMatch.ko;
    const resultDesignerTip = result.designerTip[locale as keyof typeof result.designerTip] || result.designerTip.ko;
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-base font-semibold text-purple-700 text-center">
                  {resultCreativityType}
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  ⭐ {t('ui.creativityKeywords')}
                </h3>
                <div className="flex flex-wrap justify-center gap-2">
                  {resultCreativityKeywords.split(/[·]/).map((keyword, index) => (
                    <span
                      key={index}
