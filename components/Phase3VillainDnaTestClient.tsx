...
import {
  Phase3VillainDnaQuestion,
  Phase3VillainDnaResult,
  calculatePhase3VillainDnaResult,
...
interface Phase3VillainDnaTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3VillainDnaQuestion[];
  results: Phase3VillainDnaResult[];
...
export default function Phase3VillainDnaTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3VillainDnaTestClientProps) {
  const t = useTranslations('phase3VillainDnaTest');
  ...
  const [result, setResult] = useState<Phase3VillainDnaResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3VillainDnaQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3VillainDnaQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3VillainDnaResult(answersArray);
      const villainResult = results.find(r => r.type === resultType);
      
      if (villainResult) {
        setResult(villainResult);
      }
    }
  };
  ...
  const handleShareResult = async () => {
    ...
    const snippet = result.shareSnippet[locale as keyof typeof result.shareSnippet] || result.shareSnippet.ko;
  ...
  if (showResult && result) {
    ...
    const codename = result.villainCodename[locale as keyof typeof result.villainCodename] || result.villainCodename.ko;
    const specialty = result.villainSpecialty[locale as keyof typeof result.villainSpecialty] || result.villainSpecialty.ko;
    const quote = result.quoteLine[locale as keyof typeof result.quoteLine] || result.quoteLine.ko;
    const level = result.villainLevel[locale as keyof typeof result.villainLevel] || result.villainLevel.ko;
    const awakening = result.awakeningCondition[locale as keyof typeof result.awakeningCondition] || result.awakeningCondition.ko;
    const flameProb = result.blackFlameProbability[locale as keyof typeof result.blackFlameProbability] || result.blackFlameProbability.ko;
    const ultimate = result.ultimateMove[locale as keyof typeof result.ultimateMove] || result.ultimateMove.ko;
    const danger = result.dangerIndex[locale as keyof typeof result.dangerIndex] || result.dangerIndex.ko;
    const oneLine = result.oneLineEval[locale as keyof typeof result.oneLineEval] || result.oneLineEval.ko;
          ...
          <div>
            <div className="text-center mb-3 bg-white rounded-2xl shadow-lg p-4 md:p-5">
              ...
              <p className="text-sm font-medium text-gray-700 mb-3">
                🎭 {t('ui.villainSpecialty')}: {specialty}
              </p>
              <p className="text-base italic text-gray-800 mb-4 border-l-4 border-purple-400 pl-3 text-left">
                {quote}
              </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-base font-semibold text-purple-600">{level}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  ⚡ {t('ui.awakeningCondition')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">{awakening}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  🔥 {t('ui.blackFlameProbability')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">{flameProb}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  💥 {t('ui.ultimateMove')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">{ultimate}</p>
              </div>
            </div>
            ...
            <div className="space-y-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{danger}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  💬 {t('ui.oneLineEval')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">{oneLine}</p>
              </div>
            </div>
        ...
        <div>
          ...
          <div className="space-y-4 px-4">
            {optionsArray.map((option, index) => {
              ...
              const colorIdx = index % 2;

              return (
                <button
                  key={index}
                  onClick={() => handleAnswer(option.score)}
                  className={`w-full bg-gradient-to-r ${colors[colorIdx]} border-2 text-gray-800 font-medium py-3 px-4 rounded-xl transition-all transform hover:scale-102 text-left`}
                >
                  <div className="flex items-center">
                    <div className={`w-7 h-7 ${bgColors[colorIdx]} text-white rounded-full flex items-center justify-center font-bold mr-3 flex-shrink-0 text-sm`}>
                      {label}
                    </div>
