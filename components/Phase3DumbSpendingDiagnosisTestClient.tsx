...
import {
  Phase3DumbSpendingDiagnosisQuestion,
  Phase3DumbSpendingDiagnosisResult,
  calculatePhase3DumbSpendingDiagnosisResult,
...
interface Phase3DumbSpendingDiagnosisTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3DumbSpendingDiagnosisQuestion[];
  results: Phase3DumbSpendingDiagnosisResult[];
...
export default function Phase3DumbSpendingDiagnosisTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3DumbSpendingDiagnosisTestClientProps) {
  const t = useTranslations('phase3DumbSpendingDiagnosisTest');
  ...
  const [result, setResult] = useState<Phase3DumbSpendingDiagnosisResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3DumbSpendingDiagnosisQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3DumbSpendingDiagnosisQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3DumbSpendingDiagnosisResult(answersArray);
      const dumbResult = results.find(r => r.type === resultType);
      
      if (dumbResult) {
        setResult(dumbResult);
      }
    }
  };
  ...
  if (showResult && result) {
    ...
    const resultDiagnosis = result.diagnosisGrade[locale as keyof typeof result.diagnosisGrade] || result.diagnosisGrade.ko;
    ...
    const monthlyWaste = result.monthlyWasteRange[locale as keyof typeof result.monthlyWasteRange] || result.monthlyWasteRange.ko;
    const yearlySave = result.yearlySaveRange[locale as keyof typeof result.yearlySaveRange] || result.yearlySaveRange.ko;
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            <div className="text-center mb-3 bg-white rounded-2xl shadow-lg p-4 md:p-5">
              ...
              <p className="text-base font-semibold text-orange-600 mb-2">{resultDiagnosis}</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-center text-lg font-bold text-red-600">{monthlyWaste}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  💰 {t('ui.yearlySave')}
                </h3>
                <p className="text-center text-lg font-bold text-emerald-600">{yearlySave}</p>
              </div>
            </div>
