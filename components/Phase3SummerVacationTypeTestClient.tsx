...
import {
  Phase3SummerVacationTypeQuestion,
  Phase3SummerVacationTypeResult,
  calculatePhase3SummerVacationTypeResult,
} from '@/lib/phase3SummerVacationTypeData';
...
interface Phase3SummerVacationTypeTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3SummerVacationTypeQuestion[];
  results: Phase3SummerVacationTypeResult[];
...
export default function Phase3SummerVacationTypeTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3SummerVacationTypeTestClientProps) {
  const t = useTranslations('phase3SummerVacationTypeTest');
  ...
  const [result, setResult] = useState<Phase3SummerVacationTypeResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3SummerVacationTypeQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3SummerVacationTypeQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3SummerVacationTypeResult(answersArray);
      const vacationResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (vacationResult) {
        setResult(vacationResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (vacationResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(vacationResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 휴가지 유형 결과:', vacationResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const resultVacationStyle =
      result.vacationStyle[locale as keyof typeof result.vacationStyle] || result.vacationStyle.ko;
    const resultRecommended =
      result.recommendedPlaces[locale as keyof typeof result.recommendedPlaces] || result.recommendedPlaces.ko;
    const resultBudget =
      result.budgetSense[locale as keyof typeof result.budgetSense] || result.budgetSense.ko;
    ...
    const recommendedPlaceParts = resultRecommended
      .split('/')
      .map((s) => s.trim())
      .filter(Boolean);
      ...
      <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-blue-50">
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                📍 {t('ui.recommendedPlaces')}
              </h3>
              <div className="flex flex-wrap gap-2">
                {recommendedPlaceParts.map((part, i) => (
                  <span
                    key={i}
                    className="inline-flex items-center rounded-full border border-sky-100 bg-gradient-to-r from-sky-50 to-cyan-50 px-3.5 py-2 text-sm font-medium text-gray-800 shadow-sm"
                  >
                    {part}
                  </span>
                ))}
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <p className="text-base text-gray-700 leading-relaxed">{resultBudget}</p>
            </div>
