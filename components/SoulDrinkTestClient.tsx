...
import { SoulDrinkQuestion, SoulDrinkResult, calculateSoulDrinkResult } from '@/lib/soulDrinkData';
...
interface SoulDrinkTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: SoulDrinkQuestion[];
  results: SoulDrinkResult[];
...
export default function SoulDrinkTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  playCount = 0,
  similarTests = []
}: SoulDrinkTestClientProps) {
  const t = useTranslations('soulDrinkTest');
  ...
  const [result, setResult] = useState<SoulDrinkResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<SoulDrinkQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: SoulDrinkQuestion[]) => {
  ...
  const handleAnswer = (typePoints: number[]) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      const answersArray = questions.map((_, idx) => newAnswers[idx] ?? [0, 0, 0, 0, 0]);
      
      // 결과 계산
      const resultType = calculateSoulDrinkResult(answersArray);
      const soulDrinkResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (soulDrinkResult) {
        setResult(soulDrinkResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (soulDrinkResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(soulDrinkResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 소울 드링크 결과:', soulDrinkResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const resultSoulCoffee = result.soulCoffee[locale as keyof typeof result.soulCoffee] || result.soulCoffee.ko;
    const resultSoulWine = result.soulWine[locale as keyof typeof result.soulWine] || result.soulWine.ko;
