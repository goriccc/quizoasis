...
import {
  Phase3EagleEyeUltimateQuestion,
  Phase3EagleEyeUltimateResult,
  Phase3EagleEyeUltimateOption,
  calculatePhase3EagleEyeUltimateResult,
...
interface Phase3EagleEyeUltimateTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3EagleEyeUltimateQuestion[];
  results: Phase3EagleEyeUltimateResult[];
...
export default function Phase3EagleEyeUltimateTestClient({
  locale,
  slug,
  title,
  description,
  ...
  isLatestTest = false,
  badgeType = null,
}: Phase3EagleEyeUltimateTestClientProps) {
  const t = useTranslations('phase3EagleEyeUltimateTest');
  ...
  const [result, setResult] = useState<Phase3EagleEyeUltimateResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3EagleEyeUltimateQuestion[]>([]);
  /** 답 선택 후 정답/해설 확인 단계 */
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3EagleEyeUltimateQuestion[]) => {
  ...
  const finishQuizWithAnswers = (finalAnswers: Record<number, number>) => {
    ...
    const resultType = calculatePhase3EagleEyeUltimateResult(answersArray);
  ...
  const handleOptionSelect = (option: Phase3EagleEyeUltimateOption) => {
  ...
  if (showResult && result) {
    ...
    const resultStrength = result.strength
      ...
      : null;
    const resultWeakness = result.weakness
    ...
    const resultOneLiner = result.oneLiner
      ...
      : null;
    const resultTodo = result.todoToday
      ? result.todoToday[locale as keyof typeof result.todoToday] || result.todoToday.ko
      : null;
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4 border border-sky-100">
                ...
                <p className="text-lg font-bold text-sky-700 text-center">{resultLevel}</p>
              </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3 border border-amber-100">
              <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                ⭐ {t('ui.characteristicsLabel')}
              </h3>
              <div className="flex flex-wrap gap-2 justify-center">
                {resultCharacteristics.split(/[,、，]/).map((char, idx) => (
                  <span
                    key={idx}
                    className="inline-block bg-gradient-to-r from-indigo-100 to-sky-100 text-indigo-800 text-sm font-semibold px-3 py-1 rounded-full shadow-sm"
                  >
                    {char.trim()}
                  </span>
                ))}
            ...
            {resultTodo && (
              <div className="bg-white rounded-xl shadow-lg p-4 mb-3 border border-blue-100">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  ✅ {t('ui.nextChallengeLabel')}
                </h3>
                <p className="text-sm text-gray-700">{resultTodo}</p>
              </div>
            )}
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div>
          ...
          <div className="mb-6 px-4 text-center">
            ...
            <p className="text-sm text-gray-500">{t('ui.questionHint')}</p>
          </div>

          <div className="space-y-4 px-4">
            {optionsArray.map((option, index) => {
              const optionTextRaw = option.text[locale as keyof typeof option.text] || option.text.ko;
              const optionText = optionTextRaw.replace(/^[A-D]\.\s*/, '');
              const colors = [
