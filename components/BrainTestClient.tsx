...
import { BrainQuestion, BrainResult, calculateBrainResult } from '@/lib/brainData';
...
interface BrainTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
...
const getCompatibilityDescription = (myType: string, partnerType: string, t: any): string => {
  ...
  const translation = t(`brainTest.compatibility.${key}`);
  
  // 번역이 없으면 기본 설명 반환
  if (!translation || translation === `brainTest.compatibility.${key}`) {
    return t('brainTest.result.noCompatibleTypes');
  }
...
// 주의 타입 설명 함수
const getCautionDescription = (myType: string, partnerType: string, t: any): string => {
  const key = `${myType}_${partnerType}`;
  const translation = t(`brainTest.caution.${key}`);
  
  if (translation && translation !== `brainTest.caution.${key}`) {
    return translation;
  }
  
  return t('brainTest.result.noCautionTypes');
};
...
export default function BrainTestClient({ 
  ...
  const [shuffledQuestions, setShuffledQuestions] = useState<BrainQuestion[]>(questions);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: BrainQuestion[]) => {
  ...
  const handleAnswer = (scores: any) => {
    const newAnswers = [...answers, { questionId: shuffledQuestions[currentQuestion].id, scores }];
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculateBrainResult(newAnswers);
      ...
      if (brainResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(brainResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 좌뇌/우뇌 결과:', brainResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  const calculateResult = (finalAnswers: any[]) => {
    const resultType = calculateBrainResult(finalAnswers);
    const brainResult = results.find(r => r.type === resultType);
  ...
  const handleShareResult = async () => {
    ...
    const shareText = t('brainTest.shareMessages.default', { type: resultTitle });
    ...
    if (navigator.share) {
      ...
      try {
        ...
        alert(t('brainTest.ui.linkCopySuccess'));
      } catch (error) {
        console.error('클립보드 복사 실패:', error);
        alert(t('brainTest.ui.shareError'));
      }
    }
  };
  ...
  const shareToWeChat = async () => {
    ...
    const shareText = result 
      ? t('brainTest.shareMessages.wechat', { type: resultTitle })
      : `${title}\n\n${url}`;
    ...
    try {
      ...
      alert(t('brainTest.ui.wechatLinkCopy'));
    } catch (error) {
      alert(t('brainTest.ui.shareFeatureError'));
    }
  };
  ...
  const shareToWhatsApp = () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareText = result 
      ? encodeURIComponent(t('brainTest.shareMessages.whatsapp', { type: resultTitle }))
  ...
  const shareToKakao = () => {
    ...
    if (!window.Kakao || !window.Kakao.isInitialized()) {
      alert(t('brainTest.ui.kakaoInitializing'));
      return;
    }
    ...
    const shareDescription = result 
      ? t('brainTest.shareMessages.kakao', { type: resultTitle })
      : description;
    ...
    try {
      window.Kakao.Share.sendDefault({
        ...
        buttons: [
          {
            title: t('brainTest.ui.startTest'),
            link: {
              mobileWebUrl: currentUrl,
              webUrl: currentUrl,
            },
      ...
      alert(t('brainTest.ui.kakaoShareError'));
    }
  };
  ...
  const shareToTelegram = () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareText = result 
      ? t('brainTest.shareMessages.telegram', { type: resultTitle })
      : title;
      ...
      <div className="min-h-screen bg-white">
        <div className="max-w-4xl mx-auto">
          <div className="relative w-full overflow-hidden mb-3" style={{ aspectRatio: '680/384' }}>
            <Image
              src={getThumbnailUrl(thumbnail || 'test_104_left_right_brain.jpg')}
              alt={title}
              fill
          ...
          <div className="px-4">
            ...
            <div className="text-gray-600 mb-6 leading-relaxed text-center space-y-4">
              <p className="font-bold text-gray-700">{t('brainTest.startMessage.line1')}</p>
              <p>{t('brainTest.startMessage.line2')}</p>
            ...
            <div className="mb-8 text-center">
              <h2 className="text-lg font-bold text-gray-800 mb-4">
                {t('brainTest.ui.shareWithFriends')}
              </h2>
              <div className="flex justify-center gap-2">
                <button onClick={copyLink} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/link.jpeg" alt={t('brainTest.ui.linkCopy')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToKakao} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/kakao.jpeg" alt={t('brainTest.ui.kakao')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToTelegram} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/telegram.jpeg" alt={t('brainTest.ui.telegram')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToWeChat} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/wechat.jpeg" alt={t('brainTest.ui.wechat')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToLine} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/line.jpeg" alt={t('brainTest.ui.line')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToWhatsApp} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/whatsapp.jpeg" alt={t('brainTest.ui.whatsapp')} width={46} height={46} className="rounded-lg" />
                </button>
              </div>
            </div>
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-4">
        ...
        <div className="flex flex-col items-center justify-center">
          ...
          <p className="mt-4 text-lg text-gray-700">{t('brainTest.ui.analyzing')}</p>
        </div>
  ...
  if (showResult && result) {
    ...
    const resultRecommendedJobs = result.recommendedJobs[locale as keyof typeof result.recommendedJobs] || result.recommendedJobs.ko;
      ...
      <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-blue-50">
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                {t('brainTest.ui.recommendedJobs')}
              </h3>
              <div className="flex flex-wrap gap-2">
                {resultRecommendedJobs.split(/[,、]/).map((job, index) => (
                  <span
                    key={index}
            ...
            {result.compatibility && (
              <>
                <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
                  <h3 className="text-base font-bold text-gray-800 mb-3">
                    {t('brainTest.ui.compatibleTypes')}
                  </h3>
                  {Array.isArray(result.compatibility.best) && result.compatibility.best.length > 0 ? (
                    ...
                    <p className="text-sm text-gray-700">
                      {t('brainTest.result.noCompatibleTypes')}
                    </p>
                  )}
                </div>
                ...
                <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
                  <h3 className="text-base font-bold text-gray-800 mb-3">
                    {t('brainTest.ui.cautionTypes')}
                  </h3>
                  {Array.isArray(result.compatibility.careful) && result.compatibility.careful.length > 0 ? (
                    <div className="space-y-2">
                      {result.compatibility.careful.map(type => {
                        const partner = results.find(r => r.type === type);
                        if (!partner) return null;
                        const partnerTitle = partner.title[locale as keyof typeof partner.title] || partner.title.ko;
                        const compatibilityDesc = getCautionDescription(result.type, type, t);
                        return (
                    ...
                    <p className="text-sm text-gray-700">
                      {t('brainTest.result.noCautionTypes')}
                    </p>
                  )}
                </div>
              </>
            ...
            <div className="flex flex-col sm:flex-row gap-4 px-4">
              ...
              >
                {t('brainTest.ui.retake')}
              </button>
              <Link
                href={`/${locale}`}
                className="flex-1 bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold py-4 px-6 rounded-xl hover:from-purple-700 hover:to-pink-700 transition-all text-center shadow-md"
              >
                {t('brainTest.ui.otherTests')}
              </Link>
            </div>
