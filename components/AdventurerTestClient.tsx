...
import { AdventurerQuestion, AdventurerResult, calculateAdventurerResult } from '@/lib/adventurerData';
...
interface AdventurerTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: AdventurerQuestion[];
  results: AdventurerResult[];
...
export default function AdventurerTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  playCount = 0,
  similarTests = []
}: AdventurerTestClientProps) {
  const t = useTranslations();
  const [started, setStarted] = useState(false);
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [answers, setAnswers] = useState<any[]>([]);
  const [result, setResult] = useState<AdventurerResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<AdventurerQuestion[]>(questions);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: AdventurerQuestion[]) => {
  ...
  const handleAnswer = (scores: any) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculateAdventurerResult(newAnswers);
      const adventurerResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (adventurerResult) {
        setResult(adventurerResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (adventurerResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(adventurerResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 모험가 결과:', adventurerResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  const calculateResult = (finalAnswers: any[]) => {
    const resultType = calculateAdventurerResult(finalAnswers);
    const adventurerResult = results.find(r => r.type === resultType);
  ...
  const handleShareResult = async () => {
    ...
    const shareMessage = t('adventurerTest.shareMessages.default', { type: resultTitle });
    ...
    if (navigator.share) {
      ...
      try {
        ...
        alert(t('adventurerTest.alerts.resultCopied'));
      } catch (error) {
        console.error('클립보드 복사 실패:', error);
        alert(t('adventurerTest.alerts.shareFailed'));
      }
    }
  };
  ...
  const shareToWeChat = async () => {
    ...
    const shareMessage = result 
      ? t('adventurerTest.shareMessages.wechat', { type: resultTitle })
      : title;
    const shareText = `${shareMessage}\n\n${url}`;
    ...
    try {
      ...
      alert(t('adventurerTest.alerts.wechatCopy'));
    } catch (error) {
      alert(t('adventurerTest.alerts.shareFailed'));
    }
  };
  ...
  const shareToWhatsApp = () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareMessage = result 
      ? t('adventurerTest.shareMessages.whatsapp', { type: resultTitle })
      : title;
  ...
  const shareToKakao = () => {
    ...
    if (!window.Kakao || !window.Kakao.isInitialized()) {
      alert(t('adventurerTest.alerts.kakaoInit'));
      return;
    }
    ...
    const shareDescription = result 
      ? t('adventurerTest.shareMessages.kakao', { type: resultTitle })
      : description;
    ...
    try {
      ...
      alert(t('adventurerTest.alerts.kakaoError'));
    }
  };
  ...
  const shareToTelegram = () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareMessage = result 
      ? t('adventurerTest.shareMessages.telegram', { type: resultTitle })
      : title;
  ...
  const copyLink = () => {
    ...
    alert(t('adventurerTest.alerts.linkCopied'));
  };
      ...
      <div className="min-h-screen bg-white">
        <div className="max-w-4xl mx-auto">
          <div className="relative w-full overflow-hidden mb-3" style={{ aspectRatio: '680/384' }}>
            <Image
              src={getThumbnailUrl(thumbnail || 'test_206_adventurer_vs_cautious.jpg')}
              alt={title}
              fill
          ...
          <div className="px-4">
            ...
            <div className="text-gray-600 mb-6 leading-relaxed text-center space-y-4">
              <p className="font-bold text-gray-700">{t('adventurerTest.startMessage.line1')}</p>
              <p>{t('adventurerTest.startMessage.line2')}</p>
              <p>{t('adventurerTest.startMessage.line3')}</p>
              <p>{t('adventurerTest.startMessage.line4')}</p>
              <p>{t('adventurerTest.startMessage.line5')}</p>
              <p>{t('adventurerTest.startMessage.line6')}</p>
              <p>{t('adventurerTest.startMessage.line7')}</p>
              <p>{t('adventurerTest.startMessage.line8')}</p>
              <p>{t('adventurerTest.startMessage.line9')}</p>
            </div>
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-3">
                  ✅ {t('adventurerTest.result.strengths')}
                </h3>
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-3">
                  ⚠️ {t('adventurerTest.result.weaknesses')}
                </h3>
                <div className="flex flex-wrap gap-2">
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                🎯 {t('adventurerTest.result.score')}
              </h3>
              <div className="flex flex-wrap gap-2">
                {resultScore.split(' / ').map((scoreItem, index) => (
                  <span
                    key={index}
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                💡 {t('adventurerTest.result.advice')}
              </h3>
