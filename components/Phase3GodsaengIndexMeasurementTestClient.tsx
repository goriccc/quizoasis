...
import {
  Phase3GodsaengIndexMeasurementQuestion,
  Phase3GodsaengIndexMeasurementResult,
  calculatePhase3GodsaengIndexMeasurementResult,
...
interface Phase3GodsaengIndexMeasurementTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3GodsaengIndexMeasurementQuestion[];
  results: Phase3GodsaengIndexMeasurementResult[];
...
export default function Phase3GodsaengIndexMeasurementTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3GodsaengIndexMeasurementTestClientProps) {
  const t = useTranslations('phase3GodsaengIndexMeasurementTest');
  ...
  const [result, setResult] = useState<Phase3GodsaengIndexMeasurementResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3GodsaengIndexMeasurementQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3GodsaengIndexMeasurementQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3GodsaengIndexMeasurementResult(answersArray);
      const godResult = results.find(r => r.type === resultType);
  ...
  const handleShareResult = async () => {
    ...
    const line = result.shareLine[locale as keyof typeof result.shareLine] || result.shareLine.ko;
  ...
  if (showResult && result) {
    ...
    const indexHeadline =
      result.indexHeadline[locale as keyof typeof result.indexHeadline] || result.indexHeadline.ko;
    const hookLine = result.hookLine[locale as keyof typeof result.hookLine] || result.hookLine.ko;
    const descriptionText = result.description[locale as keyof typeof result.description] || result.description.ko;
    const levelLine = result.levelLine[locale as keyof typeof result.levelLine] || result.levelLine.ko;
    const strengthLine = result.strengthLine[locale as keyof typeof result.strengthLine] || result.strengthLine.ko;
    const cautionLine = result.cautionLine[locale as keyof typeof result.cautionLine] || result.cautionLine.ko;
    const missionLine = result.missionLine[locale as keyof typeof result.missionLine] || result.missionLine.ko;
    const todayLine = result.todayLine[locale as keyof typeof result.todayLine] || result.todayLine.ko;
    const mottoLine = result.mottoLine[locale as keyof typeof result.mottoLine] || result.mottoLine.ko;
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            <div className="text-center mb-3 bg-white rounded-2xl shadow-lg p-4 md:p-5">
              ...
              <p className="text-lg font-bold text-emerald-700 mb-3">{indexHeadline}</p>
              <p className="text-sm font-semibold text-gray-800 mb-2 text-center">{levelLine}</p>
              <p className="text-lg font-medium text-gray-800 mb-3 text-left">{hookLine}</p>
              <p className="text-base text-gray-600 leading-relaxed text-left">{descriptionText}</p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-sm font-bold text-gray-800 mb-2 text-left">{t('ui.strength')}</h3>
                <p className="text-sm text-gray-700 leading-relaxed text-left">{strengthLine}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-sm font-bold text-gray-800 mb-2 text-left">{t('ui.caution')}</h3>
                <p className="text-sm text-gray-700 leading-relaxed text-left">{cautionLine}</p>
              </div>
            </div>

            <div className="mb-3 bg-white rounded-xl shadow-lg p-4 md:p-5">
              <h3 className="text-base font-bold text-gray-800 mb-2 text-left">{t('ui.mission')}</h3>
              <p className="text-sm text-gray-800 leading-relaxed text-left">{missionLine}</p>
            </div>

            <div className="mb-3 bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl shadow-lg p-4 border border-orange-100">
              <h3 className="text-base font-bold text-gray-800 mb-2">{t('ui.todayTask')}</h3>
              <p className="text-sm text-gray-800 leading-relaxed">{todayLine}</p>
            </div>

            <div className="mb-3 bg-gradient-to-r from-emerald-50 to-teal-50 rounded-xl shadow-lg p-4 border border-emerald-100">
              <p className="text-sm text-gray-800 leading-relaxed text-left font-medium">{mottoLine}</p>
            </div>
