...
import {
  Phase3TanjinjamSpendingTypeQuestion,
  Phase3TanjinjamSpendingTypeResult,
  calculatePhase3TanjinjamSpendingTypeResult,
...
interface Phase3TanjinjamSpendingTypeTestClientProps {
  ...
  title: string;
  description: string;
  questions: Phase3TanjinjamSpendingTypeQuestion[];
  results: Phase3TanjinjamSpendingTypeResult[];
...
export default function Phase3TanjinjamSpendingTypeTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3TanjinjamSpendingTypeTestClientProps) {
  const t = useTranslations('phase3TanjinjamSpendingTypeTest');
  ...
  const [result, setResult] = useState<Phase3TanjinjamSpendingTypeResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3TanjinjamSpendingTypeQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3TanjinjamSpendingTypeQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3TanjinjamSpendingTypeResult(answersArray);
      const tanjinResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (tanjinResult) {
        setResult(tanjinResult);
      }
    }
  };
  ...
  if (showResult && result) {
    ...
    const resultTanjinCategory =
      result.tanjinJamCategory[locale as keyof typeof result.tanjinJamCategory] || result.tanjinJamCategory.ko;
    const resultTanjinPoint =
      result.tanjinPoint[locale as keyof typeof result.tanjinPoint] || result.tanjinPoint.ko;
    const resultTanjinLevel =
      result.tanjinLevel[locale as keyof typeof result.tanjinLevel] || result.tanjinLevel.ko;
    const resultTanjinSignature =
      result.tanjinSignature[locale as keyof typeof result.tanjinSignature] || result.tanjinSignature.ko;
    const resultMonthlyPick =
      result.monthlyPick[locale as keyof typeof result.monthlyPick] || result.monthlyPick.ko;
          ...
          <div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-5">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {resultTanjinPoint.split(/[,，、]/).map((part, index) => (
                    <span
                      key={index}
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed text-center font-semibold">{resultTanjinLevel}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  📝 {t('ui.tanjinSignature')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed text-center">{resultTanjinSignature}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed text-center">{resultMonthlyPick}</p>
              </div>
            </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2 sm:gap-4 px-2 sm:px-4">
            {optionsArray.map((option, index) => {
              ...
              const bgColors = ['bg-purple-600', 'bg-pink-600', 'bg-blue-600', 'bg-emerald-600'];
