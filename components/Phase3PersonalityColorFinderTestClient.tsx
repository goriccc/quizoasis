...
import {
  Phase3PersonalityColorFinderQuestion,
  Phase3PersonalityColorFinderResult,
  calculatePhase3PersonalityColorFinderResult,
...
interface Phase3PersonalityColorFinderTestClientProps {
  ...
  title: string;
  description: string;
  questions: Phase3PersonalityColorFinderQuestion[];
  results: Phase3PersonalityColorFinderResult[];
...
export default function Phase3PersonalityColorFinderTestClient({
  locale,
  slug,
  title,
  description,
  ...
  isLatestTest = false,
  badgeType = null,
}: Phase3PersonalityColorFinderTestClientProps) {
  // ✅ 템플릿 UI/UX 그대로 사용: namespace만 변경
  const t = useTranslations('phase3PersonalityColorFinderTest');
  ...
  const [result, setResult] = useState<Phase3PersonalityColorFinderResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3PersonalityColorFinderQuestion[]>([]);
      ...
      const loadTests = async () => {
        try {
          ...
          const similarTestsList = allTests
            ...
            .filter((t: any) => {
              const otherTestTags =
                typeof t.tags === 'object' && !Array.isArray(t.tags)
                  ? t.tags[locale] || t.tags.ko || []
                  : t.tags || [];

              return (
                Array.isArray(currentTestTags) &&
                Array.isArray(otherTestTags) &&
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3PersonalityColorFinderQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      const resultType = calculatePhase3PersonalityColorFinderResult(answersArray);
      const match = results.find((r) => r.type === resultType);
  ...
  if (showResult && result) {
    ...
    const resultShortDescription =
      result.shortDescription[locale as keyof typeof result.shortDescription] || result.shortDescription.ko;
    const resultLongDescription = result.description[locale as keyof typeof result.description] || result.description.ko;
    const resultAlgorithmSubtype =
      result.algorithmSubtype[locale as keyof typeof result.algorithmSubtype] || result.algorithmSubtype.ko;
    const resultFeedComposition =
      result.feedComposition[locale as keyof typeof result.feedComposition] || result.feedComposition.ko;
    const resultSaveFolder =
      result.saveFolderName[locale as keyof typeof result.saveFolderName] || result.saveFolderName.ko;
    const resultSnsMain =
      result.snsMainActivity[locale as keyof typeof result.snsMainActivity] || result.snsMainActivity.ko;
    ...
    const paletteItems = resultSaveFolder
      .split(/\s*\/\s*/g)
      .map((s) => s.trim())
      .filter(Boolean);

    const goodColorItems = resultGoodMatch
      .split(/[,、，]/g)
      .map((s) => s.trim())
      .filter(Boolean);

    const badColorItems = resultBadMatch
      .split(/[,、，]/g)
      .map((s) => s.trim())
      .filter(Boolean);
          ...
          <div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">📱 {t('ui.algorithmSubtype')}</h3>
                <p className="text-lg font-bold text-purple-600 text-center leading-snug" style={{ fontSize: '1.25em' }}>
                  {resultAlgorithmSubtype}
                </p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-5">
                <h3 className="text-base font-bold text-gray-800 mb-4 text-left">📊 {t('ui.feedComposition')}</h3>
                <div className="flex flex-wrap justify-center gap-2">
                  {resultFeedComposition.split(/\s*\/\s*/).map((part, index) => (
                    <span
                      key={index}
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">📁 {t('ui.saveFolderName')}</h3>
                <div className="flex flex-wrap justify-center gap-2">
                  {paletteItems.map((part, index) => (
                    <span
                      key={index}
                      className="inline-block bg-gradient-to-r from-emerald-50 to-emerald-100 text-emerald-800 px-3 py-1.5 rounded-full text-sm font-medium border border-emerald-200"
                    >
                      {part}
                    </span>
                  ))}
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">💬 {t('ui.snsMainActivity')}</h3>
                <p className="text-sm text-gray-700 leading-relaxed text-center">{resultSnsMain}</p>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-3">💖 {t('ui.goodMatch')}</h3>
                <div className="flex flex-wrap gap-2">
                  {goodColorItems.map((c, idx) => (
                    <span
                      key={`${c}-${idx}`}
                      className="inline-block bg-gradient-to-r from-green-100 to-emerald-100 text-emerald-900 px-3 py-1.5 rounded-full text-sm font-medium"
                    >
                      {c}
                    </span>
                  ))}
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-3">💔 {t('ui.badMatch')}</h3>
                <div className="flex flex-wrap gap-2">
                  {badColorItems.map((c, idx) => (
                    <span
                      key={`${c}-${idx}`}
                      className="inline-block bg-gradient-to-r from-yellow-100 to-orange-100 text-orange-900 px-3 py-1.5 rounded-full text-sm font-medium"
                    >
                      {c}
                    </span>
                  ))}
              ...
              return (
                ...
                >
                  ...
                  <div className="p-2 sm:p-3">
                    <p className="text-xs sm:text-sm font-medium text-gray-800 leading-snug line-clamp-4">
                      {optionLabel}
                    </p>
                  </div>
                </button>
