...
import { TimePerspectiveQuestion, TimePerspectiveResult, calculateTimePerspectiveResult } from '@/lib/timePerspectiveData';
...
interface TimePerspectiveTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: TimePerspectiveQuestion[];
  results: TimePerspectiveResult[];
...
export default function TimePerspectiveTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: TimePerspectiveTestClientProps) {
  const t = useTranslations('timePerspectiveTest');
  ...
  const [result, setResult] = useState<TimePerspectiveResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<TimePerspectiveQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: TimePerspectiveQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculateTimePerspectiveResult(answersArray);
      const timePerspectiveResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (timePerspectiveResult) {
        setResult(timePerspectiveResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (timePerspectiveResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(timePerspectiveResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 시간 관점 결과:', timePerspectiveResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const resultTimePerspective = result.timePerspective[locale as keyof typeof result.timePerspective] || result.timePerspective.ko;
