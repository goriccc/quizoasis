...
import {
  Phase3EqSelfDiagnosisQuestion,
  Phase3EqSelfDiagnosisResult,
  calculatePhase3EqSelfDiagnosisResult,
  calculatePhase3EqSelfDiagnosisDomainScores,
} from '@/lib/phase3EqSelfDiagnosisData';
...
interface Phase3EqSelfDiagnosisTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3EqSelfDiagnosisQuestion[];
  results: Phase3EqSelfDiagnosisResult[];
...
export default function Phase3EqSelfDiagnosisTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3EqSelfDiagnosisTestClientProps) {
  const t = useTranslations('phase3EqSelfDiagnosisTest');
  ...
  const [result, setResult] = useState<Phase3EqSelfDiagnosisResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3EqSelfDiagnosisQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3EqSelfDiagnosisQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      const scores = calculatePhase3EqSelfDiagnosisDomainScores(answersArray);
      setDomainScores(scores);

      const resultType = calculatePhase3EqSelfDiagnosisResult(answersArray);
  ...
  const handleRetake = () => {
    ...
    setResult(null);
    setShowResult(false);
    setDomainScores({ selfAwareness: 0, selfRegulation: 0, motivation: 0, empathy: 0, socialSkills: 0, total: 0 });
  ...
  if (showResult && result) {
    ...
    const resultEqSelfDiagnosis =
      result.eqLevel[locale as keyof typeof result.eqLevel] || result.eqLevel.ko;
          ...
          <div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <p className="text-sm font-bold text-gray-700 leading-relaxed text-center">
                {resultEqSelfDiagnosis}
              </p>
            </div>

            {resultFiveElementsAnalysis && (
              <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  🧠 {t('ui.fiveElementsAnalysisTitle')}
                </h3>
                <ul className="text-sm text-gray-700 leading-relaxed list-disc list-inside pl-[2ch]">
                  {resultFiveElementsAnalysis.split('\n').map((item, idx) => (
                    item.trim() ? (
                ...
                {domainConfig.map((domain) => (
                  <div key={domain.key}>
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2.5">
                      <div
                        className="bg-gradient-to-r from-purple-500 to-violet-400 h-2.5 rounded-full transition-all"
...
{isType6 && resultCertification && (
            ...
            <div className="mt-8 mb-6 px-4">
              <button
                ...
                className="w-full bg-gradient-to-r from-purple-500 to-violet-500 text-white font-bold py-4 px-6 rounded-xl hover:from-purple-600 hover:to-violet-600 transition-all shadow-md flex items-center justify-center gap-3"
              >
