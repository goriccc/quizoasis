...
import { Phase2WorldHistoryQuizQuestion, Phase2WorldHistoryQuizResult, calculatePhase2WorldHistoryQuizResult } from '@/lib/phase2_world_history_quiz_data';
...
interface Phase2WorldHistoryQuizTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase2WorldHistoryQuizQuestion[];
  results: Phase2WorldHistoryQuizResult[];
...
export default function Phase2WorldHistoryQuizTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false
}: Phase2WorldHistoryQuizTestClientProps) {
  const t = useTranslations('phase2WorldHistoryQuizTest');
  ...
  const [result, setResult] = useState<Phase2WorldHistoryQuizResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2WorldHistoryQuizQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase2WorldHistoryQuizQuestion[]) => {
  ...
  const handleAnswer = (selectedOptionIndex: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산 (정답 체크)
      const resultType = calculatePhase2WorldHistoryQuizResult(newAnswers, questions);
      const worldHistoryQuizResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (worldHistoryQuizResult) {
        setResult(worldHistoryQuizResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (worldHistoryQuizResult) {
        // 한국어일 때는 시즌/추천 상품, 다른 언어는 결과 타입 기반
        const keyword = locale === 'ko' ? 'trending products' : getProductKeywordsForDating(worldHistoryQuizResult.type, locale)[0];
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 세계사 퀴즈 결과:', worldHistoryQuizResult.type, '→ 검색 키워드:', keyword);
  ...
  if (showResult && result) {
    ...
    const resultHistoryLevel = result.historyLevel[locale as keyof typeof result.historyLevel] || result.historyLevel.ko;
