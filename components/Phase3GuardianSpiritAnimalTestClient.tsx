...
import {
  Phase3GuardianSpiritAnimalQuestion,
  Phase3GuardianSpiritAnimalResult,
  calculatePhase3GuardianSpiritAnimalResult,
...
interface Phase3GuardianSpiritAnimalTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3GuardianSpiritAnimalQuestion[];
  results: Phase3GuardianSpiritAnimalResult[];
...
export default function Phase3GuardianSpiritAnimalTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3GuardianSpiritAnimalTestClientProps) {
  const t = useTranslations('phase3GuardianSpiritAnimalTest');
  ...
  const [result, setResult] = useState<Phase3GuardianSpiritAnimalResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3GuardianSpiritAnimalQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3GuardianSpiritAnimalQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3GuardianSpiritAnimalResult(answersArray);
      const snsResult = results.find(r => r.type === resultType);
  ...
  const handleShareResult = async () => {
    ...
    const resultShareName = result.shareTypeName[locale as keyof typeof result.shareTypeName] || result.shareTypeName.ko;
    const shareText = `${t('shareMessages.default', { name: resultShareName })}\n\n${`https://myquizoasis.com${window.location.pathname}`}`;
  ...
  const shareToLine = () => {
    ...
    const shareText = result 
      ? encodeURIComponent(t('shareMessages.line', { name: resultShareName }))
  ...
  const shareToWeChat = async () => {
    ...
    const resultShareName = result ? (result.shareTypeName[locale as keyof typeof result.shareTypeName] || result.shareTypeName.ko) : '';
    const shareText = result 
      ? `${t('shareMessages.wechat', { name: resultShareName })}\n\n${url}`
  ...
  const shareToWhatsApp = () => {
    ...
    const resultShareName = result ? (result.shareTypeName[locale as keyof typeof result.shareTypeName] || result.shareTypeName.ko) : '';
    const shareText = result 
      ? encodeURIComponent(t('shareMessages.whatsapp', { name: resultShareName }))
  ...
  const shareToKakao = () => {
    ...
    const shareDescription = result 
      ? t('shareMessages.kakao', { name: resultShareName })
  ...
  const shareToTelegram = () => {
    ...
    const resultShareName = result ? (result.shareTypeName[locale as keyof typeof result.shareTypeName] || result.shareTypeName.ko) : '';
    const shareText = result 
      ? t('shareMessages.telegram', { name: resultShareName })
  ...
  if (showResult && result) {
    ...
    const resultGuardianOfficial =
      result.guardianOfficial[locale as keyof typeof result.guardianOfficial] || result.guardianOfficial.ko;
    const resultSymbolism = result.symbolism[locale as keyof typeof result.symbolism] || result.symbolism.ko;
    const resultGuardianEnergy =
      result.guardianEnergy[locale as keyof typeof result.guardianEnergy] || result.guardianEnergy.ko;
    const resultSpiritMessage =
      result.spiritMessage[locale as keyof typeof result.spiritMessage] || result.spiritMessage.ko;
    const resultSpiritColors =
      result.spiritColors[locale as keyof typeof result.spiritColors] || result.spiritColors.ko;
    const resultLuckyDirection =
      result.luckyDirection[locale as keyof typeof result.luckyDirection] || result.luckyDirection.ko;
    const resultResemblance =
      result.resemblance[locale as keyof typeof result.resemblance] || result.resemblance.ko;
    const resultGuardianBlessing =
      result.guardianBlessing[locale as keyof typeof result.guardianBlessing] || result.guardianBlessing.ko;
          ...
          <div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  ✨ {t('ui.guardianOfficial')}
                </h3>
                <p className="text-lg font-bold text-indigo-700 text-center leading-snug">{resultGuardianOfficial}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-5">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {resultSymbolism.split(/[,、，]/).map((part, index) => (
                    <span
                      key={index}
                      className="inline-flex items-center justify-center px-3 py-1.5 rounded-full text-xs font-semibold leading-tight bg-gradient-to-br from-purple-100 to-pink-100 text-purple-900 border border-purple-200/70 shadow-sm"
                    >
                      {part.trim()}
                    </span>
                  ))}
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed text-center">{resultGuardianEnergy}</p>
              </div>
            </div>

            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4 border-l-4 border-indigo-400">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  💬 {t('ui.spiritMessage')}
                </h3>
                <p className="text-sm text-gray-800 leading-relaxed italic text-center">&ldquo;{resultSpiritMessage}&rdquo;</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-5">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {resultSpiritColors.split(/[,、，]/).map((part, index) => {
                    const pillStyles = [
                      'bg-gradient-to-br from-amber-50 to-orange-100 text-amber-950 border border-amber-200/70 shadow-sm',
                      'bg-gradient-to-br from-rose-50 to-pink-100 text-rose-950 border border-rose-200/70 shadow-sm',
                      'bg-gradient-to-br from-violet-50 to-purple-100 text-violet-950 border border-violet-200/70 shadow-sm',
                      'bg-gradient-to-br from-emerald-50 to-teal-100 text-emerald-950 border border-emerald-200/70 shadow-sm',
                    ];
                    const cls = pillStyles[index % pillStyles.length];
                    return (
                      <span
                        key={index}
                        className={`inline-flex items-center justify-center px-3 py-1.5 rounded-full text-xs font-semibold leading-tight ${cls}`}
                      >
                        {part.trim()}
                      </span>
                    );
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed text-center">{resultLuckyDirection}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed text-center">{resultResemblance}</p>
              </div>
            </div>

            <div className="mb-3">
              <div className="bg-gradient-to-r from-indigo-50 to-violet-50 rounded-xl shadow-lg p-5 border border-indigo-100">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-center">
                  {t('ui.guardianBlessing')}
                </h3>
                <p className="text-base font-semibold text-indigo-900 text-center leading-relaxed">{resultGuardianBlessing}</p>
              </div>
            </div>
