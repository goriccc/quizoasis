...
import {
  Phase3StressReliefTypeQuestion,
  Phase3StressReliefTypeResult,
  calculatePhase3StressReliefTypeResult,
...
interface Phase3StressReliefTypeTestClientProps {
  ...
  title: string;
  description: string;
  questions: Phase3StressReliefTypeQuestion[];
  results: Phase3StressReliefTypeResult[];
...
export default function Phase3StressReliefTypeTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3StressReliefTypeTestClientProps) {
  const t = useTranslations('phase3StressReliefTypeTest');
  ...
  const [result, setResult] = useState<Phase3StressReliefTypeResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3StressReliefTypeQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3StressReliefTypeQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3StressReliefTypeResult(answersArray);
      const srResult = results.find(r => r.type === resultType);
  ...
  if (showResult && result) {
    ...
    const resultScoreBand = result.scoreBand[locale as keyof typeof result.scoreBand] || result.scoreBand.ko;
    const resultTag = result.tagLine[locale as keyof typeof result.tagLine] || result.tagLine.ko;
    ...
    const mechanism = result.mechanism[locale as keyof typeof result.mechanism] || result.mechanism.ko;
    const strength = result.strength[locale as keyof typeof result.strength] || result.strength.ko;
    const caution = result.caution[locale as keyof typeof result.caution] || result.caution.ko;
    const routineText = result.routineText[locale as keyof typeof result.routineText] || result.routineText.ko;
    const actionNow = result.actionNow[locale as keyof typeof result.actionNow] || result.actionNow.ko;

    const routineLines = routineText.split('\n').map((s) => s.trim()).filter(Boolean);
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            <div className="text-center mb-3 bg-white rounded-2xl shadow-lg p-4 md:p-5">
              ...
              <p className="text-sm font-semibold text-teal-700 mb-3">{resultScoreBand}</p>
              <p className="text-base font-semibold text-gray-800 mb-2 text-left">{resultTag}</p>
              <p className="text-lg font-medium text-gray-800 mb-3 text-left">{resultHook}</p>
              <p className="text-base text-gray-600 leading-relaxed text-left">{resultLongDescription}</p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-sm font-bold text-gray-800 mb-2 text-left">⚙️ {t('ui.mechanism')}</h3>
                <p className="text-sm text-gray-700 leading-relaxed text-left">{mechanism}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-sm font-bold text-gray-800 mb-2 text-left">⭐ {t('ui.strength')}</h3>
                <p className="text-sm text-gray-700 leading-relaxed text-left">{strength}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-sm font-bold text-gray-800 mb-2 text-left">⚠️ {t('ui.caution')}</h3>
                <p className="text-sm text-gray-700 leading-relaxed text-left">{caution}</p>
              </div>
            </div>
            ...
            <div className="mb-3 bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl shadow-lg p-4 border border-orange-100">
              <h3 className="text-base font-bold text-gray-800 mb-2">✨ {t('ui.actionNow')}</h3>
              <p className="text-sm text-gray-800 leading-relaxed">{actionNow}</p>
            </div>
