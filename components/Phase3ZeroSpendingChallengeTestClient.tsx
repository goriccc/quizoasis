...
import {
  Phase3ZeroSpendingChallengeQuestion,
  Phase3ZeroSpendingChallengeResult,
  calculatePhase3ZeroSpendingChallengeResult,
...
interface Phase3ZeroSpendingChallengeTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3ZeroSpendingChallengeQuestion[];
  results: Phase3ZeroSpendingChallengeResult[];
...
export default function Phase3ZeroSpendingChallengeTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3ZeroSpendingChallengeTestClientProps) {
  const t = useTranslations('phase3ZeroSpendingChallengeTest');
  ...
  const [result, setResult] = useState<Phase3ZeroSpendingChallengeResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3ZeroSpendingChallengeQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3ZeroSpendingChallengeQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3ZeroSpendingChallengeResult(answersArray);
      const zsResult = results.find(r => r.type === resultType);
  ...
  if (showResult && result) {
    ...
    const resultSuccessRate = result.successRateLine[locale as keyof typeof result.successRateLine] || result.successRateLine.ko;
    const resultHook = result.hookLine[locale as keyof typeof result.hookLine] || result.hookLine.ko;
    const resultLongDescription = result.description[locale as keyof typeof result.description] || result.description.ko;
    const detailTitle = result.detailSectionTitle[locale as keyof typeof result.detailSectionTitle] || result.detailSectionTitle.ko;
    const detailBulletsText = result.detailBullets[locale as keyof typeof result.detailBullets] || result.detailBullets.ko;
    const prescriptionText = result.prescriptionSteps[locale as keyof typeof result.prescriptionSteps] || result.prescriptionSteps.ko;
    const savingsEst = result.savingsEstimate[locale as keyof typeof result.savingsEstimate] || result.savingsEstimate.ko;
    const yearlyApplyText = result.yearlyApply[locale as keyof typeof result.yearlyApply] || result.yearlyApply.ko;
    const todayTask = result.todayTask[locale as keyof typeof result.todayTask] || result.todayTask.ko;
    const motivation = result.motivationLine[locale as keyof typeof result.motivationLine] || result.motivationLine.ko;

    const detailLines = detailBulletsText.split('\n').map((s) => s.trim()).filter(Boolean);
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            <div className="text-center mb-3 bg-white rounded-2xl shadow-lg p-4 md:p-5">
              ...
              <p className="text-base font-semibold text-emerald-700 mb-2">{resultSuccessRate}</p>
              <p className="text-lg font-medium text-gray-800 mb-3">{resultHook}</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  💸 {t('ui.savingsEstimate')}
                </h3>
                <p className="text-center text-lg font-bold text-emerald-700">{savingsEst}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  📅 {t('ui.yearlyScenario')}
                </h3>
                <p className="text-center text-lg font-bold text-teal-700">{yearlyApplyText}</p>
              </div>
            </div>
            ...
            <div className="mb-3 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl shadow-lg p-4 border border-emerald-100">
              <h3 className="text-base font-bold text-gray-800 mb-2">💬 {t('ui.motivationLine')}</h3>
              <p className="text-sm text-gray-800 leading-relaxed">{motivation}</p>
            </div>
