...
import {
  Phase3ExamStudyTypeQuestion,
  Phase3ExamStudyTypeResult,
  calculatePhase3ExamStudyTypeResult,
  calculatePhase3ExamStudyTypeDomainScores,
} from '@/lib/phase3ExamStudyTypeData';
...
interface Phase3ExamStudyTypeTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3ExamStudyTypeQuestion[];
  results: Phase3ExamStudyTypeResult[];
...
export default function Phase3ExamStudyTypeTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3ExamStudyTypeTestClientProps) {
  const t = useTranslations('phase3ExamStudyTypeTest');
  ...
  const [result, setResult] = useState<Phase3ExamStudyTypeResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3ExamStudyTypeQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3ExamStudyTypeQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      const scores = calculatePhase3ExamStudyTypeDomainScores(answersArray);
      setDomainScores(scores);

      const resultType = calculatePhase3ExamStudyTypeResult(answersArray);
  ...
  const handleRetake = () => {
    ...
    setResult(null);
    setShowResult(false);
    setDomainScores({ startStyle: 0, focusEnv: 0, studyMethod: 0, slumpCoping: 0, examEvePattern: 0, resultAcceptance: 0, total: 0 });
  ...
  if (showResult && result) {
    ...
    const resultStudySummary =
      result.studySummary[locale as keyof typeof result.studySummary] || result.studySummary.ko;
    const resultStudyType =
      result.studyType[locale as keyof typeof result.studyType] || result.studyType.ko;
    const resultStudyKeywords =
      result.studyKeywords[locale as keyof typeof result.studyKeywords] || result.studyKeywords.ko;
    ...
    const resultStudyTip = result.studyTip[locale as keyof typeof result.studyTip] || result.studyTip.ko;
    const resultTypicalPhrase =
      result.typicalPhrase[locale as keyof typeof result.typicalPhrase] || result.typicalPhrase.ko;
    ...
    const domainConfig = [
      { key: 'startStyle', label: t('ui.startStyleDomain'), score: domainScores.startStyle, max: 6 },
      { key: 'focusEnv', label: t('ui.focusEnvDomain'), score: domainScores.focusEnv, max: 6 },
      { key: 'studyMethod', label: t('ui.studyMethodDomain'), score: domainScores.studyMethod, max: 6 },
      { key: 'slumpCoping', label: t('ui.slumpCopingDomain'), score: domainScores.slumpCoping, max: 6 },
      { key: 'examEvePattern', label: t('ui.examEvePatternDomain'), score: domainScores.examEvePattern, max: 6 },
      { key: 'resultAcceptance', label: t('ui.resultAcceptanceDomain'), score: domainScores.resultAcceptance, max: 6 },
    ];
                ...
                {domainConfig.map((domain) => (
                  <div key={domain.key}>
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2.5">
                      <div
                        className="bg-gradient-to-r from-slate-600 to-amber-500 h-2.5 rounded-full transition-all"
...
{isType6 && resultCertification && (
            ...
            <div className="mt-8 mb-6 px-4">
              <button
                ...
                className="w-full bg-gradient-to-r from-slate-600 to-amber-600 text-white font-bold py-4 px-6 rounded-xl hover:from-slate-700 hover:to-amber-700 transition-all shadow-md flex items-center justify-center gap-3"
              >
