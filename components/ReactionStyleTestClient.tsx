...
import { ReactionStyleQuestion, ReactionStyleResult, calculateReactionStyleResult } from '@/lib/reactionStyleData';
...
import { searchAliExpressProducts, getProductKeywordsForDating, getPopularProducts } from '@/lib/aliexpress';
import ProductRecommendations from './ProductRecommendations';
import AdSensePlaceholder, { ADSENSE_CONFIG, safeLoadAdSense } from '@/lib/adsense';

interface ReactionStyleTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: ReactionStyleQuestion[];
  results: ReactionStyleResult[];
...
export default function ReactionStyleTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  playCount = 0,
  similarTests = []
}: ReactionStyleTestClientProps) {
  const t = useTranslations();
  const [started, setStarted] = useState(false);
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [answers, setAnswers] = useState<any[]>([]);
  const [result, setResult] = useState<ReactionStyleResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<ReactionStyleQuestion[]>(questions);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: ReactionStyleQuestion[]) => {
  ...
  const handleAnswer = (scores: any) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculateReactionStyleResult(newAnswers);
      const reactionStyleResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (reactionStyleResult) {
        setResult(reactionStyleResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (reactionStyleResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(reactionStyleResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 반응 스타일 결과:', reactionStyleResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  const calculateResult = (finalAnswers: any[]) => {
    const resultType = calculateReactionStyleResult(finalAnswers);
    const reactionStyleResult = results.find(r => r.type === resultType);
  ...
  const handleShareResult = async () => {
    ...
    const shareText = t('reactionStyleTest.shareMessages.default', { type: resultTitle }) + `\n\n${`https://myquizoasis.com${window.location.pathname}`}`;
    ...
    if (navigator.share) {
      ...
      try {
        ...
        alert(t('reactionStyleTest.alerts.resultCopied'));
      } catch (error) {
        console.error('클립보드 복사 실패:', error);
        alert(t('reactionStyleTest.alerts.shareFailed'));
      }
    }
  };
  ...
  const shareToWeChat = async () => {
    ...
    const shareText = result 
      ? t('reactionStyleTest.shareMessages.wechat', { type: resultTitle }) + `\n\n${url}`
      : `${title}\n\n${url}`;
    ...
    try {
      ...
      alert(t('reactionStyleTest.alerts.wechatCopy'));
    } catch (error) {
      alert(t('reactionStyleTest.alerts.shareFailed'));
    }
  };
  ...
  const shareToWhatsApp = () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareText = result 
      ? encodeURIComponent(t('reactionStyleTest.shareMessages.whatsapp', { type: resultTitle }))
  ...
  const shareToKakao = () => {
    ...
    if (!window.Kakao || !window.Kakao.isInitialized()) {
      alert(t('reactionStyleTest.alerts.kakaoInit'));
      return;
    }
    ...
    const shareDescription = result 
      ? t('reactionStyleTest.shareMessages.kakao', { type: resultTitle })
      : description;
    ...
    try {
      window.Kakao.Share.sendDefault({
        ...
        buttons: [
          {
            title: t('reactionStyleTest.ui.startTest'),
            link: {
              mobileWebUrl: currentUrl,
              webUrl: currentUrl,
            },
      ...
      alert(t('reactionStyleTest.alerts.kakaoError'));
    }
  };
  ...
  const shareToTelegram = () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareText = result 
      ? t('reactionStyleTest.shareMessages.telegram', { type: resultTitle })
      : title;
  ...
  const copyLink = () => {
    ...
    alert(t('reactionStyleTest.alerts.linkCopied'));
  };
      ...
      <div className="min-h-screen bg-white">
        <div className="max-w-4xl mx-auto">
          <div className="relative w-full overflow-hidden mb-3" style={{ aspectRatio: '680/384' }}>
            <Image
              src={getThumbnailUrl(thumbnail || 'test_225_reaction_style.jpg')}
              alt={title}
              fill
          ...
          <div className="px-4">
            ...
            <div className="text-gray-600 mb-6 leading-relaxed text-center space-y-4">
              <p className="font-bold text-gray-700">{t('reactionStyleTest.startMessage.line1')}</p>
              <p>{t('reactionStyleTest.startMessage.line2')}</p>
              <p>{t('reactionStyleTest.startMessage.line3')}</p>
              <p>{t('reactionStyleTest.startMessage.line4')}</p>
              <p className="whitespace-pre-line">{t('reactionStyleTest.startMessage.line5')}</p>
            </div>
            ...
            <div className="flex justify-center mb-4">
              ...
              >
                {t('reactionStyleTest.ui.startTest')}
              </button>
            </div>

            <p className="text-sm font-bold text-center mb-6" style={{ color: '#669df6' }}>
              {t('reactionStyleTest.ui.totalParticipants', { count: formatPlayCount(displayPlayCount, locale as Locale) })}
            </p>
        ...
        <div className="bg-white rounded-2xl p-8 max-w-md w-full mx-4 text-center shadow-2xl">
          ...
          <div className="mb-6">
            {locale === 'ko' ? (
              ...
              <div className="max-w-sm mx-auto">
                <ProductRecommendations 
                  products={getPopularProducts().slice(0, 3)}
                  title=""
                  locale={locale}
                />
              </div>
          ...
          <div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                🎯 {t('reactionStyleTest.result.reactionFeatures')}
              </h3>
              <div className="flex flex-wrap gap-2">
                {resultReactionFeatures.split(/[, 、，]/)
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                💡 {t('reactionStyleTest.result.advice')}
              </h3>
        ...
        <div className="mb-8">
          <div className="flex justify-between items-center mb-2">
            <span className="text-sm font-medium text-gray-600">
              {t('reactionStyleTest.ui.progress')}
            </span>
