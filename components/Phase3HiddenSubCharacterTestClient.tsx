...
import {
  Phase3HiddenSubCharacterQuestion,
  Phase3HiddenSubCharacterResult,
  calculatePhase3HiddenSubCharacterResult,
...
interface Phase3HiddenSubCharacterTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3HiddenSubCharacterQuestion[];
  results: Phase3HiddenSubCharacterResult[];
...
export default function Phase3HiddenSubCharacterTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3HiddenSubCharacterTestClientProps) {
  const t = useTranslations('phase3HiddenSubCharacterTest');
  ...
  const [result, setResult] = useState<Phase3HiddenSubCharacterResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3HiddenSubCharacterQuestion[]>([]);
  ...
  useEffect(() => {
    if (result && locale !== 'ko') {
      const loadProducts = async () => {
        try {
          const products = await searchAliExpressProducts('trending lifestyle gifts', 4, locale);
          setAliProducts(products);
        } catch (error) {
          console.error('상품 로드 실패:', error);
        }
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3HiddenSubCharacterQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3HiddenSubCharacterResult(answersArray);
      const subResult = results.find(r => r.type === resultType);
      ...
      if (subResult && locale !== 'ko') {
        const loadStartTime = Date.now();
        console.log('🎭 [시작] 부캐 찾기 결과:', subResult.type, '→ 추천 상품 검색');
        searchAliExpressProducts('trending lifestyle gifts', 4, locale)
          .then(products => {
  ...
  if (showResult && result) {
    ...
    const subSkillText = result.subSkill[locale as keyof typeof result.subSkill] || result.subSkill.ko;
    ...
    const triggerRateText = result.triggerRate[locale as keyof typeof result.triggerRate] || result.triggerRate.ko;
    const mainSkillsRaw = result.mainSkills[locale as keyof typeof result.mainSkills] || result.mainSkills.ko;
    const disappearConditionText =
      result.disappearCondition[locale as keyof typeof result.disappearCondition] || result.disappearCondition.ko;
    const gapLevelText = result.gapLevel[locale as keyof typeof result.gapLevel] || result.gapLevel.ko;
    const oneLinerText = result.oneLiner[locale as keyof typeof result.oneLiner] || result.oneLiner.ko;
    const mainSkillChips = mainSkillsRaw.split(/[,，、]/).map((s) => s.trim()).filter(Boolean);
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            ...
            <div className="space-y-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4 border-2 border-fuchsia-100">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  ✨ {t('ui.subSkill')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">{subSkillText}</p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="bg-white rounded-xl shadow-lg p-4">
                  ...
                  <p className="text-sm text-gray-700 leading-relaxed">{triggerConditionText}</p>
                </div>
                <div className="bg-white rounded-xl shadow-lg p-4">
                  <h3 className="text-base font-bold text-gray-800 mb-2">
                    📊 {t('ui.triggerRate')}
                  </h3>
                  <p className="text-sm text-gray-700 leading-relaxed">{triggerRateText}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="bg-white rounded-xl shadow-lg p-4">
                  <h3 className="text-base font-bold text-gray-800 mb-2">
                    🫥 {t('ui.disappearCondition')}
                  </h3>
                  <p className="text-sm text-gray-700 leading-relaxed">{disappearConditionText}</p>
                </div>
                <div className="bg-white rounded-xl shadow-lg p-4">
                  <h3 className="text-base font-bold text-gray-800 mb-2">
                    ⭐ {t('ui.gapLevel')}
                  </h3>
                  <p className="text-sm text-gray-700 leading-relaxed">{gapLevelText}</p>
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl shadow-lg p-4 border-2 border-amber-100">
                ...
                <p className="text-base text-amber-900 font-medium italic leading-relaxed">{oneLinerText}</p>
              </div>
            </div>
