...
import {
  Phase3TfIndexPreciseMeasurementQuestion,
  Phase3TfIndexPreciseMeasurementResult,
  calculatePhase3TfIndexPreciseMeasurementResult,
  getFtPercentFromRawScore,
...
interface Phase3TfIndexPreciseMeasurementTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3TfIndexPreciseMeasurementQuestion[];
  results: Phase3TfIndexPreciseMeasurementResult[];
...
export default function Phase3TfIndexPreciseMeasurementTestClient({
  locale,
  slug,
  title,
  description,
  ...
  isLatestTest = false,
  badgeType = null,
}: Phase3TfIndexPreciseMeasurementTestClientProps) {
  const t = useTranslations('phase3TfIndexPreciseMeasurementTest');
  ...
  const [result, setResult] = useState<Phase3TfIndexPreciseMeasurementResult | null>(null);
  const [totalRawScore, setTotalRawScore] = useState(0);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3TfIndexPreciseMeasurementQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3TfIndexPreciseMeasurementQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      const resultType = calculatePhase3TfIndexPreciseMeasurementResult(answersArray);
      const tfResult = results.find(r => r.type === resultType);

      if (tfResult) {
        setResult(tfResult);
      }

      if (tfResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(tfResult.type, locale);
  ...
  const formatShareLine = (r: Phase3TfIndexPreciseMeasurementResult) => {
    const ft = getFtPercentFromRawScore(totalRawScore);
    const line = r.shareLine[locale as keyof typeof r.shareLine] || r.shareLine.ko;
    return line.replace(/\{fPercent\}/g, String(ft.f));
  };
