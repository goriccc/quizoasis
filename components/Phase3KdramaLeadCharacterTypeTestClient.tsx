...
import {
  Phase3KdramaLeadCharacterTypeQuestion,
  Phase3KdramaLeadCharacterTypeResult,
  calculatePhase3KdramaLeadCharacterTypeResult,
...
interface Phase3KdramaLeadCharacterTypeTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3KdramaLeadCharacterTypeQuestion[];
  results: Phase3KdramaLeadCharacterTypeResult[];
...
export default function Phase3KdramaLeadCharacterTypeTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3KdramaLeadCharacterTypeTestClientProps) {
  const t = useTranslations('phase3KdramaLeadCharacterTypeTest');
  ...
  const [result, setResult] = useState<Phase3KdramaLeadCharacterTypeResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3KdramaLeadCharacterTypeQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3KdramaLeadCharacterTypeQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3KdramaLeadCharacterTypeResult(answersArray);
      const kdramaResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (kdramaResult) {
        setResult(kdramaResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (kdramaResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(kdramaResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] K-드라마 주인공 재질 결과:', kdramaResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const resultDramaClimax = result.dramaClimax[locale as keyof typeof result.dramaClimax] || result.dramaClimax.ko;
    const resultDramaSupportingRole =
      result.dramaSupportingRole[locale as keyof typeof result.dramaSupportingRole] || result.dramaSupportingRole.ko;
    const resultDramaOst = result.dramaOst[locale as keyof typeof result.dramaOst] || result.dramaOst.ko;
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  👥 {t('ui.dramaSupportingRole')}
                </h3>
                <p className="text-sm text-gray-700 text-center leading-relaxed">
                  {resultDramaSupportingRole}
                </p>
              </div>
