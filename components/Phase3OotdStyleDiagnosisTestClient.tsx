...
import {
  Phase3OotdStyleDiagnosisQuestion,
  Phase3OotdStyleDiagnosisResult,
  calculatePhase3OotdStyleDiagnosisResult,
...
interface Phase3OotdStyleDiagnosisTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3OotdStyleDiagnosisQuestion[];
  results: Phase3OotdStyleDiagnosisResult[];
...
export default function Phase3OotdStyleDiagnosisTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3OotdStyleDiagnosisTestClientProps) {
  const t = useTranslations('phase3OotdStyleDiagnosisTest');
  ...
  const [result, setResult] = useState<Phase3OotdStyleDiagnosisResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3OotdStyleDiagnosisQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3OotdStyleDiagnosisQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3OotdStyleDiagnosisResult(answersArray);
  ...
  if (showResult && result) {
    ...
    const resultFashionIdentity =
      result.fashionIdentity[locale as keyof typeof result.fashionIdentity] || result.fashionIdentity.ko;
    const resultSignatureItems =
      result.signatureItems[locale as keyof typeof result.signatureItems] || result.signatureItems.ko;
    const resultColorPalette =
      result.colorPalette[locale as keyof typeof result.colorPalette] || result.colorPalette.ko;
    const resultOotdDirection =
      result.ootdDirection[locale as keyof typeof result.ootdDirection] || result.ootdDirection.ko;
    ...
    /** 시그니처: KO/EN 등 `/`, 일본어 `／` */
    const signatureChips = resultSignatureItems
      .split(/\s*[/／]\s*/)
      .map((s) => s.trim())
      .filter(Boolean);
    /** 팔레트: 중점 `·`(U+00B7) · カタカナ中黒 `・`(U+30FB) */
    const hasPaletteSeparators = /[·・]/.test(resultColorPalette);
    const paletteChips = hasPaletteSeparators
      ? resultColorPalette
          .split(/[·・]/)
          .map((s) => s.trim())
          .filter(Boolean)
          ...
          <div>
              ...
              <div className="bg-white rounded-xl shadow-lg p-5">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {paletteChips.length > 0
                    ...
                    : (
                      <p className="text-sm text-gray-700 leading-relaxed text-center w-full">{resultColorPalette}</p>
                    )}
                </div>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed text-center">{resultOotdDirection}</p>
              </div>
            </div>
