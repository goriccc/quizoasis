...
import {
  Phase3InstaFeedPersonaAnalysisQuestion,
  Phase3InstaFeedPersonaAnalysisResult,
  calculatePhase3InstaFeedPersonaAnalysisResult,
...
interface Phase3InstaFeedPersonaAnalysisTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3InstaFeedPersonaAnalysisQuestion[];
  results: Phase3InstaFeedPersonaAnalysisResult[];
...
export default function Phase3InstaFeedPersonaAnalysisTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3InstaFeedPersonaAnalysisTestClientProps) {
  const t = useTranslations('phase3InstaFeedPersonaAnalysisTest');
  ...
  const [result, setResult] = useState<Phase3InstaFeedPersonaAnalysisResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3InstaFeedPersonaAnalysisQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3InstaFeedPersonaAnalysisQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3InstaFeedPersonaAnalysisResult(answersArray);
      const instaResult = results.find(r => r.type === resultType);

      if (instaResult) {
        setResult(instaResult);
      }
    }
  };
  ...
  if (showResult && result) {
    ...
    const resultInstaPersona =
      result.instaPersona[locale as keyof typeof result.instaPersona] || result.instaPersona.ko;
    const resultUploadPattern =
      result.uploadPattern[locale as keyof typeof result.uploadPattern] || result.uploadPattern.ko;
    const resultMainContent =
      result.mainContent[locale as keyof typeof result.mainContent] || result.mainContent.ko;
    const resultMostUsedFeature =
      result.mostUsedFeature[locale as keyof typeof result.mostUsedFeature] || result.mostUsedFeature.ko;
    const resultInstaGoal =
      result.instaGoal[locale as keyof typeof result.instaGoal] || result.instaGoal.ko;
    ...
    const resultInfluencerGrowthTip =
      result.influencerGrowthTip[locale as keyof typeof result.influencerGrowthTip] ||
      result.influencerGrowthTip.ko;
    const comboPartner = results.find((r) => r.type === result.personaComboPartnerType);
    const comboPartnerPersona = comboPartner
      ? comboPartner.instaPersona[locale as keyof typeof comboPartner.instaPersona] ||
        comboPartner.instaPersona.ko
      : '';
    const comboDescription =
      result.personaCombo[locale as keyof typeof result.personaCombo] || result.personaCombo.ko;
    const resultPersonaCombo = comboPartnerPersona
      ? `${comboPartnerPersona} ${comboDescription}`
      : comboDescription;
    const resultOneLineReview =
      result.oneLineReview[locale as keyof typeof result.oneLineReview] || result.oneLineReview.ko;
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultUploadPattern}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  {t('ui.mainContent')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">{resultMainContent}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  {t('ui.mostUsedFeature')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">{resultMostUsedFeature}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  {t('ui.instaGoal')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">{resultInstaGoal}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultStrength}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultCaution}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  {t('ui.influencerGrowthTip')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">{resultInfluencerGrowthTip}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultPersonaCombo}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed text-center italic">
                  {result.emoji} {resultOneLineReview}
                </p>
              </div>
            </div>
