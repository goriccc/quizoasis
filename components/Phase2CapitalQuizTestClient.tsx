...
import { Phase2CapitalQuizQuestion, Phase2CapitalQuizResult, calculatePhase2CapitalQuizResult } from '@/lib/phase2_capital_quiz_data';
...
interface Phase2CapitalQuizTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase2CapitalQuizQuestion[];
  results: Phase2CapitalQuizResult[];
...
export default function Phase2CapitalQuizTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase2CapitalQuizTestClientProps) {
  const t = useTranslations('phase2CapitalQuizTest');
  ...
  const [result, setResult] = useState<Phase2CapitalQuizResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2CapitalQuizQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase2CapitalQuizQuestion[]) => {
  ...
  const handleAnswer = (selectedOptionIndex: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산 (정답 체크)
      const resultType = calculatePhase2CapitalQuizResult(newAnswers, questions);
      const capitalQuizResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (capitalQuizResult) {
        setResult(capitalQuizResult);
      }
  ...
  if (showResult && result) {
    ...
    const resultKnowledgeLevel = result.knowledgeLevel[locale as keyof typeof result.knowledgeLevel] || result.knowledgeLevel.ko;
