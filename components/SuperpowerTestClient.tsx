...
import { SuperpowerQuestion, SuperpowerResult, calculateSuperpowerResult } from '@/lib/superpowerData';
...
interface SuperpowerTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: SuperpowerQuestion[];
...
export default function SuperpowerTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false
}: SuperpowerTestClientProps) {
  const t = useTranslations('superpowerTest');
  const tGlobal = useTranslations(); // 글로벌 번역 (mbti 등)
  const [started, setStarted] = useState(false);
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [answers, setAnswers] = useState<Record<number, number[]>>({}); // 원래 질문 인덱스를 키로 사용, 값은 typePoints 배열
  const [result, setResult] = useState<SuperpowerResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<SuperpowerQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: SuperpowerQuestion[]) => {
  ...
  // 답변 처리
  const handleAnswer = (typePoints: number[]) => {
    // 현재 질문의 원래 인덱스를 찾아서 답변 저장
    const currentOriginalIndex = originalQuestionIndices[currentQuestion];
    const newAnswers = { ...answers, [currentOriginalIndex]: typePoints };
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 원래 질문 순서대로 typePoints 배열 재구성 (newAnswers에는 모든 답변이 포함됨)
      const answersArray = questions.map((_, idx) => newAnswers[idx] ?? [0, 0, 0, 0, 0, 0]);
      
      // 결과 계산
      const resultType = calculateSuperpowerResult(answersArray);
      const superpowerResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (superpowerResult) {
        setResult(superpowerResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (superpowerResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(superpowerResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 초능력 결과:', superpowerResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const resultMainAbility = result.mainAbility[locale as keyof typeof result.mainAbility] || result.mainAbility.ko;
    const resultPersonalityKeywords = result.personalityKeywords[locale as keyof typeof result.personalityKeywords] || result.personalityKeywords.ko;
    const resultUsage = result.usage[locale as keyof typeof result.usage] || result.usage.ko;
          ...
          <div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <div className="flex flex-wrap justify-center gap-2">
                {resultMainAbility.split(/[,、，]/).map((ability, index) => (
                  <span
                    key={index}
                    className="inline-block bg-gradient-to-r from-yellow-100 to-orange-100 text-orange-800 px-4 py-2 rounded-full text-sm font-medium"
                  >
                    {ability.trim()}
                  </span>
                ))}
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <div className="flex flex-wrap justify-center gap-2">
                {resultPersonalityKeywords.split(/[,、，]/).map((keyword, index) => (
                  <span
                    key={index}
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <div className="flex flex-wrap justify-center gap-2">
                {resultUsage.split(/[,、，]/).map((usage, index) => (
                  <span
                    key={index}
                    className="inline-block bg-gradient-to-r from-blue-100 to-cyan-100 text-blue-800 px-4 py-2 rounded-full text-sm font-medium"
                  >
                    {usage.trim()}
                  </span>
                ))}
          ...
          <div className="space-y-4 px-4">
            {optionsArray.map((option, index) => {
              ...
              return (
                <button
                  key={index}
                  onClick={() => handleAnswer(option.typePoints)}
