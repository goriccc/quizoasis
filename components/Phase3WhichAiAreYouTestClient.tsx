...
import {
  Phase3WhichAiAreYouQuestion,
  Phase3WhichAiAreYouResult,
  calculatePhase3WhichAiAreYouResult,
...
interface Phase3WhichAiAreYouTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3WhichAiAreYouQuestion[];
  results: Phase3WhichAiAreYouResult[];
...
export default function Phase3WhichAiAreYouTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3WhichAiAreYouTestClientProps) {
  const t = useTranslations('phase3WhichAiAreYouTest');
  ...
  const [result, setResult] = useState<Phase3WhichAiAreYouResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3WhichAiAreYouQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3WhichAiAreYouQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3WhichAiAreYouResult(answersArray);
      const whichAiResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (whichAiResult) {
        setResult(whichAiResult);
      }
    }
  };
  ...
  const handleShareResult = async () => {
    ...
    const resultShareName = result.aiShareName[locale as keyof typeof result.aiShareName] || result.aiShareName.ko;
  ...
  const shareToLine = () => {
    ...
    const resultShareName = result ? (result.aiShareName[locale as keyof typeof result.aiShareName] || result.aiShareName.ko) : '';
  ...
  if (showResult && result) {
    ...
    const resultAiSubtype = result.aiSubtype[locale as keyof typeof result.aiSubtype] || result.aiSubtype.ko;
    const resultCoreStrength = result.coreStrength[locale as keyof typeof result.coreStrength] || result.coreStrength.ko;
          ...
          <div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-5">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {resultCoreStrength.split(/[,、，]/).map((char, index) => (
                    <span
                      key={index}
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed text-center">{resultCaution}</p>
              </div>
            </div>
