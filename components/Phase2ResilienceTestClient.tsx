...
import { Phase2ResilienceQuestion, Phase2ResilienceResult, calculatePhase2ResilienceResult } from '@/lib/phase2_resilience_data';
...
interface Phase2ResilienceTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase2ResilienceQuestion[];
  results: Phase2ResilienceResult[];
...
export default function Phase2ResilienceTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase2ResilienceTestClientProps) {
  const t = useTranslations('phase2ResilienceTest');
  ...
  const [result, setResult] = useState<Phase2ResilienceResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2ResilienceQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase2ResilienceQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase2ResilienceResult(answersArray);
      const resilienceResult = results.find(r => r.type === resultType);
      ...
      if (resilienceResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(resilienceResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 회복탄력성 결과:', resilienceResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const resultResilienceScore = result.resilienceScore[locale as keyof typeof result.resilienceScore] || result.resilienceScore.ko;
