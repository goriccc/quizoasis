...
import { FlirtingStyleQuestion, FlirtingStyleResult, calculateFlirtingStyleResult } from '@/lib/flirtingStyleData';
...
interface FlirtingStyleTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: FlirtingStyleQuestion[];
  results: FlirtingStyleResult[];
...
export default function FlirtingStyleTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  playCount = 0,
  similarTests = []
}: FlirtingStyleTestClientProps) {
  const t = useTranslations('flirtingStyleTest');
  const tGlobal = useTranslations(); // 글로벌 번역 (mbti 등)
  const [started, setStarted] = useState(false);
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [answers, setAnswers] = useState<Record<number, string>>({}); // 원래 질문 인덱스를 키로 사용, types 저장 (예: "Type1,Type4")
  const [result, setResult] = useState<FlirtingStyleResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<FlirtingStyleQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: FlirtingStyleQuestion[]) => {
  ...
  const handleAnswer = (types: string[]) => {
    // ⚠️ 중요: 셔플링된 질문 순서와 관계없이 원래 질문 인덱스를 사용하여 답변 저장
    // currentQuestion은 셔플링된 질문의 인덱스이고,
    // originalQuestionIndices[currentQuestion]은 해당 질문의 원래 인덱스입니다.
    const currentOriginalIndex = originalQuestionIndices[currentQuestion];
    const typesString = types.join(',');
    // answers 객체는 원래 질문 인덱스를 키로 사용하므로, 셔플링과 무관하게 올바른 순서로 저장됩니다.
    const newAnswers = { ...answers, [currentOriginalIndex]: typesString };
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // ⚠️ 중요: 결과 계산 전에 원래 질문 순서대로 답변 배열 재구성
      // newAnswers는 원래 질문 인덱스를 키로 사용하므로, questions.map을 사용하면
      // 원래 순서대로 답변 배열을 만들 수 있습니다.
      // 이렇게 하면 셔플링과 관계없이 올바른 계산이 가능합니다.
      const answersArray = questions.map((_, idx) => newAnswers[idx] ?? '');
      
      // 결과 계산 (원래 질문 순서대로 재구성된 answersArray 사용)
      const resultType = calculateFlirtingStyleResult(answersArray);
      const flirtingStyleResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (flirtingStyleResult) {
        setResult(flirtingStyleResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (flirtingStyleResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(flirtingStyleResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 플러팅 스타일 결과:', flirtingStyleResult.type, '→ 검색 키워드:', keywords[0]);
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="px-4">
            ...
            <div className="text-gray-600 mb-6 leading-relaxed text-center space-y-4">
              ...
              <p className="whitespace-pre-line font-semibold text-purple-600">{t('startMessage.line6')}</p>
            </div>
  ...
  if (showResult && result) {
    ...
    const resultFlirtingTechnique = result.flirtingTechnique[locale as keyof typeof result.flirtingTechnique] || result.flirtingTechnique.ko;
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-center">
                  💋 {t('ui.flirtingTechnique')}
                </h3>
                <div className="flex flex-wrap justify-center gap-2">
                  {resultFlirtingTechnique.split(/[,、，]/).map((tech, index) => (
                    <span
                      key={index}
                      className="inline-block bg-gradient-to-r from-pink-100 to-purple-100 text-pink-800 px-3 py-1.5 rounded-full text-sm font-medium"
                    >
                      {tech.trim()}
                    </span>
                  ))}
