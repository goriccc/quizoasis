...
import { Phase2MentalAgeQuestion, Phase2MentalAgeResult, calculatePhase2MentalAgeResult } from '@/lib/phase2MentalAgeData';
...
interface Phase2MentalAgeTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase2MentalAgeQuestion[];
  results: Phase2MentalAgeResult[];
...
export default function Phase2MentalAgeTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase2MentalAgeTestClientProps) {
  const t = useTranslations('phase2MentalAgeTest');
  ...
  const [result, setResult] = useState<Phase2MentalAgeResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2MentalAgeQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase2MentalAgeQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase2MentalAgeResult(answersArray);
      const mentalAgeResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (mentalAgeResult) {
        setResult(mentalAgeResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (mentalAgeResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(mentalAgeResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 정신 연령 결과:', mentalAgeResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  // Helper to extract numeric/core level string for sharing
  const getLevelForShare = (levelText: string) => {
    // "5세 (응석받이)" -> "5세" 추출 시도
    const match = levelText.split('(')[0].trim();
    return match || levelText;
  };
  ...
  const handleShareResult = async () => {
    ...
    const resultLevel = result.mentalAgeLevel[locale as keyof typeof result.mentalAgeLevel] || result.mentalAgeLevel.ko;
    const levelForShare = getLevelForShare(resultLevel);
    const shareText = `${t('shareMessages.default', { level: levelForShare, type: resultTitle })}\n\n${`https://myquizoasis.com${window.location.pathname}`}`;
  ...
  const shareToLine = () => {
    ...
    const resultLevel = result ? (result.mentalAgeLevel[locale as keyof typeof result.mentalAgeLevel] || result.mentalAgeLevel.ko) : '';
    const levelForShare = result ? getLevelForShare(resultLevel) : '';
    const shareText = result 
      ? encodeURIComponent(t('shareMessages.line', { level: levelForShare, type: resultTitle }))
  ...
  const shareToWeChat = async () => {
    ...
    const levelForShare = result ? getLevelForShare(resultLevel) : '';
    const shareText = result 
      ? `${t('shareMessages.wechat', { level: levelForShare, type: resultTitle })}\n\n${url}`
  ...
  const shareToWhatsApp = () => {
    ...
    const levelForShare = result ? getLevelForShare(resultLevel) : '';
    const shareText = result 
      ? encodeURIComponent(t('shareMessages.whatsapp', { level: levelForShare, type: resultTitle }))
  ...
  const shareToKakao = () => {
    ...
    const shareDescription = result 
      ? t('shareMessages.kakao', { level: levelForShare, type: resultTitle })
  ...
  const shareToTelegram = () => {
    ...
    const levelForShare = result ? getLevelForShare(resultLevel) : '';
    const shareText = result 
      ? t('shareMessages.telegram', { level: levelForShare, type: resultTitle })
  ...
  if (showResult && result) {
    ...
    const resultMentalAgeLevel = result.mentalAgeLevel[locale as keyof typeof result.mentalAgeLevel] || result.mentalAgeLevel.ko;
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="space-y-2">
                  {(() => {
                    // "Type 5", "タイプ5", "类型5", "類型5", "Loại 5", "Tipe 5" 등 모든 언어 형식에서 숫자 추출
                    const matchType = resultGoodMatch.match(/(?:Type|タイプ|类型|類型|Loại|Tipe)\s*(\d+)/i);
                    if (matchType) {
