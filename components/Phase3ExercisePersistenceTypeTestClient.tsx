...
import {
  Phase3ExercisePersistenceTypeQuestion,
  Phase3ExercisePersistenceTypeResult,
  calculatePhase3ExercisePersistenceTypeResult,
...
interface Phase3ExercisePersistenceTypeTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3ExercisePersistenceTypeQuestion[];
  results: Phase3ExercisePersistenceTypeResult[];
...
export default function Phase3ExercisePersistenceTypeTestClient({
  locale,
  slug,
  title,
  description,
  ...
  isLatestTest = false,
  badgeType = null,
}: Phase3ExercisePersistenceTypeTestClientProps) {
  const t = useTranslations('phase3ExercisePersistenceTypeTest');
  ...
  const [result, setResult] = useState<Phase3ExercisePersistenceTypeResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3ExercisePersistenceTypeQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3ExercisePersistenceTypeQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3ExercisePersistenceTypeResult(answersArray);
      const exerciseResult = results.find(r => r.type === resultType);
  ...
  if (showResult && result) {
    ...
    const resultObstacleType =
      result.obstacleType[locale as keyof typeof result.obstacleType] || result.obstacleType.ko;
    const resultObstacleKeywords =
      result.obstacleKeywords[locale as keyof typeof result.obstacleKeywords] || result.obstacleKeywords.ko;
    const resultQuitMoment =
      result.quitMoment[locale as keyof typeof result.quitMoment] || result.quitMoment.ko;
    const resultOvercomeStrategy =
      result.overcomeStrategy[locale as keyof typeof result.overcomeStrategy] || result.overcomeStrategy.ko;
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left w-full">
                  {t('ui.obstacleKeywords')}
                </h3>
                <div className="flex flex-wrap justify-center gap-2">
                  {resultObstacleKeywords.split(/[·、，]/).map((keyword, idx) => (
                    <span
                      key={idx}
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed text-center">{resultQuitMoment}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left w-full">
                  {t('ui.overcomeStrategy')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-line">{resultOvercomeStrategy}</p>
              </div>
            </div>
