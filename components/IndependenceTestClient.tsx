...
interface IndependenceTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: IndependenceQuestion[];
  results: IndependenceResult[];
...
export default function IndependenceTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  playCount = 0,
  similarTests = []
}: IndependenceTestClientProps) {
  const t = useTranslations('independenceTest');
  ...
  const [result, setResult] = useState<IndependenceResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<IndependenceQuestion[]>(questions);
  ...
  // 질문 섞기
  const shuffleQuestions = (questionList: IndependenceQuestion[]) => {
  ...
  const handleAnswer = (answer: string) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const calculatedResult = calculateIndependenceResult(newAnswers);
      ...
      <div className="min-h-screen bg-white">
        <div className="max-w-4xl mx-auto">
          <div className="relative w-full overflow-hidden mb-3" style={{ aspectRatio: '680/384' }}>
            <Image
              src={getThumbnailUrl(thumbnail || 'test_209_independence_vs_dependence.jpg')}
              alt={title}
              fill
  ...
  if (showResult && result) {
    ...
    const resultScore = result.independenceScore[mappedLocale as keyof typeof result.independenceScore] || result.independenceScore.ko;
