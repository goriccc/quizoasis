...
import {
  Phase3PersonalityShoeRecommendationQuestion,
  Phase3PersonalityShoeRecommendationResult,
  calculatePhase3PersonalityShoeRecommendationResult,
...
interface Phase3PersonalityShoeRecommendationTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3PersonalityShoeRecommendationQuestion[];
  results: Phase3PersonalityShoeRecommendationResult[];
...
export default function Phase3PersonalityShoeRecommendationTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3PersonalityShoeRecommendationTestClientProps) {
  const t = useTranslations('phase3PersonalityShoeRecommendationTest');
  ...
  const [result, setResult] = useState<Phase3PersonalityShoeRecommendationResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3PersonalityShoeRecommendationQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3PersonalityShoeRecommendationQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3PersonalityShoeRecommendationResult(answersArray);
      const matched = results.find(r => r.type === resultType);
  ...
  if (showResult && result) {
    ...
    const resultShoeType =
      result.shoeTypeRecommended[locale as keyof typeof result.shoeTypeRecommended] ||
      result.shoeTypeRecommended.ko;
    const resultModelKeywords =
      result.modelKeywords[locale as keyof typeof result.modelKeywords] || result.modelKeywords.ko;
    const resultPersonalityLink =
      result.personalityLink[locale as keyof typeof result.personalityLink] || result.personalityLink.ko;
    const resultColorRec =
      result.colorRec[locale as keyof typeof result.colorRec] || result.colorRec.ko;
    const resultOutfitDirection =
      result.outfitDirection[locale as keyof typeof result.outfitDirection] || result.outfitDirection.ko;
    ...
    const commaOrDotChips = (s: string) =>
      s
        .split(/[,，、]/)
        .flatMap((part) => (part.includes('·') || part.includes('・') ? part.split(/[·・]/) : [part]))
        .map((x) => x.trim())
        .filter(Boolean);
    const modelChips = commaOrDotChips(resultModelKeywords);
    const personalityChips = commaOrDotChips(resultPersonalityLink);
    const colorChips = commaOrDotChips(resultColorRec);
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  👟 {t('ui.recommendedShoeType')}
                </h3>
                <p className="text-lg font-bold text-purple-600 text-center leading-snug" style={{ fontSize: '1.25em' }}>
                  {resultShoeType}
                </p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-5">
                <h3 className="text-base font-bold text-gray-800 mb-4 text-left">
                  🏷️ {t('ui.modelKeywords')}
                </h3>
                <div className="flex flex-wrap justify-center gap-2">
                  {modelChips.map((part, index) => (
                    <span
                      key={index}
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-5">
                <h3 className="text-base font-bold text-gray-800 mb-4 text-left">
                  🔗 {t('ui.personalityLink')}
                </h3>
                <div className="flex flex-wrap justify-center gap-2">
                  {personalityChips.map((part, index) => (
                    <span
                      key={index}
                      className="inline-block bg-gradient-to-r from-sky-50 to-indigo-50 text-gray-800 px-3 py-1.5 rounded-full text-sm font-medium"
                    >
                      {part}
                    </span>
                  ))}
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-5">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {colorChips.map((part, index) => (
                    <span
                      key={index}
                      className="inline-block bg-gradient-to-r from-amber-50 to-rose-50 text-gray-800 px-3 py-1.5 rounded-full text-sm font-medium"
                    >
                      {part}
                    </span>
                  ))}
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  📌 {t('ui.outfitDirection')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed text-center">{resultOutfitDirection}</p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-2 sm:gap-4 px-2 sm:px-4">
            {optionsArray.map((option, index) => {
              ...
              const colors = [
                ...
                'from-sky-50 to-blue-100 hover:from-sky-100 hover:to-blue-200 border-sky-200 hover:border-sky-400',
                'from-amber-50 to-orange-100 hover:from-amber-100 hover:to-orange-200 border-amber-200 hover:border-amber-400',
              ];
              const bgColors = ['bg-purple-600', 'bg-pink-600', 'bg-sky-600', 'bg-amber-600'];
              ...
              return (
                <button
                  ...
                  className={`min-w-0 w-full bg-gradient-to-br ${colors[colorIdx]} border-2 rounded-xl overflow-hidden transition-all transform hover:scale-[1.02] text-left shadow-sm hover:shadow-md`}
                >
                  <div className="relative w-full aspect-[4/3] bg-gray-100">
                    ...
                    />
                    <div
                      className={`absolute top-2 left-2 w-8 h-8 ${bgColors[colorIdx]} text-white rounded-full flex items-center justify-center font-bold text-sm shadow`}
                    >
                      {label}
                    </div>
                  </div>
