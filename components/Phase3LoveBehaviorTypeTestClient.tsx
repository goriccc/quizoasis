...
import {
  Phase3LoveBehaviorTypeQuestion,
  Phase3LoveBehaviorTypeResult,
  calculatePhase3LoveBehaviorTypeResult,
...
interface Phase3LoveBehaviorTypeTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3LoveBehaviorTypeQuestion[];
  results: Phase3LoveBehaviorTypeResult[];
...
export default function Phase3LoveBehaviorTypeTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3LoveBehaviorTypeTestClientProps) {
  const t = useTranslations('phase3LoveBehaviorTypeTest');
  ...
  const [result, setResult] = useState<Phase3LoveBehaviorTypeResult | null>(null);
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3LoveBehaviorTypeResult(answersArray);
      ...
      if (resolved && locale !== 'ko') {
        ...
        console.log('🔮 [시작] 연애 행동 유형 결과:', resolved.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const resultPartnerView = result.partnerView[locale as keyof typeof result.partnerView] || result.partnerView.ko;
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <p className="text-xl font-bold text-purple-600 text-center" style={{ fontSize: '1.35em' }}>
                {resultScoreBand}
              </p>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <p className="text-sm text-gray-700 leading-relaxed">{resultPartnerView}</p>
            </div>
