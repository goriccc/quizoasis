...
import { Phase2RelationshipCutQuestion, Phase2RelationshipCutResult, calculatePhase2RelationshipCutResult } from '@/lib/phase2RelationshipCutData';
...
interface Phase2RelationshipCutTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase2RelationshipCutQuestion[];
  results: Phase2RelationshipCutResult[];
...
export default function Phase2RelationshipCutTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase2RelationshipCutTestClientProps) {
  const t = useTranslations('phase2RelationshipCutTest');
  const tGlobal = useTranslations(); // 글로벌 번역 (mbti 등)
  const [started, setStarted] = useState(false);
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [answers, setAnswers] = useState<Record<number, number>>({}); // 원래 질문 인덱스를 키로 사용, 값은 선택된 옵션 인덱스
  const [result, setResult] = useState<Phase2RelationshipCutResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2RelationshipCutQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase2RelationshipCutQuestion[]) => {
  ...
  // 답변 처리
  const handleAnswer = (shuffledOptionIndex: number) => {
    // 현재 질문의 원래 인덱스
    const currentOriginalIndex = originalQuestionIndices[currentQuestion];
    // 셔플링된 옵션 배열에서 선택된 옵션 찾기
    const optionsArray = shuffledOptionsMap[currentQuestion] || shuffledQuestions[currentQuestion].options;
    const selectedOption = optionsArray[shuffledOptionIndex];
    
    // 원래 질문에서 선택된 옵션의 원래 인덱스 찾기
    const originalQuestion = questions[currentOriginalIndex];
    const originalOptionIndex = originalQuestion.options.findIndex(
      opt => opt.text.ko === selectedOption.text.ko
    );
    
    // 원래 질문 인덱스와 원래 옵션 인덱스를 저장
    const newAnswers = { ...answers, [currentOriginalIndex]: originalOptionIndex >= 0 ? originalOptionIndex : shuffledOptionIndex };
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 원래 질문 순서대로 옵션 인덱스 배열 재구성 (newAnswers에는 모든 답변이 포함됨)
      const answersArray = questions.map((_, idx) => newAnswers[idx] ?? 0);
      
      // 결과 계산
      const resultType = calculatePhase2RelationshipCutResult(answersArray);
      const relationshipCutResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (relationshipCutResult) {
        setResult(relationshipCutResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (relationshipCutResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(relationshipCutResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 인간관계 정리 유형 결과:', relationshipCutResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const resultCutSpeed = result.cutSpeed[locale as keyof typeof result.cutSpeed] || result.cutSpeed.ko;
