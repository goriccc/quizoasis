...
import {
  Phase3ExLingeringFeelingsQuestion,
  Phase3ExLingeringFeelingsResult,
  calculatePhase3ExLingeringFeelingsResult,
...
interface Phase3ExLingeringFeelingsTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3ExLingeringFeelingsQuestion[];
  results: Phase3ExLingeringFeelingsResult[];
...
export default function Phase3ExLingeringFeelingsTestClient({
  locale,
  slug,
  title,
  description,
  ...
  isLatestTest = false,
  badgeType = null,
}: Phase3ExLingeringFeelingsTestClientProps) {
  const t = useTranslations('phase3ExLingeringFeelingsTest');
  ...
  const [result, setResult] = useState<Phase3ExLingeringFeelingsResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3ExLingeringFeelingsQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3ExLingeringFeelingsQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3ExLingeringFeelingsResult(answersArray);
      const shieldResult = results.find(r => r.type === resultType);
  ...
  if (showResult && result) {
    ...
    const resultLingeringIndex =
      result.lingeringIndex[locale as keyof typeof result.lingeringIndex] || result.lingeringIndex.ko;
    const resultCurrentState =
      result.currentState[locale as keyof typeof result.currentState] || result.currentState.ko;
    const resultTwoPossibilities = result.twoPossibilities
      ? result.twoPossibilities[locale as keyof typeof result.twoPossibilities] || result.twoPossibilities.ko
      : null;
    const resultMeaning = result.meaning
      ? result.meaning[locale as keyof typeof result.meaning] || result.meaning.ko
      : null;
    const resultWhatEmotionSays = result.whatEmotionSays
      ? result.whatEmotionSays[locale as keyof typeof result.whatEmotionSays] || result.whatEmotionSays.ko
      : null;
    const resultNeedNow = result.needNow
      ? result.needNow[locale as keyof typeof result.needNow] || result.needNow.ko
      : null;
    const resultToYou = result.toYou
      ? result.toYou[locale as keyof typeof result.toYou] || result.toYou.ko
      : null;
    const resultCanDoActions = result.canDoActions
      ? result.canDoActions[locale as keyof typeof result.canDoActions] || result.canDoActions.ko
      : null;
    const resultMustDoActions = result.mustDoActions
      ? result.mustDoActions[locale as keyof typeof result.mustDoActions] || result.mustDoActions.ko
      : null;
    const resultDoOneNow = result.doOneNow
      ? result.doOneNow[locale as keyof typeof result.doOneNow] || result.doOneNow.ko
      : null;
    const resultOptionalMore = result.optionalMore
      ? result.optionalMore[locale as keyof typeof result.optionalMore] || result.optionalMore.ko
      : null;
    const resultOneLiner = result.oneLiner
      ? result.oneLiner[locale as keyof typeof result.oneLiner] || result.oneLiner.ko
      : null;

    const chipSplit = (text: string) =>
      text.split(/[,、，·]/).map((s) => s.trim()).filter(Boolean);

    const renderBulletList = (text: string) => (
      <ul className="list-disc list-outside pl-8 ml-1 space-y-2 text-sm text-gray-700 text-left leading-relaxed">
        {text
          .split(/\n+/)
          .map((line) => line.trim())
          .filter(Boolean)
          ...
          <div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-lg font-bold text-indigo-600 text-center leading-snug">
                  {resultLingeringIndex}
                </p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="flex flex-wrap gap-2 justify-center">
                  {chipSplit(resultCurrentState).map((char, idx) => (
                    <span
                      key={idx}
                      className="inline-block bg-gradient-to-r from-indigo-100 to-violet-100 text-indigo-800 text-sm font-semibold px-3 py-1 rounded-full shadow-sm"
                    >
                      {char}
                    </span>
                  ))}
            ...
            {resultTwoPossibilities ? (
              <div className="mb-3">
                <div className="bg-white rounded-xl shadow-lg p-4">
                  <h3 className="text-base font-bold text-gray-800 mb-2 text-left w-full">
                    {t('ui.twoPossibilities')}
                  </h3>
                  <p className="text-sm text-gray-700 leading-relaxed text-center">{resultTwoPossibilities}</p>
                </div>
              </div>
            ) : null}
            ...
            {resultMeaning ? (
              <div className="mb-3">
                <div className="bg-white rounded-xl shadow-lg p-4">
                  ...
                  <p className="text-sm text-gray-700 leading-relaxed text-center">{resultMeaning}</p>
                </div>
              </div>
            ) : null}

            {resultWhatEmotionSays ? (
              <div className="mb-3">
                <div className="bg-white rounded-xl shadow-lg p-4">
                  <h3 className="text-base font-bold text-gray-800 mb-2 text-left w-full">
                    {t('ui.whatEmotionSays')}
                  </h3>
                  <p className="text-sm text-gray-700 leading-relaxed text-center">{resultWhatEmotionSays}</p>
                </div>
              </div>
            ) : null}
            ...
            {resultNeedNow ? (
              <div className="mb-3">
                <div className="bg-white rounded-xl shadow-lg p-4">
                  ...
                  <p className="text-sm text-gray-700 leading-relaxed text-center">{resultNeedNow}</p>
                </div>
              </div>
            ) : null}
            ...
            {resultToYou ? (
              <div className="mb-3">
                <div className="bg-gradient-to-r from-indigo-50 to-blue-50 rounded-xl shadow-lg p-4 border border-indigo-100">
                  <h3 className="text-base font-bold text-gray-800 mb-2 text-left w-full">
                    {t('ui.toYou')}
                  </h3>
                  <p className="text-sm text-gray-800 leading-relaxed text-center font-medium">{resultToYou}</p>
                </div>
              </div>
            ) : null}
            ...
            {resultCanDoActions ? (
              <div className="mb-3">
                <div className="bg-white rounded-xl shadow-lg p-4">
                  ...
                  {renderBulletList(resultCanDoActions)}
                </div>
              </div>
            ) : null}
            ...
            {resultMustDoActions ? (
              <div className="mb-3">
                <div className="bg-white rounded-xl shadow-lg p-4">
                  ...
                  {renderBulletList(resultMustDoActions)}
                </div>
              </div>
            ) : null}
            ...
            {resultDoOneNow ? (
              <div className="mb-3">
                <div className="bg-white rounded-xl shadow-lg p-4">
                  ...
                  <p className="text-sm text-gray-700 leading-relaxed text-center font-medium">{resultDoOneNow}</p>
                </div>
              </div>
            ) : null}
            ...
            {resultOptionalMore ? (
              <div className="mb-3">
                <div className="bg-white rounded-xl shadow-lg p-4">
                  ...
                  {renderBulletList(resultOptionalMore)}
                </div>
              </div>
            ) : null}
