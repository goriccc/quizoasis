...
import {
  Phase3AdhdTendencyChecklistQuestion,
  Phase3AdhdTendencyChecklistResult,
  calculatePhase3AdhdTendencyChecklistResult,
...
interface Phase3AdhdTendencyChecklistTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3AdhdTendencyChecklistQuestion[];
  results: Phase3AdhdTendencyChecklistResult[];
...
export default function Phase3AdhdTendencyChecklistTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3AdhdTendencyChecklistTestClientProps) {
  const t = useTranslations('phase3AdhdTendencyChecklistTest');
  ...
  const [result, setResult] = useState<Phase3AdhdTendencyChecklistResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3AdhdTendencyChecklistQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3AdhdTendencyChecklistQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3AdhdTendencyChecklistResult(answersArray);
      const adhdResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (adhdResult) {
        setResult(adhdResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (adhdResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(adhdResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] ADHD 성향 체크리스트 결과:', adhdResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  // 결과 공유하기
  const getResultShareBody = () => {
    if (!result) return '';
    const map: Record<string, 'resultType1' | 'resultType2' | 'resultType3' | 'resultType4' | 'resultType5' | 'resultType6'> = {
      Type1: 'resultType1',
      Type2: 'resultType2',
      Type3: 'resultType3',
      Type4: 'resultType4',
    ...
    const sk = map[result.type];
    if (sk) return t(`shareMessages.${sk}`);
  ...
  const shareToLine = () => {
    ...
    const shareText = result
      ? encodeURIComponent(getResultShareBody())
  ...
  const shareToKakao = () => {
    ...
    const shareDescription = result ? getResultShareBody() : t('shareMessages.startKakao');
  ...
  const shareToTelegram = () => {
    const url = encodeURIComponent(`https://myquizoasis.com${window.location.pathname}`);
    const shareText = result ? getResultShareBody() : t('shareMessages.startTelegram');
  ...
  if (showResult && result) {
    ...
    const pickLocalized = (r: Record<string, string>) =>
      (r[locale as keyof typeof r] ?? r.ko ?? '').trim();
    const resultWorkStyle = pickLocalized(result.workStyle);
    const resultRelationshipStyle = pickLocalized(result.relationshipStyle);
    const resultMessageToYou = pickLocalized(result.messageToYou);
    const resultRecommendedDirection = pickLocalized(result.recommendedDirection);
              ...
              {resultWorkStyle ? (
                <div className="bg-white rounded-xl shadow-lg p-4">
                  <h3 className="text-base font-bold text-gray-800 mb-2">
                    {t('ui.workStyleSection')}
                  </h3>
                  <p className="text-base text-gray-600 leading-relaxed">{resultWorkStyle}</p>
                </div>
              ) : null}
              {resultRelationshipStyle ? (
                <div className="bg-white rounded-xl shadow-lg p-4">
                  <h3 className="text-base font-bold text-gray-800 mb-2">
                    {t('ui.relationshipStyleSection')}
                  </h3>
                  <p className="text-base text-gray-600 leading-relaxed">{resultRelationshipStyle}</p>
                </div>
              ) : null}
              {resultMessageToYou ? (
                <div className="bg-white rounded-xl shadow-lg p-4">
                  <h3 className="text-base font-bold text-gray-800 mb-2">
                    {t('ui.messageToYouSection')}
                  </h3>
                  <p className="text-base text-gray-600 leading-relaxed whitespace-pre-line">{resultMessageToYou}</p>
                </div>
              ) : null}
              {resultRecommendedDirection ? (
                <div className="bg-white rounded-xl shadow-lg p-4">
                  <h3 className="text-base font-bold text-gray-800 mb-2">
                    {t('ui.recommendedDirectionSection')}
                  </h3>
                  <p className="text-base text-gray-600 leading-relaxed">{resultRecommendedDirection}</p>
                </div>
              ) : null}
            </div>
