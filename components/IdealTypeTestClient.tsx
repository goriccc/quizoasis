...
import { IdealTypeQuestion, IdealTypeResult, calculateIdealTypeResult } from '@/lib/idealTypeData';
...
interface IdealTypeTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: IdealTypeQuestion[];
  results: IdealTypeResult[];
...
const getCompatibilityDescription = (myType: string, partnerType: string, t: any): string => {
  ...
  return t(`idealTypeTest.result.compatibility.${key}`) || '';
};
...
export default function IdealTypeTestClient({ 
  ...
  const [shuffledQuestions, setShuffledQuestions] = useState<IdealTypeQuestion[]>(questions);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: IdealTypeQuestion[]) => {
  ...
  const handleAnswer = (scores: any) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculateIdealTypeResult(newAnswers);
      const idealTypeResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (idealTypeResult) {
        setResult(idealTypeResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (idealTypeResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(idealTypeResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 이상형 결과:', idealTypeResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  const calculateResult = (finalAnswers: any[]) => {
    const resultType = calculateIdealTypeResult(finalAnswers);
    const idealTypeResult = results.find(r => r.type === resultType);
  ...
  const shareToKakao = () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareMessage = result ? t('idealTypeTest.shareMessage', { type: resultTitle }) : title;
    const shareDescription = result ? shareMessage : description;
      ...
      <div className="min-h-screen bg-white">
        <div className="max-w-4xl mx-auto">
          <div className="relative w-full overflow-hidden mb-3" style={{ aspectRatio: '680/384' }}>
            <Image
              src={getThumbnailUrl(thumbnail || 'test_032_ideal_type.jpg')}
              alt={title}
              fill
          ...
          <div className="px-4">
            ...
            <div className="text-gray-600 mb-6 leading-relaxed text-center space-y-4">
              <p className="font-bold text-gray-700">{t('idealTypeTest.startMessageLine1')}</p>
              <p>{t('idealTypeTest.startMessageLine2')}</p>
              <p>{t('idealTypeTest.startMessageLine3')}</p>
              <p>{t('idealTypeTest.startMessageLine4')}</p>
              <p className="whitespace-pre-line">{t('idealTypeTest.startMessageLine5')}</p>
              <p>{t('idealTypeTest.startMessageLine6')}</p>
            </div>
      ...
      <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-blue-50">
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                💕 {t('idealTypeTest.result.datingStyle')}
              </h3>
              <div className="flex flex-wrap gap-2">
                {result.datingStyle && result.datingStyle.map((style, index) => {
                  const styleText = style[locale as keyof typeof style] || style.ko;
                  return (
                    <span 
                      key={index}
                      className="bg-gradient-to-r from-pink-100 to-rose-100 text-pink-800 px-3 py-1.5 rounded-full text-sm font-medium"
                    >
                      {styleText}
                    </span>
                  );
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                💼 {t('idealTypeTest.result.idealJobs')}
              </h3>
              <div className="flex flex-wrap gap-2">
                {result.idealJobs && result.idealJobs.map((job, index) => {
                  const jobText = job[locale as keyof typeof job] || job.ko;
                  return (
                    <span 
                      key={index}
                      className="bg-gradient-to-r from-blue-100 to-indigo-100 text-blue-800 px-3 py-1.5 rounded-full text-sm font-medium"
                    >
                      {jobText}
                    </span>
                  );
            ...
            {result.compatibility && (
              <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
                <h3 className="text-base font-bold text-gray-800 mb-3">
                  💘 {locale === 'ko' ? '궁합 정보' : 
                      locale === 'en' ? 'Compatibility' :
                      locale === 'ja' ? '相性情報' :
                      locale === 'zh-CN' ? '匹配信息' :
                      locale === 'zh-TW' ? '匹配資訊' :
                      locale === 'vi' ? 'Thông tin tương thích' :
                      locale === 'id' ? 'Informasi Kecocokan' : '궁합 정보'}
                </h3>
                <div className="grid grid-cols-2 gap-3 mb-3">
                  {result.compatibility.best && (
                    <div className="bg-white rounded-xl shadow-lg p-4">
                      <h3 className="text-base font-bold text-gray-800 mb-3">
                        💖 {t('idealTypeTest.result.bestMatch')}
                      </h3>
                  ...
                  {result.compatibility.good && (
                    <div className="bg-white rounded-xl shadow-lg p-4">
                      <h3 className="text-base font-bold text-gray-800 mb-3">
                        😊 {t('idealTypeTest.result.goodMatch')}
                      </h3>
                ...
                <div className="grid grid-cols-2 gap-3 mb-3">
                  {result.compatibility.careful && (
                    <div className="bg-white rounded-xl shadow-lg p-4">
                      <h3 className="text-base font-bold text-gray-800 mb-3">
                        ⚠️ {t('idealTypeTest.result.carefulMatch')}
                      </h3>
                  ...
                  {result.compatibility.difficult && (
                    <div className="bg-white rounded-xl shadow-lg p-4">
                      <h3 className="text-base font-bold text-gray-800 mb-3">
                        ❌ {t('idealTypeTest.result.difficultMatch')}
                      </h3>
