...
import {
  Phase3ApprovalSeekingLevelQuestion,
  Phase3ApprovalSeekingLevelResult,
  calculatePhase3ApprovalSeekingLevelResult,
  calculatePhase3ApprovalSeekingLevelDomainScores,
} from '@/lib/phase3ApprovalSeekingLevelData';
...
interface Phase3ApprovalSeekingLevelTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3ApprovalSeekingLevelQuestion[];
  results: Phase3ApprovalSeekingLevelResult[];
...
export default function Phase3ApprovalSeekingLevelTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3ApprovalSeekingLevelTestClientProps) {
  const t = useTranslations('phase3ApprovalSeekingLevelTest');
  ...
  const [result, setResult] = useState<Phase3ApprovalSeekingLevelResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3ApprovalSeekingLevelQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3ApprovalSeekingLevelQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      const scores = calculatePhase3ApprovalSeekingLevelDomainScores(answersArray);
      setDomainScores(scores);

      const resultType = calculatePhase3ApprovalSeekingLevelResult(answersArray);
  ...
  const handleRetake = () => {
    ...
    setResult(null);
    setShowResult(false);
    setDomainScores({ snsOnline: 0, praiseCriticism: 0, othersGaze: 0, relationship: 0, selfDecision: 0, psychological: 0, total: 0 });
  ...
  if (showResult && result) {
    ...
    const resultApprovalSeekingLevel =
      result.desireLevel[locale as keyof typeof result.desireLevel] || result.desireLevel.ko;
    const resultApprovalSeekingLevelType =
      result.desireType[locale as keyof typeof result.desireType] || result.desireType.ko;
    const resultEvaluationImpact =
      result.evaluationImpact[locale as keyof typeof result.evaluationImpact] || result.evaluationImpact.ko;
    ...
    const domainConfig = [
      { key: 'snsOnline', label: t('ui.snsOnlineDomain'), score: domainScores.snsOnline, max: 6 },
      { key: 'praiseCriticism', label: t('ui.praiseCriticismDomain'), score: domainScores.praiseCriticism, max: 6 },
      { key: 'othersGaze', label: t('ui.othersGazeDomain'), score: domainScores.othersGaze, max: 6 },
      { key: 'relationship', label: t('ui.relationshipDomain'), score: domainScores.relationship, max: 6 },
      { key: 'selfDecision', label: t('ui.selfDecisionDomain'), score: domainScores.selfDecision, max: 6 },
      { key: 'psychological', label: t('ui.psychologicalDomain'), score: domainScores.psychological, max: 6 },
    ];
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm font-bold text-gray-700 leading-relaxed text-center">
                  {resultApprovalSeekingLevelType}
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  💜 {t('ui.evaluationImpact')}
                </h3>
                <p className="text-sm font-bold text-gray-700 leading-relaxed text-center">
                  {resultEvaluationImpact}
                </p>
              </div>
            </div>
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
                ...
                {domainConfig.map((domain) => (
                  <div key={domain.key}>
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2.5">
                      <div
                        className="bg-gradient-to-r from-purple-500 to-pink-400 h-2.5 rounded-full transition-all"
              ...
              <div className="mt-4 space-y-2 text-sm text-gray-700">
                ...
                <p>
                  📌 {t('ui.mostIndependentDomain')}: <span className="font-semibold">{weakestLabels.join(' · ')}</span>
                </p>
              </div>
            </div>
