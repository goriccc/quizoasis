...
import { EntrepreneurSpiritQuestion, EntrepreneurSpiritResult, calculateEntrepreneurSpiritResult } from '@/lib/entrepreneurSpiritData';
...
interface EntrepreneurSpiritTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: EntrepreneurSpiritQuestion[];
  results: EntrepreneurSpiritResult[];
...
export default function EntrepreneurSpiritTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  playCount = 0,
  similarTests = []
}: EntrepreneurSpiritTestClientProps) {
  const t = useTranslations();
  const [started, setStarted] = useState(false);
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [answers, setAnswers] = useState<any[]>([]);
  const [result, setResult] = useState<EntrepreneurSpiritResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<EntrepreneurSpiritQuestion[]>(questions);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: EntrepreneurSpiritQuestion[]) => {
  ...
  const handleAnswer = (scores: any) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculateEntrepreneurSpiritResult(newAnswers);
      const entrepreneurResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (entrepreneurResult) {
        setResult(entrepreneurResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (entrepreneurResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(entrepreneurResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 창업가 기질 결과:', entrepreneurResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  const handleShareResult = async () => {
    ...
    if (navigator.share) {
      ...
      try {
        ...
        alert(t('entrepreneurSpiritTest.alerts.shareError'));
      }
    }
  };
  ...
  const shareToWeChat = async () => {
    ...
    try {
      ...
      alert(t('entrepreneurSpiritTest.alerts.linkCopied') + ' ' + t('entrepreneurSpiritTest.alerts.wechatInstruction'));
    } catch (error) {
      alert(t('entrepreneurSpiritTest.alerts.shareError'));
    }
  };
  ...
  const copyLink = () => {
    ...
    alert(t('entrepreneurSpiritTest.alerts.linkCopied'));
  };
      ...
      <div className="min-h-screen bg-white">
        <div className="max-w-4xl mx-auto">
          <div className="relative w-full overflow-hidden mb-3" style={{ aspectRatio: '680/384' }}>
            <Image
              src={getThumbnailUrl(thumbnail || 'test_050_entrepreneur_spirit.jpg')}
              alt={title}
              fill
          ...
          <div className="px-4">
            ...
            <div className="text-gray-600 mb-6 leading-relaxed text-center space-y-4">
              <p className="font-bold text-gray-700">{t('entrepreneurSpiritTest.startMessage.line1')}</p>
              <p>{t('entrepreneurSpiritTest.startMessage.line2')}</p>
              <p>{t('entrepreneurSpiritTest.startMessage.line3')}</p>
              <p>{t('entrepreneurSpiritTest.startMessage.line4')}</p>
              <p className="whitespace-pre-line">{t('entrepreneurSpiritTest.startMessage.line5')}</p>
              <p>{t('entrepreneurSpiritTest.startMessage.line6')}</p>
            </div>
...
{t('mbti.startTest')}
            ...
            <div className="mb-8 text-center">
              ...
              <div className="flex justify-center gap-2">
                <button onClick={copyLink} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/link.jpeg" alt={t('entrepreneurSpiritTest.ui.linkCopy')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToKakao} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/kakao.jpeg" alt={t('entrepreneurSpiritTest.ui.kakao')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToTelegram} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/telegram.jpeg" alt={t('entrepreneurSpiritTest.ui.telegram')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToWeChat} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/wechat.jpeg" alt={t('entrepreneurSpiritTest.ui.wechat')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToLine} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/line.jpeg" alt={t('entrepreneurSpiritTest.ui.line')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToWhatsApp} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/whatsapp.jpeg" alt={t('entrepreneurSpiritTest.ui.whatsapp')} width={46} height={46} className="rounded-lg" />
                </button>
              </div>
            </div>
  ...
  if (showResult && result) {
    ...
    const resultRecommendedFields = result.recommendedFields[locale as keyof typeof result.recommendedFields] || result.recommendedFields.ko;
          ...
          <div>
            <div className="text-center mb-3 bg-white rounded-2xl shadow-lg p-4 md:p-5">
              ...
              <p className="text-sm text-gray-500 leading-relaxed">
                {result.longDescription ? (typeof result.longDescription === 'string' ? result.longDescription : (result.longDescription[locale as keyof typeof result.longDescription] || result.longDescription.ko)) : ''}
              </p>
            </div>

            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                📊 {t('entrepreneurSpiritTest.result.successRate')}
              </h3>
              <div className="text-center">
                <div className="text-2xl mb-2">{result.successRate}</div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-3">
                  ✅ {t('entrepreneurSpiritTest.result.pros')}
                </h3>
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-3">
                  ⚠️ {t('entrepreneurSpiritTest.result.cons')}
                </h3>
                <div className="flex flex-wrap gap-2">
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                🚀 {t('entrepreneurSpiritTest.result.recommendedFields')}
              </h3>
              <div className="flex flex-wrap gap-2">
                {resultRecommendedFields.split(/,\s+|，\s*|、\s*/).map((field, index) => (
                  <span
                    key={index}
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                💡 {t('entrepreneurSpiritTest.result.advice')}
              </h3>
