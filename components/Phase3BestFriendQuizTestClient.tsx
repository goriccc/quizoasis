...
import {
  Phase3BestFriendQuizQuestion,
  Phase3BestFriendQuizResult,
  Phase3BestFriendQuizPayload,
  encodeQuizPayload,
  decodeQuizPayload,
  formatQuestionWithName,
  phase3BestFriendQuizQuestions,
  phase3BestFriendQuizResults,
  calculatePhase3BestFriendQuizResult,
  scoreFriendAnswers,
...
interface Props {
  ...
  title: string;
  description: string;
  questions: Phase3BestFriendQuizQuestion[];
  results: Phase3BestFriendQuizResult[];
...
function Phase3BestFriendQuizTestClientInner({
  locale,
  slug,
  title,
  ...
  const t = useTranslations('phase3BestFriendQuizTest');
  const tGlobal = useTranslations();
  const tBadges = useTranslations('badges');
  const searchParams = useSearchParams();
  const kRaw = searchParams.get('k');
  const challengePayload = useMemo(
    () => (kRaw ? decodeQuizPayload(decodeURIComponent(kRaw)) : null),
    [kRaw]
  );

  const [phase, setPhase] = useState<Phase>(() => (kRaw ? 'friendIntro' : 'creatorIntro'));
  const [friendPayload, setFriendPayload] = useState<Phase3BestFriendQuizPayload | null>(
    () => challengePayload
  );
  ...
  const [result, setResult] = useState<Phase3BestFriendQuizResult | null>(null);
  const [friendScore, setFriendScore] = useState(0);
  const [wrongQuestions, setWrongQuestions] = useState<number[]>([]);

  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3BestFriendQuizQuestion[]>([]);
  ...
  const shuffleQuestions = (list: Phase3BestFriendQuizQuestion[]) => {
  ...
  const handleAnswer = (displayOptionIndex: number) => {
    ...
    if (phase === 'creatorRun') {
      ...
      const payload: Phase3BestFriendQuizPayload = { n: creatorName.trim(), a: arr };
      const encoded = encodeQuizPayload(payload);
    ...
    if (phase === 'friendRun' && friendPayload) {
      const { score, wrongQuestionNumbers } = scoreFriendAnswers(friendPayload, newAnswers);
      const type = calculatePhase3BestFriendQuizResult(score);
  ...
  const friendResultShareLine = () => {
    ...
    return t('shareMessages.friendResult', {
  ...
  const hostSummaryText = useMemo(() => {
    ...
    return t('hostNotifyBody', {
      name: friendPayload.n,
      score: friendScore,
      wrong: wrongQuestions.length ? wrongQuestions.join(', ') : t('noneWrong'),
      type: typeTitle,
    });
  }, [friendPayload, result, friendScore, wrongQuestions, t, locale]);
      ...
      <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-rose-50">
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div className="text-center mb-3 bg-white rounded-2xl shadow-lg p-4 md:p-5">
            ...
            <p className="text-lg font-bold text-orange-600 mb-2">{t('scoreLine', { score: friendScore })}</p>
          ...
          <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
            <h3 className="font-bold text-gray-800 mb-2">{t('wrongListTitle')}</h3>
            {wrongQuestions.length === 0 ? (
              <p className="text-green-600 font-medium">{t('allCorrect')}</p>
            ) : (
              <p className="text-gray-800">{wrongQuestions.join(', ')}</p>
            )}
          </div>
  ...
  const qText = formatQuestionWithName(q.question, locale, hostName);
...
export default function Phase3BestFriendQuizTestClient(props: Props) {
  return (
    <Suspense
      fallback={
    ...
    >
      <Phase3BestFriendQuizTestClientInner {...props} />
    </Suspense>
  );
}
