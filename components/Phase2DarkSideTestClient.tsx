...
import { Phase2DarkSideQuestion, Phase2DarkSideResult, calculatePhase2DarkSideResult } from '@/lib/phase2_dark_side_data';
...
interface Phase2DarkSideTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase2DarkSideQuestion[];
  results: Phase2DarkSideResult[];
...
export default function Phase2DarkSideTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase2DarkSideTestClientProps) {
  const t = useTranslations('phase2DarkSideTest');
  ...
  const [result, setResult] = useState<Phase2DarkSideResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2DarkSideQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase2DarkSideQuestion[]) => {
  ...
  const handleAnswer = (selectedTypes: string[]) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase2DarkSideResult(newAnswers);
      const darkSideResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (darkSideResult) {
        setResult(darkSideResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (darkSideResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(darkSideResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 흑화 빌런 결과:', darkSideResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const resultDarkStyle = result.darkStyle[locale as keyof typeof result.darkStyle] || result.darkStyle.ko;
            ...
            <div className="space-y-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {resultDarkStyle.split(/[,、，]/).map((keyword, index) => (
                    <span
                      key={index}
                      className="inline-block bg-gradient-to-r from-gray-800 to-black text-white px-3 py-1.5 rounded-full text-sm font-medium"
                    >
                      {keyword.trim()}
                    </span>
                  ))}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-white rounded-xl shadow-lg p-4">
                  ...
                  <div className="space-y-2">
                    {(() => {
                      // Type 번호로 결과 찾기 (예: "Type 4"에서 "4" 추출)
                      const typeMatch = resultGoodMatch.match(/Type\s*(\d+)/i);
                      if (typeMatch) {
                        const typeNumber = typeMatch[1];
                        const matchResult = results.find(r => r.type === `Type${typeNumber}`);
                        if (matchResult) {
                ...
                <div className="bg-white rounded-xl shadow-lg p-4">
                  ...
                  <div className="space-y-2">
                    {(() => {
                      // Type 번호로 결과 찾기 (예: "Type 2"에서 "2" 추출)
                      const typeMatch = resultBadMatch.match(/Type\s*(\d+)/i);
                      if (typeMatch) {
                        const typeNumber = typeMatch[1];
                        const matchResult = results.find(r => r.type === `Type${typeNumber}`);
                        if (matchResult) {
                          const matchTitle = matchResult.title[locale as keyof typeof matchResult.title] || matchResult.title.ko;
                          return (
                            <div className="bg-gradient-to-r from-red-100 to-orange-100 rounded-lg px-3 py-2">
                              <div className="flex items-center gap-2.5">
                                <span className="text-xl">{matchResult.emoji}</span>
                                <span className="text-sm font-medium text-gray-800">{matchTitle}</span>
                              </div>
