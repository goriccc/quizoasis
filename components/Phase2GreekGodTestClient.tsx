...
import { Phase2GreekGodQuestion, Phase2GreekGodResult, calculatePhase2GreekGodResult } from '@/lib/phase2_greek_god_data';
...
interface Phase2GreekGodTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase2GreekGodQuestion[];
  results: Phase2GreekGodResult[];
...
export default function Phase2GreekGodTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase2GreekGodTestClientProps) {
  const t = useTranslations('phase2GreekGodTest');
  const tGlobal = useTranslations(); // 글로벌 번역 (mbti 등)
  const [started, setStarted] = useState(false);
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [answers, setAnswers] = useState<Array<{ questionId: number; selectedTypes: string[] }>>([]);
  const [result, setResult] = useState<Phase2GreekGodResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2GreekGodQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase2GreekGodQuestion[]) => {
  ...
  // 답변 처리
  const handleAnswer = (selectedTypes: string[]) => {
    ...
    const newAnswers = [...answers, { questionId: currentQuestionId, selectedTypes }];
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase2GreekGodResult(newAnswers);
      const greekGodResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (greekGodResult) {
        setResult(greekGodResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (greekGodResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(greekGodResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 그리스 신 결과:', greekGodResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const resultSymbols = result.symbols[locale as keyof typeof result.symbols] || result.symbols.ko;
    const resultTraits = result.traits[locale as keyof typeof result.traits] || result.traits.ko;
    const resultRecommendedItems = result.recommendedItems[locale as keyof typeof result.recommendedItems] || result.recommendedItems.ko;
            ...
            <div className="space-y-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {resultSymbols.split(/[,、，]/).map((symbol, index) => (
                    <span
                      key={index}
                      className="inline-block px-4 py-2 bg-gradient-to-r from-purple-100 to-pink-100 text-purple-700 font-medium rounded-full text-sm border border-purple-200"
                    >
                      {symbol.trim()}
                    </span>
                  ))}
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {resultTraits.split(/[,、，]/).map((trait, index) => (
                    <span
                      key={index}
                      className="inline-block px-4 py-2 bg-gradient-to-r from-blue-100 to-cyan-100 text-blue-700 font-medium rounded-full text-sm border border-blue-200"
                    >
                      {trait.trim()}
                    </span>
                  ))}
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  💎 {t('ui.recommendedItems')}
                </h3>
                <div className="flex flex-wrap gap-2">
                  {resultRecommendedItems.split(/[,、，]/).map((item, index) => (
                    <span
                      key={index}
                      className="inline-block px-4 py-2 bg-gradient-to-r from-amber-100 to-yellow-100 text-amber-700 font-medium rounded-full text-sm border border-amber-200"
                    >
                      {item.trim()}
                    </span>
                  ))}
