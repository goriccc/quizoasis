...
import { DatingStyleQuestion, DatingStyleResult, calculateDatingStyleResult, datingStyleResults } from '../lib/datingStyleData';
...
interface DatingStyleTestClientProps {
  ...
  title: string;
  description: string;
  questions: DatingStyleQuestion[];
  results: DatingStyleResult[];
...
export default function DatingStyleTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  const t = useTranslations('datingStyleTest');
  const tGlobal = useTranslations();
  
  const [started, setStarted] = useState(false);
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [answers, setAnswers] = useState<any[]>([]);
  const [result, setResult] = useState<DatingStyleResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<DatingStyleQuestion[]>(questions);
  ...
  useEffect(() => {
    if (locale !== 'ko' && !started && aliProducts.length === 0) {
      const loadProducts = async () => {
        try {
          const keyword = locale === 'id' ? 'fashion' : 'couple gifts';
  ...
  const handleAnswer = (answer: string) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 테스트 완료
      const calculatedResult = calculateDatingStyleResult(newAnswers);
      const datingStyleResult = results.find(r => r.type === calculatedResult);
      if (datingStyleResult) {
        setResult(datingStyleResult);
      }
      ...
      window.Kakao.Share.sendDefault({
        ...
        content: {
          ...
          description: shareText,
              imageUrl: getThumbnailUrl(thumbnail || 'test_221_dating_style.jpg'),
          link: {
  ...
  const shuffleQuestions = (questionList: DatingStyleQuestion[]) => {
      ...
      <div className="min-h-screen bg-white">
        <div className="max-w-4xl mx-auto">
          <div className="relative w-full overflow-hidden mb-3" style={{ aspectRatio: '680/384' }}>
            <Image
              src={getThumbnailUrl(thumbnail || 'test_221_dating_style.jpg')}
              alt={title}
              fill
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-4">
        ...
        <div className="text-center">
          ...
          <p className="text-gray-600">{t('result.analyzingDate')}</p>
        </div>
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
        <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6 text-center">
          ...
          <p className="text-gray-600 mb-6">{t('result.analyzed')}</p>
          <button
            onClick={handleShowResult}
          ...
          <div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                💡 {t('result.datingFeatures')}
              </h3>
              <div className="flex flex-wrap gap-2">
                {result.datingFeatures[localeKey as keyof typeof result.datingFeatures].map((feature, index) => (
                  <span
                    key={index}
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {result.pros[localeKey as keyof typeof result.pros].map((pro, index) => (
                    <span
                      key={index}
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {result.cons[localeKey as keyof typeof result.cons].map((con, index) => (
                    <span
                      key={index}
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                📍 {t('result.recommendedPlaces')}
              </h3>
              <div className="flex flex-wrap gap-2">
                {result.recommendedPlaces[localeKey as keyof typeof result.recommendedPlaces].map((place, index) => (
                  <span
                    key={index}
            ...
            {/* 궁합 분석 */}
            {(() => {
              const parseMatchInfo = (matchText: string) => {
                const typeMatch = matchText.match(/Type (\d+)/);
                if (!typeMatch) return null;
                const typeNum = parseInt(typeMatch[1]);
                const matchResult = datingStyleResults.find(r => r.type === typeNum);
                if (!matchResult) return null;
                
                // "- " 뒤의 설명 추출 (모든 언어 지원)
                const descMatch = matchText.match(/-\s*(.+)$/);
                const description = descMatch ? descMatch[1].trim() : '';
                
                return {
                  emoji: matchResult.emoji,
                  name: matchResult.title[localeKey as keyof typeof matchResult.title],
                  description: description
                };
              };

              const bestMatchInfo = parseMatchInfo(result.bestMatch[localeKey as keyof typeof result.bestMatch]);
              const goodMatchInfo = parseMatchInfo(result.goodMatch[localeKey as keyof typeof result.goodMatch]);
              const carefulMatchInfo = parseMatchInfo(result.carefulMatch[localeKey as keyof typeof result.carefulMatch]);
              ...
              return (
                <>
                  {bestMatchInfo && (
                    <div className="bg-white rounded-xl shadow-lg p-3 mb-2">
                      <h3 className="text-sm font-bold text-gray-800">
                        💖 {t('result.bestMatch')}: <span className="font-normal">{bestMatchInfo.emoji} {bestMatchInfo.name} - {bestMatchInfo.description}</span>
                      </h3>
                    </div>
                  )}
                  ...
                  {goodMatchInfo && (
                    <div className="bg-white rounded-xl shadow-lg p-3 mb-2">
                      <h3 className="text-sm font-bold text-gray-800">
                        😊 {t('result.goodMatch')}: <span className="font-normal">{goodMatchInfo.emoji} {goodMatchInfo.name} - {goodMatchInfo.description}</span>
                      </h3>
                    </div>
                  )}
                  ...
                  {carefulMatchInfo && (
                    <div className="bg-white rounded-xl shadow-lg p-3 mb-3">
                      <h3 className="text-sm font-bold text-gray-800">
                        ⚠️ {t('result.carefulMatch')}: <span className="font-normal">{carefulMatchInfo.emoji} {carefulMatchInfo.name} - {carefulMatchInfo.description}</span>
                      </h3>
                    </div>
                  )}
                </>
