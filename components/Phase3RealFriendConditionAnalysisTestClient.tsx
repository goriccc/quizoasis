...
import {
  Phase3RealFriendConditionAnalysisQuestion,
  Phase3RealFriendConditionAnalysisResult,
  calculatePhase3RealFriendConditionAnalysisResult,
...
interface Phase3RealFriendConditionAnalysisTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3RealFriendConditionAnalysisQuestion[];
  results: Phase3RealFriendConditionAnalysisResult[];
...
function getResultShareSnippet(t: (key: string) => string, result: Phase3RealFriendConditionAnalysisResult): string {
  switch (result.type) {
    case 'Type1':
      return t('shareMessages.type1');
    case 'Type2':
      return t('shareMessages.type2');
    case 'Type3':
      return t('shareMessages.type3');
    case 'Type4':
      return t('shareMessages.type4');
    case 'Type5':
      return t('shareMessages.type5');
    default:
      return t('shareMessages.type6');
  }
}

export default function Phase3RealFriendConditionAnalysisTestClient({
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3RealFriendConditionAnalysisTestClientProps) {
  const t = useTranslations('phase3RealFriendConditionAnalysisTest');
  ...
  const [result, setResult] = useState<Phase3RealFriendConditionAnalysisResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3RealFriendConditionAnalysisQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3RealFriendConditionAnalysisQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3RealFriendConditionAnalysisResult(answersArray);
      const found = results.find((r) => r.type === resultType);
      if (found) {
        setResult(found);
      }
    }
  ...
  const handleShareResult = async () => {
    ...
    const snippet = getResultShareSnippet(t, result);
  ...
  const shareToKakao = () => {
    ...
    const shareDescription = result ? getResultShareSnippet(t, result) : t('shareMessages.startKakao');
  ...
  if (showResult && result) {
    ...
    const resultWhyStays = result.whyStays[locale as keyof typeof result.whyStays] || result.whyStays.ko;
    const resultWhyDrifts = result.whyDrifts[locale as keyof typeof result.whyDrifts] || result.whyDrifts.ko;
    const resultOftenHeard = result.oftenHeard[locale as keyof typeof result.oftenHeard] || result.oftenHeard.ko;
    const resultHappyMoment = result.happyMoment[locale as keyof typeof result.happyMoment] || result.happyMoment.ko;
    const resultContactPattern = result.contactPattern[locale as keyof typeof result.contactPattern] || result.contactPattern.ko;
    const resultBestMatch = result.bestMatch[locale as keyof typeof result.bestMatch] || result.bestMatch.ko;
    const resultClashStyle = result.clashStyle[locale as keyof typeof result.clashStyle] || result.clashStyle.ko;
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            <div className="text-center mb-3 bg-white rounded-2xl shadow-lg p-4 md:p-5">
              ...
              <div className="text-base text-gray-600 leading-relaxed whitespace-pre-line text-left">
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {resultKeywords.split(/[·,、，]/).map((char, index) => (
                    <span
                      key={index}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultWhyStays}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  💔 {t('ui.whyDrifts')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">{resultWhyDrifts}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultOftenHeard}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  ✨ {t('ui.happyMoment')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">{resultHappyMoment}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultContactPattern}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  🤝 {t('ui.bestMatch')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">{resultBestMatch}</p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4 border border-amber-100">
                <h3 className="text-base font-bold text-gray-800 mb-2">
                  ⚡ {t('ui.clashStyle')}
                </h3>
                <p className="text-sm text-gray-800 font-medium leading-relaxed">{resultClashStyle}</p>
              </div>
            </div>
