...
import { EmpathyFQuestion, EmpathyFResult, calculateEmpathyFResult } from '@/lib/empathyFData';
...
interface EmpathyFTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: EmpathyFQuestion[];
  results: EmpathyFResult[];
...
export default function EmpathyFTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  ...
  const t = useTranslations('empathyFTest');
  ...
  const [result, setResult] = useState<EmpathyFResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<EmpathyFQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: EmpathyFQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculateEmpathyFResult(answersArray);
