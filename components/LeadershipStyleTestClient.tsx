...
import { LeadershipStyleQuestion, LeadershipStyleResult, calculateLeadershipStyleResult } from '@/lib/leadershipStyleData';
...
interface LeadershipStyleTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: LeadershipStyleQuestion[];
  results: LeadershipStyleResult[];
...
export default function LeadershipStyleTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: LeadershipStyleTestClientProps) {
  const t = useTranslations('leadershipStyleTest');
  ...
  const [shuffledQuestions, setShuffledQuestions] = useState<LeadershipStyleQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: LeadershipStyleQuestion[]) => {
  ...
  const handleAnswer = (types: string[]) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      const resultType = calculateLeadershipStyleResult(answersArray);
      const leadershipStyleResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (leadershipStyleResult) {
        setResult(leadershipStyleResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (leadershipStyleResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(leadershipStyleResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 리더십 스타일 결과:', leadershipStyleResult.type, '→ 검색 키워드:', keywords[0]);
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-base font-bold text-purple-600 text-center">
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-base text-purple-600 text-center">
                  {resultStrengths}
                </p>
              </div>
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-base text-orange-600 text-center">
                  {resultWeaknesses}
                </p>
              </div>
