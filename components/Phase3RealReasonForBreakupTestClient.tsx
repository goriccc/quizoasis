...
import { Phase3RealReasonForBreakupQuestion, Phase3RealReasonForBreakupResult, calculatePhase3RealReasonForBreakupResult } from '@/lib/phase3RealReasonForBreakupData';
...
interface Phase3RealReasonForBreakupTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3RealReasonForBreakupQuestion[];
  results: Phase3RealReasonForBreakupResult[];
...
export default function Phase3RealReasonForBreakupTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3RealReasonForBreakupTestClientProps) {
  const t = useTranslations('phase3RealReasonForBreakupTest');
  ...
  const [result, setResult] = useState<Phase3RealReasonForBreakupResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3RealReasonForBreakupQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3RealReasonForBreakupQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3RealReasonForBreakupResult(answersArray);
      const breakupResult = results.find(r => r.type === resultType);
      ...
      if (breakupResult && locale !== 'ko') {
        ...
        console.log('🔮 [시작] 이별 원인 결과:', breakupResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const resultImprovePoint = result.improvePoint[locale as keyof typeof result.improvePoint] || result.improvePoint.ko;
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            <div className="text-center mb-3 bg-white rounded-2xl shadow-lg p-4 md:p-5">
              ...
              <p className="text-lg font-semibold text-gray-700 mb-3 whitespace-pre-line">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {resultGoodMatch.split(/[,、，]/).map((char, index) => (
                    <span
                      key={index}
                      className="inline-block bg-gradient-to-r from-green-100 to-emerald-100 text-green-800 text-sm font-semibold px-3 py-1 rounded-full shadow-sm"
                    >
                      {char.trim()}
                    </span>
                  ))}
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <ul className="list-disc list-outside pl-5 space-y-2 text-sm text-gray-700 text-left leading-relaxed">
                {resultPrescription
                  .split('\n')
                  .map((line) => line.trim())
                  .filter(Boolean)
