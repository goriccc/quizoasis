...
import {
  Phase3SpendingPersonalityTypeQuestion,
  Phase3SpendingPersonalityTypeResult,
  calculatePhase3SpendingPersonalityTypeResult,
...
interface Phase3SpendingPersonalityTypeTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3SpendingPersonalityTypeQuestion[];
  results: Phase3SpendingPersonalityTypeResult[];
...
export default function Phase3SpendingPersonalityTypeTestClient({
  locale,
  slug,
  title,
  description,
  ...
  isLatestTest = false,
  badgeType = null,
}: Phase3SpendingPersonalityTypeTestClientProps) {
  const t = useTranslations('phase3SpendingPersonalityTypeTest');
  ...
  const [result, setResult] = useState<Phase3SpendingPersonalityTypeResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3SpendingPersonalityTypeQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3SpendingPersonalityTypeQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3SpendingPersonalityTypeResult(answersArray);
  ...
  if (showResult && result) {
    ...
    const resultSpendingDna =
      result.spendingDna[locale as keyof typeof result.spendingDna] || result.spendingDna.ko;
    const resultKeywords =
      result.keywords[locale as keyof typeof result.keywords] || result.keywords.ko;
    ...
    const keywordChips = resultKeywords.split(/[,、，]/).map((s) => s.trim()).filter(Boolean);
