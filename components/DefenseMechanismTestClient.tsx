...
import { DefenseMechanismQuestion, DefenseMechanismResult, calculateDefenseMechanismResult } from '@/lib/defenseMechanismData';
...
interface DefenseMechanismTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: DefenseMechanismQuestion[];
  results: DefenseMechanismResult[];
...
,
  isLatestTest = false,
  badgeType = null
}: DefenseMechanismTestClientProps) {
  const t = useTranslations('defenseMechanismTest');
  const tGlobal = useTranslations(); // 글로벌 번역 (mbti 등)
  const [started, setStarted] = useState(false);
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [answers, setAnswers] = useState<Record<number, string>>({}); // 원래 질문 인덱스를 키로 사용, 값은 Type (예: "Type1")
  const [result, setResult] = useState<DefenseMechanismResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<DefenseMechanismQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: DefenseMechanismQuestion[]) => {
  ...
  const handleAnswer = (type: string) => {
    ...
    const newAnswers = { ...answers, [currentOriginalIndex]: type };
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 원래 질문 순서대로 Type 배열 재구성 (newAnswers에는 모든 답변이 포함됨)
      const answersArray = questions.map((_, idx) => newAnswers[idx] ?? 'Type1');
      
      // 결과 계산
      const resultType = calculateDefenseMechanismResult(answersArray);
      const defenseMechanismResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (defenseMechanismResult) {
        setResult(defenseMechanismResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (defenseMechanismResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(defenseMechanismResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 방어기제 결과:', defenseMechanismResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const resultDefenseMechanism = result.defenseMechanism[locale as keyof typeof result.defenseMechanism] || result.defenseMechanism.ko;
