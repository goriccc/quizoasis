...
import { Phase2FactBomberQuestion, Phase2FactBomberResult, calculatePhase2FactBomberResult } from '@/lib/phase2_fact_bomber_data';
...
interface Phase2FactBomberTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase2FactBomberQuestion[];
  results: Phase2FactBomberResult[];
...
export default function Phase2FactBomberTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  playCount = 0,
  similarTests = []
}: Phase2FactBomberTestClientProps) {
  const t = useTranslations('phase2FactBomberTest');
  ...
  const [result, setResult] = useState<Phase2FactBomberResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2FactBomberQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase2FactBomberQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase2FactBomberResult(answersArray);
      const phase2FactBomberResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (phase2FactBomberResult) {
        setResult(phase2FactBomberResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (phase2FactBomberResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(phase2FactBomberResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 팩폭 능력 결과:', phase2FactBomberResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const resultFactBomberLevel = result.factBomberLevel[locale as keyof typeof result.factBomberLevel] || result.factBomberLevel.ko;
