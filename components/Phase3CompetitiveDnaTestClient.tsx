...
import {
  Phase3CompetitiveDnaQuestion,
  Phase3CompetitiveDnaResult,
  calculatePhase3CompetitiveDnaResult,
  calculatePhase3CompetitiveDnaDomainScores,
} from '@/lib/phase3CompetitiveDnaData';
...
interface Phase3CompetitiveDnaTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3CompetitiveDnaQuestion[];
  results: Phase3CompetitiveDnaResult[];
...
export default function Phase3CompetitiveDnaTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3CompetitiveDnaTestClientProps) {
  const t = useTranslations('phase3CompetitiveDnaTest');
  ...
  const [result, setResult] = useState<Phase3CompetitiveDnaResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3CompetitiveDnaQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3CompetitiveDnaQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      const scores = calculatePhase3CompetitiveDnaDomainScores(answersArray);
      setDomainScores(scores);

      const resultType = calculatePhase3CompetitiveDnaResult(answersArray);
  ...
  const handleRetake = () => {
    ...
    setResult(null);
    setShowResult(false);
    setDomainScores({ reaction: 0, winLoss: 0, motivation: 0, comparison: 0, strategy: 0, lifeImpact: 0, total: 0 });
  ...
  if (showResult && result) {
    ...
    const resultCompetitiveDna =
      result.competitiveDna[locale as keyof typeof result.competitiveDna] || result.competitiveDna.ko;
    const resultCompetitiveType =
      result.competitiveType[locale as keyof typeof result.competitiveType] || result.competitiveType.ko;
    const resultCompetitiveKeywords =
      result.competitiveKeywords[locale as keyof typeof result.competitiveKeywords] || result.competitiveKeywords.ko;
    ...
    const domainConfig = [
      { key: 'reaction', label: t('ui.reactionDomain'), score: domainScores.reaction, max: 6 },
      { key: 'winLoss', label: t('ui.winLossDomain'), score: domainScores.winLoss, max: 6 },
      { key: 'motivation', label: t('ui.motivationDomain'), score: domainScores.motivation, max: 6 },
      { key: 'comparison', label: t('ui.comparisonDomain'), score: domainScores.comparison, max: 6 },
      { key: 'strategy', label: t('ui.strategyDomain'), score: domainScores.strategy, max: 6 },
      { key: 'lifeImpact', label: t('ui.lifeImpactDomain'), score: domainScores.lifeImpact, max: 6 },
    ];
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  🎯 {t('ui.competitiveType')}
                </h3>
                <p className="text-sm font-bold text-gray-700 leading-relaxed text-center">
                  {resultCompetitiveType}
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  🔑 {t('ui.competitiveKeywords')}
                </h3>
                <p className="text-sm font-bold text-gray-700 leading-relaxed text-center">
                  {resultCompetitiveKeywords}
                </p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <div className="mt-4 space-y-2 text-sm text-gray-700">
                ...
                <p>
                  📌 {t('ui.lowestDomain')}: <span className="font-semibold">{weakestLabels.join(' · ')}</span>
                </p>
              </div>
            </div>
            ...
            {(resultBestEnvironments || resultSportsGameTypes) && (
              <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  🧭 {t('ui.environmentGuideTitle')}
                </h3>
                <div className="space-y-2 text-sm text-gray-700">
                  {resultBestEnvironments && (
                    <p><span className="font-semibold">{t('ui.bestEnvironmentsTitle')}:</span> {resultBestEnvironments}</p>
                  )}
                  {resultSportsGameTypes && (
                    <p><span className="font-semibold">{t('ui.sportsGameTypesTitle')}:</span> {resultSportsGameTypes}</p>
                  )}
                </div>
              </div>
            )}
