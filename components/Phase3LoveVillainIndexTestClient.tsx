...
import {
  Phase3LoveVillainIndexQuestion,
  Phase3LoveVillainIndexResult,
  calculatePhase3LoveVillainIndexResult,
...
interface Phase3LoveVillainIndexTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3LoveVillainIndexQuestion[];
  results: Phase3LoveVillainIndexResult[];
...
export default function Phase3LoveVillainIndexTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3LoveVillainIndexTestClientProps) {
  const t = useTranslations('phase3LoveVillainIndexTest');
  ...
  const [result, setResult] = useState<Phase3LoveVillainIndexResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3LoveVillainIndexQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3LoveVillainIndexQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3LoveVillainIndexResult(answersArray);
  ...
  if (showResult && result) {
    ...
    const resultVillainLevel = result.villainLevel[locale as keyof typeof result.villainLevel] || result.villainLevel.ko;
    const resultVillainIndex = result.villainIndex[locale as keyof typeof result.villainIndex] || result.villainIndex.ko;
