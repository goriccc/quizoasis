...
import {
  Phase3HundredBillionProbabilityQuestion,
  Phase3HundredBillionProbabilityResult,
  calculatePhase3HundredBillionProbabilityResult,
...
interface Phase3HundredBillionProbabilityTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3HundredBillionProbabilityQuestion[];
  results: Phase3HundredBillionProbabilityResult[];
...
export default function Phase3HundredBillionProbabilityTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3HundredBillionProbabilityTestClientProps) {
  const t = useTranslations('phase3HundredBillionProbabilityTest');
  ...
  const [result, setResult] = useState<Phase3HundredBillionProbabilityResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3HundredBillionProbabilityQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3HundredBillionProbabilityQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3HundredBillionProbabilityResult(answersArray);
  ...
  if (showResult && result) {
    ...
    const resultProbability = result.probability[locale as keyof typeof result.probability] || result.probability.ko;
