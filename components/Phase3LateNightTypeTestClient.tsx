...
import {
  Phase3LateNightTypeQuestion,
  Phase3LateNightTypeResult,
  calculatePhase3LateNightTypeResult,
...
interface Phase3LateNightTypeTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3LateNightTypeQuestion[];
  results: Phase3LateNightTypeResult[];
...
export default function Phase3LateNightTypeTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3LateNightTypeTestClientProps) {
  const t = useTranslations('phase3LateNightTypeTest');
  ...
  const [result, setResult] = useState<Phase3LateNightTypeResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3LateNightTypeQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3LateNightTypeQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3LateNightTypeResult(answersArray);
  ...
  if (showResult && result) {
    ...
    const resultLateNightType = result.lateNightType[locale as keyof typeof result.lateNightType] || result.lateNightType.ko;
    const resultLateNightKeywords = result.lateNightKeywords[locale as keyof typeof result.lateNightKeywords] || result.lateNightKeywords.ko;
    ...
    const resultDawnBgm = result.dawnBgm[locale as keyof typeof result.dawnBgm] || result.dawnBgm.ko;
          ...
          <div>
            <div className="text-center mb-3 bg-white rounded-2xl shadow-lg p-4 md:p-5">
              ...
              <p className="mt-4 text-sm font-semibold text-indigo-600">
                🌙 {t('ui.currentDawnMoment', { hour: currentHour })}
              </p>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                🔑 {t('ui.lateNightKeywords')}
              </h3>
              <div className="flex flex-wrap justify-center gap-2">
                {resultLateNightKeywords.split(/[·]/).map((keyword, index) => (
                  <span
                    key={index}
