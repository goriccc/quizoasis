...
import {
  Phase3CuriosityTypeQuestion,
  Phase3CuriosityTypeResult,
  calculatePhase3CuriosityTypeResult,
  calculatePhase3CuriosityTypeDomainScores,
} from '@/lib/phase3CuriosityTypeData';
...
interface Phase3CuriosityTypeTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3CuriosityTypeQuestion[];
  results: Phase3CuriosityTypeResult[];
...
export default function Phase3CuriosityTypeTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3CuriosityTypeTestClientProps) {
  const t = useTranslations('phase3CuriosityTypeTest');
  ...
  const [result, setResult] = useState<Phase3CuriosityTypeResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3CuriosityTypeQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3CuriosityTypeQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      const scores = calculatePhase3CuriosityTypeDomainScores(answersArray);
      setDomainScores(scores);

      const resultType = calculatePhase3CuriosityTypeResult(answersArray);
  ...
  const handleRetake = () => {
    ...
    setResult(null);
    setShowResult(false);
    setDomainScores({ interestScope: 0, exploreMethod: 0, questionTendency: 0, learningPattern: 0, interestTrigger: 0, curiosityPersistence: 0, total: 0 });
  ...
  if (showResult && result) {
    ...
    const resultCuriositySummary =
      result.curiositySummary[locale as keyof typeof result.curiositySummary] || result.curiositySummary.ko;
    const resultCuriosityType =
      result.curiosityType[locale as keyof typeof result.curiosityType] || result.curiosityType.ko;
    const resultExploreKeywords =
      result.exploreKeywords[locale as keyof typeof result.exploreKeywords] || result.exploreKeywords.ko;
    ...
    const domainConfig = [
      { key: 'interestScope', label: t('ui.interestScopeDomain'), score: domainScores.interestScope, max: 6 },
      { key: 'exploreMethod', label: t('ui.exploreMethodDomain'), score: domainScores.exploreMethod, max: 6 },
      { key: 'questionTendency', label: t('ui.questionTendencyDomain'), score: domainScores.questionTendency, max: 6 },
      { key: 'learningPattern', label: t('ui.learningPatternDomain'), score: domainScores.learningPattern, max: 6 },
      { key: 'interestTrigger', label: t('ui.interestTriggerDomain'), score: domainScores.interestTrigger, max: 6 },
      { key: 'curiosityPersistence', label: t('ui.curiosityPersistenceDomain'), score: domainScores.curiosityPersistence, max: 6 },
    ];
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              ...
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  🔑 {t('ui.exploreKeywordsTitle')}
                </h3>
                <p className="text-sm font-bold text-gray-700 leading-relaxed text-center">
                  {resultExploreKeywords}
                </p>
              </div>
            </div>
<div className="bg-white rounded-xl shadow-lg p-4 mb-3">
...
{isType6 && resultCertification && (
            ...
            <div className="mt-8 mb-6 px-4">
              <button
                ...
                className="w-full bg-gradient-to-r from-indigo-500 to-purple-500 text-white font-bold py-4 px-6 rounded-xl hover:from-indigo-600 hover:to-purple-600 transition-all shadow-md flex items-center justify-center gap-3"
              >
