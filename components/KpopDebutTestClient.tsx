...
import { KpopDebutQuestion, KpopDebutResult, calculateKpopDebutResult } from '@/lib/kpopDebutData';
...
interface KpopDebutTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: KpopDebutQuestion[];
  results: KpopDebutResult[];
...
export default function KpopDebutTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: KpopDebutTestClientProps) {
  const t = useTranslations('kpopDebutTest');
  const tGlobal = useTranslations();
  const [started, setStarted] = useState(false);
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [answers, setAnswers] = useState<any[]>([]);
  const [result, setResult] = useState<KpopDebutResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<KpopDebutQuestion[]>(questions);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: KpopDebutQuestion[]) => {
  ...
  const handleAnswer = (scores: any) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculateKpopDebutResult(newAnswers);
      const kpopDebutResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (kpopDebutResult) {
        setResult(kpopDebutResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (kpopDebutResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(kpopDebutResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] K-POP 데뷔 결과:', kpopDebutResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  const calculateResult = (finalAnswers: any[]) => {
    const resultType = calculateKpopDebutResult(finalAnswers);
    const kpopDebutResult = results.find(r => r.type === resultType);
  ...
  const shareToKakao = () => {
    ...
    const thumbnailUrl = getThumbnailUrl(thumbnail || 'phase2_test_038_kpop_debut.jpg');
      ...
      <div className="min-h-screen bg-white">
        <div className="max-w-4xl mx-auto">
          <div className="relative w-full overflow-hidden mb-3" style={{ aspectRatio: '680/384' }}>
            <Image
              src={getThumbnailUrl(thumbnail || 'phase2_test_038_kpop_debut.jpg')}
              alt={title}
              fill
          ...
          <div className="px-4">
            ...
            {similarTestsState.length > 0 && (
              ...
              <h2 className="text-xl font-bold text-gray-800 mb-6">
                {tGlobal('recommendations.similarTests')}
              </h2>
  ...
  if (showResult && result) {
    ...
    const resultPosition = result.position[locale as keyof typeof result.position] || result.position.ko;
    const resultAgency = result.agency[locale as keyof typeof result.agency] || result.agency.ko;
    const resultEntryPoint = result.entryPoint[locale as keyof typeof result.entryPoint] || result.entryPoint.ko;
          ...
          <div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <div className="flex flex-wrap gap-2">
                {resultPosition.split(/[,、，]/).map((position, index) => (
                  <span
                    key={index}
                    className="inline-block bg-gradient-to-r from-blue-100 to-cyan-100 text-blue-800 font-medium px-4 py-2 rounded-full text-sm border border-blue-200"
                  >
                    {position.trim()}
                  </span>
                ))}
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <div className="flex flex-wrap gap-2">
                {resultEntryPoint.split(/[,、，]/).map((point, index) => (
                  <span
                    key={index}
