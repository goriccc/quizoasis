...
import {
  Phase3YoloFireGodlifeTypeQuestion,
  Phase3YoloFireGodlifeTypeResult,
  calculatePhase3YoloFireGodlifeTypeResult,
...
interface Phase3YoloFireGodlifeTypeTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3YoloFireGodlifeTypeQuestion[];
  results: Phase3YoloFireGodlifeTypeResult[];
...
export default function Phase3YoloFireGodlifeTypeTestClient({
  locale,
  slug,
  title,
  description,
  ...
  isLatestTest = false,
  badgeType = null,
}: Phase3YoloFireGodlifeTypeTestClientProps) {
  const t = useTranslations('phase3YoloFireGodlifeTypeTest');
  ...
  const [result, setResult] = useState<Phase3YoloFireGodlifeTypeResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3YoloFireGodlifeTypeQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3YoloFireGodlifeTypeQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3YoloFireGodlifeTypeResult(answersArray);
      const yfgResult = results.find(r => r.type === resultType);
  ...
  if (showResult && result) {
    ...
    const resultLifeFinance =
      result.lifeFinanceType[locale as keyof typeof result.lifeFinanceType] || result.lifeFinanceType.ko;
