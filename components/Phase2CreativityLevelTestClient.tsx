...
import { Phase2CreativityLevelQuestion, Phase2CreativityLevelResult, calculatePhase2CreativityLevelResult } from '@/lib/phase2CreativityLevelData';
...
interface Phase2CreativityLevelTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase2CreativityLevelQuestion[];
  results: Phase2CreativityLevelResult[];
...
export default function Phase2CreativityLevelTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  playCount = 0,
  similarTests = []
}: Phase2CreativityLevelTestClientProps) {
  const t = useTranslations('phase2CreativityLevelTest');
  ...
  const [result, setResult] = useState<Phase2CreativityLevelResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase2CreativityLevelQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase2CreativityLevelQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase2CreativityLevelResult(answersArray);
      const creativityResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (creativityResult) {
        setResult(creativityResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (creativityResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(creativityResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 창의력 레벨 결과:', creativityResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  const handleShareResult = async () => {
    ...
    const resultCreativityLevel = result.creativityLevel[locale as keyof typeof result.creativityLevel] || result.creativityLevel.ko;
    const shareText = `${t('shareMessages.default', { level: resultCreativityLevel, type: resultTitle })}\n\n${`https://myquizoasis.com${window.location.pathname}`}`;
  ...
  const shareToLine = () => {
    ...
    const resultCreativityLevel = result ? (result.creativityLevel[locale as keyof typeof result.creativityLevel] || result.creativityLevel.ko) : '';
    const shareText = result 
      ? encodeURIComponent(t('shareMessages.line', { level: resultCreativityLevel, type: resultTitle }))
  ...
  const shareToWhatsApp = () => {
    ...
    const resultCreativityLevel = result ? (result.creativityLevel[locale as keyof typeof result.creativityLevel] || result.creativityLevel.ko) : '';
    const shareText = result 
      ? encodeURIComponent(t('shareMessages.whatsapp', { level: resultCreativityLevel, type: resultTitle }))
  ...
  const shareToKakao = () => {
    ...
    const shareDescription = result 
      ? t('shareMessages.kakao', { level: resultCreativityLevel, type: resultTitle })
  ...
  const shareToTelegram = () => {
    ...
    const resultCreativityLevel = result ? (result.creativityLevel[locale as keyof typeof result.creativityLevel] || result.creativityLevel.ko) : '';
    const shareText = result 
      ? t('shareMessages.telegram', { level: resultCreativityLevel, type: resultTitle })
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="space-y-2">
                  {(() => {
                    // "Type 2", "タイプ2", "类型2", "類型2", "Loại 2", "Tipe 2" 등 모든 언어 형식에서 숫자 추출
