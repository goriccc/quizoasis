...
import {
  Phase3LonelinessConcentrationQuestion,
  Phase3LonelinessConcentrationResult,
  calculatePhase3LonelinessConcentrationResult,
...
interface Phase3LonelinessConcentrationTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3LonelinessConcentrationQuestion[];
  results: Phase3LonelinessConcentrationResult[];
...
export default function Phase3LonelinessConcentrationTestClient({
  locale,
  slug,
  title,
  description,
  ...
  isLatestTest = false,
  badgeType = null,
}: Phase3LonelinessConcentrationTestClientProps) {
  const t = useTranslations('phase3LonelinessConcentrationTest');
  ...
  const [result, setResult] = useState<Phase3LonelinessConcentrationResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3LonelinessConcentrationQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3LonelinessConcentrationQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3LonelinessConcentrationResult(answersArray);
      const lonelinessResult = results.find(r => r.type === resultType);

      if (lonelinessResult) {
        setResult(lonelinessResult);
      }
    }
  };
  ...
  if (showResult && result) {
    ...
    const resultConcentration =
      result.concentration[locale as keyof typeof result.concentration] || result.concentration.ko;
    ...
    const resultCommunity =
      result.community[locale as keyof typeof result.community] || result.community.ko;
    const resultCrisis = result.crisisNote
          ...
          <div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <div className="flex flex-wrap gap-2 justify-center">
                  {resultCurrentState.split(/[,、，·]/).map((char, idx) => (
                    <span
                      key={idx}
            ...
            {resultStrength ? (
              <div className="mb-3">
                <div className="bg-white rounded-xl shadow-lg p-4">
                  ...
                  <div className="flex flex-wrap gap-2 justify-center">
                    {resultStrength.split(/[,、，·]/).map((char, idx) => (
                      <span
                        key={idx}
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{resultCommunity}</p>
              </div>
            </div>

            {resultCrisis ? (
              <div className="mb-3">
                <div className="bg-rose-50 border border-rose-200 rounded-xl shadow-lg p-4">
                  <h3 className="text-base font-bold text-rose-800 mb-2 text-left w-full">
                    {t('ui.crisisNote')}
                  </h3>
                  <p className="text-sm text-rose-900 leading-relaxed">{resultCrisis}</p>
                </div>
              </div>
            ) : null}
