...
import {
  Phase3DailyMindWeatherReportQuestion,
  Phase3DailyMindWeatherReportResult,
  calculatePhase3DailyMindWeatherReportResult,
  saveMindWeatherResult,
  computeStreak,
  getStreakBadgeMessage,
  getYesterdayCompareMessage,
  getMonthCalendarDays,
  getMonthPatternMessage,
  type MindWeatherHistoryEntry,
...
interface Phase3DailyMindWeatherReportTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3DailyMindWeatherReportQuestion[];
  results: Phase3DailyMindWeatherReportResult[];
...
export default function Phase3DailyMindWeatherReportTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3DailyMindWeatherReportTestClientProps) {
  const t = useTranslations('phase3DailyMindWeatherReportTest');
  ...
  const [result, setResult] = useState<Phase3DailyMindWeatherReportResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3DailyMindWeatherReportQuestion[]>([]);
  ...
  const [weatherHistory, setWeatherHistory] = useState<MindWeatherHistoryEntry[]>([]);
  const [streakCount, setStreakCount] = useState(0);
  const [streakBadge, setStreakBadge] = useState<string | null>(null);
  const [yesterdayCompare, setYesterdayCompare] = useState<string | null>(null);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3DailyMindWeatherReportQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3DailyMindWeatherReportResult(answersArray);
      const weatherResult = results.find(r => r.type === resultType);

      if (weatherResult) {
        setResult(weatherResult);
        const history = saveMindWeatherResult(weatherResult.type, weatherResult.emoji);
        setWeatherHistory(history);
        const streak = computeStreak(history);
        setStreakCount(streak);
        setStreakBadge(getStreakBadgeMessage(streak, locale));
        setYesterdayCompare(
          getYesterdayCompareMessage(history, weatherResult.type, weatherResult.emoji, locale)
        );
      }
    }
  };
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="px-4">
            ...
            <div className="text-gray-600 mb-6 leading-relaxed text-center space-y-4">
              ...
              <p className="text-sm text-gray-500 mt-2">{t('startMessage.note')}</p>
            </div>
  ...
  if (showResult && result) {
    const resultTitleRaw = result.title[locale as keyof typeof result.title] || result.title.ko;
    ...
    const resultWeatherLabel =
      result.weatherLabel[locale as keyof typeof result.weatherLabel] || result.weatherLabel.ko;
    const resultTemperature =
      result.temperature[locale as keyof typeof result.temperature] || result.temperature.ko;
    const resultTodayAdvice =
      result.todayAdvice[locale as keyof typeof result.todayAdvice] || result.todayAdvice.ko;
    const resultSuitableThings =
      result.suitableThings[locale as keyof typeof result.suitableThings] || result.suitableThings.ko;
    const resultEmotionLine =
      result.emotionLine[locale as keyof typeof result.emotionLine] || result.emotionLine.ko;
    const resultTomorrowCheck =
      result.tomorrowCheck[locale as keyof typeof result.tomorrowCheck] || result.tomorrowCheck.ko;
    const resultCrisisNote = result.crisisNote
      ? result.crisisNote[locale as keyof typeof result.crisisNote] || result.crisisNote.ko
      : null;

    const now = new Date();
    const calendarDays = getMonthCalendarDays(weatherHistory, now.getFullYear(), now.getMonth());
    const monthPattern = getMonthPatternMessage(weatherHistory, now.getFullYear(), now.getMonth(), locale);
      ...
      <div className="min-h-screen bg-gradient-to-br from-sky-50 via-blue-50 to-indigo-50">
        <div className="max-w-3xl mx-auto px-4 py-8">
          <div>
            ...
            {streakBadge && (
              <div className="mb-3 bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 rounded-xl shadow-sm p-4 text-center">
                <p className="text-base font-semibold text-emerald-800">{streakBadge}</p>
                <p className="text-sm text-emerald-600 mt-1">{t('ui.streakCount', { count: streakCount })}</p>
              </div>
            )}

            {yesterdayCompare && (
              <div className="mb-3 bg-white rounded-xl shadow-lg p-4 text-center">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left w-full">
                  {t('ui.yesterdayCompare')}
                </h3>
                <p className="text-sm text-gray-700 leading-relaxed">{yesterdayCompare}</p>
              </div>
            )}
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-lg font-bold text-sky-600 text-center leading-snug">
                  {resultWeatherLabel}
                </p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-base text-gray-700 text-center leading-snug">{resultTemperature}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed text-center">&quot;{resultTodayAdvice}&quot;</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-5">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {resultSuitableThings.split(/[,、，]/).map((part, index) => (
                    <span
                      key={index}
                      className="inline-block bg-gradient-to-r from-sky-100 to-indigo-100 text-sky-800 px-3 py-1.5 rounded-full text-sm font-medium"
                    >
                      {part.trim()}
                    </span>
                  ))}
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed text-center">&quot;{resultEmotionLine}&quot;</p>
              </div>
            </div>
            ...
            {resultCrisisNote && (
              <div className="mb-3">
                <div className="bg-blue-50 border border-blue-200 rounded-xl shadow-sm p-4">
                  <h3 className="text-base font-bold text-blue-900 mb-2 text-left">
                    {t('ui.crisisNote')}
                  </h3>
                  <p className="text-sm text-blue-800 leading-relaxed">{resultCrisisNote}</p>
                </div>
              </div>
            )}
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                ...
                <p className="text-sm text-gray-700 leading-relaxed text-center">{resultTomorrowCheck}</p>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  {t('ui.monthCalendar', { year: now.getFullYear(), month: now.getMonth() + 1 })}
                </h3>
                <p className="text-sm text-gray-600 mb-3 text-center">{monthPattern}</p>
                ...
                <div className="grid grid-cols-7 gap-1">
                  ...
                  {calendarDays.map((d) => (
                    <div
                      key={d.dateKey}
                      className="aspect-square rounded-lg bg-sky-50 flex flex-col items-center justify-center text-[10px] sm:text-xs"
                    >
          ...
          <div className="grid grid-cols-2 gap-2 sm:gap-4 px-2 sm:px-4">
            {optionsArray.map((option, index) => {
              ...
              const colors = [
                ...
                'from-amber-50 to-amber-100 hover:from-amber-100 hover:to-amber-200 border-amber-200 hover:border-amber-400',
              ];
              const bgColors = ['bg-purple-600', 'bg-pink-600', 'bg-blue-600', 'bg-amber-600'];
