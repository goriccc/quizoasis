...
import {
  Phase3OptimismIndexQuestion,
  Phase3OptimismIndexResult,
  calculatePhase3OptimismIndexResult,
  calculatePhase3OptimismIndexDomainScores,
} from '@/lib/phase3OptimismIndexData';
...
interface Phase3OptimismIndexTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3OptimismIndexQuestion[];
  results: Phase3OptimismIndexResult[];
...
export default function Phase3OptimismIndexTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3OptimismIndexTestClientProps) {
  const t = useTranslations('phase3OptimismIndexTest');
  ...
  const [result, setResult] = useState<Phase3OptimismIndexResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3OptimismIndexQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3OptimismIndexQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      const scores = calculatePhase3OptimismIndexDomainScores(answersArray);
      setDomainScores(scores);

      const resultType = calculatePhase3OptimismIndexResult(answersArray);
  ...
  const handleRetake = () => {
    ...
    setResult(null);
    setShowResult(false);
    setDomainScores({ permanence: 0, pervasiveness: 0, personalization: 0, total: 0 });
  ...
  if (showResult && result) {
    ...
    const resultOptimismIndex =
      result.optimismIndex[locale as keyof typeof result.optimismIndex] || result.optimismIndex.ko;
    const resultOptimismType =
      result.optimismType[locale as keyof typeof result.optimismType] || result.optimismType.ko;
                ...
                {domainConfig.map((domain) => (
                  <div key={domain.key}>
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2.5">
                      <div
                        className="bg-gradient-to-r from-amber-400 to-orange-400 h-2.5 rounded-full transition-all"
