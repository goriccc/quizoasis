...
import { TravelStyleQuestion, TravelStyleResult, calculateTravelStyleResult } from '@/lib/travelStyleData';
...
interface TravelStyleTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: TravelStyleQuestion[];
  results: TravelStyleResult[];
...
export default function TravelStyleTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  playCount = 0,
  similarTests = []
}: TravelStyleTestClientProps) {
  const t = useTranslations('travelStyleTest');
  ...
  const [shuffledQuestions, setShuffledQuestions] = useState<TravelStyleQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: TravelStyleQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculateTravelStyleResult(answersArray);
      const travelStyleResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (travelStyleResult) {
        setResult(travelStyleResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (travelStyleResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(travelStyleResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 여행 스타일 결과:', travelStyleResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const resultJScore = result.jScore[locale as keyof typeof result.jScore] || result.jScore.ko;
