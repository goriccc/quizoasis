...
import { ConflictStyleQuestion, ConflictStyleResult, calculateConflictStyleResult } from '@/lib/conflictStyleData';
...
interface ConflictStyleTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: ConflictStyleQuestion[];
  results: ConflictStyleResult[];
...
export default function ConflictStyleTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  const [result, setResult] = useState<ConflictStyleResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<ConflictStyleQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: ConflictStyleQuestion[]) => {
  ...
  const handleAnswer = (type: string) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      const answersArray = questions.map((_, idx) => newAnswers[idx] ?? 'Type3');
      
      // 결과 계산
      const resultType = calculateConflictStyleResult(answersArray);
      const conflictStyleResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (conflictStyleResult) {
        setResult(conflictStyleResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (conflictStyleResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(conflictStyleResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 갈등 해결 스타일 결과:', conflictStyleResult.type, '→ 검색 키워드:', keywords[0]);
