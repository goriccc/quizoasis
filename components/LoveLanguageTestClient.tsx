...
import { LoveLanguageQuestion, LoveLanguageResult, calculateLoveLanguageResult } from '@/lib/loveLanguageData';
...
interface LoveLanguageTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
...
export default function LoveLanguageTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  playCount = 0,
  similarTests = []
}: LoveLanguageTestClientProps) {
  const t = useTranslations('loveLanguageTest');
  const tGlobal = useTranslations(); // 글로벌 번역 (mbti 등)
  const [started, setStarted] = useState(false);
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [answers, setAnswers] = useState<Record<number, string>>({}); // 원래 질문 인덱스를 키로 사용 (type 저장)
  const [result, setResult] = useState<LoveLanguageResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<LoveLanguageQuestion[]>([]);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: LoveLanguageQuestion[]) => {
  ...
  const handleAnswer = (type: string) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculateLoveLanguageResult(answersArray);
      const loveLanguageResult = results.find(r => r.type === resultType);
      ...
      if (loveLanguageResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(loveLanguageResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 사랑의 언어 결과:', loveLanguageResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const resultTip = result.tip[locale as keyof typeof result.tip] || result.tip.ko;
