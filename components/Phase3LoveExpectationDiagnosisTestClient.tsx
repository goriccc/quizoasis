...
import {
  Phase3LoveExpectationDiagnosisQuestion,
  Phase3LoveExpectationDiagnosisResult,
  calculatePhase3LoveExpectationDiagnosisResult,
  calculatePhase3LoveExpectationDiagnosisDomainScores,
} from '@/lib/phase3LoveExpectationDiagnosisData';
...
interface Phase3LoveExpectationDiagnosisTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3LoveExpectationDiagnosisQuestion[];
  results: Phase3LoveExpectationDiagnosisResult[];
...
export default function Phase3LoveExpectationDiagnosisTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3LoveExpectationDiagnosisTestClientProps) {
  const t = useTranslations('phase3LoveExpectationDiagnosisTest');
  ...
  const [result, setResult] = useState<Phase3LoveExpectationDiagnosisResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3LoveExpectationDiagnosisQuestion[]>([]);
  const [originalQuestionIndices, setOriginalQuestionIndices] = useState<number[]>([]); // 셔플링된 질문의 원래 인덱스 매핑
  const [domainScores, setDomainScores] = useState({
    contactCommunication: 0,
    timeMeeting: 0,
    emotionalSupport: 0,
    understandingEmpathy: 0,
    futureDevotion: 0,
    overallExpectLevel: 0,
    total: 0,
  });
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3LoveExpectationDiagnosisQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      const scores = calculatePhase3LoveExpectationDiagnosisDomainScores(answersArray);
      setDomainScores(scores);

      const resultType = calculatePhase3LoveExpectationDiagnosisResult(answersArray);
  ...
  const handleRetake = () => {
    ...
    setResult(null);
    setShowResult(false);
    setDomainScores({ contactCommunication: 0, timeMeeting: 0, emotionalSupport: 0, understandingEmpathy: 0, futureDevotion: 0, overallExpectLevel: 0, total: 0 });
  ...
  if (showResult && result) {
    ...
    const resultExpectationSummary =
      result.expectationSummary[locale as keyof typeof result.expectationSummary] || result.expectationSummary.ko;
    const resultExpectationType =
      result.expectationType[locale as keyof typeof result.expectationType] || result.expectationType.ko;
    const resultExpectationKeywords =
      result.expectationKeywords[locale as keyof typeof result.expectationKeywords] || result.expectationKeywords.ko;
    ...
    const resultGuidanceAdvice = result.guidanceAdvice[locale as keyof typeof result.guidanceAdvice] || result.guidanceAdvice.ko;
    ...
    const domainConfig = [
      { key: 'contactCommunication', label: t('ui.contactCommunicationDomain'), score: domainScores.contactCommunication, max: 6 },
      { key: 'timeMeeting', label: t('ui.timeMeetingDomain'), score: domainScores.timeMeeting, max: 6 },
      { key: 'emotionalSupport', label: t('ui.emotionalSupportDomain'), score: domainScores.emotionalSupport, max: 6 },
      { key: 'understandingEmpathy', label: t('ui.understandingEmpathyDomain'), score: domainScores.understandingEmpathy, max: 6 },
      { key: 'futureDevotion', label: t('ui.futureDevotionDomain'), score: domainScores.futureDevotion, max: 6 },
      { key: 'overallExpectLevel', label: t('ui.overallExpectLevelDomain'), score: domainScores.overallExpectLevel, max: 6 },
    ];
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  🎯 {t('ui.expectationType')}
                </h3>
                <p className="text-sm font-bold text-gray-700 leading-relaxed text-center">
                  {resultExpectationType}
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-2 text-left">
                  🔑 {t('ui.expectationKeywordsTitle')}
                </h3>
                <p className="text-sm font-bold text-gray-700 leading-relaxed text-center">
                  {resultExpectationKeywords}
                </p>
              </div>
            </div>
<div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              ...
              <div className="mt-4 space-y-2 text-sm text-gray-700">
                ...
                <p>
                  📌 {t('ui.boostDomain')}: <span className="font-semibold">{weakestLabels.join(' · ')}</span>
                </p>
              </div>
            </div>
...
{isType6 && resultCertification && (
            ...
            <div className="mt-8 mb-6 px-4">
              <button
                ...
                className="w-full bg-gradient-to-r from-rose-500 to-pink-600 text-white font-bold py-4 px-6 rounded-xl hover:from-rose-600 hover:to-pink-700 transition-all shadow-md flex items-center justify-center gap-3"
              >
