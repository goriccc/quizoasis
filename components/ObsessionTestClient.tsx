...
import { ObsessionQuestion, ObsessionResult, calculateObsessionResult } from '@/lib/obsessionData';
...
interface ObsessionTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: ObsessionQuestion[];
  results: ObsessionResult[];
...
export default function ObsessionTestClient({ 
  locale, 
  slug, 
  title, 
  description,
  ...
  playCount = 0,
  similarTests = []
}: ObsessionTestClientProps) {
  const t = useTranslations();
  const [started, setStarted] = useState(false);
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [answers, setAnswers] = useState<any[]>([]);
  const [result, setResult] = useState<ObsessionResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<ObsessionQuestion[]>(questions);
  ...
  // 질문 섞기 함수
  const shuffleQuestions = (questionList: ObsessionQuestion[]) => {
  ...
  const handleAnswer = (scores: any) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculateObsessionResult(newAnswers);
      const obsessionResult = results.find(r => r.type === resultType);
      
      // 결과 설정
      if (obsessionResult) {
        setResult(obsessionResult);
      }
      
      // 결과에 맞는 상품 백그라운드 로드 (로딩 시간 동안)
      if (obsessionResult && locale !== 'ko') {
        const keywords = getProductKeywordsForDating(obsessionResult.type, locale);
        const loadStartTime = Date.now();
        console.log('🔮 [시작] 강박 결과:', obsessionResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  const calculateResult = (finalAnswers: any[]) => {
    const resultType = calculateObsessionResult(finalAnswers);
    const obsessionResult = results.find(r => r.type === resultType);
  ...
  const handleShareResult = async () => {
    ...
    const shareText = `${t('obsessionTest.shareMessages.default', { type: resultTitle })}\n\n${`https://myquizoasis.com${window.location.pathname}`}`;
    ...
    if (navigator.share) {
      try {
        ...
        if (error instanceof Error && error.name !== 'AbortError') {
          console.error(t('obsessionTest.alerts.shareFailed'), error);
        }
      }
    } else {
      try {
        await navigator.clipboard.writeText(shareText);
        alert(t('obsessionTest.alerts.resultCopied'));
      } catch (error) {
        console.error(t('obsessionTest.alerts.clipboardError'), error);
        alert(t('obsessionTest.alerts.shareUnavailable'));
      }
    }
  };
  ...
  const shareToWeChat = async () => {
    ...
    const shareText = result 
      ? `${t('obsessionTest.shareMessages.wechat', { type: resultTitle })}\n\n${url}`
      : `${title}\n\n${url}`;
    ...
    try {
      ...
      alert(t('obsessionTest.alerts.wechatCopy'));
    } catch (error) {
      alert(t('obsessionTest.alerts.shareUnavailable'));
    }
  };
  ...
  const shareToWhatsApp = () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareText = result 
      ? encodeURIComponent(t('obsessionTest.shareMessages.whatsapp', { type: resultTitle }))
  ...
  const shareToKakao = () => {
    ...
    if (!window.Kakao || !window.Kakao.isInitialized()) {
      alert(t('obsessionTest.alerts.kakaoInit'));
      return;
    }
    ...
    const shareDescription = result 
      ? t('obsessionTest.shareMessages.kakao', { type: resultTitle })
      : description;
    ...
    try {
      ...
      console.error(t('obsessionTest.alerts.shareFailed'), error);
      alert(t('obsessionTest.alerts.kakaoError'));
    }
  };
  ...
  const shareToTelegram = () => {
    ...
    const resultTitle = result ? (result.title[locale as keyof typeof result.title] || result.title.ko) : '';
    const shareText = result 
      ? t('obsessionTest.shareMessages.telegram', { type: resultTitle })
      : title;
  ...
  const copyLink = () => {
    ...
    alert(t('obsessionTest.alerts.linkCopied'));
  };
      ...
      <div className="min-h-screen bg-white">
        <div className="max-w-4xl mx-auto">
          <div className="relative w-full overflow-hidden mb-3" style={{ aspectRatio: '680/384' }}>
            <Image
              src={getThumbnailUrl(thumbnail || 'test_203_obsession_vs_healthy.jpg')}
              alt={title}
              fill
          ...
          <div className="px-4">
            ...
            <div className="text-gray-600 mb-6 leading-relaxed text-center space-y-4">
              <p className="font-bold text-gray-700">{t('obsessionTest.startMessage.line1')}</p>
              <p>{t('obsessionTest.startMessage.line2')}</p>
              <p>{t('obsessionTest.startMessage.line3')}</p>
              <p>{t('obsessionTest.startMessage.line4')}</p>
              <p>{t('obsessionTest.startMessage.line5')}</p>
              <p>{t('obsessionTest.startMessage.line6')}</p>
            </div>
            ...
            <div className="mb-8 text-center">
              ...
              <div className="flex justify-center gap-2">
                <button onClick={copyLink} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/link.jpeg" alt={t('obsessionTest.ui.linkCopy')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToKakao} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/kakao.jpeg" alt={t('obsessionTest.ui.kakao')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToTelegram} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/telegram.jpeg" alt={t('obsessionTest.ui.telegram')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToWeChat} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/wechat.jpeg" alt={t('obsessionTest.ui.wechat')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToLine} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/line.jpeg" alt={t('obsessionTest.ui.line')} width={46} height={46} className="rounded-lg" />
                </button>
                <button onClick={shareToWhatsApp} className="flex items-center justify-center w-12 h-12 hover:scale-110 transition-transform">
                  <Image src="/icons/whatsapp.jpeg" alt={t('obsessionTest.ui.whatsapp')} width={46} height={46} className="rounded-lg" />
                </button>
              </div>
            </div>
  ...
  if (showResult && result) {
    ...
    const resultSymptoms = result.symptoms[locale as keyof typeof result.symptoms] || result.symptoms.ko;
    const resultRiskLevel = result.riskLevel[locale as keyof typeof result.riskLevel] || result.riskLevel.ko;
    const resultImpact = result.impact[locale as keyof typeof result.impact] || result.impact.ko;
    const resultAdvice = result.advice[locale as keyof typeof result.advice] || result.advice.ko;
    const resultManagementTips = result.managementTips;
    ...
    const symptomsArray = splitByCommas(resultSymptoms);
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-base font-bold text-gray-800 mb-3">
                  ⚠️ {t('obsessionTest.ui.riskLevel')}
                </h3>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 mb-3">
              <h3 className="text-base font-bold text-gray-800 mb-3">
                ✅ {t('obsessionTest.ui.managementTips')}
              </h3>
              <div className="space-y-2">
                {resultManagementTips.map((tip, index) => (
                  <div key={index} className="flex items-start gap-2">
                    <span className="text-gray-800 font-medium">•</span>
                    <span className="text-sm text-gray-700">
                      {tip[locale as keyof typeof tip] || tip.ko}
                    </span>
                  </div>
                ))}
              </div>
