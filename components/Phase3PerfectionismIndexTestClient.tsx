...
import {
  Phase3PerfectionismIndexQuestion,
  Phase3PerfectionismIndexResult,
  calculatePhase3PerfectionismIndexResult,
...
interface Phase3PerfectionismIndexTestClientProps {
  locale: string;
  slug: string;
  title: string;
  description: string;
  questions: Phase3PerfectionismIndexQuestion[];
  results: Phase3PerfectionismIndexResult[];
...
export default function Phase3PerfectionismIndexTestClient({ 
  locale, 
  slug, 
  title, 
  description,
...
,
  isLatestTest = false,
  badgeType = null
}: Phase3PerfectionismIndexTestClientProps) {
  const t = useTranslations('phase3PerfectionismIndexTest');
  ...
  const [result, setResult] = useState<Phase3PerfectionismIndexResult | null>(null);
  const [showResult, setShowResult] = useState(false);
  const [shuffledQuestions, setShuffledQuestions] = useState<Phase3PerfectionismIndexQuestion[]>([]);
...
// 질문 섞기 함수
  const shuffleQuestions = (questionList: Phase3PerfectionismIndexQuestion[]) => {
  ...
  const handleAnswer = (score: number) => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      // 결과 계산
      const resultType = calculatePhase3PerfectionismIndexResult(answersArray);
      ...
      if (perfectionismResult && locale !== 'ko') {
        ...
        console.log('🔮 [시작] 완벽주의 지수 결과:', perfectionismResult.type, '→ 검색 키워드:', keywords[0]);
  ...
  if (showResult && result) {
    ...
    const resultLifeImpact = pickLocalized(result.lifeImpact);
            ...
            <div className="flex flex-col gap-3 mb-3">
              {resultLifeImpact ? (
                <div className="bg-white rounded-xl shadow-lg p-4">
                  <h3 className="text-base font-bold text-gray-800 mb-2">
                    {t('ui.lifeImpactSection')}
                  </h3>
                  <p className="text-base text-gray-600 leading-relaxed">{resultLifeImpact}</p>
                </div>
              ) : null}
              {resultWorkStyle ? (
                <div className="bg-white rounded-xl shadow-lg p-4">
              ...
              {resultMessageToYou ? (
                <div className="bg-white rounded-xl shadow-lg p-4">
                  ...
                  <p className="text-base text-gray-600 leading-relaxed">{resultMessageToYou}</p>
                </div>
              ) : null}
